...
const BASE_URL_HEADER = "x-tokenash-base-url";
const PROXY_ENV = "TOKENASH_OPENCODE_TRANSPORT_PROXY_URL";
const STATE_KEY = Symbol.for("tokenash.opencode.transport");
...
interface GlobalWithTokenashTransport {
...
function getState(): TransportState | undefined {
  return (globalThis as GlobalWithTokenashTransport)[STATE_KEY];
}

function setState(state: TransportState | undefined): void {
  (globalThis as GlobalWithTokenashTransport)[STATE_KEY] = state;
}
...
function wrapSpawn(originalSpawn: ChildSpawn): ChildSpawn {
  return function tokenashSpawn(this: unknown, ...args: unknown[]) {
...
function wrapExec(originalExec: ChildExec): ChildExec {
  return function tokenashExec(this: unknown, ...args: unknown[]) {
    const state = getState();
    if (!state) {
...
function wrapExecFile(originalExecFile: ChildExecFile): ChildExecFile {
  return function tokenashExecFile(this: unknown, ...args: unknown[]) {
    const state = getState();
    if (!state) {
...
function wrapFork(originalFork: ChildFork): ChildFork {
  return function tokenashFork(this: unknown, ...args: unknown[]) {
    const state = getState();
    if (!state) {
...
function wrapRequest(
  ...
  return function tokenashRequest(this: unknown, ...args: unknown[]) {
    const state = getState();
    if (!state) {
...
function wrapGet(request: HttpRequest | HttpsRequest): HttpGet | HttpsGet {
  return function tokenashGet(this: unknown, ...args: unknown[]) {
...
function wrapHttp2Connect(originalConnect: Http2Connect): Http2Connect {
  return function tokenashHttp2Connect(this: unknown, authority: string | URL, ...args: unknown[]) {
    const state = getState();
    if (state) {
      const proxy = normalizeProxyUrl(state.proxyUrl);
      ...
      if (shouldRoute(upstream, proxy)) {
        throw new Error(
          `Tokenash OpenCode wrap blocked direct HTTP/2 connection to ${upstream.origin}. ` +
            "Use fetch, http, or https so traffic can be routed through Tokenash.",
        );
      }
    }
...
export function installTokenashTransport(options: InstallOptions): () => void {
  ...
  if (existing) {
    ...
    return () => uninstallTokenashTransport();
  }
...
export function uninstallTokenashTransport(): void {
  const state = getState();
  if (!state) {
    return;
  }
