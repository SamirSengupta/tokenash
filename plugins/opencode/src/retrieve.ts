import type { CompressResult } from "tokenash";
...
export function getDefaultProxyUrl(): string {
  return _proxyUrlCache ?? process.env.TOKENASH_BASE_URL ?? "http://localhost:8787";
}
...
export function createTokenashRetrieveTool(config: RetrieveToolConfig) {
  const origin = config.proxyBaseUrl.replace(/\/+$/, "");

  return {
    name: "tokenash_retrieve",
    description:
      "Retrieve original uncompressed content from Tokenash's compression store. " +
...
export async function compressWithTokenash(
  messages: unknown[],
  options: {
    model?: string;
    tokenBudget?: number;
