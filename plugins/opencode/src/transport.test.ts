...
import { installTokenashTransport, uninstallTokenashTransport } from "./transport.js";

afterEach(() => {
  uninstallTokenashTransport();
  vi.restoreAllMocks();
});
...
describe("Tokenash OpenCode transport", () => {
  it("routes external fetch calls through the proxy without pre-registering providers", async () => {
    ...
    installTokenashTransport({ proxyUrl: "http://127.0.0.1:8787/v1" });
    ...
    expect(new Headers(fetchMock.mock.calls[0][1]?.headers).get("x-tokenash-base-url")).toBe(
      "https://api.deepseek.com",
    );
    expect(fetchMock.mock.calls[1][0]).toEqual(new URL("http://127.0.0.1:8787/base/v1/messages"));
    expect(new Headers(fetchMock.mock.calls[1][1]?.headers).get("x-tokenash-base-url")).toBe(
  ...
  it("bypasses local, OpenCode, and Tokenash proxy fetch URLs", async () => {
    const originalFetch = globalThis.fetch;
    const fetchMock = vi.fn(async (..._args: FetchCall) => new Response("ok"));
    globalThis.fetch = fetchMock as unknown as typeof fetch;
  ...
  it("routes external https.request calls through the proxy", async () => {
    ...
    installTokenashTransport({ proxyUrl: proxy.url });
    ...
    expect(proxy.seen[0].headers["x-tokenash-base-url"]).toBe("https://api.anthropic.com");
  ...
  it("preloads the Tokenash shim into child Node processes", () => {
    const originalNodeOptions = process.env.NODE_OPTIONS;
    const originalProxyUrl = process.env.TOKENASH_OPENCODE_TRANSPORT_PROXY_URL;

    try {
      process.env.NODE_OPTIONS = "--trace-warnings";
      delete process.env.TOKENASH_OPENCODE_TRANSPORT_PROXY_URL;

      installTokenashTransport({ proxyUrl: "http://127.0.0.1:8787/v1" });

      expect(process.env.TOKENASH_OPENCODE_TRANSPORT_PROXY_URL).toBe("http://127.0.0.1:8787/v1");
      ...
      if (originalProxyUrl === undefined) {
        ...
        process.env.TOKENASH_OPENCODE_TRANSPORT_PROXY_URL = originalProxyUrl;
      }
      uninstallTokenashTransport();
    }
  });

  it("injects the Tokenash shim into child processes with custom env", () => {
    ...
    try {
      ...
      expect(options.env.TOKENASH_OPENCODE_TRANSPORT_PROXY_URL).toBe("http://127.0.0.1:8787/v1");
  ...
  it("restores patched transports only after the final disposer", () => {
    ...
    const firstDispose = installTokenashTransport({ proxyUrl: "http://127.0.0.1:8787/v1" });
    const secondDispose = installTokenashTransport({ proxyUrl: "http://127.0.0.1:8788/v1" });
