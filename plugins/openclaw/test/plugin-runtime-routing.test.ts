...
const mocked = vi.hoisted(() => ({
  ...
  createTokenashRetrieveTool: vi.fn(({ proxyUrl }: { proxyUrl: string }) => ({ proxyUrl })),
}));

const proxyReadyListeners: Array<(proxyUrl: string) => void | Promise<void>> = [];

vi.mock("../src/engine.js", () => ({
  TokenashContextEngine: class {
...
vi.mock("../src/tools/tokenash-retrieve.js", () => ({
  createTokenashRetrieveTool: mocked.createTokenashRetrieveTool,
}));

import tokenashPlugin from "../src/plugin/index.js";
...
afterEach(() => {
  ...
  mocked.createTokenashRetrieveTool.mockClear();
  proxyReadyListeners.length = 0;
});

describe("tokenashPlugin runtime routing", () => {
  function stubConfiguredProxyProbe(response: "tokenash" | "non-tokenash" | "down") {
    if (response === "down") {
      ...
      vi.fn((url: string) => {
        ...
        if (url.endsWith("/v1/retrieve/stats")) {
          return Promise.resolve({
            ok: response === "tokenash",
            status: response === "tokenash" ? 200 : 404,
        ...
        if (url.endsWith("/stats")) {
          return Promise.resolve({
            ok: response === "tokenash",
            status: response === "tokenash" ? 200 : 200,
            text: () =>
              Promise.resolve(response === "tokenash" ? JSON.stringify({ proxy_inbound: { total: 1 } }) : "{}"),
          });
        }
  ...
  it("routes configured proxyUrl only after it probes as Tokenash", async () => {
    const gatewayHandlers = new Map<string, () => Promise<void>>();
    stubConfiguredProxyProbe("tokenash");
  ...
  it("does not route configured proxyUrl when only generic liveness endpoints respond", async () => {
    const gatewayHandlers = new Map<string, () => Promise<void>>();
    stubConfiguredProxyProbe("non-tokenash");
    ...
    expect(api.logger.warn).toHaveBeenCalledWith(
      expect.stringContaining("configured proxyUrl is not a ready Tokenash proxy"),
    );
  });
  ...
  it("documents that the retrieve tool can be created from configured proxyUrl before routing is validated", () => {
    ...
    expect(mocked.createTokenashRetrieveTool).toHaveBeenCalledWith({
      proxyUrl: "http://127.0.0.1:8787",
    });
  });
});
