/**
 * TokenashContextEngine — ContextEngine implementation for OpenClaw.
 *
 * Compresses tool outputs and conversation context using the Tokenash proxy.
...
export interface TokenashEngineConfig extends ProxyManagerConfig {
  enabled?: boolean;
}

export class TokenashContextEngine {
  readonly info = {
    id: "tokenash",
    name: "Tokenash Context Compression",
    version: "0.1.0",
    ownsCompaction: true,
  };
  ...
  private config: TokenashEngineConfig;
  ...
  constructor(config: TokenashEngineConfig = {}, logger?: ProxyManagerLogger) {
    this.config = config;
      ...
      return {
        ...
        systemPromptAddition:
          result.tokensSaved > 100
            ? `[Context compressed by Tokenash: ${result.tokensSaved} tokens saved. Use tokenash_retrieve with the hash to get full details.]`
            : undefined,
      };
    } catch (error) {
   ...
   * - CCR: stores originals for retrieval via tokenash_retrieve tool
   *
  ...
  async compact(params: {
    ...
    return {
      ok: true,
      compacted: true,
      reason: "Tokenash applies SmartCrusher + Kompress + RollingWindow on next assemble()",
    };
  }
  ...
  ensureProxyStarted(): void {
    ...
    this.proxyStartupPromise = this.proxyManager
      ...
      .then(async (proxyUrl) => {
        ...
        this.logger.info(`Tokenash proxy ready at ${proxyUrl}`);
        return proxyUrl;
      })
      .catch((error) => {
        this.proxyStartupError = error;
        this.logger.warn(`Tokenash proxy unavailable: ${error}`);
        throw error;
      })
      .finally(() => {
  ...
  async ensureProxyUrl(): Promise<string> {
    ...
    if (!this.proxyStartupPromise) {
      throw new Error("Tokenash proxy startup is disabled");
    }
  ...
  private async notifyProxyReady(proxyUrl: string): Promise<void> {
    for (const listener of this.proxyReadyListeners) {
      try {
        ...
        this.logger.warn(`Tokenash proxy ready listener failed: ${error}`);
      }
    }
  }
}
