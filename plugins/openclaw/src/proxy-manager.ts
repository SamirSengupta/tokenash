/**
 * Manages connectivity to a Tokenash proxy (local or remote).
 *
 * Security model:
...
/** Default logger that prefixes all messages with `[tokenash]`. */
export const defaultLogger: ProxyManagerLogger = {
  info: (m) => console.log(`[tokenash] ${m}`),
  warn: (m) => console.warn(`[tokenash] ${m}`),
  error: (m) => console.error(`[tokenash] ${m}`),
  debug: () => {},
};
...
const TOKENASH_MODULE_DISCOVERY_SNIPPET =
  "import importlib.util, sys; sys.exit(0 if importlib.util.find_spec('tokenash') else 1)";
...
export class ProxyManager {
  ...
  async start(): Promise<string> {
    ...
    for (const url of candidateUrls) {
      const probe = await probeTokenashProxy(url);
      probeByUrl.set(url, probe);
      if (probe.reachable && probe.isTokenash) {
        this.proxyUrl = url;
        this.logger.info(`Tokenash proxy already running at ${url}`);
        return url;
      }
    }
    ...
    if (explicitUrl) {
      ...
      if (explicitProbe?.reachable && !explicitProbe.isTokenash) {
        throw new Error(
          `Service reachable at ${explicitUrl}, but it does not appear to be a Tokenash proxy (${explicitProbe.reason ?? "unknown service"}).`,
        );
      }
    }
    ...
    if (explicitUrl && !isLocalProxyUrl(explicitUrl)) {
      throw new Error(
        `Remote Tokenash proxy not reachable at ${explicitUrl}. Ensure the proxy is running at that address.`,
      );
    }
    ...
    if (this.config.autoStart === true) {
      ...
      if (startupProbe?.reachable && !startupProbe.isTokenash) {
        throw new Error(
          `Cannot auto-start Tokenash at ${startupUrl}: port is in use by a non-Tokenash service (${startupProbe.reason ?? "unknown service"}).`,
        );
      }

      this.logger.info(
        `No Tokenash proxy detected${explicitUrl ? ` at ${startupUrl}` : " on default local endpoints"}; attempting to auto-start...`,
      );
      await this.startTokenashProxy(startupUrl, port);

      const startedProbe = await waitForTokenashProxy(
        startupUrl,
        this.config.startupTimeoutMs ?? 20_000,
      );
      if (startedProbe.reachable && startedProbe.isTokenash) {
        this.proxyUrl = startupUrl;
        this.logger.info(`Tokenash proxy started and reachable at ${startupUrl}`);
        return startupUrl;
      }
      throw new Error(
        `Attempted to start Tokenash proxy, but it was not reachable at ${startupUrl} (${startedProbe.reason ?? "unknown"}).`,
      );
    }

    if (explicitUrl) {
      throw new Error(
        `Tokenash proxy not reachable at ${explicitUrl}. Ensure the proxy is running first.`,
      );
    }

    throw new Error(
      `Tokenash proxy not detected on default endpoints (${defaultCandidates.join(", ")}). ` +
  ...
  private async startTokenashProxy(proxyUrl: string, defaultPort: number): Promise<void> {
    ...
    throw new Error(
      "No usable Tokenash launcher found. Tried PATH, local npm, global npm, and Python. " +
        "Install tokenash (npm or pip) and ensure one launcher is available.\n" +
  ...
  private buildLaunchSpecs(host: string, port: string): LaunchSpec[] {
    ...
    if (configuredPython) {
      specs.push({
        label: `Configured Python: ${configuredPython} -m tokenash.cli`,
        command: configuredPython,
        args: ["-m", "tokenash.cli", ...commonArgs],
        checkCommand: configuredPython,
        checkArgs: ["-c", TOKENASH_MODULE_DISCOVERY_SNIPPET],
      });
    }

    // 2) Windows pyenv: resolve the real executable so we avoid shim .bat wrappers.
    if (process.platform === "win32") {
      const pyenvTokenash = this.getPyenvResolvedTokenash();
      if (pyenvTokenash) {
        specs.push({
          label: `pyenv: ${pyenvTokenash}`,
          command: pyenvTokenash,
          args: commonArgs,
          checkCommand: pyenvTokenash,
          checkArgs: ["--version"],
          useShell: false,
        });
      }
    ...
    specs.push({
      ...
      checkArgs: process.platform === "win32"
        ? ["tokenash"]
        : ["-lc", "command -v tokenash >/dev/null 2>&1"],
    ...
    const localBins = process.platform === "win32"
      ? [join(localBinDir, "tokenash.cmd"), join(localBinDir, "tokenash")]
      : [join(localBinDir, "tokenash")];
    ...
    if (npmPrefix) {
      const globalBins = process.platform === "win32"
        ? [join(npmPrefix, "tokenash.cmd"), join(npmPrefix, "tokenash")]
        : [join(npmPrefix, "bin", "tokenash"), join(npmPrefix, "tokenash")];
    ...
    for (const pyCmd of pythonCommands) {
      ...
      specs.push({
        label: `Python: ${pyCmd} -m tokenash.cli`,
        command: pyCmd,
        args: ["-m", "tokenash.cli", ...commonArgs],
        checkCommand: pyCmd,
        checkArgs: ["-c", TOKENASH_MODULE_DISCOVERY_SNIPPET],
  ...
  private getPyenvResolvedTokenash(): string | null {
    if (process.platform !== "win32") return null;

    try {
      const result = spawnSync("pyenv", ["which", "tokenash"], {
        encoding: "utf8",
...
/**
 * Probe a configured URL and verify whether it is a running Tokenash proxy.
 */
export async function probeTokenashProxy(proxyUrl: string): Promise<ProxyProbeResult> {
  ...
  if (retrieveStats.ok && hasTokenashStatsShape(retrieveStats.body)) {
    return { reachable: true, isTokenash: true };
  }

  const stats = await probeEndpoint("/stats", { readBody: true });
  if (stats.ok && hasTokenashStatsShape(stats.body)) {
    return { reachable: true, isTokenash: true };
  }
  ...
  if (!anyReachable) {
    return { reachable: false, isTokenash: false, reason: "proxy probe failed" };
  }
  ...
  return { reachable: true, isTokenash: false, reason: reasons.join("; ") };
}

function hasTokenashStatsShape(body: string | undefined): boolean {
  if (!body) {
    return false;
  }
...
async function waitForTokenashProxy(proxyUrl: string, timeoutMs: number): Promise<ProxyProbeResult> {
  const start = Date.now();
  while (Date.now() - start < timeoutMs) {
    const result = await probeTokenashProxy(proxyUrl);
    if (result.reachable && result.isTokenash) {
      return result;
    }
    await new Promise((resolve) => setTimeout(resolve, 500));
  }
  return probeTokenashProxy(proxyUrl);
}
