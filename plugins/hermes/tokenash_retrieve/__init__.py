"""Tokenash CCR retrieve plugin.

The tokenash proxy (127.0.0.1:8787) compresses large tool outputs in LLM
...
TOKENASH_RETRIEVE_SCHEMA = {
    "name": "tokenash_retrieve",
    "description": (
        "Retrieve the original uncompressed content behind a tokenash "
...
def _handle_tokenash_retrieve(args: dict, **kw) -> str:
    ...
    try:
        ...
        return tool_error(
            f"tokenash proxy unreachable at {_PROXY_URL} ({type(exc).__name__}). "
    ...
    if resp.status_code != 200:
        return tool_error(f"tokenash proxy returned HTTP {resp.status_code}: {resp.text[:200]}")
...
def register(ctx) -> None:
    """Register the tokenash_retrieve tool. Called by the plugin loader."""
    ctx.register_tool(
        name="tokenash_retrieve",
        toolset="tokenash",
        schema=TOKENASH_RETRIEVE_SCHEMA,
        handler=_handle_tokenash_retrieve,
        emoji="🗜️",
    )
