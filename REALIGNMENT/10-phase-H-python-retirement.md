...
**Worktree:** `~/claude-projects/tokenash-worktrees/realign-H1-retire-python-proxy-request-path`
**Risk:** **HIGH** (production-facing change; canary deploy mandatory)
**LOC:** **-15,000 / +500**

### Scope
Delete the Python FastAPI server, all handlers, the responses converter (already gone in Phase C PR-C5), memory subsystem (replaced by live-zone tail injection from Phase A PR-A2 and Phase B PR-B6), semantic cache, batch handler, etc. The Rust proxy becomes the canonical request-path implementation. Operators run only `tokenash-proxy` (Rust binary).

### Files

**Delete:**
- `tokenash/proxy/server.py` (2864 LOC)
- `tokenash/proxy/handlers/anthropic.py` (2423 LOC)
- `tokenash/proxy/handlers/openai.py` (2742 LOC)
- `tokenash/proxy/handlers/streaming.py` (1131 LOC)
- `tokenash/proxy/handlers/gemini.py` (839 LOC)
- `tokenash/proxy/handlers/batch.py` (1010 LOC)
- `tokenash/proxy/responses_converter.py` — already deleted in PR-C5; verify gone.
- `tokenash/proxy/memory_handler.py` (1756 LOC)
- `tokenash/proxy/memory_tool_adapter.py` (1273 LOC)
- `tokenash/proxy/semantic_cache.py` (142 LOC)
- `tokenash/proxy/savings_tracker.py` (934 LOC) — re-implemented in Rust as part of `observability/`.
- `tokenash/proxy/loopback_guard.py` (92 LOC) — Rust equivalent in `crates/tokenash-proxy/src/handlers/debug.rs`.
- `tokenash/proxy/ws_session_registry.py` (226 LOC) — Rust equivalent.
- `tokenash/proxy/interceptors/` — all of this directory.
- `tokenash/proxy/cost.py`, `helpers.py`, `rate_limiter.py`, `request_logger.py` — re-implemented in Rust as part of compression dispatch / observability.
- `tokenash/proxy/prometheus_metrics.py` — re-implemented in Rust as `observability/prometheus.rs`.
- `tokenash/proxy/extensions.py`, `models.py`, `modes.py`, `stage_timer.py`, `warmup.py`, `responses_converter.py`, `debug_introspection.py`.
- `tokenash/transforms/cache_aligner.py` — already gutted in Phase A PR-A2; delete the remaining stub.

**Move:**
- `tokenash/proxy/loopback_guard.py` test logic → `crates/tokenash-proxy/tests/integration_loopback_guard.rs`.

**Modify:**
- `tokenash/cli/proxy.py` — `tokenash proxy start` now spawns the Rust binary (`./target/release/tokenash-proxy`) instead of `uvicorn tokenash.proxy.server:app`.
- `tokenash/cli/wrap/*.py` — same: env-var setup remains, but `proxy_url` points at the Rust binary's listen address.
...
- Manual test: `tokenash proxy start` boots the Rust binary; `curl -s http://127.0.0.1:8787/healthz` returns OK.
...
**Worktree:** `~/claude-projects/tokenash-worktrees/realign-H2-retire-litellm-backends`
**Risk:** **MEDIUM**
**LOC:** **-3,000 / +100**
...
**Delete:**
- `tokenash/backends/litellm.py` (~1500 LOC, the lossy converter)
- `tokenash/backends/__init__.py` if the only contents was the LiteLLM backend.

**Modify:**
- `pyproject.toml` — remove `litellm` from dependencies. (Saves ~50 MB of installed-deps size.)
- `tokenash/providers/registry.py` — delete `litellm-bedrock`, `litellm-vertex` provider entries.
- `tokenash/cli/wrap/*` — verify no wrap CLIs route to LiteLLM (they shouldn't; they go through the proxy).
...
**Worktree:** `~/claude-projects/tokenash-worktrees/realign-H3-final-cleanup`
**Risk:** **LOW**
**LOC:** -2,000 / +500
...
- `tokenash/__init__.py` — drop unused module imports.
...
- `crates/tokenash-parity/` — the parity harness itself becomes irrelevant after Python side is gone. **Decision needed** (see `12-decisions-needed.md` Q3): keep parity-run as a "prior-version-vs-current-version" harness, or delete?

**Add:**
- `CHANGELOG.md` entry: "**Breaking**: Python proxy retired. Operators must use the Rust binary `tokenash-proxy`. See migration guide at `docs/operations/python-to-rust-migration.md`."
- `docs/operations/python-to-rust-migration.md` — operator migration guide.

### Acceptance criteria

- `git grep -i "uvicorn\|fastapi" tokenash/` returns nothing in non-test code.
...
| Module | Role | Reason |
|---|---|---|
| `tokenash/cli/wrap/*.py` | Agent launchers | Off-path; orchestrates filesystem + subprocess. Python is the right tool. |
| `tokenash/cli/{evals,init,install,learn,memory,perf,proxy,tools}.py` | CLI admin commands | Click-based; off-path. |
| `tokenash/rtk/installer.py` | RTK binary downloader | Off-path; filesystem operations. |
| `tokenash/providers/codex/install.py`, `claude/install.py` | Client config installation | Off-path; filesystem. |
| `tokenash/evals/`, `learn/`, `memory/` writers | Research / batch tooling | Off-path; long-running batch. |
| `tokenash/tokenizers/` | Parity backstop | Used only by parity harness if H3 keeps it. |
| `tokenash/telemetry/toin.py` | TOIN learning loop | Off-path; observation-only after Phase B PR-B5. |
| `tokenash/subscription/tracker.py`, `client.py` | Subscription usage poller | Off-path. |
| `tokenash/copilot_auth.py` | Copilot OAuth refresh | Off-path; specific to Copilot integration. |
...
- ✅ Operators run only the Rust `tokenash-proxy` binary
