...
Sources: 10 parallel deep-audit subagents (Rust proxy passthrough; Rust compression correctness; Python proxy + bridges; prefix cache safety; streaming + wire-format; RTK + tests/parity; over-engineering; OpenAI long-tail + Bedrock; Tokenash-side injections; auth-mode handling).
...
- **File:** `tokenash/proxy/server.py:1050-1058`; `tokenash/proxy/handlers/openai.py:1212`
...
- **File:** `tokenash/proxy/server.py:1088, 1090`; `tokenash/proxy/handlers/streaming.py:651`; `tokenash/proxy/handlers/openai.py:2392-2397`; `tokenash/proxy/handlers/batch.py:344`
...
- **File:** `crates/tokenash-proxy/src/compression/anthropic.rs:151-156`
...
- **File:** `crates/tokenash-proxy/src/compression/anthropic.rs:146-157`; `crates/tokenash-core/src/context/strategy/drop_by_score.rs:64-80`; `crates/tokenash-core/src/context/manager.rs`; `tokenash/transforms/intelligent_context.py:354-450`
...
- **File:** `crates/tokenash-proxy/src/compression/anthropic.rs:91, 172`
...
- **File:** `tokenash/proxy/memory_handler.py:389-398`; `tokenash/proxy/handlers/anthropic.py:1147-1171`
...
- **File:** `tokenash/proxy/responses_converter.py:94, 221-256`
...
- **File:** `tokenash/proxy/handlers/streaming.py:58, 772`; `tokenash/ccr/response_handler.py:672`
...
- **File:** `tokenash/proxy/handlers/streaming.py:213-298`
...
- **File:** `tokenash/proxy/handlers/streaming.py:300-391` (`_response_to_sse`)
...
- **File:** `tokenash/backends/litellm.py:860`
...
- **File:** `tokenash/proxy/handlers/openai.py:2422-2447`
...
- **File:** `crates/tokenash-proxy/src/compression/anthropic.rs:90-188`
...
- **File:** `tokenash/proxy/handlers/streaming.py:160-211, 213-298`
- **Evidence:** No `event_type == "error"` arm. The wire passthrough is byte-faithful (good) but Tokenash's bookkeeping (`stream_state.input_tokens` etc.) silently doesn't reflect the failure; `_finalize_stream_response` reports a clean PERF line for an errored stream.
- **Guide:** §1.7, §2.21.
- **Fix:** Add `error` handling to telemetry.
- **Phase C → PR-C1 (Rust SSE).**

### P1-15. Connection drop without `message_stop`/`[DONE]` not surfaced
- **File:** `tokenash/proxy/handlers/streaming.py:899` (`finally: await self._finalize_stream_response`)
...
- **File:** `tokenash/proxy/handlers/streaming.py:227, 700`
...
- **Files:** `tokenash/transforms/intelligent_context.py`; `crates/tokenash-core/src/context/manager.rs`; `crates/tokenash-proxy/src/compression/icm.rs`
- **Status:** Delete in Phase B (PR-B1).

### P2-19. `RollingWindow`, `ProgressiveSummarizer` (head-truncation strategies)
- **Files:** `tokenash/transforms/rolling_window.py` (395 LOC); `tokenash/transforms/progressive_summarizer.py` (508 LOC)
...
- **Files:** `crates/tokenash-core/src/scoring/{scorer,score,weights,traits,mod}.rs` (~1500 LOC); `crates/tokenash-core/src/relevance/{embedding,bm25,hybrid,base,mod}.rs` (~1600 LOC); `tokenash/transforms/scoring.py` (459 LOC)
...
### P2-21. `crates/tokenash-core/src/context/` — except `safety.rs`
- **Files:** `crates/tokenash-core/src/context/{config,workspace,candidate,ccr_drop,manager,strategy/}.rs` (~1500 LOC)
- **Status:** Delete in Phase B (PR-B1). `safety.rs` (tool-pair atomicity) is moved to `crates/tokenash-core/src/transforms/safety.rs` and kept.

### P2-22. `ToolCrusher` operates without `frozen_message_count`
- **File:** `tokenash/transforms/tool_crusher.py:106`
...
- **File:** `tokenash/transforms/cache_aligner.py:160-262`
...
- **File:** `tokenash/proxy/memory_handler.py:498-510`; `tokenash/proxy/handlers/openai.py:535-540`
...
- **File:** `tokenash/ccr/tool_injection.py:302-328`
...
- **File:** `crates/tokenash-core/src/context/manager.rs:172-185`; `crates/tokenash-proxy/src/proxy.rs:285`
...
- **File:** `tokenash/telemetry/toin.py:853-927`
...
- **File:** Missing entirely in `crates/tokenash-proxy/`
...
- **Evidence:** Compression acceptance is `bytes_saved > 0` (`crates/tokenash-core/src/transforms/pipeline/orchestrator.rs:158-165`); ICM aggregate-checks tokens (`anthropic.rs:162-168`) but per-block transforms don't.
...
- **File:** `tokenash/backends/litellm.py:486-628`
...
- **File:** Same — `tokenash/backends/litellm.py`
- **Phase D → PR-D4 builds native Vertex.**

### P4-39. No native Bedrock/Vertex paths in Rust
- **File:** `crates/tokenash-proxy/src/compression/mod.rs:50` only matches `/v1/messages`.
- **Phase D → PR-D1-D4.**

### P4-40. `/v1/conversations` blind spot (§4.14)
- **Evidence:** Zero references. Server-side prepended items invisible to Tokenash; tokenizer count over-reports.
- **Phase C → PR-C4.**
...
- **File:** `tokenash/backends/litellm.py:600`; `tokenash/learn/plugins/codex.py:283`
- **Guide:** §4.4.
...
- **File:** `tokenash/proxy/responses_converter.py:94, 235`
...
- **File:** `tokenash/proxy/request_logger.py` — no base64/image redaction
- **Guide:** §11.6.
...
- **File:** `tokenash/proxy/responses_converter.py:99` — "Unknown item type: preserve"
...
### P5-49. `X-Tokenash-*` request headers leak upstream
- **File:** `tokenash/proxy/handlers/anthropic.py:526` — `dict(request.headers.items())` captured unmodified, no strip step before `httpx.post(headers=headers)`.
- **Risk:** Subscription-revocation fingerprint.
- **Phase A → PR-A5.**

### P5-50. `anthropic-beta` mutated when memory enabled, not session-sticky
- **File:** `tokenash/proxy/handlers/anthropic.py:1162-1168`
- **Status:** Already covered by P0-6. **Phase A → PR-A6, A7.**

### P5-51. `OpenAI-Beta` auto-injection on WS path
- **File:** `tokenash/proxy/handlers/openai.py:1566-1567`
...
- **File:** `tokenash/telemetry/toin.py:477, 496`
...
- **File:** `crates/tokenash-proxy/src/proxy.rs:355-358, 377-383`
- **Guide:** §11.10.
...
- **Files:** `tokenash/cli/wrap.py` — only Claude/Codex/Aider/Copilot/Cursor today.
- **Phase G → PR-G1.**
...
- **File:** `crates/tokenash-parity/src/lib.rs:172-174`
...
- **File:** `tokenash/proxy/prometheus_metrics.py` — only aggregate by provider
- **Phase G → PR-G3.**
