...
about: Report results of testing `tokenash wrap copilot --subscription` on Linux/Windows/macOS
...
- **How you installed tokenash**: (pipx/pip `--pre` wheel · Docker install.sh/ps1 · built from source)
- **tokenash version**: (`tokenash --version`)
...
- **Command run**:
  ```
  tokenash wrap copilot --subscription -- --model gpt-4o -p "Reply with exactly: TOKENASH_OK"
  ```
- **Did it print `TOKENASH_OK`?**: yes / no
...
(logs from `~/.tokenash/logs/proxy.log`, surprises, etc.)
