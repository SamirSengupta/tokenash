...
from tokenash.cache.compression_feedback import (
...
from tokenash.transforms.smart_crusher import (
    SmartCrusherConfig,
...
def test_tokenash_marker_collision() -> AdversarialResult:
    """
    ATTACK: Input data already contains __tokenash_ fields.
    """
    result = AdversarialResult(
        name="Marker Field Collision",
        category="injection",
        expected_behavior="Should not confuse existing __tokenash_ fields with our markers",
        severity="high",
    )

    # Data that already has __tokenash_ fields
    items = [
        {
            "id": i,
            "__tokenash_compressed": True,  # Fake marker!
            "__tokenash_hash": "fakehash12345678",
            "__tokenash_stats": {"fake": True},
        }
        for i in range(100)
    ]
...
def main():
    ...
    tests = [
        ...
        test_tokenash_marker_collision,
