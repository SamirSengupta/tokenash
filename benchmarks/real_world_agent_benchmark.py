"""
Real-World Agent Benchmark: MCP Tools + Tokenash
...
We measure:
- Token usage with vs without Tokenash
- Cost savings
...
try:
    ...
    TOKENASH_AVAILABLE = True
except ImportError:
    TOKENASH_AVAILABLE = False
...
def run_agent_scenario(
    ...
    # Determine if using TokenashClient
    is_tokenash = isinstance(client, TokenashClient) if TOKENASH_AVAILABLE else False
    mode = "tokenash" if is_tokenash else "baseline"
...
def run_full_benchmark(api_key: str = None) -> dict:
    """Run complete benchmark comparing baseline vs Tokenash."""
    ...
    if not TOKENASH_AVAILABLE:
        raise RuntimeError("Tokenash not available")
    ...
    # Tokenash-wrapped client
    db_path = os.path.join(tempfile.gettempdir(), "tokenash_benchmark.db")
    tokenash_client = TokenashClient(
    ...
    print("\n" + "=" * 70)
    print("REAL-WORLD AGENT BENCHMARK: MCP Tools + Tokenash")
    print("=" * 70)
    ...
    for scenario in scenarios:
        ...
        # Run with Tokenash
        print("\n[2/2] Running TOKENASH (optimized)...")
        tokenash_result = run_agent_scenario(tokenash_client, scenario)
        print(f"   Input tokens: {tokenash_result.total_input_tokens:,}")
        print(f"   Output tokens: {tokenash_result.total_output_tokens:,}")
        print(f"   Cost: ${tokenash_result.cost_usd:.4f}")
        print(f"   Answer quality: {tokenash_result.answer_quality:.1%}")
        results.append(tokenash_result)

        # Calculate savings
        if baseline_result.total_input_tokens > 0:
            token_savings = 1 - (
                tokenash_result.total_input_tokens / baseline_result.total_input_tokens
            )
            cost_savings = (
                1 - (tokenash_result.cost_usd / baseline_result.cost_usd)
            ...
            print(
                f"   Quality preserved: {'✓' if tokenash_result.answer_quality >= baseline_result.answer_quality * 0.9 else '✗'}"
            )
    ...
    tokenash_results = [r for r in results if r.mode == "tokenash"]

    total_baseline_tokens = sum(r.total_input_tokens for r in baseline_results)
    total_tokenash_tokens = sum(r.total_input_tokens for r in tokenash_results)
    total_baseline_cost = sum(r.cost_usd for r in baseline_results)
    total_tokenash_cost = sum(r.cost_usd for r in tokenash_results)

    print(f"\n{'Metric':<25} {'Baseline':>15} {'Tokenash':>15} {'Savings':>15}")
    print("-" * 70)

    token_savings = (
        (1 - total_tokenash_tokens / total_baseline_tokens) if total_baseline_tokens > 0 else 0
    )
    cost_savings = (1 - total_tokenash_cost / total_baseline_cost) if total_baseline_cost > 0 else 0

    print(
        f"{'Total Input Tokens':<25} {total_baseline_tokens:>15,} {total_tokenash_tokens:>15,} {token_savings:>14.1%}"
    )
    print(
        f"{'Total Cost':<25} ${total_baseline_cost:>14.4f} ${total_tokenash_cost:>14.4f} {cost_savings:>14.1%}"
    )
    ...
    avg_tokenash_quality = (
        sum(r.answer_quality for r in tokenash_results) / len(tokenash_results)
        if tokenash_results
        else 0
    )
    print(
        f"{'Avg Answer Quality':<25} {avg_baseline_quality:>14.1%} {avg_tokenash_quality:>14.1%} {'preserved' if avg_tokenash_quality >= avg_baseline_quality * 0.9 else 'degraded':>15}"
    )

    return {
        "baseline": [r.__dict__ for r in baseline_results],
        "tokenash": [r.__dict__ for r in tokenash_results],
        "summary": {
            "total_baseline_tokens": total_baseline_tokens,
            "total_tokenash_tokens": total_tokenash_tokens,
            "token_savings": token_savings,
            "total_baseline_cost": total_baseline_cost,
            "total_tokenash_cost": total_tokenash_cost,
