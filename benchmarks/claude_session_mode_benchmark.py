...
from tokenash.cache.prefix_tracker import PrefixCacheTracker
from tokenash.pricing.litellm_pricing import get_model_pricing
...
try:
    from tokenash.proxy.modes import PROXY_MODE_CACHE, PROXY_MODE_TOKEN
except ImportError:
...
def _make_proxy(mode: str) -> TokenashProxy:
    cfg = ProxyConfig(
        mode=mode,
        optimize=True,
        image_optimize=True,
    ...
    return TokenashProxy(cfg)


def _apply_mode_to_messages(
    proxy: TokenashProxy | None,
    mode: str,
...
def _disable_tokenash_benchmark_logging() -> None:
    logging.raiseExceptions = False
    for logger_name in (
        "tokenash",
        "tokenash.cache",
        "tokenash.cache.compression_cache",
        "tokenash.proxy",
        "tokenash.transforms",
    ):
...
def _simulate_single_replay_mode(
    ...
    _disable_tokenash_benchmark_logging()
    ...
    for mode in ("baseline", PROXY_MODE_TOKEN, PROXY_MODE_CACHE):
        ...
        if worker_count > 1 and total > 1:
            with concurrent.futures.ProcessPoolExecutor(
                ...
                initializer=_disable_tokenash_benchmark_logging,
            ) as executor:
...
<body>
  <div class="shell">
    <section class="hero">
      ...
      <p>Observed usage is read directly from <code>~/.claude/projects</code>. Baseline, token, and cache are replayed locally through Tokenash without making API calls.</p>
...
def main() -> int:
    args = parse_args()
    logging.getLogger("tokenash.transforms").setLevel(logging.WARNING)
    logging.getLogger("tokenash.proxy").setLevel(logging.WARNING)
