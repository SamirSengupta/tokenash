"""
Truncation vs Summarization vs Tokenash: A Fair Benchmark
...
3. Tokenash - Statistical compression with retrieval
...
try:
    ...
    from tokenash.tokenizers import TiktokenCounter
...
try:
    from tokenash.transforms.kompress_compressor import KompressCompressor, is_kompress_available
...
def generate_metrics_data(n_points: int = 500) -> tuple[list[dict], list[Question]]:
    ...
    This is where Tokenash should excel - detecting statistical outliers.
    """
...
def kompress_compress(data: list[dict]) -> tuple[str, dict]:
    ...
    if not KOMPRESS_AVAILABLE:
        raise RuntimeError("Kompress not available. Install with: pip install tokenash[ml]")
...
def tokenash_compress(data: list[dict], query_context: str = "") -> tuple[list[dict], dict]:
    """
    Use Tokenash's SmartCrusher for statistical compression.
...
def run_scenario_benchmark(
    ...
    if config.run_kompress:
        ...
        if KOMPRESS_AVAILABLE:
            ...
            print("   Kompress not available. Install with: pip install tokenash[ml]")

    # --- TOKENASH ---
    print("\n[4/4] Running Tokenash...")
    if TOKENASH_AVAILABLE:
        try:
            # Use first question as query context (realistic usage)
            query_context = scenario.questions[0].text if scenario.questions else ""
            compressed, metadata = tokenash_compress(scenario.data, query_context)
            ...
            print(f"   Tokenash failed: {e}")
            import traceback

            traceback.print_exc()
    else:
        print("   Tokenash not available")
...
def run_full_benchmark(client: "OpenAI", config: BenchmarkConfig = None) -> dict:
    ...
    print("\n" + "=" * 70)
    print("TRUNCATION vs SUMMARIZATION vs LLMLINGUA-2 vs TOKENASH BENCHMARK")
    print("=" * 70)

    # Generate scenarios
    scenarios = []

    # Scenario 1: Logs (Tokenash should win - needs anomaly detection)
    ...
    # Scenario 3: Metrics (Tokenash should win - statistical outliers)
...
def generate_summary(results: list[ApproachResult], scenarios: list[Scenario]) -> str:
    ...
    for approach in ["truncation", "summarization", "llmlingua-2", "tokenash"]:
