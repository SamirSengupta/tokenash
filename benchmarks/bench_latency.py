#!/usr/bin/env python3
"""Latency benchmark for Tokenash compression pipeline.
...
class ProfilingPipeline:
    ...
    def apply(
        ...
        from tokenash.utils import deep_copy_messages
...
def format_terminal_report(results: list[LatencyResult]) -> str:
    ...
    lines.append("")
    lines.append("=" * 100)
    lines.append("  TOKENASH LATENCY BENCHMARK")
    lines.append("=" * 100)
    lines.append("")
...
def format_markdown_report(results: list[LatencyResult]) -> str:
    ...
    lines.append("# Tokenash Latency Benchmarks")
    lines.append("")
    lines.append(
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Tokenash latency benchmark",
    ...
    print("Tokenash Latency Benchmark")
    print("=" * 40)
    print()
