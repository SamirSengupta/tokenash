"""
Tokenash ADVERSARIAL Benchmark: True Worst Cases
...
Focus on methodology differences and key findings.""",
        ...
        expected_behavior="Tokenash should have minimal compression - prose has no structural redundancy",
    )
...
Don't skip any file or change.""",
        ...
        expected_behavior="Tokenash should struggle - code changes are unique and can't be summarized",
    )
...
Describe what you observe about the data format and structure.""",
        ...
        expected_behavior="Tokenash CANNOT compress this - random data has no patterns",
    )
...
def create_small_data_scenario() -> AdversarialScenario:
    return AdversarialScenario(
        ...
        expected_behavior="Tokenash has no opportunity - data is already minimal",
    )
...
Pay attention to who said what and how opinions evolved.""",
        ...
        expected_behavior="Tokenash should preserve conversation flow - context matters",
    )
...
def run_adversarial_benchmark(api_key: str = None) -> dict:
    ...
    print("=" * 70)
    print("TOKENASH ADVERSARIAL BENCHMARK")
    ...
    if TOKENASH_AVAILABLE:
        db_path = os.path.join(tempfile.gettempdir(), "tokenash_adversarial.db")
    ...
    for scenario in scenarios:
        ...
        if tokenash_client:
            print("\n[2/2] TOKENASH...")
            tokenash = run_scenario(tokenash_client, scenario, "tokenash")
            print(f"   Input tokens: {tokenash.input_tokens:,}")
            print(f"   Cost: ${tokenash.cost_usd:.4f}")
            results.append(tokenash)

            if baseline.input_tokens > 0:
                change = (tokenash.input_tokens - baseline.input_tokens) / baseline.input_tokens
                print(f"\n   📊 Token change: {change:+.1%}")
                if change > 0:
                    print("   ⚠️  TOKENASH INCREASED TOKENS (overhead > savings)")
                elif change > -0.1:
    ...
    print(f"\n{'Scenario':<30} {'Baseline':>12} {'Tokenash':>12} {'Change':>12}")
    print("-" * 66)
