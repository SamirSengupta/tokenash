...
This benchmark demonstrates WHY Tokenash matters by showing:
...
2. THE SOLUTION: Tokenash's impact on real workloads
...
# Tokenash imports
from tokenash.transforms.smart_crusher import SmartCrusherConfig, smart_crush_tool_output
...
def benchmark_coding_agent_explosion() -> BenchmarkResult:
    ...
    # Apply Tokenash transforms using convenience function
...
def benchmark_cache_alignment() -> BenchmarkResult:
    ...
    """
    from tokenash.cache import DetectorConfig, DynamicContentDetector
...
def benchmark_rag_scaling() -> BenchmarkResult:
    """
    Show how RAG context grows and how Tokenash manages it.
...
def benchmark_conversation_scaling() -> list[BenchmarkResult]:
    ...
    and shows the scaling curve with and without Tokenash.
    """
    results = []
...
def _generate_terminal_report(results: list[BenchmarkResult]) -> str:
    ...
    lines.append("")
    lines.append("=" * 80)
    lines.append("  TOKENASH AGENT COST BENCHMARK")
    ...
    lines.append(f"    Without Tokenash:    ${total_baseline * 1_000_000:.2f}")
    lines.append(f"    With Tokenash:       ${(total_baseline - total_savings) * 1_000_000:.2f}")
...
def _generate_markdown_report(results: list[BenchmarkResult]) -> str:
    ...
    lines.append("")
    lines.append("This benchmark demonstrates Tokenash's impact on real-world agent workloads:")
    lines.append("")
    ...
    lines.append("")
    lines.append("| Scale | Without Tokenash | With Tokenash | Monthly Savings |")
    lines.append("|-------|------------------|---------------|-----------------|")
...
def main():
    parser = argparse.ArgumentParser(description="Tokenash Agent Cost Benchmark")
