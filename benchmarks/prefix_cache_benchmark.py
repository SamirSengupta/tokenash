...
Strategies:
  1. Baseline — no Tokenash, no markers
  2. Tokenash compression — full pipeline, CompressionCache keeps bytes stable
  3. Tokenash + prefix freeze — pipeline skips frozen (already-cached) messages
  4. Tokenash + explicit markers — pipeline + 4 cache_control breakpoints
...
CACHE_MODE_EXPLICIT = "explicit_4"  # Tokenash's 4 strategic breakpoints
...
def main():
    ...
    # Strategy 3: Tokenash explicit markers (4 strategic breakpoints)
    if "markers" in strategies_to_run:
        step += 1
        print(f"[{step}/{num_strategies}] Tokenash explicit (4 strategic breakpoints)...")
        r = run_strategy(
            "Tokenash 4x",
            api_key,
            model,
