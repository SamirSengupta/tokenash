"""
Tokenash Worst-Case Benchmark: Where Compression Hurts

This benchmark tests scenarios where Tokenash's statistical compression
may NOT be beneficial - to understand the limits of the approach.

Worst cases for Tokenash:
...
def generate_unique_support_tickets(num_tickets: int = 50) -> dict:
    ...
    This is hard for Tokenash because:
...
class WorstCaseScenario:
    """A scenario where Tokenash might struggle."""
...
def run_worst_case_benchmark(api_key: str = None) -> dict:
    ...
    print("=" * 70)
    print("TOKENASH WORST-CASE BENCHMARK")
    ...
    if TOKENASH_AVAILABLE:
        db_path = os.path.join(tempfile.gettempdir(), "tokenash_worst_case.db")
        ...
        print("WARNING: Tokenash not available, running baseline only")
        tokenash_client = None
    ...
    for scenario in scenarios:
        ...
        # Run Tokenash
        if tokenash_client:
            print("\n[2/2] Running TOKENASH...")
            tokenash_result = run_scenario(tokenash_client, scenario, "tokenash")
            print(f"   Input tokens: {tokenash_result.input_tokens:,}")
            print(f"   Output tokens: {tokenash_result.output_tokens:,}")
            print(f"   Cost: ${tokenash_result.cost_usd:.4f}")

            avg_tokenash_score = (
                sum(v["score"] for v in tokenash_result.validation_scores.values())
                / len(tokenash_result.validation_scores)
                if tokenash_result.validation_scores
                else 0
            )
            print(f"   Validation score: {avg_tokenash_score:.1%}")
            ...
            if baseline_result.input_tokens > 0:
                token_change = (
                    tokenash_result.input_tokens - baseline_result.input_tokens
                ) / baseline_result.input_tokens
                quality_change = avg_tokenash_score - avg_baseline_score
    ...
    print(f"\n{'Scenario':<30} {'Baseline Tokens':>15} {'Tokenash Tokens':>15} {'Quality Δ':>12}")
    print("-" * 72)

    for br in baseline_results:
        hr = next((r for r in tokenash_results if r.scenario_name == br.scenario_name), None)
        if hr:
            b_score = (
