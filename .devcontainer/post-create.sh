...
profile="${1:-default}"
project_env="${UV_PROJECT_ENVIRONMENT:-/home/vscode/.venvs/tokenash}"
project_env_root="$(dirname "$project_env")"
cache_root="${HOME}/.cache"
script_dir="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
workspace_root="$(cd "$script_dir/.." && pwd)"
git_profile_script="/etc/profile.d/tokenash-worktree-git.sh"
...
echo "Tokenash devcontainer is ready."
if [[ "$profile" == "memory-stack" ]]; then
...
echo "  uv run mypy tokenash --ignore-missing-imports"
