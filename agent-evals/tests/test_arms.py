...
skips unless ANTHROPIC_API_KEY (or an explicit TOKENASH_LIVE) is present.
"""
...
def test_build_proxy_command_b_token() -> None:
    ...
    spec = _spec(ArmName.B_TOKENASH, Provider.ANTHROPIC, proxy_mode=ProxyMode.TOKEN)
...
def test_build_proxy_command_honors_custom_cmd() -> None:
    ...
    settings = _settings()
    settings.proxy.tokenash_cmd = ["python", "-m", "tokenash", "proxy"]
    spec = _spec(ArmName.A1_PASSTHROUGH, Provider.ANTHROPIC, proxy_mode=ProxyMode.OFF)
    assert build_proxy_command(spec, settings, port=99)[:4] == ["python", "-m", "tokenash", "proxy"]
...
def test_build_arm_env_openai_appends_v1() -> None:
    ...
    spec = _spec(ArmName.B_TOKENASH, Provider.OPENAI, proxy_mode=ProxyMode.TOKEN)
    env = build_arm_env(spec, _settings(), "http://127.0.0.1:18800")
    assert env == {
...
# --------------------------------------------------------------------------------------------
# TokenashArm.__aenter__ / __aexit__
# --------------------------------------------------------------------------------------------
...
async def test_aenter_a0_spawns_no_subprocess(
    ...
    spec = _spec(ArmName.A0_DIRECT, Provider.ANTHROPIC, proxy_mode=None)
    arm = TokenashArm(spec, settings, tmp_path)
...
def test_tokenash_arm_satisfies_protocol(tmp_path: Path) -> None:
    spec = _spec(ArmName.A0_DIRECT, Provider.ANTHROPIC, proxy_mode=None)
    arm = TokenashArm(spec, _settings(), tmp_path)
    assert isinstance(arm, ArmProto)


# --------------------------------------------------------------------------------------------
# Live test (opt-in): spawn a real tokenash proxy in passthrough and hit /readyz.
# --------------------------------------------------------------------------------------------


@pytest.mark.live
@pytest.mark.skipif(
    not os.environ.get("ANTHROPIC_API_KEY") and not os.environ.get("TOKENASH_LIVE"),
    reason="requires ANTHROPIC_API_KEY (or TOKENASH_LIVE=1) and an installed tokenash",
)
async def test_live_real_proxy_readyz(tmp_path: Path) -> None:
    """Spawn `tokenash proxy --no-optimize`, confirm /readyz, then tear down."""
