"""Opt-in LIVE tests (``-m live``): spawn a real ``tokenash proxy`` and hit a real upstream.
...
* ``test_b_arm_captures_savings_live`` — the B arm emits ``x-tokenash-*`` headers that the client
...
from agent_evals.arms import TokenashArm
...
def _arm(
    ...
    return TokenashArm(spec, settings, run_dir, **kw)  # type: ignore[arg-type]
