...
Two attribution surfaces, matching the Tokenash proxy's two reporting channels:

* **Per-request** — every optimized response carries ``x-tokenash-*`` headers
...
``tokenash/proxy/handlers/{openai,anthropic}.py``). ``x-tokenash-savings-percent`` is
...
# --- Per-response header names (verified against tokenash proxy handlers) -------------------
# Required token headers — absent => no Tokenash optimization happened on this response.
HEADER_TOKENS_BEFORE = "x-tokenash-tokens-before"
HEADER_TOKENS_AFTER = "x-tokenash-tokens-after"
...
HEADER_TOKENS_SAVED = "x-tokenash-tokens-saved"
HEADER_MODEL = "x-tokenash-model"
# Conditional headers — only present when their condition holds.
HEADER_TRANSFORMS = "x-tokenash-transforms"
HEADER_CACHED = "x-tokenash-cached"
HEADER_COMPRESSION_FAILED = "x-tokenash-compression-failed"

# String the proxy writes for a true boolean header (e.g. ``x-tokenash-cached: "true"``).
_HEADER_TRUE = "true"
...
def parse_savings_headers(
    ...
    """Parse the per-response ``x-tokenash-*`` headers into a :class:`TaskSavings`.

    Returns ``None`` (never a fabricated zero) when the required token headers are absent —
    that signals "no Tokenash optimization on this response" (e.g. a pass-through / A1 arm, or
    a non-optimized request). ``savings_percent``/``ratio``/``cost_*`` are DERIVED by
    :meth:`TaskSavings.from_token_counts`; ``x-tokenash-savings-percent`` is never read (it is
    batch-path-only).
    """
    ...
    try:
        ...
        logger.warning(
            "malformed tokenash token headers; skipping savings capture",
            extra={
                "fields": {
...
def make_response_hook(
    ...
    On each response it reads the ``x-tokenash-*`` headers; if ``task_id_getter()`` resolves to
    ...
    Tokenash headers, nothing is recorded — savings are never fabricated.
    """
