...
End-to-end accuracy A/B framework for **Tokenash**: run trusted coding-agent benchmarks
**WITH vs WITHOUT** Tokenash's context-compression proxy and produce a statistically
defensible verdict — *does compression preserve what the agent can solve, and how much
does it save?*

This is a self-contained nested project inside the `tokenash` repo. It consumes Tokenash
only as the system-under-test (via `base_url`); it is **not** part of the `tokenash`
wheel and is **not** wired into tokenash's `make ci-precheck`.

## The clean A/B (why a proxy helps)

Tokenash sits in the request path, so the only variable between arms is `base_url`:

| Arm | base_url | Tokenash mode | Isolates |
|-----|----------|---------------|----------|
| `A0_DIRECT` | provider API | none | native agent score |
| `A1_PASSTHROUGH` | `localhost:N` | `--no-optimize` | proxy-hop cost only |
| `B_TOKENASH` | `localhost:M` | `--mode token` | compression cost (vs A1) |
