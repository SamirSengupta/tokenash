#!/usr/bin/env python3
"""Comprehensive Strands + Bedrock Demo for Tokenash SDK.

This demo showcases two Tokenash integration patterns for AWS Strands Agents:

1. **TokenashHookProvider** - Compresses tool outputs as they happen
...
2. **TokenashStrandsModel** - Optimizes entire conversation context
...
Requirements:
    ...
    - pip install strands-agents tokenash[strands]
"""
...
def check_dependencies() -> bool:
    ...
    # Check tokenash
    try:
        from tokenash.integrations.strands import (  # noqa: F401
            TokenashHookProvider,
            TokenashStrandsModel,
        )
    except ImportError:
        missing.append("tokenash[strands]")
...
# ============================================================================
# Demo 1: TokenashHookProvider
# ============================================================================


def run_hook_provider_demo(region: str = "us-west-2") -> dict[str, Any]:
    """Demonstrate TokenashHookProvider for tool output compression.
    ...
    from tokenash.integrations.strands import TokenashHookProvider

    print_box(
        "Demo 1: TokenashHookProvider",
        [
            "The TokenashHookProvider intercepts tool outputs and compresses",
    ...
    # Create TokenashHookProvider
    hook_provider = TokenashHookProvider(
...
# ============================================================================
# Demo 2: TokenashStrandsModel
# ============================================================================


def run_model_wrapper_demo(region: str = "us-west-2") -> dict[str, Any]:
    """Demonstrate TokenashStrandsModel for conversation optimization.
    ...
    print_box(
        "Demo 2: TokenashStrandsModel",
        [
            "TokenashStrandsModel wraps the Bedrock model to optimize the",
            ...
            "",
            "Using: Claude 3 Haiku wrapped with TokenashStrandsModel",
        ],
    )
    ...
    # Wrap with TokenashStrandsModel
    optimized_model = TokenashStrandsModel(
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Tokenash + Strands Bedrock Demo",
...
Environment Variables:
  ...
  AWS_PROFILE             AWS profile name (alternative to keys)
    ...
    parser.add_argument(
        "--hook",
        action="store_true",
        help="Run only the TokenashHookProvider demo",
    )
    parser.add_argument(
        "--model",
        action="store_true",
        help="Run only the TokenashStrandsModel demo",
    )
    parser.add_argument(
        "--region",
    ...
    # Print header
    print_box(
        "Tokenash + Strands Bedrock Demo",
        [
            "This demo showcases Tokenash's integration with AWS Strands Agents.",
            "",
            "Tokenash provides two integration patterns:",
            "  1. TokenashHookProvider - Compress tool outputs in real-time",
            "  2. TokenashStrandsModel - Optimize entire conversation context",
            "",
        ...
        if run_hook and run_model:
            ...
            print_box(
                ...
                [
                    ...
                    "  [x] TokenashHookProvider - Real-time tool output compression",
                    "  [x] TokenashStrandsModel - Full context optimization",
                ],
                style="success",
            )
