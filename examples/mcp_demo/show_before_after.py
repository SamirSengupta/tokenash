...
This demonstrates the minimal code change needed to add Tokenash
...
def main():
    print("\n" + "=" * 70)
    print("TOKENASH MCP INTEGRATION - DEVELOPER EXPERIENCE")
    print("=" * 70)
...
"""
    ...
    print("\nAFTER (with Tokenash compression):")
    print("-" * 40)
    after_standalone = """
from tokenash.integrations.mcp import compress_tool_result  # ADD THIS
...
"""
    ...
    after_wrapper = """
from mcp import Client
from tokenash.integrations.mcp import TokenashMCPClientWrapper  # ADD THIS

# Create MCP client
base_client = Client(transport)
client = TokenashMCPClientWrapper(base_client)  # WRAP IT (1 line)
...
"""
    ...
    print("-" * 40)
    with_metrics = """
from tokenash.integrations.mcp import compress_tool_result_with_metrics
