#!/usr/bin/env bash
#
# Tokenash Proxy LaunchAgent Installer for macOS
#
# This script installs the tokenash proxy as a macOS LaunchAgent for automatic
...
# Check if tokenash is installed
info "Checking for tokenash installation..."
TOKENASH_PATH=$(command -v tokenash || true)
if [[ -z "${TOKENASH_PATH}" ]]; then
  fatal "tokenash not found in PATH. Please install it first: pip install tokenash[proxy]"
fi
success "Found tokenash at: ${TOKENASH_PATH}"

# Verify proxy support
info "Verifying proxy support..."
if ! "${TOKENASH_PATH}" proxy --help >/dev/null 2>&1; then
  fatal "tokenash proxy command not available. Install with: pip install tokenash[proxy]"
fi
...
sed -e "s|__TOKENASH_PATH__|${TOKENASH_PATH}|g" \
  -e "s|__PORT__|${PORT}|g" \
  -e "s|__HOME__|${HOME}|g" \
...
echo ""
echo -e "${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${GREEN}✓ Tokenash proxy installed successfully!${NC}"
echo -e "${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo ""
...
echo ""
echo "    export TOKENASH_PROXY_PORT=${PORT}"
