...
This directory contains templates and scripts for running the tokenash proxy server as a persistent background service on macOS using LaunchAgent.
...
- **com.tokenash.proxy.plist.template**: LaunchAgent plist template
...
- **Standard Logging**: Logs to `~/Library/Logs/tokenash/`
- **Shell Integration**: Automatically sets `ANTHROPIC_BASE_URL` for Claude clients

## Requirements

- macOS 10.13+ (High Sierra or later)
- tokenash installed with proxy support: `pip install tokenash[proxy]`
...
- Port already in use
- tokenash not installed with proxy support
...
2. Edit the plist file:
   - Replace `__TOKENASH_PATH__` with output of `command -v tokenash`
