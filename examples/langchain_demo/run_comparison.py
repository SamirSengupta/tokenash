"""Real-world LangChain Agent: Before/After Tokenash Comparison.

This script demonstrates the impact of Tokenash optimization on a realistic
...
def run_agent_baseline(scenario: dict, api_key: str) -> AgentRun:
    """Run agent WITHOUT Tokenash (baseline)."""
...
def run_agent_tokenash(scenario: dict, api_key: str) -> AgentRun:
    """Run agent WITH Tokenash optimization."""

    # Import Tokenash integration
    from tokenash import TokenashConfig
    ...
    tokenash_model = TokenashChatModel(
    ...
    for _ in range(5):
        ...
        # Call model (Tokenash optimizes internally)
        response = tokenash_model.invoke(messages)
        messages.append(response)
    ...
    # Get Tokenash metrics
    tokens_saved = tokenash_model.get_total_tokens_saved()
...
def print_comparison(baseline: AgentRun, tokenash: AgentRun):
    """Print comparison between baseline and tokenash runs."""
    ...
    # Token comparison
    input_saved = baseline.total_input_tokens - tokenash.total_input_tokens
    input_pct = (
        (input_saved / baseline.total_input_tokens * 100) if baseline.total_input_tokens > 0 else 0
    )

    print(f"\n{'METRIC':<30} {'BASELINE':>15} {'TOKENASH':>15} {'SAVINGS':>15}")
    print("-" * 75)
    print(
        f"{'Input Tokens':<30} {baseline.total_input_tokens:>15,} {tokenash.total_input_tokens:>15,} {input_saved:>14,} ({input_pct:.1f}%)"
    )
    print(
        f"{'Output Tokens':<30} {baseline.total_output_tokens:>15,} {tokenash.total_output_tokens:>15,} {'N/A':>15}"
    )
    print(
        f"{'Tool Output Tokens':<30} {baseline.tool_output_tokens:>15,} {tokenash.tool_output_tokens:>15,} {'(raw)':>15}"
    )
    print(f"{'Tool Calls':<30} {baseline.tool_calls:>15} {tokenash.tool_calls:>15} {'':>15}")
    print(f"{'Messages':<30} {baseline.messages_count:>15} {tokenash.messages_count:>15} {'':>15}")
    print(
        f"{'Duration (ms)':<30} {baseline.duration_ms:>15.0f} {tokenash.duration_ms:>15.0f} {'':>15}"
    )
    ...
    tokenash_cost = (
        tokenash.total_input_tokens * input_cost_per_1m
        + tokenash.total_output_tokens * output_cost_per_1m
    ) / 1_000_000
    cost_saved = baseline_cost - tokenash_cost
    cost_pct = (cost_saved / baseline_cost * 100) if baseline_cost > 0 else 0

    print(
        f"\n{'Estimated Cost (USD)':<30} ${baseline_cost:>14.6f} ${tokenash_cost:>14.6f} ${cost_saved:>13.6f} ({cost_pct:.1f}%)"
    )
...
def main():
    ...
    print("\n" + "=" * 70)
    print("LANGCHAIN AGENT: BEFORE/AFTER TOKENASH COMPARISON")
    print("=" * 70)
    ...
    for scenario in SCENARIOS:
        ...
        # Run with Tokenash
        print("  - Running with Tokenash optimization...")
        tokenash = run_agent_tokenash(scenario, api_key)
        all_tokenash.append(tokenash)

        # Print comparison
        print_comparison(baseline, tokenash)

    # Print summary
    print_summary(all_baseline, all_tokenash)
...
def run_simulation():
    ...
    for scenario in SCENARIOS:
        ...
        print(f"  With Tokenash (20 items max), estimated: ~{total_tool_tokens // 5:,} tokens")
        print(
            f"  Estimated savings: ~{total_tool_tokens * 2 - total_tool_tokens // 5:,} tokens (~80%)"
        )


def print_summary(baseline_runs: list[AgentRun], tokenash_runs: list[AgentRun]):
    ...
    total_tokenash_input = sum(r.total_input_tokens for r in tokenash_runs)
    total_saved = total_baseline_input - total_tokenash_input
    pct_saved = (total_saved / total_baseline_input * 100) if total_baseline_input > 0 else 0

    print(f"\n{'Metric':<30} {'Baseline':>15} {'Tokenash':>15} {'Savings':>15}")
    print("-" * 75)
    print(
        f"{'Total Input Tokens':<30} {total_baseline_input:>15,} {total_tokenash_input:>15,} {total_saved:>14,}"
    )
    ...
    tokenash_cost = total_tokenash_input * input_cost
    cost_saved = baseline_cost - tokenash_cost

    print(
        f"\n{'Est. Input Cost (USD)':<30} ${baseline_cost:>14.4f} ${tokenash_cost:>14.4f} ${cost_saved:>13.4f}"
    )
    ...
    print("=" * 70)
    print(f"""
Tokenash reduced input tokens by {pct_saved:.1f}% across all scenarios.
