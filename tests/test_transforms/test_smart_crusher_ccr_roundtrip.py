...
The Rust core integration test (`crates/tokenash-core/tests/ccr_roundtrip.rs`)
...
def _build_extension() -> None:
    try:
        ...
        pytest.skip(
            "tokenash._core not built — run `bash scripts/build_rust_extension.sh`",
...
def _force_lossy_config():
    ...
    from tokenash._core import SmartCrusherConfig
...
def test_native_default_crusher_has_a_store() -> None:
    ...
    from tokenash._core import SmartCrusher
...
def test_shim_exposes_ccr_get_and_ccr_len() -> None:
    from tokenash.config import SmartCrusherConfig as PyConfig
...
def test_shim_lossy_crush_populates_store() -> None:
    ...
    `tokenash.transforms.smart_crusher.SmartCrusher` shim — the path
...
def test_explicit_before_after_roundtrip_native() -> None:
    ...
    from tokenash._core import SmartCrusher, SmartCrusherConfig
