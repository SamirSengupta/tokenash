...
`tests/parity/fixtures/smart_crusher/` through `tokenash._core.SmartCrusher`
...
same fixtures via `cargo run -p tokenash-parity --bin parity-run --
...
def test_rust_backend_matches_recorded_output(fixture_path: Path):
    ...
    Rust regression — cross-check with `cargo run -p tokenash-parity`.
    """
