...
PyO3 (`tokenash._core`). These tests guard against regressions by
...
Skipped automatically when the `tokenash._core` wheel isn't installed
(e.g. CI lane without the maturin step). The Rust crate's own
`cargo run -p tokenash-parity` covers the same fixtures from the Rust
...
def _has_core() -> bool:
    try:
        import tokenash._core  # noqa: F401
...
def test_rust_backend_matches_recorded_output(fixture_path: Path):
    ...
    real Rust regression (cross-check with `cargo run -p tokenash-parity`
    to localize).
    """
    from tokenash.transforms.diff_compressor import DiffCompressor, DiffCompressorConfig
...
def test_compress_with_stats_returns_python_dataclass_and_pyo3_stats():
    ...
    """
    from tokenash.transforms.diff_compressor import (
...
def test_content_router_uses_rust_backend():
    ...
    from tokenash.transforms.diff_compressor import DiffCompressor
