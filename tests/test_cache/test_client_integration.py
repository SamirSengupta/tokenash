"""Tests for TokenashClient cache optimizer integration."""
...
from tokenash.cache.base import CacheMetrics, CacheResult
...
class TestTokenashClientCacheIntegration:
    """Test TokenashClient cache optimizer integration."""
...
class TestCacheOptimizerInvocation:
    ...
    @patch("tokenash.storage.sqlite.SQLiteStorage.save")
    def test_optimizer_optimize_is_called_during_chat(self, mock_save, temp_db):
        ...
        """
        from tokenash import TokenashMode
        ...
        client.chat.completions.create(
            ...
            messages=messages,
            max_tokens=100,
            tokenash_mode=TokenashMode.OPTIMIZE,
        )
    ...
    def test_optimizer_transforms_applied_in_response(self, mock_save, temp_db):
        ...
        assert hasattr(result, "tokenash"), "Response should have tokenash metadata"
        tokenash_meta = result.tokenash

        # Check that cache optimizer was reported
        assert tokenash_meta.cache_optimizer_used is not None or any(
            "cache_optimizer" in t for t in (tokenash_meta.transforms_applied or [])
    ...
    def test_optimizer_not_called_in_audit_mode(self, mock_save, temp_db):
        ...
        client.chat.completions.create(
            ...
            messages=messages,
            max_tokens=100,
            tokenash_mode=TokenashMode.AUDIT,
        )
...
class TestSemanticCacheIntegration:
    """Test semantic cache integration with TokenashClient.
    ...
    def test_semantic_cache_hit_returns_cached_response_without_api_call(self, mock_save, temp_db):
        ...
        if client._semantic_cache_layer is not None:
            from tokenash.cache import OptimizationContext
...
class TestSessionStatsTracking:
    """Test session statistics tracking in TokenashClient.
