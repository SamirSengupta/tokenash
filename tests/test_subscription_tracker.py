...
async def test_tracker_start_stop_and_rollover_reset(
    ...
    tracker._state.contribution = TokenashContribution(tokens_submitted=99)
...
async def test_maybe_poll_handles_inactive_and_none_snapshot(
    ...
    monkeypatch.setattr("tokenash.subscription.client.read_cached_oauth_token", lambda: None)
    await tracker._maybe_poll()
    assert tracker._state.poll_count == 0

    monkeypatch.setattr(
        "tokenash.subscription.client.read_cached_oauth_token", lambda: "cached-token"
    )
...
async def test_maybe_poll_success_updates_state_and_metrics(
    ...
    monkeypatch.setitem(
        sys.modules,
        "tokenash.observability.metrics",
        SimpleNamespace(
