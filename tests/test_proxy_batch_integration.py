...
class TestOpenAIBatchCreate:
    ...
    def test_batch_create_with_compression(self, openai_batch_client, openai_api_key):
        ...
        try:
            ...
            if "x-tokenash-tokens-saved" in response.headers:
                tokens_saved = int(response.headers["x-tokenash-tokens-saved"])
                assert tokens_saved >= 0

            if "x-tokenash-savings-percent" in response.headers:
                savings_percent = float(response.headers["x-tokenash-savings-percent"])
                assert 0 <= savings_percent <= 100

            # Verify compression metadata was added
            metadata = batch_data.get("metadata", {})
            if metadata.get("tokenash_compressed") == "true":
                # Compression was applied
                assert "tokenash_tokens_saved" in metadata
                assert "tokenash_original_tokens" in metadata
                assert "tokenash_compressed_tokens" in metadata
                tokens_saved = int(metadata["tokenash_tokens_saved"])
                assert tokens_saved >= 0
