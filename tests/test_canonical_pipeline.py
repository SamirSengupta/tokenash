...
from tokenash.client import TokenashClient
from tokenash.compress import compress
from tokenash.config import TokenashConfig, TokenashMode, TransformResult
from tokenash.hooks import CompressionHooks
from tokenash.pipeline import (
...
from tokenash.transforms import ContentRouter, TransformPipeline
from tokenash.transforms.content_router import CompressionStrategy
...
def test_pipeline_extension_manager_replaces_events_and_ignores_failures(caplog):
    ...
    with caplog.at_level("WARNING", logger="tokenash.pipeline"):
        event = manager.emit(
...
def test_discover_pipeline_extensions_handles_load_and_init_failures(monkeypatch):
    pipeline_module = importlib.import_module("tokenash.pipeline")
...
def test_tokenash_client_emits_canonical_pipeline_events(tmp_path):
    recorder = RecordingExtension()
    original = DummyOriginalClient()
    config = TokenashConfig(
        store_url=f"jsonl://{tmp_path / 'tokenash.jsonl'}",
    ...
    client = TokenashClient(
        ...
        store_url=f"jsonl://{tmp_path / 'tokenash-client.jsonl'}",
