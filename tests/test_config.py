...
class TestTokenashMode:
    """Tests for TokenashMode enum."""

    def test_enum_values(self):
        """All expected enum values exist with correct string values."""
        assert TokenashMode.AUDIT.value == "audit"
        assert TokenashMode.OPTIMIZE.value == "optimize"
        assert TokenashMode.SIMULATE.value == "simulate"

    def test_string_conversion(self):
        """TokenashMode inherits from str for string compatibility."""
        # Enum value access works as string
        assert TokenashMode.AUDIT.value == "audit"
        assert TokenashMode.OPTIMIZE.value == "optimize"
        assert TokenashMode.SIMULATE.value == "simulate"
        # Can compare directly with strings since it inherits from str
        assert TokenashMode.AUDIT == "audit"
        assert TokenashMode.OPTIMIZE == "optimize"
        assert TokenashMode.SIMULATE == "simulate"
        # isinstance check confirms str inheritance
        assert isinstance(TokenashMode.AUDIT, str)
...
class TestTokenashConfig:
    """Tests for TokenashConfig main configuration class."""

    def test_default_values(self):
        """Default values are correctly set."""
        config = TokenashConfig()
        assert config.store_url == "sqlite:///tokenash.db"
        assert config.default_mode == TokenashMode.AUDIT
    ...
    def test_get_context_limit_direct_match(self):
        ...
        config = TokenashConfig(model_context_limits={"gpt-4o": 128000, "claude-3-opus": 200000})
    ...
    def test_get_context_limit_prefix_match(self):
        ...
        config = TokenashConfig(model_context_limits={"gpt-4": 128000, "claude-3": 200000})
        # Prefix matches
    ...
    def test_get_context_limit_not_found(self):
        ...
        config = TokenashConfig(model_context_limits={"gpt-4": 128000})
    ...
    def test_model_context_limits_isolation(self):
        ...
        config1 = TokenashConfig()
        config2 = TokenashConfig()
