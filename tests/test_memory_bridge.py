"""Tests for the Memory Bridge (markdown <-> Tokenash bidirectional sync).
...
## Project Overview
- **Tokenash**: Context optimization layer for LLM applications
- **Repos**: OSS at ~/claude-projects/tokenash
...
class TestRelationshipExtraction:
    def test_bold_colon_pattern(self):
        section = ParsedSection(
            heading="Test",
            heading_level=2,
            content="- **Tokenash**: Context optimization layer",
            facts=["**Tokenash**: Context optimization layer"],
        )
        rels = extract_relationships_from_section(section)
        assert len(rels) >= 1
        assert rels[0]["source"] == "Tokenash"
    ...
    def test_verb_patterns(self):
        section = ParsedSection(
            heading="Test",
            heading_level=2,
            content="Tokenash uses SQLite for storage",
            facts=["Tokenash uses SQLite for storage"],
        )
        rels = extract_relationships_from_section(section)
...
class TestMemoryBridgeExport:
    ...
    async def test_export_claude_code_style(self, bridge, tmp_dir, backend):
        ...
        await backend.save_memory(
            content="Tokenash is a context optimization layer",
            user_id="test_user",
            importance=0.8,
