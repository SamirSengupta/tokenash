...
Tokenash only compresses content the model has already seen — assistant
...
def main() -> int:
    ...
    proc = subprocess.Popen(
        [
            str(REPO_ROOT / ".venv/bin/tokenash"),
            "proxy",
            "--port",
            str(port),
            "--no-telemetry",
        ],
        env={**os.environ, "TOKENASH_REQUIRE_RUST_CORE": "true"},
        stdout=log_fp,
    ...
    try:
        ...
        time.sleep(1.5)
        canonical_log = Path.home() / ".tokenash" / "logs" / "proxy.log"
