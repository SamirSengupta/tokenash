...
from tokenash.install.models import DeploymentManifest, SupervisorKind
from tokenash.install.supervisors import (
    _command_for_script,
    _linux_service_unit,
    _linux_task_spec,
...
def _manifest(
    *, profile: str = "default", scope: str = "user", supervisor: str = "service"
    ...
    return DeploymentManifest(
        ...
        host="127.0.0.1",
        backend="anthropic",
        service_name=f"tokenash-{profile}",
    )
...
def test_linux_service_unit_uses_user_systemd_path(monkeypatch, tmp_path: Path) -> None:
    ...
    unit_path, content = _linux_service_unit(manifest, tmp_path / "run-tokenash.sh")

    assert unit_path == tmp_path / ".config" / "systemd" / "user" / "tokenash-default.service"
    assert "ExecStart=" + str(tmp_path / "run-tokenash.sh") in content
    assert "Restart=on-failure" in content


def test_command_for_script_and_unix_runner(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setattr(
        "tokenash.install.supervisors.resolve_tokenash_command",
        lambda: ["python", "-m", "tokenash"],
    )
    ...
    record = _render_unix_runner(
        tmp_path / "scripts" / "run-tokenash.sh", ["tokenash", "run", "--flag"]
    )
    assert record.kind == "script"
    content = Path(record.path).read_text(encoding="utf-8")
    assert content.startswith("#!/usr/bin/env bash")
    assert "exec tokenash run --flag" in content
...
def test_linux_task_spec_for_user_scope_includes_crontab_markers(tmp_path: Path) -> None:
    ...
    cron_path, content = _linux_task_spec(manifest, tmp_path / "ensure-tokenash.sh")

    assert cron_path is None
    assert "# >>> tokenash smoke >>>" in content
    assert "# <<< tokenash smoke <<<" in content
...
def test_macos_launchd_plist_switches_between_keepalive_and_interval(
    ...
    service_path, service_content = _macos_launchd_plist(
        service_manifest, tmp_path / "run-tokenash.sh"
    )
    assert service_path == tmp_path / "Library" / "LaunchAgents" / "com.tokenash.default.plist"
    ...
    task_path, task_content = _macos_launchd_plist(
        task_manifest, tmp_path / "ensure-tokenash.sh", interval=300
    )
    assert task_path == tmp_path / "Library" / "LaunchAgents" / "com.tokenash.tasky.plist"
...
def test_render_windows_runner_writes_ps1_and_cmd_wrappers(tmp_path: Path) -> None:
    ps1_path = tmp_path / "run-tokenash.ps1"
    cmd_path = tmp_path / "run-tokenash.cmd"

    records = _render_windows_runner(
        ps1_path,
        cmd_path,
        ["C:\\Program Files\\Python\\python.exe", "tokenash", "install", "agent", "run"],
    )
    ...
    assert '& "C:\\Program Files\\Python\\python.exe" tokenash install agent run' in ps1_content
    assert (
        'powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0run-tokenash.ps1" %*'
        in cmd_content
    )


def test_render_runner_scripts_writes_unix_scripts(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setattr("tokenash.install.supervisors.sys.platform", "linux")
    monkeypatch.setattr(
        "tokenash.install.supervisors.resolve_tokenash_command", lambda: ["tokenash"]
    )
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    manifest = _manifest()
...
def test_render_runner_scripts_writes_windows_scripts(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setattr("tokenash.install.supervisors.sys.platform", "win32")
    monkeypatch.setattr(
        "tokenash.install.supervisors.resolve_tokenash_command", lambda: ["tokenash.exe"]
    )
    monkeypatch.setattr(
        "tokenash.install.supervisors.windows_run_script_path",
        lambda profile: tmp_path / "run-tokenash.ps1",
    )
    monkeypatch.setattr(
        "tokenash.install.supervisors.windows_run_cmd_path",
        lambda profile: tmp_path / "run-tokenash.cmd",
    )
    monkeypatch.setattr(
        "tokenash.install.supervisors.windows_ensure_script_path",
        lambda profile: tmp_path / "ensure-tokenash.ps1",
    )
    monkeypatch.setattr(
        "tokenash.install.supervisors.windows_ensure_cmd_path",
        lambda profile: tmp_path / "ensure-tokenash.cmd",
    )
...
def test_start_and_stop_supervisor_use_linux_systemctl(monkeypatch) -> None:
    ...
    monkeypatch.setattr("tokenash.install.supervisors.sys.platform", "linux")
    monkeypatch.setattr(
        "tokenash.install.supervisors.subprocess.run",
    ...
    assert calls == [
        ["systemctl", "--user", "restart", "tokenash-default"],
        ["systemctl", "--user", "stop", "tokenash-default"],
    ]
...
def test_install_supervisor_linux_service_and_tasks(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setattr("tokenash.install.supervisors.sys.platform", "linux")
    monkeypatch.setattr("tokenash.install.supervisors.sys.platform", "linux")
    run_script = tmp_path / "run-tokenash.sh"
    ensure_script = tmp_path / "ensure-tokenash.sh"
    monkeypatch.setattr(
        "tokenash.install.supervisors.render_runner_scripts",
        lambda manifest: [
            type("Record", (), {"kind": "script", "path": run_script.as_posix()})(),
            type("Record", (), {"kind": "script", "path": ensure_script.as_posix()})(),
        ],
    )
    unit_path = tmp_path / "tokenash-default.service"
    monkeypatch.setattr(
        "tokenash.install.supervisors._linux_service_unit",
    ...
    monkeypatch.setattr("tokenash.install.supervisors.subprocess.run", fake_run)
    ...
    assert ["systemctl", "--user", "enable", "tokenash-default"] in [call[0] for call in calls]
    assert service_records[-1].kind == "service-unit"

    cron_path = tmp_path / "tokenash-system"
    monkeypatch.setattr(
        "tokenash.install.supervisors._linux_task_spec",
...
def test_install_supervisor_darwin_windows_and_unsupported(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("tokenash.install.supervisors.os.getuid", lambda: 123, raising=False)

    plist_path = tmp_path / "com.tokenash.default.plist"
    monkeypatch.setattr(
        "tokenash.install.supervisors._macos_launchd_plist",
        lambda manifest, script, interval=None: (plist_path, f"plist-{interval}"),
    )
    monkeypatch.setattr("tokenash.install.supervisors.sys.platform", "darwin")
    monkeypatch.setattr("tokenash.install.supervisors.sys.platform", "darwin")
    service_records = install_supervisor(_manifest(supervisor=SupervisorKind.SERVICE.value))
    ...
    monkeypatch.setattr(
        "tokenash.install.supervisors.windows_run_cmd_path",
        lambda profile: Path(f"C:\\tmp\\{profile}\\run-tokenash.cmd"),
    )
    monkeypatch.setattr(
        "tokenash.install.supervisors.windows_ensure_cmd_path",
        lambda profile: Path(f"C:\\tmp\\{profile}\\ensure-tokenash.cmd"),
    )
    ...
    assert [
        ...
        "create",
        "tokenash-default",
        'binPath= cmd.exe /c "C:\\tmp\\default\\run-tokenash.cmd"',
        "start= auto",
    ] in calls
    assert [
        "schtasks",
        "/Create",
        "/TN",
        "tokenash-default-health",
        "/TR",
        "C:\\tmp\\default\\ensure-tokenash.cmd",
        "/SC",
        "MINUTE",
        "/MO",
        "5",
    ...
    monkeypatch.setattr("tokenash.install.supervisors.sys.platform", "plan9")
    monkeypatch.setattr("tokenash.install.supervisors.sys.platform", "plan9")
...
def test_start_and_stop_supervisor_darwin_windows_and_none(monkeypatch) -> None:
    ...
    monkeypatch.setattr("tokenash.install.supervisors.os.getuid", lambda: 77, raising=False)
    ...
    assert calls == [
        ["launchctl", "kickstart", "-k", "gui/77/com.tokenash.default"],
        ["launchctl", "bootout", "gui/77/com.tokenash.default"],
    ]
    ...
    stop_supervisor(_manifest(supervisor=SupervisorKind.SERVICE.value))
    assert calls == [
        ["sc.exe", "start", "tokenash-default"],
        ["sc.exe", "stop", "tokenash-default"],
    ]
...
def test_macos_start_bootstraps_when_job_not_registered(monkeypatch, tmp_path: Path) -> None:
    ...
    plist_path = tmp_path / "Library" / "LaunchAgents" / "com.tokenash.default.plist"
    assert calls == [
        ["launchctl", "kickstart", "-k", "gui/77/com.tokenash.default"],
...
def test_macos_start_retries_bootstrap_until_launchd_settles(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("tokenash.install.supervisors.os.getuid", lambda: 77, raising=False)
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setattr("tokenash.install.supervisors.time.sleep", lambda _s: None)
...
def test_macos_start_raises_after_bootstrap_keeps_failing(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setattr("tokenash.install.supervisors.time.sleep", lambda _s: None)
    monkeypatch.setattr("tokenash.install.supervisors._MACOS_BOOTSTRAP_RETRIES", 3)
...
def test_macos_stop_tolerates_missing_job(monkeypatch) -> None:
    ...
    stop_supervisor(_manifest(supervisor=SupervisorKind.SERVICE.value))
    assert calls == [["launchctl", "bootout", "gui/77/com.tokenash.default"]]
...
def test_remove_supervisor_linux_service_cron_path_and_missing_crontab(
    ...
    assert ["systemctl", "--user", "disable", "--now", "tokenash-default"] in calls
    assert ["systemctl", "--user", "daemon-reload"] in calls

    cron_path = tmp_path / "tokenash-task"
...
def test_remove_supervisor_darwin_and_windows(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("tokenash.install.supervisors.os.getuid", lambda: 55, raising=False)

    plist_path = tmp_path / "com.tokenash.default.plist"
    plist_path.write_text("plist", encoding="utf-8")
    monkeypatch.setattr(
        "tokenash.install.supervisors.unix_run_script_path",
        lambda profile: tmp_path / "run-tokenash.sh",
    )
    monkeypatch.setattr(
        "tokenash.install.supervisors.unix_ensure_script_path",
        lambda profile: tmp_path / "ensure-tokenash.sh",
    )
    monkeypatch.setattr(
        "tokenash.install.supervisors._macos_launchd_plist",
    ...
    assert calls[0] == ["launchctl", "bootout", "gui/55/com.tokenash.default"]
    ...
    assert calls == [
        ["sc.exe", "stop", "tokenash-default"],
        ["sc.exe", "delete", "tokenash-default"],
        ["schtasks", "/Delete", "/TN", "tokenash-default-startup", "/F"],
        ["schtasks", "/Delete", "/TN", "tokenash-default-health", "/F"],
    ]
