...
def test_bash_native_installer_supports_persistent_docker_lifecycle(tmp_path: Path) -> None:
    ...
    env["TOKENASH_DOCKER_IMAGE"] = "tokenash:test-image"
    ...
    try:
        ...
        assert "TOKENASH_IMAGE_DEFAULT=tokenash:test-image" in wrapper.read_text(encoding="utf-8")
        ...
        manifest_path = home / ".tokenash" / "deploy" / "smoke" / "manifest.json"
        manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
        assert manifest["preset"] == "persistent-docker"
        assert manifest["port"] == port
        assert manifest["memory_enabled"] is True
        assert manifest["memory_db_path"] == "/tmp/tokenash-home/.tokenash/memory.db"
        assert manifest["telemetry_enabled"] is False

        state_path = home / ".tokenash" / "deploy" / "smoke" / "docker-native.env"
        ...
        assert "/tmp/tokenash-home/.tokenash/memory.db" in install_call
        ...
        assert "TOKENASH_WORKSPACE_DIR=/tmp/tokenash-home/.tokenash" in install_call
        assert "TOKENASH_CONFIG_DIR=/tmp/tokenash-home/.tokenash/config" in install_call
...
def test_powershell_native_installer_supports_persistent_docker_lifecycle(tmp_path: Path) -> None:
    ...
    try:
        ...
        wrapper = home / ".local" / "bin" / "tokenash.ps1"
        assert wrapper.exists()
        assert "__TOKENASH_INSTALL_IMAGE__" not in wrapper.read_text(encoding="utf-8")
        assert "tokenash:test-image" in wrapper.read_text(encoding="utf-8")
        cmd_wrapper = home / ".local" / "bin" / "tokenash.cmd"
        ...
        manifest_path = home / ".tokenash" / "deploy" / "smoke" / "manifest.json"
        state_path = home / ".tokenash" / "deploy" / "smoke" / "docker-native.json"
        ...
        assert manifest["memory_db_path"] == "/tmp/tokenash-home/.tokenash/memory.db"
        assert manifest["telemetry_enabled"] is False
        assert state["container_name"] == "tokenash-smoke"
