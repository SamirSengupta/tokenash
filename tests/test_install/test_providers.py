...
from tokenash.install.models import DeploymentManifest, ManagedMutation
from tokenash.install.providers import _apply_windows_env_scope, _remove_windows_env_scope
from tokenash.providers.claude.install import apply_provider_scope as apply_claude_provider_scope
from tokenash.providers.claude.install import build_install_env as build_claude_install_env
from tokenash.providers.claude.install import revert_provider_scope as revert_claude_provider_scope
from tokenash.providers.codex.install import apply_provider_scope as apply_codex_provider_scope
from tokenash.providers.codex.install import build_install_env as build_codex_install_env
from tokenash.providers.codex.install import revert_provider_scope as revert_codex_provider_scope
from tokenash.providers.copilot.install import build_install_env as build_copilot_install_env
from tokenash.providers.opencode.install import (
    apply_provider_scope as apply_opencode_provider_scope,
)
from tokenash.providers.opencode.install import build_install_env as build_opencode_install_env
from tokenash.providers.opencode.install import (
...
def test_apply_and_revert_claude_provider_scope(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "tokenash.providers.claude.install.claude_settings_path", lambda: settings_path
    )
...
def test_apply_and_revert_codex_provider_scope(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("tokenash.providers.codex.install.codex_config_path", lambda: config_path)
    manifest = _manifest(tmp_path)

    mutation = apply_codex_provider_scope(manifest)
    content = config_path.read_text()
    assert 'model_provider = "tokenash"' in content
    ...
    assert 'model_provider = "tokenash"' not in reverted
...
def test_apply_codex_provider_scope_replaces_existing_managed_block(
    ...
    config_path.write_text(
        'model = "gpt-4o"\n\n'
        "# --- Tokenash persistent provider ---\n"
        'model_provider = "tokenash"\n\n'
        "[model_providers.tokenash]\n"
        'name = "Tokenash persistent proxy"\n'
        'base_url = "http://127.0.0.1:1111/v1"\n'
        "requires_openai_auth = true\n"
        "supports_websockets = true\n"
        "# --- end Tokenash persistent provider ---\n"
    )
    monkeypatch.setattr("tokenash.providers.codex.install.codex_config_path", lambda: config_path)
    manifest = _manifest(tmp_path)
    manifest.port = 9999
    ...
    content = config_path.read_text()
    assert content.count("# --- Tokenash persistent provider ---") == 1
...
def test_apply_openclaw_provider_scope_uses_manifest_port(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("tokenash.providers.openclaw.install.shutil_which", lambda name: "openclaw")
    monkeypatch.setattr(
        "tokenash.providers.openclaw.install.resolve_tokenash_command",
        lambda: ["tokenash"],
    )
    monkeypatch.setattr(
        "tokenash.providers.openclaw.install._invoke_openclaw",
        lambda command: recorded.append(command),
    )
    monkeypatch.setattr(
        "tokenash.providers.openclaw.install.openclaw_config_path",
        lambda: tmp_path / "openclaw.json",
    )
    manifest = _manifest(tmp_path)
    manifest.port = 9999

    from tokenash.providers.openclaw.install import (
        apply_provider_scope as apply_openclaw_provider_scope,
    )

    apply_openclaw_provider_scope(manifest)

    assert recorded == [["tokenash", "wrap", "openclaw", "--no-auto-start", "--proxy-port", "9999"]]


def test_openclaw_apply_provider_scope_requires_installed_binary(
    tmp_path: Path, monkeypatch
) -> None:
    monkeypatch.setattr("tokenash.providers.openclaw.install.shutil_which", lambda name: None)
...
def test_openclaw_helper_wrappers_delegate_to_stdlib(monkeypatch) -> None:
    ...
    from tokenash.providers.openclaw.install import _invoke_openclaw, shutil_which

    assert shutil_which("openclaw") == "/fake/openclaw"
    _invoke_openclaw(["tokenash", "wrap", "openclaw"])

    assert recorded == [(["tokenash", "wrap", "openclaw"], True)]
...
def test_openclaw_revert_provider_scope_skips_without_binary(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("tokenash.providers.openclaw.install._invoke_openclaw", fail_if_called)
...
def test_openclaw_revert_provider_scope_invokes_unwrap(monkeypatch, tmp_path: Path) -> None:
    ...
    assert recorded == [["tokenash", "unwrap", "openclaw"]]
...
def test_windows_env_scope_restores_previous_values(monkeypatch, tmp_path: Path) -> None:
    ...
    manifest.base_env = {"TOKENASH_PORT": "8787"}
    ...
    def fake_run(command: list[str], **kwargs):
        ...
        if "GetEnvironmentVariable" in script:
            ...
            value = previous_values.get(name, "__TOKENASH_UNSET__")
            return Result(stdout=value)
        return Result()

    monkeypatch.setattr("tokenash.install.providers.subprocess.run", fake_run)
    ...
    assert previous_by_name["TOKENASH_PORT"] == "7777"
    assert previous_by_name["ANTHROPIC_BASE_URL"] == "https://old"
    assert any(
        "[Environment]::SetEnvironmentVariable('TOKENASH_PORT','7777','User')" in command[-1]
...
def test_apply_mutations_runs_openclaw_for_user_scope(monkeypatch, tmp_path: Path) -> None:
    ...
    if os.name == "nt":
        monkeypatch.setattr(
            "tokenash.install.providers._apply_windows_env_scope", lambda deployment: []
        )
    else:
        monkeypatch.setattr(
            "tokenash.install.providers._apply_unix_env_scope", lambda deployment: []
        )
    monkeypatch.setattr(
        "tokenash.install.providers.apply_provider_scope_mutations",
        lambda deployment: [ManagedMutation(target="openclaw", kind="openclaw-wrap")],
    )

    from tokenash.install.providers import apply_mutations
...
def test_apply_and_revert_opencode_provider_scope(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "tokenash.providers.opencode.install.opencode_config_path", lambda: config_path
    )
    manifest = _manifest(tmp_path)
    ...
    assert data["provider"]["tokenash"]["options"]["baseURL"] == "http://127.0.0.1:8787/v1"
    ...
    assert "tokenash" not in reverted.get("provider", {})
...
# must never appear in the tokenash provider block.
# ---------------------------------------------------------------------------


def test_tokenash_provider_block_never_sets_requires_openai_auth(
    monkeypatch, tmp_path: Path
) -> None:
    """apply_provider_scope must NOT emit requires_openai_auth in the tokenash block.

    Bug 3 (#406): requires_openai_auth = true on a custom [model_providers.tokenash]
    block forces codex to demand OpenAI OAuth login for tokenash-routed traffic.
    Tokenash is a local proxy — it must not require OpenAI auth.
    """
    for port in (8787, 9999):
        config_path = tmp_path / f"config_{port}.toml"
        monkeypatch.setattr(
            "tokenash.providers.codex.install.codex_config_path",
        ...
        content = config_path.read_text()
        # The rendered TOML for the tokenash provider block must not contain this
        # field.  If it does, codex will prompt for OpenAI OAuth on every startup.
        assert "requires_openai_auth" not in content, (
            f"requires_openai_auth must be absent from the tokenash provider block "
            f"(port={port}); got:\n{content}"
        )
        # Sanity: the block itself is present and points at the right port.
        assert f'base_url = "http://127.0.0.1:{port}/v1"' in content
        assert "[model_providers.tokenash]" in content
...
def test_inject_codex_provider_config_writes_openai_base_url(
    ...
    1. Strip ``requires_openai_auth`` from the tokenash provider block (done in 3ca48d3).
    2. Inject ``openai_base_url`` at the top level so subscription (ChatGPT plan)
       users are also routed through tokenash.
    ...
    """
    from tokenash.cli import wrap as wrap_mod
    ...
    assert 'openai_base_url = "http://127.0.0.1:8787/v1"' in content, (
        ...
        "so that Codex subscription (ChatGPT plan) users are routed through tokenash; "
        f"got:\n{content}"
    )
...
def test_unwrap_removes_top_level_openai_base_url(
    ...
    # Also verify via _strip_codex_tokenash_blocks directly — the orphan-cleanup
    ...
    stripped = wrap_mod._strip_codex_tokenash_blocks(orphan_content)
    assert "openai_base_url" not in stripped, (
        f"_strip_codex_tokenash_blocks must remove orphaned openai_base_url lines; got:\n{stripped}"
    )
...
def test_apply_provider_scope_writes_openai_base_url(monkeypatch, tmp_path: Path) -> None:
    ...
    through tokenash regardless of which entry point they used."""
    config_path = tmp_path / "config.toml"
    config_path.write_text('model = "gpt-4o"\n')
    monkeypatch.setattr("tokenash.providers.codex.install.codex_config_path", lambda: config_path)
    manifest = _manifest(tmp_path)
...
def test_planner_resolves_opencode_as_install_target() -> None:
    from tokenash.install.planner import resolve_targets
...
def test_planner_opencode_in_supported_targets_enum() -> None:
    from tokenash.install.models import ToolTarget
    from tokenash.install.planner import PROVIDER_SCOPE_TARGETS, SUPPORTED_TARGETS
...
def test_planner_build_tool_envs_includes_opencode() -> None:
    from tokenash.install.planner import build_tool_envs
...
def test_revert_opencode_provider_scope_fallback_on_oserror(monkeypatch, tmp_path: Path) -> None:
    ...
    config_path = tmp_path / "opencode.json"
    backup_path = config_path.with_suffix(".json.tokenash-backup")
    config_path.parent.mkdir(parents=True, exist_ok=True)

    from tokenash.install.models import ManagedMutation
    ...
    provider_json = '{"tokenash":{"npm":"@ai-sdk/openai-compatible","name":"Tokenash Proxy","options":{"baseURL":"http://127.0.0.1:8787/v1"}}}'
    config_path.write_text(
        f'{_PROVIDER_MARKER_START}\n"provider": {provider_json},\n{_PROVIDER_MARKER_END}\n'
    )

    monkeypatch.setattr(
        "tokenash.providers.opencode.install.opencode_config_path",
        lambda: config_path,
    )

    from tokenash.providers.opencode.install import revert_provider_scope
    ...
    assert not config_path.exists() or "tokenash" not in config_path.read_text()
