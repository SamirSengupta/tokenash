...
from tokenash.providers.copilot.wrap import (
    build_launch_env,
...
def test_detect_running_proxy_backend_requires_string_backend(monkeypatch) -> None:
    monkeypatch.setattr(
        "tokenash.providers.copilot.wrap.query_proxy_config",
...
def test_resolve_provider_type_prefers_explicit_and_env() -> None:
    ...
    assert resolve_provider_type(None, "auto", {"TOKENASH_BACKEND": "anthropic"}) == "anthropic"
    assert resolve_provider_type(None, "auto", {"TOKENASH_BACKEND": "anyllm"}) == "openai"
