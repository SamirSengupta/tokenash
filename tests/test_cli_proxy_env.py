...
Verifies that:
1. Provider target URL env vars are read by `tokenash proxy`
2. litellm-* backends are accepted by both CLI and argparse paths
3. TOKENASH_WRAP_PROXY_TIMEOUT controls `tokenash wrap` proxy readiness waits
"""
...
from tokenash.cli import wrap as wrap_mod  # noqa: E402
from tokenash.cli.main import main  # noqa: E402
...
class TestCLIProxyEnvVars:
    ...
    def test_tokenash_host_from_env(self, runner):
        """TOKENASH_HOST env var should be passed to ProxyConfig."""
        captured_config = {}

        def mock_run_server(config, **kwargs):
            captured_config["config"] = config

        with patch("tokenash.proxy.server.run_server", mock_run_server):
            result = runner.invoke(
                main,
                ["proxy"],
                env={"TOKENASH_HOST": "0.0.0.0"},
    ...
    def test_tokenash_port_from_env(self, runner):
        """TOKENASH_PORT env var should be passed to ProxyConfig."""
        captured_config = {}
    ...
    def test_tokenash_min_tokens_from_env(self, runner):
        """TOKENASH_MIN_TOKENS env var should be passed to ProxyConfig."""
        captured_config = {}
    ...
    def test_tokenash_budget_from_env(self, runner):
        """TOKENASH_BUDGET env var should be passed to ProxyConfig."""
        captured_config = {}
    ...
    def test_budget_period_flag_and_env(self, runner):
        """--budget-period and TOKENASH_BUDGET_PERIOD should reach ProxyConfig."""
        captured_config = {}
        ...
        with patch("tokenash.proxy.server.run_server", mock_run_server):
            result = runner.invoke(
                main,
                ["proxy"],
                env={"TOKENASH_BUDGET_PERIOD": "hourly"},
                catch_exceptions=False,
            )
    ...
    def test_code_aware_enabled_from_env(self, runner):
        """TOKENASH_CODE_AWARE_ENABLED env var should be passed to ProxyConfig."""
        captured_config = {}
        ...
        with patch("tokenash.proxy.server.run_server", mock_run_server):
            result = runner.invoke(
                main,
                ["proxy"],
                env={"TOKENASH_CODE_AWARE_ENABLED": "true"},
                catch_exceptions=False,
            )
    ...
    def test_code_aware_enabled_defaults_false(self, runner):
        """Without TOKENASH_CODE_AWARE_ENABLED, code-aware stays disabled in the wrapper."""
        captured_config = {}

        def mock_run_server(config, **kwargs):
            captured_config["config"] = config

        env = {k: v for k, v in os.environ.items() if k != "TOKENASH_CODE_AWARE_ENABLED"}

        with (
            patch("tokenash.proxy.server.run_server", mock_run_server),
    ...
    def test_disable_kompress_from_env(self, runner):
        """TOKENASH_DISABLE_KOMPRESS should be passed to ProxyConfig."""
        captured_config = {}
    ...
    def test_code_aware_flag_overrides_env_var(self, runner):
        """--code-aware should win over TOKENASH_CODE_AWARE_ENABLED=false."""
        captured_config = {}
        ...
        with patch("tokenash.proxy.server.run_server", mock_run_server):
            result = runner.invoke(
                main,
                ["proxy", "--code-aware"],
                env={"TOKENASH_CODE_AWARE_ENABLED": "false"},
                catch_exceptions=False,
            )
    ...
    def test_request_timeout_from_env(self, runner, timeout):
        """TOKENASH_REQUEST_TIMEOUT env var should be passed to ProxyConfig."""
        captured_config = {}
        ...
        with patch("tokenash.proxy.server.run_server", mock_run_server):
            result = runner.invoke(
                main,
                ["proxy"],
                env={"TOKENASH_REQUEST_TIMEOUT": f"{timeout}"},
                catch_exceptions=False,
            )
    ...
    def test_production_scaling_env_vars(self, runner):
        ...
        with patch("tokenash.proxy.server.run_server", mock_run_server):
            result = runner.invoke(
                ...
                env={
                    "TOKENASH_WORKERS": "4",
                    "TOKENASH_LIMIT_CONCURRENCY": "250",
                    "TOKENASH_MAX_CONNECTIONS": "200",
                    "TOKENASH_MAX_KEEPALIVE": "50",
                },
                catch_exceptions=False,
            )
    ...
    def test_keepalive_expiry_env_var(self, runner):
        ...
        with patch("tokenash.proxy.server.run_server", mock_run_server):
            result = runner.invoke(
                main,
                ["proxy"],
                env={"TOKENASH_KEEPALIVE_EXPIRY": "45"},
                catch_exceptions=False,
            )
...
class TestCLIAnyllmProviderEnv:
    """Test that TOKENASH_ANYLLM_PROVIDER env var is read by the CLI."""

    def test_anyllm_provider_from_env(self, runner):
        """TOKENASH_ANYLLM_PROVIDER env var should override the default."""
        captured_config = {}
        ...
        with patch("tokenash.proxy.server.run_server", mock_run_server):
            result = runner.invoke(
                ...
                env={"TOKENASH_ANYLLM_PROVIDER": "llamacpp"},
                catch_exceptions=False,
            )
...
class TestCLICompressionOnlyFlags:
    ...
    that can't resolve the injected tokenash_retrieve tool (issue #645).
    """
    ...
    def test_no_ccr_marker_from_env(self, runner):
        """TOKENASH_NO_CCR_MARKER env var disables marker injection."""
        captured_config = {}
...
class TestArgparseBackendValidation:
    """Test that the argparse path (python -m tokenash.proxy.server) accepts litellm-* backends."""
    ...
    def test_proxy_config_from_env_reads_disable_kompress(self):
        """The direct server env path should honor TOKENASH_DISABLE_KOMPRESS."""
        from tokenash.proxy.server import _proxy_config_from_env

        with patch.dict(os.environ, {"TOKENASH_DISABLE_KOMPRESS": "1"}):
    ...
    def test_proxy_config_from_env_reads_disable_kompress_fallback(self):
        """The direct server env path should honor TOKENASH_DISABLE_KOMPRESS_FALLBACK."""
        from tokenash.proxy.server import _proxy_config_from_env

        with patch.dict(os.environ, {"TOKENASH_DISABLE_KOMPRESS_FALLBACK": "1"}):
            config = _proxy_config_from_env()

        assert config.disable_kompress_fallback is True

    def test_argparse_registers_keepalive_expiry_flag(self):
        """The argparse path (python -m tokenash.proxy.server) must register
        --keepalive-expiry as a float flag, so it can override the
        TOKENASH_KEEPALIVE_EXPIRY fallback. A bad value makes argparse exit
        ...
        result = subprocess.run(
            [sys.executable, "-m", "tokenash.proxy.server", "--keepalive-expiry", "notafloat"],
            capture_output=True,
            text=True,
...
class TestCLIProxyExcludeToolsEnvVar:
    """TOKENASH_EXCLUDE_TOOLS and TOKENASH_TOOL_PROFILES must reach ProxyConfig via the Click path.

    Regression coverage for issue #825: the Click entrypoint (tokenash/cli/proxy.py)
    ...
    shared/deployed services that launch via `tokenash proxy`.
    """

    def test_exclude_tools_single_name_from_env(self, runner):
        """TOKENASH_EXCLUDE_TOOLS=WebSearch propagates to ProxyConfig.exclude_tools."""
        captured_config = {}
        ...
        with patch("tokenash.proxy.server.run_server", mock_run_server):
            result = runner.invoke(
                main,
                ["proxy"],
                env={"TOKENASH_EXCLUDE_TOOLS": "WebSearch"},
                catch_exceptions=False,
            )
    ...
    def test_exclude_tools_multi_name_from_env(self, runner):
        """TOKENASH_EXCLUDE_TOOLS=WebSearch,WebFetch yields both names (and lowercased) in result."""
        captured_config = {}
        ...
        with patch("tokenash.proxy.server.run_server", mock_run_server):
            result = runner.invoke(
                main,
                ["proxy"],
                env={"TOKENASH_EXCLUDE_TOOLS": "WebSearch,WebFetch"},
                catch_exceptions=False,
            )
    ...
    def test_exclude_tools_unset_leaves_none(self, runner):
        """Without TOKENASH_EXCLUDE_TOOLS, exclude_tools stays None (DEFAULT_EXCLUDE_TOOLS used)."""
        captured_config = {}

        def mock_run_server(config, **kwargs):
            captured_config["config"] = config

        env = {k: v for k, v in os.environ.items() if k != "TOKENASH_EXCLUDE_TOOLS"}
    ...
    def test_tool_profiles_from_env(self, runner):
        """TOKENASH_TOOL_PROFILES=Grep:conservative propagates to ProxyConfig.tool_profiles."""
        captured_config = {}
        ...
        with patch("tokenash.proxy.server.run_server", mock_run_server):
            result = runner.invoke(
                main,
                ["proxy"],
                env={"TOKENASH_TOOL_PROFILES": "Grep:conservative"},
                catch_exceptions=False,
            )
    ...
    def test_tool_profiles_unset_leaves_none(self, runner):
        """Without TOKENASH_TOOL_PROFILES, tool_profiles stays None."""
        captured_config = {}

        def mock_run_server(config, **kwargs):
            captured_config["config"] = config

        env = {k: v for k, v in os.environ.items() if k != "TOKENASH_TOOL_PROFILES"}
