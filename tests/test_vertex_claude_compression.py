...
Covers the fixes that let `tokenash wrap claude` + Vertex actually deliver
compression:
...
from tokenash.providers import proxy_routes, registry
...
def test_vertex_rawpredict_anthropic_runs_compression_handler(monkeypatch) -> None:
    ...
    monkeypatch.setattr(TokenashProxy, "handle_anthropic_messages", fake)
