...
def _build(**overrides: object) -> TokenashProxy:
    config = ProxyConfig(
        optimize=False,
        cache_enabled=False,
...
def _routers(proxy: TokenashProxy):
...
def test_proxy_config_from_env_reads_per_provider_kompress() -> None:
    with patch.dict(
        os.environ,
        {
            "TOKENASH_DISABLE_KOMPRESS_ANTHROPIC": "1",
            "TOKENASH_DISABLE_KOMPRESS_OPENAI": "0",
        },
    ):
...
def test_cli_enable_kompress_openai_from_env() -> None:
    ...
    with patch("tokenash.proxy.server.run_server", mock_run_server):
        result = CliRunner().invoke(
            main,
            ["proxy"],
            env={"TOKENASH_DISABLE_KOMPRESS_OPENAI": "0"},
            catch_exceptions=False,
        )
    assert result.exit_code == 0, result.output
