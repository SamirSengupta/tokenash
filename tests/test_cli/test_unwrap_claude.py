...
def test_unwrap_claude_removes_mcp_rtk_and_stops_proxy(
    ...
    with (
        patch("tokenash.mcp_registry.ClaudeRegistrar", return_value=Registrar()),
        patch(
    ...
    assert unregistered == ["tokenash", "codebase-memory-mcp"]
    assert stopped == [9999]
...
def test_unwrap_claude_preserves_user_managed_serena(
    ...
    monkeypatch.setenv("TOKENASH_WORKSPACE_DIR", str(tmp_path / ".tokenash"))
    unregistered: list[str] = []
    ...
    with (
        patch("tokenash.mcp_registry.ClaudeRegistrar", return_value=Registrar()),
        patch("tokenash.cli.wrap._remove_claude_rtk_hooks", return_value=False),
        patch("tokenash.cli.wrap._stop_local_proxy_for_unwrap"),
    ):
...
def test_unwrap_claude_removes_tokenash_installed_serena(
    runner: CliRunner,
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    ...
    from tokenash.mcp_registry import build_serena_spec
    from tokenash.mcp_registry.ledger import record_install
    ...
    assert result.exit_code == 0, result.output
    assert unregistered == ["tokenash", "codebase-memory-mcp", "serena"]
    assert "Removed Tokenash-installed Serena MCP server" in result.output
...
def test_unwrap_claude_keep_flags_skip_cleanup(
    ...
    with (
        patch("tokenash.mcp_registry.ClaudeRegistrar") as registrar,
        patch("tokenash.cli.wrap._remove_claude_rtk_hooks") as remove_rtk,
        patch("tokenash.cli.wrap._stop_local_proxy_for_unwrap") as stop_proxy,
    ):
                            ...
                            "hooks": [
                                {
                                    "type": "command",
                                    "command": (
                                        "/home/u/.local/bin/tokenash init hook ensure "
                                        "--profile init-user --marker tokenash-init-claude"
                                    ),
                                    "timeout": 15,
                                }
                            ],
