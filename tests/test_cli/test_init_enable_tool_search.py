"""`tokenash init claude` must set ENABLE_TOOL_SEARCH (GH #746).
...
`tokenash wrap claude` already sets it; `init` (and the persistent install) must
...
from tokenash.providers.claude import install as claude_install
