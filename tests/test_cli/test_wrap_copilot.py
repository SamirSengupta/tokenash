"""Tests for `tokenash wrap copilot` command."""
...
from tokenash.copilot_auth import DEFAULT_API_URL, CopilotSubscriptionTokenResolution
...
def _subscription_resolution(
    ...
    *,
    ...
    source: str = "tokenash-copilot-auth:/tmp/copilot_auth.json:token-exchange",
...
def wrap_modules(monkeypatch: pytest.MonkeyPatch) -> tuple[types.ModuleType, click.Group]:
    tokenash_pkg = sys.modules.get("tokenash")
    saved_tokenash_cli_attr = (
        tokenash_pkg.cli if tokenash_pkg is not None and hasattr(tokenash_pkg, "cli") else None
    )
    saved_modules = {
        name: sys.modules.get(name)
        for name in ("tokenash.cli", "tokenash.cli.main", "tokenash.cli.wrap")
    }

    fake_main_module = types.ModuleType("tokenash.cli.main")
    fake_main_module.main = click.Group()
    sys.modules["tokenash.cli.main"] = fake_main_module
    sys.modules.pop("tokenash.cli", None)
    sys.modules.pop("tokenash.cli.wrap", None)

    wrap_cli = importlib.import_module("tokenash.cli.wrap")
    monkeypatch.setattr(wrap_cli, "_check_proxy", lambda _port: False)

    try:
        yield wrap_cli, fake_main_module.main
    finally:
        for name in ("tokenash.cli.wrap", "tokenash.cli.main", "tokenash.cli"):
            sys.modules.pop(name, None)
        for name, module in saved_modules.items():
            if module is not None:
                sys.modules[name] = module
        if saved_modules["tokenash.cli"] is not None:
            cli_pkg = saved_modules["tokenash.cli"]
            if saved_modules["tokenash.cli.main"] is not None:
                cli_pkg.main = saved_modules["tokenash.cli.main"]
            if saved_modules["tokenash.cli.wrap"] is not None:
                cli_pkg.wrap = saved_modules["tokenash.cli.wrap"]
        if tokenash_pkg is not None:
            if saved_tokenash_cli_attr is None:
                if hasattr(tokenash_pkg, "cli"):
                    delattr(tokenash_pkg, "cli")
            else:
                tokenash_pkg.cli = saved_tokenash_cli_attr
...
def test_wrap_copilot_auto_anthropic_injects_instructions(
    ...
    with (
        patch("tokenash.cli.wrap.shutil.which", return_value="copilot"),
        patch("tokenash.cli.wrap.has_oauth_auth", return_value=False),
        patch("tokenash.cli.wrap._ensure_rtk_binary", return_value=Path("/tmp/rtk")),
        patch("tokenash.cli.wrap._launch_tool", side_effect=fake_launch_tool),
    ):
...
def test_wrap_copilot_byok_rejects_auto_model_before_launch(
    ...
    with (
        patch("tokenash.cli.wrap.shutil.which", return_value="copilot"),
        patch("tokenash.cli.wrap.has_oauth_auth", return_value=False),
        patch("tokenash.cli.wrap._launch_tool", side_effect=fail_launch_tool),
    ):
        result = runner.invoke(
            main,
            [
...
def test_wrap_copilot_auto_detects_running_proxy_backend(
    ...
    with (
        patch("tokenash.cli.wrap.shutil.which", return_value="copilot"),
        patch("tokenash.cli.wrap.has_oauth_auth", return_value=False),
        patch("tokenash.cli.wrap._check_proxy", return_value=True),
        patch("tokenash.cli.wrap._detect_running_proxy_backend", return_value="anyllm"),
        patch("tokenash.cli.wrap._launch_tool", side_effect=fake_launch_tool),
    ):
        result = runner.invoke(
            main,
...
def test_wrap_copilot_prefers_existing_oauth_session(
    ...
    with patch("tokenash.cli.wrap.shutil.which", return_value="copilot"):
        with patch("tokenash.cli.wrap.resolve_client_bearer_token", return_value="gho-existing"):
            with patch("tokenash.cli.wrap.has_oauth_auth", return_value=True):
                with patch("tokenash.cli.wrap._launch_tool", side_effect=fake_launch_tool):
                    result = runner.invoke(
                        main,
...
def test_wrap_copilot_subscription_uses_github_auth_without_provider_key(
    ...
    with (
        patch("tokenash.cli.wrap.shutil.which", return_value="copilot"),
        patch(
            "tokenash.cli.wrap.resolve_subscription_bearer_token_details",
...
def test_wrap_copilot_subscription_requires_reusable_auth(
    ...
    with (
        patch("tokenash.cli.wrap.shutil.which", return_value="copilot"),
        patch("tokenash.cli.wrap.resolve_subscription_bearer_token_details", return_value=None),
    ):
        result = runner.invoke(main, ["wrap", "copilot", "--subscription", "--no-rtk"])

    assert result.exit_code != 0
    assert "subscription mode requires a reusable GitHub/Copilot bearer token" in result.output
    assert "tokenash copilot-auth login" in result.output
...
def test_wrap_copilot_fails_when_binary_missing(
    ...
    _wrap_cli, main = wrap_modules
    with patch("tokenash.cli.wrap.shutil.which", return_value=None):
...
def test_unwrap_copilot_removes_rtk_instructions_and_stops_proxy(
    ...
    assert "Removed Tokenash rtk instructions from Copilot." in result.output
    assert "Stopped local Tokenash proxy on port 9999" in result.output
...
def test_unwrap_copilot_leaves_malformed_marker_content_unchanged(
    ...
    instructions = tmp_path / ".github" / "copilot-instructions.md"
    instructions.parent.mkdir()
    content = f"<!-- /tokenash:rtk-instructions -->\nKeep user guidance.\n{wrap_cli._RTK_MARKER}\n"
    ...
    assert "No Tokenash rtk instructions found for Copilot." in result.output
...
def test_wrap_copilot_oauth_keeps_generic_endpoint_when_account_advertised(
    ...
    with (
        patch("tokenash.cli.wrap.shutil.which", return_value="copilot"),
        patch("tokenash.cli.wrap.resolve_client_bearer_token", return_value="gho-oauth"),
        patch("tokenash.cli.wrap.has_oauth_auth", return_value=True),
        patch("tokenash.copilot_auth._fetch_copilot_user_info", return_value=_ACCOUNT_USER_INFO),
        patch("tokenash.cli.wrap._launch_tool", side_effect=fake_launch_tool),
    ):
...
def test_wrap_copilot_byok_never_resolves_copilot_endpoint(
    ...
    with (
        patch("tokenash.cli.wrap.shutil.which", return_value="copilot"),
        patch("tokenash.cli.wrap.has_oauth_auth", return_value=False),
        patch("tokenash.cli.wrap.resolve_copilot_api_url", side_effect=tripwire),
        patch("tokenash.cli.wrap._launch_tool", side_effect=fake_launch_tool),
    ):
        result = runner.invoke(
            main,
...
def test_resolve_copilot_api_url_ignores_user_info_and_never_calls_network(
    ...
    from tokenash import copilot_auth
