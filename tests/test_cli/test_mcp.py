...
from tokenash.cli.mcp import (
    get_tokenash_command,
    load_mcp_config,
    save_mcp_config,
)
...
def mock_claude_config_path(temp_claude_dir):
    ...
    with patch("tokenash.cli.mcp.MCP_CONFIG_PATH", config_path):
        with patch("tokenash.cli.mcp.CLAUDE_CONFIG_DIR", temp_claude_dir):
            with patch("tokenash.cli.mcp.shutil.which", side_effect=which_no_claude):
                yield config_path
...
class TestMCPConfigFunctions:
    ...
    def test_get_tokenash_command_returns_list(self):
        """Command should be a list suitable for subprocess."""
        cmd = get_tokenash_command()
    ...
    def test_save_config_creates_directory(self, tmp_path):
        ...
        with patch("tokenash.cli.mcp.MCP_CONFIG_PATH", config_path):
            with patch("tokenash.cli.mcp.CLAUDE_CONFIG_DIR", claude_dir):
...
class TestMCPUninstallCommand:
    """Test 'tokenash mcp uninstall' command."""

    def test_uninstall_removes_tokenash(self, mock_claude_config_path, mock_mcp_available):
        """Uninstall removes tokenash from the legacy config file."""
        ...
        mock_claude_config_path.write_text(
            json.dumps(
                {
                    "mcpServers": {
                        "tokenash": {"command": "tokenash", "args": ["mcp", "serve"]},
                    }
                }
            )
        )
    ...
    def test_uninstall_not_configured(self, mock_claude_config_path):
        """Uninstall when tokenash not in config exits cleanly."""
        # Create config without tokenash
...
class TestMCPStatusCommand:
    """Test 'tokenash mcp status' command."""
    ...
    def test_status_configured(self, mock_claude_config_path, mock_mcp_available):
        """Status reports configured when the legacy config has tokenash."""
...
class TestMCPServeCommand:
    """Test 'tokenash mcp serve' command."""
...
class TestMCPServerInitialization:
    ...
    def test_mcp_server_has_correct_tool_name(self):
        """MCP server is configured for tokenash_retrieve tool."""
        from tokenash.ccr.mcp_server import create_ccr_mcp_server
        ...
        assert server.server.name == "tokenash"
        # The tool name should be tokenash_retrieve
        assert CCR_TOOL_NAME == "tokenash_retrieve"
...
class TestMCPUninstallWithClaudeCLI:
    ...
    def test_uninstall_calls_claude_mcp_remove(self):
        """Uninstall calls claude mcp remove when tokenash is registered."""
        calls = []
        ...
        runner = CliRunner()
        with patch("tokenash.cli.mcp.shutil.which", return_value="/usr/bin/claude"):
            with patch("tokenash.cli.mcp.subprocess.run", side_effect=capturing_run):
                result = runner.invoke(main, ["mcp", "uninstall"])
    ...
    def test_uninstall_skips_remove_when_not_registered(self):
        """Uninstall does not call remove when tokenash is not registered via claude CLI."""
        calls = []
