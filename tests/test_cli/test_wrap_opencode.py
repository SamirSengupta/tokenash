"""Tests for `tokenash wrap opencode` and `tokenash unwrap opencode`."""
...
def test_wrap_opencode_sets_config_content_env(
    ...
    """OPENCODE_CONFIG_CONTENT env var is set with the tokenash provider."""
    monkeypatch.chdir(tmp_path)
    monkeypatch.delenv("TOKENASH_CONTEXT_TOOL", raising=False)
    ...
    assert config["provider"]["tokenash"]["options"]["baseURL"] == "http://127.0.0.1:9000/v1"
...
def test_wrap_opencode_injects_mcp_by_default(
    ...
    assert config["mcp"]["tokenash"]["type"] == "remote"
...
def test_unwrap_opencode_restores_from_backup(
    ...
    config_file = tmp_path / ".config" / "opencode" / "opencode.json"
    backup_file = config_file.with_suffix(".json.tokenash-backup")
...
def test_unwrap_opencode_strips_blocks_when_no_backup(
    ...
    """Unwrap strips Tokenash blocks when no backup exists."""
    monkeypatch.chdir(tmp_path)
    _set_test_home(monkeypatch, tmp_path)
    ...
    assert result.exit_code == 0, result.output
    assert "Removed Tokenash block" in result.output
...
def test_wrap_opencode_preserves_existing_user_providers(
    ...
    """Wrap merges tokenash provider without disturbing user's existing providers."""
    monkeypatch.chdir(tmp_path)
    monkeypatch.delenv("TOKENASH_CONTEXT_TOOL", raising=False)
    _set_test_home(monkeypatch, tmp_path)
    ...
    assert result.exit_code == 0, result.output
    config = json.loads(config_file.read_text())
    assert "tokenash" in config["provider"], "tokenash provider not injected"
...
def test_wrap_opencode_port_change_updates_existing_config(
    ...
    config_file = tmp_path / ".config" / "opencode" / "opencode.json"
    config = json.loads(config_file.read_text())
    assert config["provider"]["tokenash"]["options"]["baseURL"] == "http://127.0.0.1:9001/v1"
...
def test_wrap_opencode_handles_malformed_config_file(
    ...
    # The config file is now valid JSON with tokenash provider.
    config = json.loads(config_file.read_text())
...
def test_wrap_opencode_tokenash_project_from_cwd(
    runner: CliRunner,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """TOKENASH_PROJECT is set based on the current working directory name."""
    ...
    assert result.exit_code == 0, result.output
    env = captured["env"]
    assert env.get("TOKENASH_PROJECT") == "my-project"


def test_wrap_opencode_respects_existing_tokenash_project(
    runner: CliRunner,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """User-set TOKENASH_PROJECT env var is preserved, not overridden."""
    monkeypatch.chdir(tmp_path)
    monkeypatch.delenv("TOKENASH_CONTEXT_TOOL", raising=False)
    _set_test_home(monkeypatch, tmp_path)
    monkeypatch.setenv("TOKENASH_PROJECT", "user-set-value")
    ...
    assert result.exit_code == 0, result.output
    env = captured["env"]
    assert env["TOKENASH_PROJECT"] == "user-set-value"
...
def test_unwrap_opencode_removes_config_when_only_tokenash_content(
    runner: CliRunner,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Unwrap removes the config file entirely when it contained only Tokenash content."""
    monkeypatch.chdir(tmp_path)
    _set_test_home(monkeypatch, tmp_path)
...
def test_unwrap_opencode_noop_when_no_tokenash_markers(
    runner: CliRunner,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Unwrap is a safe no-op when the config has no Tokenash markers."""
    monkeypatch.chdir(tmp_path)
    _set_test_home(monkeypatch, tmp_path)
    ...
    assert result.exit_code == 0, result.output
    assert "no Tokenash wrap markers" in result.output
...
def test_wrap_unwrap_rewrap_is_idempotent(
    ...
    assert "tokenash" not in after_unwrap.get("provider", {})
    ...
    # After re-wrap, tokenash should be back, model unchanged
    after_rewrap = json.loads(config_file.read_text())
    assert after_rewrap["model"] == "openai/gpt-4o"
    assert "tokenash" in after_rewrap.get("provider", {})
...
def test_wrap_opencode_no_arguments_is_valid(
    ...
    """`tokenash wrap opencode` with no additional arguments is a valid command."""
    monkeypatch.chdir(tmp_path)
    monkeypatch.delenv("TOKENASH_CONTEXT_TOOL", raising=False)
    _set_test_home(monkeypatch, tmp_path)
