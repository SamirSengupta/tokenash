...
The fix is a shared wrapper at ``tokenash._subprocess`` that automatically sets
...
_PACKAGE = Path(__file__).resolve().parents[2] / "tokenash"
...
def test_text_mode_subprocess_calls_use_wrapper() -> None:
    ...
    assert not offenders, (
        "raw subprocess calls with text=True found (use tokenash._subprocess wrapper):\n"
        + "\n".join(offenders)
    )
