"""Tests for `tokenash wrap codex` and `tokenash unwrap codex`.

These exercise the Codex-specific ``config.toml`` injection and restoration
helpers that route Codex through the Tokenash proxy.  They are deliberately
...
from tokenash.cli.main import main
...
class TestStripCodexTokenashBlocks:
    ...
    def test_removes_complete_tokenash_block(self) -> None:
        wrapped = (
        ...
        assert wrap_mod._strip_codex_tokenash_blocks(wrapped) == ""
    ...
    def test_preserves_user_content_around_block(self) -> None:
        ...
        cleaned = wrap_mod._strip_codex_tokenash_blocks(wrapped)
    ...
    def test_removes_stray_top_level_model_provider_line(self) -> None:
        # Old wrap versions left `model_provider = "tokenash"` outside markers.
        content = 'foo = 1\nmodel_provider = "tokenash"\nbar = 2\n'
        cleaned = wrap_mod._strip_codex_tokenash_blocks(content, remove_mcp=True)
        assert 'model_provider = "tokenash"' not in cleaned
    ...
    def test_removes_codex_mcp_blocks(self) -> None:
        content = (
            ...
            "[mcp_servers.tokenash]\n"
            'command = "tokenash"\n'
            f"{wrap_mod._CODEX_MCP_END}\n\n"
            "# --- Tokenash MCP server: serena ---\n"
            "[mcp_servers.serena]\n"
            'command = "uvx"\n'
            "# --- end Tokenash MCP server: serena ---\n\n"
            f"{wrap_mod._MEMORY_MCP_MARKER}\n"
            "[mcp_servers.tokenash_memory]\n"
            'command = "python"\n'
        ...
        assert "[mcp_servers.tokenash]" not in cleaned
        assert "[mcp_servers.serena]" not in cleaned
        assert "[mcp_servers.tokenash_memory]" not in cleaned
        assert 'model = "gpt-4o"' in cleaned
    ...
    def test_preserves_named_mcp_blocks_when_remove_named_mcp_false(self) -> None:
        ...
        cleaned = wrap_mod._strip_codex_tokenash_blocks(
...
class TestSnapshotCodexConfig:
    ...
    def test_creates_backup_on_first_call(self, tmp_path: Path) -> None:
        ...
        backup_file = tmp_path / "config.toml.tokenash-backup"
    ...
    def test_no_backup_when_config_already_contains_memory_mcp_block(self, tmp_path: Path) -> None:
        ...
        config_file.write_text(
            ...
            "[mcp_servers.tokenash_memory]\n"
            'command = "python"\n'
            'args = ["-m", "tokenash.memory.mcp_server", "--user", "codex-user"]\n'
            f"{wrap_mod._MEMORY_MCP_END}\n"
        )
    ...
    def test_backup_when_config_contains_named_mcp_marker(self, tmp_path: Path) -> None:
        ...
        backup_file = tmp_path / "config.toml.tokenash-backup"
        original = (
            "# --- Tokenash MCP server: tokenash ---\n"
            "[mcp_servers.tokenash]\n"
            'command = "tokenash"\n'
            "# --- end Tokenash MCP server: tokenash ---\n"
        )
...
class TestCodexMemoryMcpConfig:
    ...
    def test_inject_omits_db_and_replaces_existing_memory_block(
        ...
        config_file.write_text(
            ...
            "[mcp_servers.tokenash_memory]\n"
            'command = "python"\n'
            'args = ["-m", "tokenash.memory.mcp_server", "--db", "/tmp/project-a/.tokenash/memory.db", "--user", "old-user"]\n'
            f"{wrap_mod._MEMORY_MCP_END}\n"
        )
        ...
        assert "[mcp_servers.tokenash_memory]" in content
...
class TestInjectAndRestoreRoundTrip:
    ...
    def test_wrap_unwrap_restores_empty_state(
        ...
        assert 'model_provider = "tokenash"' in config_file.read_text()

        status, _ = wrap_mod._restore_codex_provider_config()
        # No prior config existed → the injected file is fully removed.
        assert status == "removed"
        assert not config_file.exists()
        assert not (tmp_path / ".codex" / "config.toml.tokenash-backup").exists()
    ...
    def test_wrap_unwrap_restores_prior_model_provider(
        ...
        assert 'model_provider = "tokenash"' in wrapped
        assert "[model_providers.tokenash]" in wrapped

        status, _ = wrap_mod._restore_codex_provider_config()
        assert status == "restored"
        assert config_file.read_text() == original
        assert not (config_dir / "config.toml.tokenash-backup").exists()
    ...
    def test_wrap_is_idempotent(self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
        ...
        content = config_file.read_text()
        # Exactly two Tokenash blocks — a top-level-key block and the
    ...
    def test_unwrap_without_backup_removes_provider_and_mcp_blocks(
        ...
        cleaned = config_file.read_text()
        assert 'model = "gpt-4o"' in cleaned
        assert "tokenash" not in cleaned
    ...
    def test_memory_only_wrap_without_backup_preserves_named_mcp_block(
        ...
        config_dir.mkdir()
        config_file = config_dir / "config.toml"
        backup_file = config_dir / "config.toml.tokenash-backup"
        original = (
            "# --- Tokenash MCP server: tokenash ---\n"
            "[mcp_servers.tokenash]\n"
            'command = "tokenash"\n'
...
# ---------------------------------------------------------------------------
# Thread retag: wrap pulls native threads into the tokenash menu, unwrap hands
# them back, so the Codex history list stays whole across the proxy boundary.
# ---------------------------------------------------------------------------


class TestWrapRetagsThreadProviders:
    """``wrap codex`` retags ``openai`` threads to ``tokenash`` and back."""
    ...
    def test_wrap_unwrap_round_trips_thread_providers(
        ...
        self._seed_threads(gui_db, [("a", "openai"), ("b", "tokenash"), ("c", "anthropic")])
        self._seed_threads(cli_db, [("d", "openai")])

        with patch("tokenash.cli.wrap._ensure_rtk_binary", return_value=None):
            wrap_result = runner.invoke(main, ["wrap", "codex", "--prepare-only", "--port", "8787"])
        assert wrap_result.exit_code == 0, wrap_result.output
        # Native threads are now visible under the tokenash provider menu;
        # third-party providers are left untouched.
        assert self._count(gui_db, "tokenash") == 2
        assert self._count(gui_db, "openai") == 0
        assert self._count(gui_db, "anthropic") == 1
        assert self._count(cli_db, "tokenash") == 1

        with patch("tokenash.cli.wrap._stop_local_proxy_for_unwrap", return_value="stopped"):
        ...
        assert self._count(gui_db, "tokenash") == 0
        assert self._count(gui_db, "anthropic") == 1
...
class TestSubscriptionRouting:
    ...
    def test_strip_cleans_orphaned_openai_base_url(self) -> None:
        ...
        cleaned = wrap_mod._strip_codex_tokenash_blocks(content)
...
class TestInjectAvoidsDuplicateTopLevelKeys:
    ...
    def test_inject_does_not_create_duplicate_model_provider(
        ...
        # And the rewritten values are the tokenash ones.
...
# ---------------------------------------------------------------------------
# Integration tests: full `tokenash wrap codex` / `tokenash unwrap codex`
# ---------------------------------------------------------------------------
...
def test_wrap_codex_prepare_only_creates_backup_and_config(
    ...
    assert 'model_provider = "tokenash"' in config_file.read_text()
    backup = tmp_path / ".codex" / "config.toml.tokenash-backup"
...
def test_wrap_codex_prepare_only_respects_codex_home(
    ...
    content = config_file.read_text()
    assert 'model_provider = "tokenash"' in content
    assert "[mcp_servers.tokenash]" in content
    assert not (tmp_path / ".codex" / "config.toml").exists()
...
def test_wrap_codex_injects_rtk_globally_without_changing_project_agents(
    ...
    with patch(
        "tokenash.cli.wrap._ensure_rtk_binary",
...
def test_unwrap_codex_without_codex_home_warns_on_ambiguous_noop(
    ...
    assert unwrap_result.exit_code == 0, unwrap_result.output
    assert "Warning: found no Tokenash wrap markers in the default Codex config" in (
        unwrap_result.output
    )
    assert "If you wrapped Codex with CODEX_HOME" in unwrap_result.output
    assert "CODEX_HOME=/path/to/codex-home tokenash unwrap codex" in unwrap_result.output
...
def test_start_proxy_does_not_apply_agent_90_defaults(
    ...
    assert "TOKENASH_SAVINGS_PROFILE" not in env
    assert "TOKENASH_TARGET_RATIO" not in env
    assert "TOKENASH_MAX_ITEMS" not in env
    assert "TOKENASH_SMART_CRUSHER_COMPACTION" not in env
...
def test_start_proxy_preserves_explicit_savings_overrides(
    ...
    monkeypatch.setenv("TOKENASH_TARGET_RATIO", "0.20")
    monkeypatch.setenv("TOKENASH_MAX_ITEMS", "12")
    monkeypatch.setattr(wrap_mod, "_get_log_path", lambda: tmp_path / "proxy.log")
    monkeypatch.setattr(wrap_mod, "_check_proxy", lambda port: True)
    monkeypatch.setattr(wrap_mod.subprocess, "Popen", fake_popen)
    ...
    env = popen_kwargs["env"]
    assert isinstance(env, dict)
    assert env["TOKENASH_TARGET_RATIO"] == "0.20"
    assert env["TOKENASH_MAX_ITEMS"] == "12"
...
def test_wrap_codex_prepare_only_updates_stale_mcp_proxy_url(
    ...
    config_file.write_text(
        ...
        'args = ["mcp", "serve"]\n'
        "\n"
        "[mcp_servers.tokenash.env]\n"
        'TOKENASH_PROXY_URL = "http://127.0.0.1:9000"\n'
        "# --- end Tokenash MCP server ---\n"
    )
    ...
    content = config_file.read_text()
    assert "[mcp_servers.tokenash]" in content
    assert 'command = "tokenash"' in content
...
def test_wrap_codex_memory_prepare_only_uses_local_db_without_persisting_it(
    ...
    with patch("tokenash.cli.wrap._ensure_rtk_binary", return_value=None):
        with patch("tokenash.memory.sync._build_sync_backend", side_effect=fake_build_sync_backend):
            with patch("tokenash.memory.sync.sync_import", side_effect=fake_sync_import):
                with patch(
                    "tokenash.memory.sync_adapters.claude_code.ClaudeCodeAdapter",
                    FakeClaudeCodeAdapter,
                ):
                    with patch(
                        "tokenash.memory.sync_adapters.claude_code.get_claude_memory_dir",
    ...
    assert result.exit_code == 0, result.output
    assert backend_paths == [str(project_dir / ".tokenash" / "memory.db")]
...
def test_wrap_codex_prepare_only_registers_serena_when_uvx_exists(
    ...
    with patch("tokenash.cli.wrap._ensure_rtk_binary", return_value=None):
        with patch("tokenash.cli.wrap.shutil.which", side_effect=fake_which):
...
def test_unwrap_codex_restores_prior_config_end_to_end(
    ...
    with patch(
        "tokenash.cli.wrap._stop_local_proxy_for_unwrap",
    ...
    # proxy is stopped.
    assert config_file.read_text() == original
    assert 'model_provider = "tokenash"' not in config_file.read_text()
    assert not (tmp_path / ".codex" / "config.toml.tokenash-backup").exists()
    assert stopped == [9999]
    assert "Stopped local Tokenash proxy on port 9999" in unwrap_result.output
...
def test_unwrap_codex_no_stop_proxy_leaves_proxy_alone(
    ...
    with patch("tokenash.cli.wrap._stop_local_proxy_for_unwrap") as stop_proxy:
...
def test_wrap_codex_memory_prepare_only_unwrap_removes_memory_mcp_without_prior_config(
    ...
    with patch("tokenash.cli.wrap._ensure_rtk_binary", return_value=None):
        with patch("tokenash.memory.sync._build_sync_backend", return_value=FakeBackend()):
            with patch("tokenash.memory.sync.sync_import", side_effect=fake_sync_import):
                with patch(
                    "tokenash.memory.sync_adapters.claude_code.ClaudeCodeAdapter",
                    autospec=True,
...
def test_stop_local_proxy_for_unwrap_kills_identified_tokenash_proxy(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_stop_local_proxy_for_unwrap_refuses_unidentified_listener(
    ...
    with patch("tokenash.cli.wrap._kill_proxy_by_pid") as kill_proxy:
...
def test_unwrap_codex_removes_tokenash_only_config_file(
    runner: CliRunner, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    _set_test_home(monkeypatch, tmp_path)
...
class TestCodexProjectHeaderConfig:
    """The injected provider maps X-Tokenash-Project to TOKENASH_PROJECT.

    Codex's ``env_http_headers`` sends a header only when the mapped env var
    is set at Codex runtime, so `tokenash wrap codex` exports
    ``TOKENASH_PROJECT`` and the proxy attributes savings per project.
    """
    ...
    def test_inject_writes_env_http_headers_mapping(
        ...
        content = (tmp_path / ".codex" / "config.toml").read_text()
        assert 'env_http_headers = { "X-Tokenash-Project" = "TOKENASH_PROJECT" }' in content

    def test_env_http_headers_inside_provider_section(
        self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
    ) -> None:
        """The mapping must live inside [model_providers.tokenash], before
        the closing marker, so it applies to the Tokenash provider."""
        _set_test_home(monkeypatch, tmp_path)

        wrap_mod._inject_codex_provider_config(8787)

        content = (tmp_path / ".codex" / "config.toml").read_text()
        section_start = content.index("[model_providers.tokenash]")
    ...
    def test_strip_removes_block_with_env_http_headers(
        ...
        """_strip_codex_tokenash_blocks removes the whole injected block,
        ...
        assert "X-Tokenash-Project" not in cleaned
        assert "[model_providers.tokenash]" not in cleaned
        assert 'model = "gpt-4o"' in cleaned
