...
Serena is installed by default, so a prior `tokenash wrap` persists a
`serena` MCP entry and the agent keeps launching it (dashboard popup and
all). These tests pin that a later `--no-serena` removes the entry Tokenash
...
def test_disable_preserves_user_managed_serena(
    ...
    monkeypatch.setenv("TOKENASH_WORKSPACE_DIR", str(tmp_path / ".tokenash"))
    # Present in the agent config but NOT in Tokenash's ledger → user-owned.
...
def test_unwrap_codex_removes_tokenash_installed_serena(
    ...
    with (
        patch("tokenash.mcp_registry.CodexRegistrar", return_value=registrar),
        patch(
            "tokenash.cli.wrap._restore_codex_provider_config",
    ...
    assert registrar.unregistered == ["serena"]
    assert "Removed Tokenash-installed Serena MCP server from Codex" in result.output
