...
def _load_init_module(monkeypatch):
    monkeypatch.delitem(sys.modules, "tokenash.cli.init", raising=False)
    monkeypatch.delitem(sys.modules, "tokenash.cli.main", raising=False)
    ...
    monkeypatch.setitem(sys.modules, "tokenash.cli.main", fake_main_module)
    importlib.invalidate_caches()
    init_cli = importlib.import_module("tokenash.cli.init")
    monkeypatch.delitem(sys.modules, "tokenash.cli.init", raising=False)
...
def test_init_fails_when_auto_detection_empty(monkeypatch) -> None:
    """Bare ``tokenash init`` with no agents on PATH prints a guided error.
    ...
    assert "tokenash init -g claude" in result.output
...
def test_format_empty_detection_error_local_scope(monkeypatch) -> None:
    ...
    assert "tokenash init copilot" not in message
    assert "tokenash init openclaw" not in message
    assert "tokenash init claude" in message
    assert "tokenash init codex" in message
...
def test_init_verbose_enables_debug_logging_on_stderr(monkeypatch) -> None:
    """``tokenash init -v`` should emit diagnostic lines to stderr.
    ...
    assert "[tokenash init]" in stderr
...
def test_init_codex_creates_hooks_feature_flag_on_first_init(
    ...
    assert parsed["model_provider"] == "tokenash"
...
def test_init_openclaw_delegates_to_wrap(monkeypatch) -> None:
    ...
    monkeypatch.setattr(init_cli, "resolve_tokenash_command", lambda: ["tokenash"])
    monkeypatch.setattr(
        init_cli.subprocess,
        "run",
    ...
    assert calls == [["tokenash", "wrap", "openclaw", "--proxy-port", "9999"]]
...
def test_marketplace_source_prefers_env_override(monkeypatch) -> None:
    init_cli, _ = _load_init_module(monkeypatch)
    monkeypatch.setenv("TOKENASH_MARKETPLACE_SOURCE", "custom/source")
...
def test_command_string_and_matcher_on_windows(monkeypatch) -> None:
    ...
    assert init_cli._command_string(["tokenash", "init"]) == "joined-command"
...
def test_command_string_normalizes_backslashes_on_windows(monkeypatch) -> None:
    ...
    result = init_cli._command_string(
        ["C:\\Users\\user\\.local\\bin\\tokenash.exe", "init", "hook", "ensure"]
    )
    assert "\\" not in result
    assert "C:/Users/user/.local/bin/tokenash.exe" in result
...
def test_command_string_quotes_spaces_after_normalization(monkeypatch) -> None:
    ...
    result = init_cli._command_string(
        ["C:\\Program Files\\tokenash\\tokenash.exe", "init", "hook", "ensure"]
    )
    assert "\\" not in result
    assert '"C:/Program Files/tokenash/tokenash.exe"' in result
...
def test_ensure_claude_hooks_rewrites_existing_entries(monkeypatch, tmp_path: Path) -> None:
                    ...
                    "SessionStart": [
                        ...
                        },
                        {
                            ...
                            "hooks": [
                                {
                                    "type": "command",
                                    "command": "tokenash init hook ensure --marker tokenash-init-claude",
                                }
                            ],
                        },
                    ]
    ...
    monkeypatch.setattr(init_cli, "_hook_command", lambda *parts: "tokenash init hook ensure")
    ...
    assert session_entries[-1]["hooks"][0]["command"].endswith("--marker tokenash-init-claude")
...
def test_ensure_copilot_hooks_replaces_existing_marker(monkeypatch, tmp_path: Path) -> None:
        ...
        json.dumps(
            {
                "hooks": {
                    "SessionStart": [
                        ...
                        {
                            "type": "command",
                            "command": "tokenash init hook ensure --marker tokenash-init-copilot",
                        },
                    ]
                }
            }
    ...
    assert commands == ["echo keep", "tokenash init hook ensure --marker tokenash-init-copilot"]
...
def test_ensure_codex_provider_keeps_root_keys_above_existing_table(
    ...
    'invalid type: string "tokenash", expected a boolean in features'.
    """
    init_cli, _ = _load_init_module(monkeypatch)
    path = tmp_path / "config.toml"
    ...
    assert parsed["features"]["hooks"] is True
    assert parsed["model_providers"]["tokenash"]["base_url"] == "http://127.0.0.1:8787/v1"
...
def test_marketplace_source_prefers_repo_checkout(monkeypatch) -> None:
    init_cli, _ = _load_init_module(monkeypatch)
    monkeypatch.delenv("TOKENASH_MARKETPLACE_SOURCE", raising=False)
...
def test_install_claude_marketplace_runs_expected_commands(monkeypatch) -> None:
    ...
    assert calls == [
        ...
        (
            ["claude", "plugin", "install", "tokenash@tokenash-marketplace", "--scope", "user"],
...
def test_install_copilot_marketplace_runs_expected_commands(monkeypatch) -> None:
    ...
    assert calls == [
        ...
        (
            ["copilot", "plugin", "install", "tokenash@tokenash-marketplace"],
...
def test_init_codex_writes_openai_base_url(monkeypatch, tmp_path: Path) -> None:
    ...
    subscription (ChatGPT plan) users are routed through tokenash even when the
