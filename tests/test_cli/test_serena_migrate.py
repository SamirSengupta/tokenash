"""Re-wrap must migrate a stale Tokenash-installed Serena entry.
...
``_setup_serena_mcp`` closes that gap: when the ledger proves Tokenash
...
from tokenash.mcp_registry.base import RegisterResult, RegisterStatus, ServerSpec
from tokenash.mcp_registry.ledger import tokenash_installed_matching, record_install

# The Serena spec Tokenash wrote before the dashboard flag existed.
...
def test_rewrap_migrates_stale_tokenash_serena(
    capsys: pytest.CaptureFixture[str],
) -> None:
    # Ledger proves Tokenash installed the stale entry that's on disk.
    ...
    assert tokenash_installed_matching("claude", fresh)


def test_rewrap_leaves_user_managed_serena(
    capsys: pytest.CaptureFixture[str],
) -> None:
    # Differs from the current spec but is NOT in Tokenash's ledger.
...
def test_rewrap_fresh_install_records_dashboard_off_spec(
    ...
    assert tokenash_installed_matching("claude", entry)
