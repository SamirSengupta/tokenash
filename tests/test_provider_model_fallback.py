...
from tokenash.providers.anthropic import (
    AnthropicProvider,
    _infer_model_tier,
)
from tokenash.providers.anthropic import (
...
from tokenash.providers.openai import (
    OpenAIProvider,
    _infer_model_family,
)
from tokenash.providers.openai import (
...
class TestAnthropicConfigLoading:
    ...
    def test_load_from_env_var_json(self):
        ...
        with patch.dict(os.environ, {"TOKENASH_MODEL_LIMITS": json.dumps(config)}):
    ...
    def test_load_from_env_var_file(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch.dict(os.environ, {"TOKENASH_MODEL_LIMITS": str(config_path)}):
                loaded = anthropic_load_config()
                assert loaded["context_limits"]["file-model"] == 400000

    def test_load_from_config_file(self):
        """Test loading from ~/.tokenash/models.json."""
        config = {
            "anthropic": {
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            config_dir = Path(tmpdir) / ".tokenash"
            config_dir.mkdir()
    ...
    def test_env_var_overrides_config_file(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch.object(Path, "home", return_value=Path(tmpdir)):
                with patch.dict(os.environ, {"TOKENASH_MODEL_LIMITS": json.dumps(env_config)}):
                    loaded = anthropic_load_config()
                    # Env var should win
...
class TestCrossProviderConsistency:
    ...
    def test_both_providers_use_same_env_var(self):
        """Test that both providers use TOKENASH_MODEL_LIMITS."""
        config = {
    ...
    def test_both_providers_warn_for_unknown_models(self):
        ...
        from tokenash.providers import anthropic as anthropic_module
        from tokenash.providers import openai as openai_module
