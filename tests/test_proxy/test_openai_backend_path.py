...
2. Backend response with tokenash_retrieve tool call → ccr_response_handler.handle_response
...
from tokenash.backends.base import BackendResponse  # noqa: E402
...
def test_backend_response_updates_prefix_tracker_with_bedrock_cache_fields():
    ...
    with patch("tokenash.proxy.server.AnyLLMBackend", return_value=mock_backend):
...
def test_backend_response_with_ccr_tool_call_is_intercepted_and_resolved():
    """OpenAI-shape response carrying tokenash_retrieve → CCR handler resolves it."""
    config = _make_config()
    # First response: tool_call for tokenash_retrieve
    tool_call_response = {
        "id": "chatcmpl-ccr-1",
        "object": "chat.completion",
        "model": "anthropic.claude-3-5-sonnet-20241022-v2:0",
