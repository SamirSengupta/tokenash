#!/usr/bin/env python3
"""End-to-end token-savings test for Cortex Code (CoCo) + Tokenash.
...
emit.  Tokenash's SmartCrusher compresses JSON natively without any ML
...
Usage:
    ...
    cd tokenash && uv run python tests/test_cortex_code_compression.py

    # Pytest (CI-friendly assertions):
    cd tokenash && uv run --with pytest pytest tests/test_cortex_code_compression.py -v -s
"""
...
def test_cortex_code_tokenash_compression_saves_tokens() -> None:
    """Tokenash must compress a realistic multi-turn CoCo session."""
...
if __name__ == "__main__":
    ...
    print()
    print("=" * 65)
    print("  Cortex Code × Tokenash  —  token savings benchmark")
    ...
    if total_saved > 0:
        print(
            f"  PASS  tokenash saved {total_saved:,} tokens ({total_pct:.0f}%) across all CoCo payload types"
        )
    else:
        print("  FAIL  no compression — run: pip install 'tokenash[all]'")
    print()
