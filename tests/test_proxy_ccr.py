...
class TestCCRRetrieveEndpoint:
    ...
    def test_retrieve_expired_hash_reports_expiration_detail(self, client):
        ...
        with patch("tokenash.cache.compression_store.time.time", return_value=1000.0):
            hash_key = store.store(original="payload", compressed="payload")

        with patch("tokenash.cache.compression_store.time.time", return_value=1002.0):
...
class TestCCRStatsEndpoint:
    ...
    def test_stats_exposes_env_configured_ttl(self, client, monkeypatch):
        ...
        reset_compression_store()
        monkeypatch.setenv("TOKENASH_CCR_TTL_SECONDS", "7200")
...
class TestEndToEndTOINIntegration:
    ...
    def test_pipeline_compresses_tool_output_and_records_toin(
        ...
        """
        from tokenash.config import CCRConfig, SmartCrusherConfig
        from tokenash.providers import AnthropicProvider
        from tokenash.telemetry import ToolSignature
        from tokenash.transforms import SmartCrusher, TransformPipeline
    ...
    def test_retrieval_through_proxy_updates_toin_field_semantics(
        ...
        from tokenash.telemetry.toin import _make_pattern_key
