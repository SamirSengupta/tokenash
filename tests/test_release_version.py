...
from tokenash.release_version import (
    CommitInfo,
...
def test_list_release_commits_parses_empty_body_entries(
    ...
    monkeypatch.setattr("tokenash.release_version.run", run)
...
def test_release_version_script_runs_directly_without_importing_tokenash_package(
    tmp_path: Path,
) -> None:
    ...
    result = subprocess.run(
        [sys.executable, str(ROOT / "tokenash" / "release_version.py")],
        cwd=ROOT,
        capture_output=True,
        text=True,
        env=env,
