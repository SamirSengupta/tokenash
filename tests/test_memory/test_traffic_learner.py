...
from tokenash.memory.traffic_learner import (
    ExtractedPattern,
    PatternCategory,
    TrafficLearner,
...
class TestCommandsRelatedAsRetry:
    ...
    def test_same_binary_unrelated_args_rejected(self):
        ...
        failed = (
            'grep -nE "smoke|TOKENASH_SMOKE_TEST_TIMEOUT|smoke_test" '
...
class TestCommandsRelatedAsRetrySubstantiveToken:
    def test_substantive_token_beats_distance(self):
        ...
        # share the substantive token "tokenash-config", so the token-overlap
        # path accepts the pair.
        failed = "python -m foo --tokenash-config=/etc/h.toml"
        success = "python -m bar --tokenash-config=/etc/h.toml --extra"
...
class TestProjectForPattern:
    def _project(self, path: str):
        ...
        from tokenash.learn.models import ProjectInfo
...
class TestPatternsToRecommendations:
    def test_routes_preference_to_memory_file(self):
        from tokenash.learn.models import RecommendationTarget
...
class TestFlushDebounce:
    ...
    async def test_flush_worker_rate_limits(self, monkeypatch):
        ...
        from tokenash.memory import traffic_learner as tl_mod
...
def _install_plugin_registry(monkeypatch, plugin):
    """Stub out tokenash.learn.registry so flush_to_file uses our fake."""
    import sys
    import types as _types

    fake = _types.ModuleType("tokenash.learn.registry")
    ...
    monkeypatch.setitem(sys.modules, "tokenash.learn.registry", fake)
    import tokenash.learn as learn_pkg
...
class TestBackendResolution:
    def test_resolve_none_backend(self):
        from tokenash.memory.traffic_learner import _resolve_backend_db_path
