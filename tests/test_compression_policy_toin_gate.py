...
from tokenash.proxy.auth_mode import AuthMode
from tokenash.telemetry.toin import TOINConfig, get_toin, reset_toin
from tokenash.tokenizer import Tokenizer
from tokenash.tokenizers import EstimatingTokenCounter
from tokenash.transforms.compression_policy import policy_for_mode


def _has_core() -> bool:
    """Match the pattern in ``test_smart_crusher_rust_parity.py``.

    SmartCrusher's __init__ hard-imports ``tokenash._core`` (the Rust
    ...
    """
    try:
        from tokenash._core import SmartCrusher  # noqa: F401
...
_skip_no_core = pytest.mark.skipif(
    not _has_core(),
    reason="tokenash._core wheel not installed (run `scripts/build_rust_extension.sh`)",
)
...
def test_content_router_apply_stores_runtime_policy():
    ...
    """
    from tokenash.transforms.content_router import ContentRouter
