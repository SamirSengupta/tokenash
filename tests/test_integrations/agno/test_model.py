...
Tests cover:
1. TokenashAgnoModel - Wrapper for any Agno model
...
class TestAgnoAvailable:
    ...
    def test_returns_bool(self):
        ...
        from tokenash.integrations.agno import agno_available
...
class TestTokenashAgnoModel:
    """Tests for TokenashAgnoModel wrapper."""

    def test_init_with_defaults(self, mock_agno_model):
        """Initialize with default config."""
        from tokenash.integrations.agno import TokenashAgnoModel

        model = TokenashAgnoModel(wrapped_model=mock_agno_model)

        assert model.wrapped_model is mock_agno_model
        assert model.tokenash_config is not None
    ...
    def test_init_with_custom_config(self, mock_agno_model):
        ...
        model = TokenashAgnoModel(
            wrapped_model=mock_agno_model,
            tokenash_config=config,
            tokenash_mode=TokenashMode.SIMULATE,
        )

        assert model.tokenash_config is config
        assert model.tokenash_mode == TokenashMode.SIMULATE

    def test_init_auto_detect_provider(self, mock_agno_model):
        """Auto-detect provider from wrapped model."""
        from tokenash.integrations.agno import TokenashAgnoModel

        model = TokenashAgnoModel(wrapped_model=mock_agno_model, auto_detect_provider=True)
    ...
    def test_convert_messages_normalizes_streaming_tool_call_objects(self, mock_agno_model):
        ...
        must flatten them to OpenAI-format dicts so neither the Tokenash
    ...
    def test_response_applies_optimization(self, mock_agno_model, sample_messages):
        """response() applies Tokenash optimization."""
        from tokenash.integrations.agno import TokenashAgnoModel
        from tokenash.providers import OpenAIProvider

        model = TokenashAgnoModel(wrapped_model=mock_agno_model)

        # Initialize provider and pipeline for mocking
        model._tokenash_provider = OpenAIProvider()
    ...
    def test_response_stream_applies_optimization(self, mock_agno_model, sample_messages):
        """response_stream() applies Tokenash optimization."""
        from tokenash.integrations.agno import TokenashAgnoModel
        from tokenash.providers import OpenAIProvider
    ...
    def test_get_savings_summary_with_data(self, mock_agno_model):
        ...
        from tokenash.integrations.agno import TokenashAgnoModel
        from tokenash.integrations.agno.model import OptimizationMetrics
...
class TestProviderDetection:
    ...
    def test_detect_openai_provider(self, mock_agno_model):
        ...
        from tokenash.integrations.agno.providers import get_tokenash_provider
        from tokenash.providers import OpenAIProvider

        provider = get_tokenash_provider(mock_agno_model)
    ...
    def test_detect_anthropic_provider(self, mock_claude_model):
        ...
        from tokenash.integrations.agno.providers import get_tokenash_provider
        from tokenash.providers import AnthropicProvider

        provider = get_tokenash_provider(mock_claude_model)
    ...
    def test_detect_from_model_id(self):
        ...
        from tokenash.integrations.agno.providers import get_tokenash_provider
        from tokenash.providers import AnthropicProvider, GoogleProvider, OpenAIProvider
        ...
        assert isinstance(get_tokenash_provider(mock_gpt), OpenAIProvider)
        ...
        assert isinstance(get_tokenash_provider(mock_claude), AnthropicProvider)
        ...
        assert isinstance(get_tokenash_provider(mock_gemini), GoogleProvider)
    ...
    def test_fallback_to_openai(self):
        ...
        provider = get_tokenash_provider(mock)

        assert isinstance(provider, OpenAIProvider)

    def test_get_model_name(self, mock_agno_model):
        """Extract model name from Agno model."""
        from tokenash.integrations.agno.providers import get_model_name_from_agno
...
class TestOptimizeMessages:
    ...
    def test_basic_optimization(self, sample_messages):
        ...
        from tokenash.integrations.agno import optimize_messages

        with patch("tokenash.integrations.agno.model.TransformPipeline") as MockPipeline:
...
class TestIntegrationWithRealTokenash:
    """Integration tests using real Tokenash components (no mocking)."""

    def test_real_optimization_pipeline(self, sample_messages):
        """Test with real Tokenash client (no API calls)."""
        from tokenash.integrations.agno import optimize_messages

        # This uses real Tokenash transforms but no LLM API calls
        optimized, metrics = optimize_messages(
            sample_messages,
            mode=TokenashMode.OPTIMIZE,
        )
    ...
    def test_model_wrapper_real_optimization(self, mock_agno_model, sample_messages):
        """Test TokenashAgnoModel with real Tokenash optimization."""
        from tokenash.integrations.agno import TokenashAgnoModel
...
class TestReasoningCapabilityForwarding:
    """Tests for reasoning capability forwarding in TokenashAgnoModel.

    These tests verify that TokenashAgnoModel properly forwards
    ...
    def test_underlying_model_property_returns_wrapped_model(self, mock_agno_model):
        ...
        wrapped = TokenashAgnoModel(wrapped_model=mock_agno_model)
...
class TestRealAgnoIntegration:
    ...
    These tests verify that TokenashAgnoModel:
    ...
    def test_is_subclass_of_agno_model(self):
        """TokenashAgnoModel must be a subclass of agno.models.base.Model."""
        from agno.models.base import Model

        from tokenash.integrations.agno import TokenashAgnoModel

        assert issubclass(TokenashAgnoModel, Model)

    def test_passes_agno_get_model_validation(self):
        """TokenashAgnoModel must pass Agno's get_model() validation."""
        from agno.models.openai import OpenAIChat
        ...
        base_model = OpenAIChat(id="gpt-4o")
        tokenash_model = TokenashAgnoModel(wrapped_model=base_model)

        # This should NOT raise "Model must be a Model instance, string, or None"
        result = get_model(tokenash_model)

        assert result is tokenash_model
        assert isinstance(result, TokenashAgnoModel)

    def test_agent_accepts_tokenash_model(self):
        """Agno Agent must accept TokenashAgnoModel as model parameter."""
        ...
        agent = Agent(model=tokenash_model, markdown=False)

        assert agent.model is tokenash_model
        assert agent.model.wrapped_model is base_model

    def test_model_id_reflects_wrapped_model(self):
        """TokenashAgnoModel id should reflect the wrapped model."""
        from agno.models.openai import OpenAIChat
        ...
        assert "gpt-4o-mini" in tokenash_model.id
        assert tokenash_model.id.startswith("tokenash:")

    def test_tokenash_model_has_required_abstract_methods(self):
        """TokenashAgnoModel must implement all required abstract methods."""
        from agno.models.openai import OpenAIChat
        ...
        assert hasattr(tokenash_model, "invoke")
        assert callable(tokenash_model.invoke)

        assert hasattr(tokenash_model, "ainvoke")
        assert callable(tokenash_model.ainvoke)

        assert hasattr(tokenash_model, "invoke_stream")
        assert callable(tokenash_model.invoke_stream)

        assert hasattr(tokenash_model, "ainvoke_stream")
        assert callable(tokenash_model.ainvoke_stream)

        assert hasattr(tokenash_model, "_parse_provider_response")
        assert callable(tokenash_model._parse_provider_response)

        assert hasattr(tokenash_model, "_parse_provider_response_delta")
        assert callable(tokenash_model._parse_provider_response_delta)
    ...
    def test_isinstance_check_passes(self):
        ...
        assert isinstance(tokenash_model, Model)

    def test_model_with_custom_tokenash_config(self):
        """Test with custom Tokenash configuration."""
        from agno.agent import Agent
        from agno.models.openai import OpenAIChat
        ...
        config = TokenashConfig(default_mode=TokenashMode.AUDIT)
        base_model = OpenAIChat(id="gpt-4o")
        tokenash_model = TokenashAgnoModel(
        ...
        assert agent.model.tokenash_config is config
        assert agent.model.tokenash_config.default_mode == TokenashMode.AUDIT
    ...
    def test_response_method_delegates_to_wrapped(self):
        ...
        sig = inspect.signature(tokenash_model.response)
    ...
    def test_optimization_tracked_across_calls(self):
        ...
        assert tokenash_model.total_tokens_saved == 0
        assert len(tokenash_model.metrics_history) == 0
        ...
        optimized, metrics = tokenash_model._optimize_messages(messages)

        # Should have tracked metrics
        assert len(tokenash_model.metrics_history) == 1
        assert tokenash_model.total_tokens_saved >= 0
...
class TestOllamaIntegration:
    ...
    def test_agent_with_ollama_model(self, ollama_model_name):
        """Test Agent with TokenashAgnoModel wrapping real Ollama model."""
        from agno.agent import Agent
        ...
        # Create agent - this validates TokenashAgnoModel works with Agent
        agent = Agent(model=tokenash_model, markdown=False)

        assert agent.model is tokenash_model
        assert isinstance(agent.model, TokenashAgnoModel)
    ...
    def test_agent_run_with_ollama(self, ollama_model_name):
        ...
        # Verify Tokenash optimization was applied
        assert len(tokenash_model.metrics_history) >= 1
    ...
    def test_agent_with_system_prompt_and_ollama(self, ollama_model_name):
        ...
        # Tokenash should have processed the system prompt
        assert tokenash_model.total_tokens_saved >= 0
    ...
    def test_multiple_turns_with_ollama(self, ollama_model_name):
        ...
        assert len(tokenash_model.metrics_history) >= 2

    def test_tokenash_optimization_reduces_tokens(self, ollama_model_name, large_conversation):
        """Test that Tokenash actually reduces tokens on large conversations."""
        from agno.models.ollama import Ollama
        ...
        optimized, metrics = tokenash_model._optimize_messages(large_conversation)
