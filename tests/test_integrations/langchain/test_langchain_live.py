...
# --- TokenashChatModel: invoke (sync) ---


class TestTokenashChatModelLiveOpenAI:
    """Live tests: TokenashChatModel wrapping ChatOpenAI."""

    def test_wrap_openai_and_invoke(self, openai_llm):
        from tokenash.integrations import TokenashChatModel

        model = TokenashChatModel(openai_llm)
...
class TestTokenashChatModelLiveAnthropic:
    """Live tests: TokenashChatModel wrapping ChatAnthropic.
    ...
    def test_wrap_anthropic_and_invoke(self, anthropic_llm):
        ...
        model = TokenashChatModel(anthropic_llm)
        messages = [HumanMessage(content="Reply with exactly: OK")]
        try:
            response = model.invoke(messages)
...
class TestTokenashChatModelStreamingLive:
...
class TestTokenashChatModelToolCallsLive:
    ...
    def test_messages_with_tool_result_compressed(self, openai_llm):
        """Conversation with tool call + large tool result; Tokenash should compress the tool result."""
        import json
...
class TestTokenashLCELive:
    """Live LCEL chain tests."""

    def test_prompt_pipe_tokenash_pipe_llm(self, openai_llm):
...
class TestOptimizeMessagesLive:
    """Live optimize_messages with real Tokenash pipeline (no API key needed for this)."""
