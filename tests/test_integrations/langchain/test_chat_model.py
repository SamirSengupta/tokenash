...
Tests cover:
1. TokenashChatModel - Wrapper for any BaseChatModel
2. TokenashCallbackHandler - Metrics and observability
3. TokenashRunnable - LCEL chain composition
...
class TestLangchainAvailable:
    ...
    def test_returns_bool(self):
        ...
        from tokenash.integrations.langchain import langchain_available
...
class TestTokenashChatModel:
    ...
    def test_init_with_custom_config(self, mock_chat_model):
        ...
        assert model.mode == TokenashMode.SIMULATE
    ...
    def test_llm_type(self, mock_chat_model):
        ...
        model = TokenashChatModel(mock_chat_model)
        assert "tokenash" in model._llm_type
    ...
    def test_identifying_params(self, mock_chat_model):
        ...
        assert "tokenash_mode" in params
    ...
    def test_generate_applies_optimization(self, mock_chat_model, sample_messages):
        """_generate applies Tokenash optimization."""
        from tokenash.integrations import TokenashChatModel
    ...
    def test_get_savings_summary_with_data(self, mock_chat_model):
        ...
        from tokenash.integrations import TokenashChatModel
        from tokenash.integrations.langchain import OptimizationMetrics
...
class TestTokenashCallbackHandler:
    """Tests for TokenashCallbackHandler."""
    ...
    def test_on_chat_model_start_triggers_alert(self):
        ...
        handler = TokenashCallbackHandler(token_alert_threshold=5)
...
class TestTokenashRunnable:
    """Tests for TokenashRunnable LCEL component."""

    def test_init_defaults(self):
        """Initialize with defaults."""
        from tokenash.integrations.langchain import TokenashRunnable

        runnable = TokenashRunnable()

        assert runnable.mode == TokenashMode.OPTIMIZE
    ...
    def test_init_custom_config(self):
        ...
        config = TokenashConfig(default_mode=TokenashMode.AUDIT)
        runnable = TokenashRunnable(config=config, mode=TokenashMode.SIMULATE)

        assert runnable.config is config
        assert runnable.mode == TokenashMode.SIMULATE
...
class TestOptimizeMessages:
    ...
    def test_basic_optimization(self, sample_messages):
        ...
        with patch("tokenash.integrations.langchain.chat_model.TransformPipeline") as MockPipeline:
...
class TestAinvokeStreamingTrue:
    ...
    def _patched_pipeline(self, streaming_mock_model):
        """Create a TokenashChatModel with a mocked optimization pipeline."""
        from tokenash.integrations import TokenashChatModel
        from tokenash.providers import OpenAIProvider

        model = TokenashChatModel(streaming_mock_model)
        model._provider = OpenAIProvider()
        _ = model.pipeline  # Force lazy init
...
class TestOllamaIntegration:
    ...
    def test_tokenash_chat_model_with_ollama(self, ollama_model_name):
        """Test TokenashChatModel wrapping real ChatOllama model."""
        ...
        tokenash_model = TokenashChatModel(base_model)

        assert tokenash_model.wrapped_model is base_model
        assert isinstance(tokenash_model, TokenashChatModel)
    ...
    def test_invoke_with_ollama(self, ollama_model_name):
        ...
        result = tokenash_model.invoke(messages)
    ...
    def test_generate_with_ollama(self, ollama_model_name):
        ...
        result = tokenash_model._generate(messages)
    ...
    def test_optimization_tracked_with_ollama(self, ollama_model_name):
        ...
        tokenash_model.invoke(messages)

        # Metrics should be tracked
        assert len(tokenash_model._metrics_history) >= 1
    ...
    def test_tokenash_optimization_reduces_tokens(self, ollama_model_name):
        """Test that Tokenash optimization actually reduces token count."""
        from langchain_ollama import ChatOllama

        from tokenash.integrations import TokenashChatModel

        base_model = ChatOllama(model=ollama_model_name)
        tokenash_model = TokenashChatModel(base_model, mode=TokenashMode.OPTIMIZE)
        ...
        if tokenash_model._metrics_history:
            metrics = tokenash_model._metrics_history[-1]
            # With a large conversation, we expect some savings
            assert metrics.tokens_before >= metrics.tokens_after

    def test_callback_handler_with_ollama(self, ollama_model_name):
        """Test TokenashCallbackHandler with real Ollama calls."""
        from langchain_ollama import ChatOllama

        from tokenash.integrations import TokenashCallbackHandler, TokenashChatModel
        ...
        tokenash_model.invoke(messages, config={"callbacks": [handler]})
...
class TestRealLangChainIntegration:
    ...
    def test_lcel_chain_with_tokenash(self, ollama_model_name):
        """Test LCEL chain composition with TokenashChatModel."""
        ...
        # Create chain: prompt -> tokenash model -> output parser
        base_model = ChatOllama(model=ollama_model_name)
        tokenash_model = TokenashChatModel(base_model)
        ...
        chain = prompt | tokenash_model | StrOutputParser()
