"""Tests for :class:`tokenash.proxy.memory_decision.MemoryDecision`.
...
* Sites 1/2/3 (Anthropic ``/v1/messages``, Gemini
  ...
  mutated requests under ``x-tokenash-bypass: true``.
...
* ``MemoryMode`` values (``auto_tail`` / ``tool``) and the env-driven
  ``TOKENASH_MEMORY_INJECTION_MODE`` (``disabled`` / ``auto_tail`` /
...
def test_bypass_header_wins_over_every_other_gate() -> None:
    """``x-tokenash-bypass: true`` is the user's "do not touch my
...
def test_passthrough_mode_header_also_triggers_bypass_skip() -> None:
    """``x-tokenash-mode: passthrough`` is the alternate spelling of
    the bypass signal — mirrors _tokenash_bypass_enabled semantics."""
    d = MemoryDecision.decide(
...
def test_mode_disabled_is_skip() -> None:
    """Operator override via TOKENASH_MEMORY_INJECTION_MODE=disabled."""
    d = MemoryDecision.decide(
