...
from tokenash.observability import TokenashOtelMetrics, reset_otel_metrics, set_otel_metrics
...
def test_tokenash_otel_metrics_records_proxy_and_pipeline_metrics() -> None:
    reader = InMemoryMetricReader()
    provider = MeterProvider(metric_readers=[reader])
    otel_metrics = TokenashOtelMetrics(meter_provider=provider)
    ...
    requests = metrics["tokenash.proxy.requests"]
    ...
    latency = metrics["tokenash.proxy.request.duration"]
    ...
    ttl_tokens = metrics["tokenash.proxy.cache.write_ttl_tokens"]
    ...
    compression_runs = metrics["tokenash.compression.runs"]
    ...
    stage_duration = metrics["tokenash.compression.stage.duration"]
    ...
    waste_tokens = metrics["tokenash.compression.waste.tokens"]
...
def test_proxy_failure_and_rate_limit_metrics_include_provider_labels() -> None:
    ...
    failed_point = _find_point(metrics["tokenash.proxy.requests.failed"], provider="openai")
    assert failed_point.value == 1

    rate_limited_point = _find_point(
        metrics["tokenash.proxy.requests.rate_limited"],
        provider="anthropic",
        model="claude-sonnet",
    )
