...
class _DummyOpenAIHandler(OpenAIHandlerMixin):
    ...
    async def _run_compression_in_executor(self, fn, *, timeout: float):
        # Test stub for TokenashProxy._run_compression_in_executor.
    ...
    async def _record_request_outcome(self, outcome) -> None:
        ...
        # matches TokenashProxy._record_request_outcome.
...
def test_handle_openai_responses_memory_timeout_fails_open(monkeypatch):
    ...
    request = _build_request(
        {"model": "gpt-5.4", "input": "hello"},
        {"Authorization": "Bearer sk-test", "x-tokenash-user-id": "user-1"},
    )
    handler = _DummyOpenAIHandler()
    handler.memory_handler = _SlowMemoryHandler()

    monkeypatch.setattr("tokenash.tokenizers.get_tokenizer", lambda model: _DummyTokenizer())
    monkeypatch.setattr("tokenash.proxy.handlers.openai.asyncio.wait_for", _timeout_wait_for)
...
def test_handle_openai_responses_ws_resolves_codex_routing_headers():
    ...
    with patch.dict(sys.modules, {"websockets": MagicMock()}):
        with patch(
            "tokenash.proxy.handlers.openai._resolve_codex_routing_headers",
