...
Verifies that ``TokenashProxy`` gates the pre-upstream phase of
...
- env var ``TOKENASH_ANTHROPIC_PRE_UPSTREAM_CONCURRENCY`` with flag override
"""
...
class _DummyAnthropicHandler(AnthropicHandlerMixin):
    ...
    def __init__(
        ...
        *,
        ...
        # ``TokenashProxy._run_compression_in_executor`` for bounded thread
    ...
    async def _run_compression_in_executor(self, fn, *, timeout):  # noqa: ANN001
        # Mirror of ``TokenashProxy._run_compression_in_executor`` for the
    ...
    async def _record_request_outcome(self, outcome) -> None:  # noqa: ANN001
        # Mirror of ``TokenashProxy._record_request_outcome`` for the
...
def stage_log_capture():
    target = logging.getLogger("tokenash.proxy")
...
def _tokenizer_patch():
    import tokenash.tokenizers as _tk
...
def _run_cli_capture(args: list[str], env: dict | None = None) -> ProxyConfig:
    ...
    via ``from tokenash.proxy.server import ... run_server``) short-
...
def test_env_var_sets_pre_upstream_concurrency():
    ...
    env = {"TOKENASH_ANTHROPIC_PRE_UPSTREAM_CONCURRENCY": "4"}
...
def test_cli_env_sets_pre_upstream_timeouts():
    env = {
        "TOKENASH_ANTHROPIC_PRE_UPSTREAM_ACQUIRE_TIMEOUT_SECONDS": "9.5",
        "TOKENASH_ANTHROPIC_PRE_UPSTREAM_MEMORY_CONTEXT_TIMEOUT_SECONDS": "3.25",
    }
    config = _run_cli_capture([], env=env)
...
# --------------------------------------------------------------------------- #
# Sanity: TokenashProxy auto-computes default when config value is None.       #
# --------------------------------------------------------------------------- #
