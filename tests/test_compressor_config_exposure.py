...
Requires the rebuilt `tokenash._core` extension — these tests fail loudly
...
from tokenash.transforms.search_compressor import SearchCompressor, SearchCompressorConfig
...
class TestSmartCrusherConfigExposure:
    ...
    def test_rust_default_lockstep(self):
        ...
        from tokenash._core import SmartCrusherConfig as RustConfig
    ...
    def test_foreign_config_object_tolerated(self):
        """tokenash.config.SmartCrusherConfig (the SDK-surface class) is
        structurally similar and flows through getattr fallbacks."""
        from tokenash.config import SmartCrusherConfig as SdkConfig
