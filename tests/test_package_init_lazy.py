...
import tokenash._version as version_module


def test_tokenash_import_stays_lazy() -> None:
    script = textwrap.dedent(
        ...
        print(json.dumps({
            "version": tokenash.__version__,
            "cache_loaded": "tokenash.cache" in sys.modules,
            "models_registry_loaded": "tokenash.models.registry" in sys.modules,
            "memory_loaded": "tokenash.memory" in sys.modules,
        }))
        """
    )
...
def test_version_prefers_installed_distribution_metadata() -> None:
    ...
    package_version.assert_called_once_with("tokenash")
...
def test_proxy_package_import_does_not_eagerly_load_server() -> None:
    script = textwrap.dedent(
        ...
        import tokenash.proxy

        print(json.dumps({
            "server_loaded": "tokenash.proxy.server" in sys.modules,
        }))
        """
    )
...
def test_proxy_server_import_skips_litellm_backend() -> None:
    script = textwrap.dedent(
        ...
        import tokenash.proxy.server

        print(json.dumps({
            "litellm_backend_loaded": "tokenash.backends.litellm" in sys.modules,
            "anyllm_backend_loaded": "tokenash.backends.anyllm" in sys.modules,
...
def test_dynamic_detector_import_skips_optional_ml_dependencies(tmp_path: Path) -> None:
    ...
    script = textwrap.dedent(
        ...
        import tokenash.cache.dynamic_detector
...
def test_compress_spreadsheet_public_import_survives_ort_pin() -> None:
    """`from tokenash import compress_spreadsheet` stays eagerly exported, and the
    ...
    script = textwrap.dedent(
        ...
        import tokenash
        from tokenash import compress_spreadsheet

        print(json.dumps({
            "eager": "compress_spreadsheet" in tokenash.__dict__,
