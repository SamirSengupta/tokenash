...
HANDLER_FILES = [
    Path("tokenash/proxy/handlers/anthropic.py"),
    Path("tokenash/proxy/handlers/openai.py"),
    Path("tokenash/proxy/handlers/gemini.py"),
    Path("tokenash/proxy/handlers/batch.py"),
]
...
def test_every_search_and_format_context_call_passes_query_kwarg() -> None:
    ...
    if offenders:
        ...
        pytest.fail(
            ...
            "untruncated query value type. See tokenash/proxy/memory_query.py."
        )
