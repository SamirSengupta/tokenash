"""Shared pytest fixtures for Tokenash tests."""
...
def _reset_tokenash_logger_propagation():
    """Keep `tokenash.*` log records flowing to pytest's caplog handler.

    `tokenash.proxy.helpers._setup_file_logging` sets
    ``logging.getLogger("tokenash").propagate = False`` once any test
    triggers a proxy startup with `--log-file`. After that, every
    subsequent test's `caplog` fixture stops capturing `tokenash.*`
    log records (caplog attaches to root, propagation is now blocked
    at the tokenash-logger boundary). Reset before every test so the
    ...
    for logger_name in (
        "tokenash",
        "tokenash.proxy",
        "tokenash.proxy.forwarded_headers",
        "tokenash.transforms",
        "tokenash.transforms.kompress_compressor",
    ):
...
def openai_tokenizer():
    ...
    from tokenash.providers.openai import OpenAITokenCounter
...
@pytest.fixture
def default_config():
    """Default TokenashConfig."""
    from tokenash.config import TokenashConfig

    return TokenashConfig()
...
def sample_request_metrics():
    ...
    from tokenash.config import RequestMetrics
