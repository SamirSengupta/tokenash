...
class TestGlobalTelemetryCollector:
    ...
    def test_env_var_disables_telemetry(self, monkeypatch):
        """TOKENASH_TELEMETRY_DISABLED environment variable disables telemetry."""
        reset_telemetry_collector()
        monkeypatch.setenv("TOKENASH_TELEMETRY_DISABLED", "1")
    ...
    def test_tokenash_telemetry_off_disables_collector(self, monkeypatch, off_value):
        """TOKENASH_TELEMETRY=off (and other documented opt-out values) disables
        the collector — closes #390.

        Pre-#390 the collector only honoured TOKENASH_TELEMETRY_DISABLED, which
        is undocumented. Users following the docs set TOKENASH_TELEMETRY=off and
        ...
        """
        reset_telemetry_collector()
        monkeypatch.delenv("TOKENASH_TELEMETRY_DISABLED", raising=False)
        monkeypatch.setenv("TOKENASH_TELEMETRY", off_value)

        collector = get_telemetry_collector()

        assert collector._config.enabled is False, (
            f"TOKENASH_TELEMETRY={off_value!r} must disable the collector — "
    ...
    def test_tokenash_telemetry_on_keeps_collector_enabled(self, monkeypatch):
        """Sanity check: the explicit opt-in path (TOKENASH_TELEMETRY=on) leaves
