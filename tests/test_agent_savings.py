...
compress_module = import_module("tokenash.compress")
...
def test_agent_90_profile_exports_cross_agent_proxy_env() -> None:
    ...
    assert env["TOKENASH_MODE"] == "token"
    assert env["TOKENASH_SAVINGS_PROFILE"] == "agent-90"
    assert env["TOKENASH_SAVINGS_TARGET"] == "0.90"
    assert env["TOKENASH_TARGET_RATIO"] == "0.10"
    assert env["TOKENASH_COMPRESS_USER_MESSAGES"] == "1"
    assert env["TOKENASH_COMPRESS_SYSTEM_MESSAGES"] == "1"
    assert env["TOKENASH_MAX_ITEMS"] == "8"
    assert env["TOKENASH_SMART_CRUSHER_COMPACTION"] == "0"
    assert env["TOKENASH_FORCE_KOMPRESS"] == "1"
    assert env["TOKENASH_ACCURACY_GUARD"] == "strict"


def test_agent_savings_env_defaults_preserve_user_overrides() -> None:
    env = {
        "TOKENASH_TARGET_RATIO": "0.25",
        "TOKENASH_MAX_ITEMS": "12",
    }

    apply_agent_savings_env_defaults(env, AGENT_90_PROFILE)

    assert env["TOKENASH_SAVINGS_PROFILE"] == "agent-90"
    assert env["TOKENASH_TARGET_RATIO"] == "0.25"
...
def test_agent_savings_cli_renders_shell_exports() -> None:
    ...
    assert 'export TOKENASH_SAVINGS_PROFILE="agent-90"' in result.output
    assert 'export TOKENASH_SAVINGS_TARGET="0.90"' in result.output
    assert 'export TOKENASH_ACCURACY_GUARD="strict"' in result.output
...
def test_agent_savings_cli_renders_json() -> None:
    ...
    assert result.exit_code == 0
    assert '"TOKENASH_TARGET_RATIO": "0.10"' in result.output
...
def test_wrap_agent_savings_profile_is_opt_in(monkeypatch) -> None:
    monkeypatch.delenv("TOKENASH_SAVINGS_PROFILE", raising=False)

    assert wrap_module._wrap_agent_savings_profile("codex") is None

    monkeypatch.setenv("TOKENASH_SAVINGS_PROFILE", AGENT_90_PROFILE)
...
def test_start_proxy_does_not_inject_agent_savings_by_default(monkeypatch, tmp_path) -> None:
    ...
    assert "TOKENASH_SAVINGS_PROFILE" not in captured_env
    assert "TOKENASH_TARGET_RATIO" not in captured_env
...
def test_start_proxy_injects_explicit_agent_savings_profile(monkeypatch, tmp_path) -> None:
    ...
    assert captured_env["TOKENASH_SAVINGS_PROFILE"] == AGENT_90_PROFILE
    assert captured_env["TOKENASH_TARGET_RATIO"] == "0.10"
...
def test_router_lossless_only_flag_reaches_crusher() -> None:
    # TOKENASH_LOSSLESS_ONLY=1 sets this field on the proxy router; it
...
def test_proxy_cli_reads_agent_90_profile_env() -> None:
    ...
    with pytest.MonkeyPatch.context() as mp:
        mp.setattr("tokenash.proxy.server.run_server", mock_run_server)
        result = runner.invoke(
            main,
            ["proxy"],
            env={"TOKENASH_SAVINGS_PROFILE": "agent-90"},
...
def test_agent_savings_requires_each_agent_to_meet_target(monkeypatch) -> None:
    from tokenash.perf import analyzer
    from tokenash.perf.analyzer import PerfRecord, PerfReport
...
def test_agent_savings_smoke_fixture_passes_real_gate(tmp_path) -> None:
    ...
    gate_result = runner.invoke(
        ...
        env={"TOKENASH_WORKSPACE_DIR": str(workspace)},
    )
