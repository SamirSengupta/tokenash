"""Tests for :class:`tokenash.mcp_registry.opencode.OpencodeRegistrar`."""
...
from tokenash.mcp_registry.opencode import (
    OpencodeRegistrar,
    _diff_specs,
    _entry_to_spec,
    _spec_to_entry,
...
def test_get_server_returns_none_when_absent(tmp_path: Path) -> None:
    ...
    registrar = _registrar(tmp_path)
    assert registrar.get_server("tokenash") is None
...
def test_get_server_returns_spec_when_present(tmp_path: Path) -> None:
    ...
    registrar = _registrar(tmp_path)
    spec = registrar.get_server("tokenash")
    assert spec is not None
    assert spec.name == "tokenash"
...
def test_register_server_creates_config_when_missing(tmp_path: Path) -> None:
    ...
    registrar = _registrar(tmp_path)
    from tokenash.mcp_registry.base import ServerSpec

    spec = ServerSpec(name="tokenash", command="tokenash", args=("mcp", "serve"))
...
def test_unregister_server_removes_entry(tmp_path: Path) -> None:
    ...
    spec = ServerSpec(name="tokenash", command="tokenash", args=("mcp", "serve"))
    registrar.register_server(spec)
    assert registrar.unregister_server("tokenash") is True
    assert registrar.get_server("tokenash") is None
...
def test_unregister_server_returns_false_when_absent(tmp_path: Path) -> None:
    ...
    registrar = _registrar(tmp_path)
    assert registrar.unregister_server("tokenash") is False


def test_specs_equivalent_true() -> None:
    from tokenash.mcp_registry.base import ServerSpec

    a = ServerSpec(name="tokenash", command="tokenash", args=("mcp", "serve"))
    b = ServerSpec(name="tokenash", command="tokenash", args=("mcp", "serve"))
...
def test_specs_equivalent_false() -> None:
    ...
    a = ServerSpec(name="tokenash", command="tokenash", args=("mcp", "serve"))
    b = ServerSpec(name="tokenash", command="other", args=("mcp", "serve"))
...
def test_register_server_force_overwrites_mismatch(tmp_path: Path) -> None:
    ...
    spec_a = ServerSpec(name="tokenash", command="old", args=("serve",))
    spec_b = ServerSpec(name="tokenash", command="tokenash", args=("mcp", "serve"))
    ...
    updated = registrar.get_server("tokenash")
    assert updated is not None
    assert updated.command == "tokenash"
...
def test_unregister_preserves_other_mcp_servers(tmp_path: Path) -> None:
    ...
    spec = ServerSpec(name="tokenash", command="tokenash", args=("mcp", "serve"))
    registrar.register_server(spec)
    registrar.unregister_server("tokenash")

    data = json.loads((tmp_path / "opencode.json").read_text())
    assert "tokenash" not in data["mcp"]
    assert "existing-server" in data["mcp"]
...
def test_register_server_with_env_vars(tmp_path: Path) -> None:
    ...
    spec = ServerSpec(
        ...
        command="tokenash",
        args=("mcp", "serve"),
        env={"TOKENASH_PROXY_URL": "http://127.0.0.1:9090"},
    )
    registrar.register_server(spec)

    data = json.loads((tmp_path / "opencode.json").read_text())
    assert data["mcp"]["tokenash"]["env"] == {"TOKENASH_PROXY_URL": "http://127.0.0.1:9090"}
...
def test_register_then_re_register_with_different_env_returns_mismatch(tmp_path: Path) -> None:
    ...
    spec_a = ServerSpec(name="tokenash", command="tokenash", args=("mcp", "serve"))
    spec_b = ServerSpec(
        name="tokenash",
        command="tokenash",
        args=("mcp", "serve"),
...
def test_unregister_server_handles_oserror(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("tokenash.mcp_registry.opencode._write_json", _fail_write)
    ok = r.unregister_server("bad-unregister")
    assert ok is False


def test_get_all_registrars_includes_opencode() -> None:
    from tokenash.mcp_registry.install import get_all_registrars
