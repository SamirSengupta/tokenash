...
~/.tokenash/ccr_store.db by default) and shared across processes, so stale
...
def test_verbatim_read_never_cache_written_before_maturation(monkeypatch):
    ...
    from tokenash.cache.compression_store import reset_compression_store
            ...
            for n in range(0, 7):
                r = client.post(
                    ...
                    headers={
                        ...
                        "x-tokenash-session-id": "nobust-1",
...
def test_read_maturation_knobs_from_env(monkeypatch):
    ...
    from tokenash.proxy.server import _MULTI_WORKER_CONFIG_ENV, _proxy_config_from_env

    # _proxy_config_from_env short-circuits on a prebuilt multi-worker JSON
    # config and ignores the TOKENASH_* vars entirely. Clear it so this test
    # actually exercises the env-var parsing path it claims to (and isn't
    # poisoned by a leaked TOKENASH_PROXY_CONFIG_JSON from another test).
    monkeypatch.delenv(_MULTI_WORKER_CONFIG_ENV, raising=False)

    monkeypatch.setenv("TOKENASH_READ_MATURATION", "1")
    monkeypatch.setenv("TOKENASH_READ_MATURATION_QUIESCE_TURNS", "3")
    monkeypatch.setenv("TOKENASH_READ_MATURATION_MAX_HOLD_TURNS", "10")
    monkeypatch.setenv("TOKENASH_READ_MATURATION_MIN_SIZE_BYTES", "4096")
