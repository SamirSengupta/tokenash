...
This module is new (no modifications to existing Python under `tokenash/`).
...
LOG = logging.getLogger("tokenash.parity.recorder")
...
def record_all(root: Path | None = None) -> dict[str, str]:
    ...
    # --- log_compressor ----------------------------------------------------
    try:
        from tokenash.transforms.log_compressor import LogCompressor
    ...
    try:
        from tokenash.transforms.cache_aligner import CacheAligner
    ...
    try:
        from tokenash.ccr.tool_injection import CCRToolInjector
    ...
    try:
        from tokenash.transforms import content_detector as _cd_mod
