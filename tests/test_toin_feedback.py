"""Tests for TOIN feedback loop: tokenash_retrieve calls flow back to TOIN."""
...
from tokenash.cache.compression_store import get_compression_store, reset_compression_store
from tokenash.telemetry import (
    TOINConfig,
...
def reset_globals(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("TOKENASH_TOIN_PATH", temp_toin_path)
    reset_toin()
...
def test_kompress_ccr_retrieval_updates_toin():
    ...
    original = "\n".join(
        [
            "TOKENASH_MODE=debug PATH=/tmp/tokenash",
            "ordinary line without the target token",
            "another ordinary line",
        ]
    )
    compressed = "TOKENASH_MODE=debug"
    ...
    results = store.search(hash_key, "TOKENASH", score_threshold=0.0)
    assert results
...
class TestCCRFeedbackExtraction:
    """Bug 2: _record_ccr_feedback_from_response extracts tokenash_retrieve calls."""

    def test_extract_tokenash_retrieve_from_response(self):
        """Should detect tokenash_retrieve tool_use blocks in response content."""
        response = {
            "content": [
        ...
        for block in content:
            if not isinstance(block, dict):
                continue
            if block.get("type") == "tool_use" and block.get("name") == "tokenash_retrieve":
    ...
    def test_ignore_non_retrieve_tool_calls(self):
        """Should ignore tool_use blocks that are not tokenash_retrieve."""
        response = {
            "content": [
                {
                    "type": "tool_use",
    ...
    def test_missing_hash_skipped(self):
        """Should skip tokenash_retrieve calls without a hash."""
        response = {
            "content": [
                {
                    "type": "tool_use",
...
class TestStreamingFeedbackIntegration:
    """Bug 2: Full feedback loop — streaming tokenash_retrieve reaches TOIN."""
    ...
    def test_record_ccr_feedback_calls_store_search(self):
        ...
        with patch(
            "tokenash.cache.compression_store.get_compression_store",
            return_value=mock_store,
        ):
            # Create a minimal proxy to test the method
            proxy = TokenashProxy.__new__(TokenashProxy)
...
class TestParseSSEToolUse:
    ...
    def test_parse_sse_extracts_tool_use(self):
        ...
        sse_data = (
            ...
            "\n"
            'data: {"type":"content_block_start","index":1,"content_block":{"type":"tool_use","id":"toolu_abc","name":"tokenash_retrieve"}}\n'
            "\n"
        ...
        assert tool_block["name"] == "tokenash_retrieve"
