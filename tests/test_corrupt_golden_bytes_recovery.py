...
@pytest.fixture(autouse=True)
def _enable_tokenash_log_propagation() -> Iterator[None]:
    """Re-enable propagation on the tokenash logger during tests.

    configure_proxy_logging() calls tokenash_logger.propagate = False to prevent
    ...
    for the duration of each test so caplog captures tokenash log records correctly.
    """
    tokenash_logger = logging.getLogger("tokenash")
    original = tokenash_logger.propagate
    tokenash_logger.propagate = True
    yield
    tokenash_logger.propagate = original
...
class TestCorruptMemoryGoldenBytes:
    ...
    def test_corrupt_bytes_logs_error(self, caplog: pytest.LogCaptureFixture) -> None:
        ...
        with caplog.at_level(logging.ERROR, logger="tokenash.proxy"):
            apply_session_sticky_memory_tools(
                provider="anthropic",
                session_id=session_id,
                request_id="req-2",
...
class TestCorruptCcrGoldenBytes:
    ...
    def test_valid_golden_bytes_still_injected(self) -> None:
        ...
        from tokenash.ccr.tool_injection import create_ccr_tool_definition
        from tokenash.proxy.helpers import serialize_tool_definition_canonical
