...
def test_proxy_provider_runtime_routes_model_metadata_and_passthrough() -> None:
    ...
    assert runtime.select_passthrough_base_url({"api-key": "azure", "x-tokenash-base-url": ""}) == (
...
def test_proxy_provider_runtime_loaders_cache_backend_types(monkeypatch) -> None:
    import tokenash.providers.registry as registry
    ...
    def fake_import(name, globals=None, locals=None, fromlist=(), level=0):
        ...
        if name == "tokenash.backends.anyllm":
            anyllm_loads += 1
            return type("Module", (), {"AnyLLMBackend": FakeAnyLLMBackend})()
        if name == "tokenash.backends.litellm":
            litellm_loads += 1
