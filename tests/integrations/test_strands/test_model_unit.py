"""Unit tests for Strands TokenashStrandsModel.

These tests use mocks and do NOT require AWS credentials or strands-agents.
They test the internal logic of TokenashStrandsModel in isolation.
...
class TestTokenashStrandsModelInit:
    """Tests for TokenashStrandsModel initialization."""

    def test_init_with_defaults(self, mock_strands_model):
        """Initialize with default settings."""
        from tokenash.integrations.strands import TokenashStrandsModel

        model = TokenashStrandsModel(wrapped_model=mock_strands_model)
    ...
    def test_init_with_custom_config(self, mock_strands_model):
        """Initialize with custom TokenashConfig."""
    ...
    def test_init_requires_wrapped_model(self):
        ...
        with pytest.raises(ValueError, match="wrapped_model cannot be None"):
            TokenashStrandsModel(wrapped_model=None)
...
class TestOptimizeMessages:
    ...
    def test_optimize_returns_metrics(self, mock_strands_model, sample_messages):
        ...
        # Mock the pipeline by setting _pipeline directly and mocking _tokenash_provider
        ...
        model._tokenash_provider = MagicMock()
        model._tokenash_provider.get_context_limit.return_value = 128000
...
class TestPipelineLazyInit:
    ...
    def test_pipeline_is_lazily_initialized(self, mock_strands_model):
        ...
        with patch("tokenash.integrations.strands.model.TransformPipeline"):
            _ = model.pipeline
...
class TestGetSavingsSummary:
    ...
    def test_summary_with_metrics(self, mock_strands_model):
        ...
        from tokenash.integrations.strands import TokenashStrandsModel
        from tokenash.integrations.strands.model import OptimizationMetrics
...
class TestOptimizeMessagesFunction:
    ...
    def test_optimize_messages_basic(self):
        ...
        with patch("tokenash.integrations.strands.model.TransformPipeline") as MockPipeline:
...
class TestStrandsAvailableFunction:
    ...
    def test_strands_available_returns_bool(self):
        ...
        from tokenash.integrations.strands import strands_available
...
class TestRealTokenashIntegration:
    """Integration tests with real Tokenash (no mocking)."""

    def test_real_optimization_with_mock_model(self, mock_strands_model, sample_messages):
        """Test with real Tokenash transforms (no API calls)."""
        from tokenash.integrations.strands import TokenashStrandsModel
        ...
        # This calls real Tokenash optimization
        optimized, metrics = model._optimize_messages(sample_messages)
