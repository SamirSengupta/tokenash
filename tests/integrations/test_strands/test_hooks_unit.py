"""Unit tests for Strands TokenashHookProvider.

These tests use mocks and do NOT require AWS credentials or strands-agents.
They test the internal logic of TokenashHookProvider in isolation.
...
class TestTokenashHookProviderInit:
    """Tests for TokenashHookProvider initialization."""

    def test_init_with_defaults(self):
        """Initialize with default settings."""
        from tokenash.integrations.strands import TokenashHookProvider

        hook = TokenashHookProvider()
    ...
    def test_init_creates_default_config_if_none(self):
        """Initialize creates a default TokenashConfig if none provided."""
        from tokenash import TokenashConfig
        from tokenash.integrations.strands import TokenashHookProvider
        ...
        assert isinstance(hook.config, TokenashConfig)


class TestRegisterHooks:
    """Tests for TokenashHookProvider.register_hooks method."""

    def test_register_hooks_adds_callback_to_registry(self):
        """register_hooks adds AfterToolCallEvent callback to registry."""
        from tokenash.integrations.strands import TokenashHookProvider

        hook = TokenashHookProvider(compress_tool_outputs=True)
    ...
    def test_register_hooks_skips_when_compression_disabled(self):
        ...
        hook = TokenashHookProvider(compress_tool_outputs=False)
        mock_registry = MagicMock()
...
class TestCrusherLazyInit:
    ...
    def test_crusher_uses_configured_min_tokens(self):
        ...
        hook = TokenashHookProvider(min_tokens_to_compress=250)
...
class TestShouldSkipCompression:
    ...
    def test_skip_error_results_when_preserve_errors_true(self):
        ...
        hook = TokenashHookProvider(preserve_errors=True)
    ...
    def test_allow_error_results_when_preserve_errors_false(self):
        ...
        hook = TokenashHookProvider(preserve_errors=False)
        result = {"status": "error", "content": [{"text": "Error message"}]}
...
class TestGetSavingsSummary:
    ...
    def test_summary_with_compressions(self):
        ...
        from tokenash.integrations.strands import TokenashHookProvider
        from tokenash.integrations.strands.hooks import CompressionMetrics
