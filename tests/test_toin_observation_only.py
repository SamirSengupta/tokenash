...
def test_compression_hint_is_not_publicly_exported():
    """`CompressionHint` is no longer re-exported from `tokenash.telemetry`."""
    import tokenash.telemetry as telemetry_pkg

    assert not hasattr(telemetry_pkg, "CompressionHint"), (
        "PR-B5: CompressionHint was retired and must not be importable from tokenash.telemetry."
    )
...
def test_record_does_not_alter_compression_decision():
    ...
    """
    smart_crusher_module = pytest.importorskip("tokenash.transforms.smart_crusher")
...
def test_smart_crusher_determinism_property():
    ...
    Tokenash). The parametrized test above covers the deterministic
