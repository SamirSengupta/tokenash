...
from tokenash.learn.scanner import ClaudeCodeScanner, _decode_project_path, _greedy_path_decode
...
class TestGreedyPathDecode:
    ...
    def test_simple_directory(self, tmp_path: Path) -> None:
        _make_dirs(tmp_path, "tokenash")
        result = _greedy_path_decode(tmp_path, ["tokenash"])
        assert result == tmp_path / "tokenash"
    ...
    def test_multi_hyphen_dot_dir_containing_subproject(self, tmp_path: Path) -> None:
        """Path like my-cool-project.nosync/tokenash — hardest combination."""
        _make_dirs(tmp_path, "my-cool-project.nosync/tokenash")
        result = _greedy_path_decode(tmp_path, ["my", "cool", "project.nosync", "tokenash"])
        assert result == tmp_path / "my-cool-project.nosync" / "tokenash"
...
class TestDecodeProjectPath:
    ...
    def test_simple_replace_finds_dot_path(self, users_tmp: Path) -> None:
        ...
        """
        project = users_tmp / "GitHub.nosync" / "tokenash"
    ...
    def test_home_dir_username_stays_single_component(self) -> None:
        ...
        ``/Users/first/last`` — causing ``tokenash learn --apply`` to fail with
        ...
        base = home / f"pytest_tokenash_{uuid4().hex}"
        try:
            base.mkdir()
