"""Tests for :class:`tokenash.proxy.compression_decision.CompressionDecision`,
...
* ``handlers/gemini.py`` had THREE compression sites that NEVER checked
  the ``x-tokenash-bypass`` header — explicit user requests to skip
...
def _config(*, optimize: bool = True) -> Any:
    """Minimal stand-in for ``TokenashConfig`` — only the fields the
    decision reads."""
...
def test_bypass_header_wins_over_every_other_gate() -> None:
    """``x-tokenash-bypass`` is the user's explicit "do not touch my
    ...
    d = CompressionDecision.decide(
        headers={"x-tokenash-bypass": "true"},
        config=_config(optimize=True),
        usage_reporter=_usage_reporter(should_compress=True),
        messages=_msgs(),
    )
...
def test_passthrough_mode_header_also_triggers_bypass() -> None:
    """``x-tokenash-mode: passthrough`` is the alternate spelling of the
    bypass signal — both go through the same path. This mirrors the
    pre-existing ``_tokenash_bypass_enabled`` semantics in helpers.py;
    re-asserted here so a refactor of that helper can't silently
    diverge."""
    d = CompressionDecision.decide(
        headers={"x-tokenash-mode": "passthrough"},
        config=_config(optimize=True),
        usage_reporter=_usage_reporter(should_compress=True),
        messages=_msgs(),
    )
...
def test_bypass_header_is_case_insensitive() -> None:
    """Real client UAs send ``X-Tokenash-Bypass`` (title-case) or
    ``x-tokenash-bypass`` (lower-case). Both must work — the existing
    helper normalised both, so the consolidated decision must too."""
    for header_key in ("X-Tokenash-Bypass", "x-tokenash-bypass", "X-TOKENASH-BYPASS"):
        ...
        if header_key == "x-tokenash-bypass":
...
def test_bypass_header_value_must_be_true() -> None:
    ...
    for value in ("false", "0", "", "yes", "no"):
        d = CompressionDecision.decide(
            headers={"x-tokenash-bypass": value},
            config=_config(),
            usage_reporter=_usage_reporter(),
            messages=_msgs(),
        )
...
def test_decide_accepts_fastapi_style_starlette_headers() -> None:
    ...
    h = _FakeStarletteHeaders({"X-Tokenash-Bypass": "true"})
    d = CompressionDecision.decide(
