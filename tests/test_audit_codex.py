"""Tests for the Codex read-pattern audit (tokenash.audit.codex)."""
...
from tokenash.audit.codex import audit_codex, classify_command, render_codex_text, strip_wrappers
...
class TestClassifier:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("nl tokenash/config.py", "read", False),
            ("rg -n 'def apply' tokenash/", "search", False),
    ...
    def test_sed_range_not_mistaken_for_path(self):
        _, path, _ = classify_command("sed -n '5,30p' tokenash/config.py")
        assert path == "tokenash/config.py"
