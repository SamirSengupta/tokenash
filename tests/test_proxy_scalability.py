...
class TestWorkerConfiguration:
    ...
    def test_run_server_uses_import_string_for_multiple_workers(self, monkeypatch):
        ...
        from tokenash.proxy.server import _MULTI_WORKER_CONFIG_ENV, run_server
        ...
        try:
            with patch("tokenash.proxy.server.uvicorn.run", fake_run):
                run_server(config, workers=4, limit_concurrency=250)

            assert captured["app"] == "tokenash.proxy.server:create_app_from_env"
            ...
            # later tests (e.g. _proxy_config_from_env then ignores TOKENASH_*).
