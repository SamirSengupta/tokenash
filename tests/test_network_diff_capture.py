...
from tokenash.capture.network_diff import (
    compare_captures,
    load_capture_file,
...
def test_network_diff_redacts_and_reports_body_json_deltas(tmp_path: Path) -> None:
    ...
    tokenash_path = tmp_path / "tokenash.jsonl"
    _write_jsonl(
        direct_path,
        [
            {
    ...
    tokenash = load_capture_file(tokenash_path, fallback_lane="tokenash")
    ...
    diff = compare_captures(direct, tokenash)
    assert diff.direct_count == 1
    assert diff.tokenash_count == 1
    paired = diff.paired[0]
    assert paired["headers"]["only_tokenash"] == ["x-tokenash-mode"]
    assert "$.metadata" in paired["json"]["only_tokenash"]
    assert "$.messages[0].content" in paired["json"]["changed"]
    assert paired["anthropic"]["direct"]["tools_count"] == 0
    assert paired["anthropic"]["tokenash"]["tools_count"] == 1
...
def test_network_diff_cli_writes_markdown_and_json(tmp_path: Path) -> None:
    ...
    _write_jsonl(tokenash_path, [record])
    ...
    assert payload["tokenash_count"] == 1
