...
from tokenash.providers.opencode.config import (
    TOKENASH_OPENCODE_PLUGIN,
    _inject_key_into_json,
    _parse_json_loose,
    append_tokenash_plugin,
...
def test_opencode_config_paths_default(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert backup_file == tmp_path / ".config" / "opencode" / "opencode.json.tokenash-backup"
...
def test_opencode_config_paths_from_env(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert backup_file == tmp_path / "custom" / "opencode.json.tokenash-backup"
...
def test_snapshot_creates_backup(tmp_path: Path) -> None:
    ...
    backup_file = tmp_path / "opencode.json.tokenash-backup"
...
def test_snapshot_skips_if_markers_present(tmp_path: Path) -> None:
    """snapshot skips if the config already contains Tokenash markers."""
    config_file = tmp_path / "opencode.json"
    backup_file = tmp_path / "opencode.json.tokenash-backup"
    config_file.write_text("// --- Tokenash proxy provider ---\n{}")
    snapshot_opencode_config_if_unwrapped(config_file, backup_file)
...
def test_strip_blocks_removes_provider_and_mcp() -> None:
    ...
    content = (
        "// --- Tokenash proxy provider ---\n"
        '{"provider": {}}\n'
        "// --- end Tokenash proxy provider ---\n"
        "// --- Tokenash MCP server ---\n"
        '{"mcp": {}}\n'
...
def test_inject_key_merges_dicts() -> None:
    ...
    data = _inject_key_into_json(data, "provider", {"tokenash": {}})
    assert "openai" in data["provider"]
    assert "tokenash" in data["provider"]


def test_inject_key_overwrites_non_dict() -> None:
    """_inject_key_into_json overwrites when existing value is not a dict."""
    data = {"model": "gpt-4o"}
    data = _inject_key_into_json(data, "model", "tokenash/claude-sonnet-4-6")
    assert data["model"] == "tokenash/claude-sonnet-4-6"
...
def test_inject_provider_config_creates_file(
    ...
    assert config["provider"]["tokenash"]["npm"] == "@ai-sdk/openai-compatible"
    assert "model" not in config  # tokenash provider is a transparent pass-through
...
def test_inject_provider_config_idempotent(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    config_file = tmp_path / ".config" / "opencode" / "opencode.json"
    config = _parse_json_loose(config_file.read_text())
    assert config["provider"]["tokenash"]["options"]["baseURL"] == "http://127.0.0.1:9999/v1"
...
def test_parse_json_loose_handles_comments_and_urls() -> None:
    ...
    data = _parse_json_loose(text)
    assert data["baseURL"] == "http://127.0.0.1:8787/v1"
    assert data["name"] == "Tokenash // Proxy"
...
def test_strip_blocks_handles_empty_string() -> None:
    """strip_opencode_tokenash_blocks returns empty string for empty input."""
    assert strip_opencode_tokenash_blocks("") == ""


def test_strip_blocks_handles_whitespace_only() -> None:
    """strip_opencode_tokenash_blocks returns empty string for whitespace input."""
    assert strip_opencode_tokenash_blocks("  \n  ") == ""


def test_strip_blocks_preserves_non_tokenash_jsonc() -> None:
    """strip_opencode_tokenash_blocks preserves JSONC comments not from Tokenash."""
...
def test_strip_blocks_removes_only_one_of_two_identical_blocks() -> None:
    """strip_opencode_tokenash_blocks removes all provider blocks, not just the first."""
    from tokenash.providers.opencode.config import _PROVIDER_MARKER_END, _PROVIDER_MARKER_START
...
def test_strip_blocks_handles_only_mcp_markers() -> None:
    """strip_opencode_tokenash_blocks also strips MCP markers."""
    from tokenash.providers.opencode.config import _MCP_MARKER_END, _MCP_MARKER_START
...
def test_inject_provider_config_merges_with_existing_mcp(
    ...
    """inject_opencode_provider_config merges tokenash MCP with existing MCP servers."""
    _set_test_home(monkeypatch, tmp_path)
    config_file = tmp_path / ".config" / "opencode" / "opencode.json"
...
def test_inject_provider_config_idempotent_with_complex_config(
    ...
    assert "tokenash" in config["provider"]
...
def test_inject_provider_config_preserves_unrelated_top_level_keys(
    ...
    assert "tokenash" in config.get("provider", {})


def test_append_tokenash_plugin_adds_plugin_once() -> None:
    config: dict[str, object] = {"plugin": ["some-plugin"]}

    assert append_tokenash_plugin(config) is True
    assert append_tokenash_plugin(config) is False

    assert config["plugin"] == ["some-plugin", TOKENASH_OPENCODE_PLUGIN]


def test_append_tokenash_plugin_preserves_configured_tuple_entry() -> None:
    config: dict[str, object] = {
        "plugin": [[TOKENASH_OPENCODE_PLUGIN, {"proxyUrl": "http://127.0.0.1:8787"}]]
    }

    assert append_tokenash_plugin(config) is False
    assert config["plugin"] == [[TOKENASH_OPENCODE_PLUGIN, {"proxyUrl": "http://127.0.0.1:8787"}]]
...
def test_build_opencode_config_content_without_mcp() -> None:
    from tokenash.providers.opencode.runtime import build_opencode_config_content

    config = build_opencode_config_content(port=8787, include_mcp=False)
    assert "provider" in config
    assert "mcp" not in config
    assert "model" not in config
    assert config["plugin"] == [["tokenash-opencode", {"proxyUrl": "http://127.0.0.1:8787/v1"}]]


def test_build_launch_env_with_project(monkeypatch: pytest.MonkeyPatch) -> None:
    from tokenash.providers.opencode.runtime import build_launch_env

    monkeypatch.delenv("TOKENASH_PROJECT", raising=False)
    ...
    assert env["TOKENASH_PROJECT"] == "test-proj"
    assert "tokenash-opencode" in env["OPENCODE_CONFIG_CONTENT"]
...
def test_proxy_base_url() -> None:
    from tokenash.providers.opencode.runtime import proxy_base_url
...
def test_inject_provider_config_strips_existing_markers(
    ...
    assert "tokenash" in first

    inject_opencode_provider_config(port=9001)
    second = config_file.read_text()
    assert "tokenash" in second
    assert second.count("tokenash") == first.count("tokenash")
