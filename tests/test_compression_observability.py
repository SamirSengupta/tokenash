...
from tokenash.transforms.content_detector import ContentType
...
from tokenash.transforms.observability import CompressionObserver
...
def isolated_toin(tmp_path, monkeypatch):
    ...
    `~/.tokenash/toin.json`, which persists across pytest invocations.
    ...
    from tokenash.telemetry.toin import TOIN_PATH_ENV_VAR, reset_toin
...
def test_smart_crusher_apply_records_observer_per_crushed_message(isolated_toin):
    ...
    from tokenash.tokenizer import Tokenizer
...
def test_prometheus_export_does_not_leak_per_strategy_metrics():
    ...
    assert "tokenash_compressions_total" not in output
    assert "tokenash_tokens_saved_by_strategy_total" not in output
