...
def _ensure_binding():
    ...
    try:
        from tokenash._core import compress_openai_responses_live_zone

        return compress_openai_responses_live_zone
    except ImportError:
        pytest.skip("tokenash._core not built — run scripts/build_rust_extension.sh")
...
class TestModelDefault:
    """Empty `model` defaults to `tokenash_core`'s `DEFAULT_MODEL`."""
...
class TestTelemetryFields:
    ...
    without recounting tokens. See `crates/tokenash-core/src/transforms/
