...
from tokenash import copilot_macos_keychain
...
def test_read_copilot_oauth_token_uses_security(
    ...
    monkeypatch.setenv("GITHUB_COPILOT_KEYCHAIN_ACCOUNT", "SamirSengupta")
...
def test_read_copilot_oauth_token_tries_copilot_cli_host_login_account(
    ...
    (copilot_home / "config.json").write_text(
        '{"lastLoggedInUser":{"host":"https://github.com","login":"SamirSengupta"}}',
        encoding="utf-8",
    )
    ...
    expected = [
        ...
        "copilot-cli",
        "-a",
        "https://github.com:SamirSengupta",
        "-w",
    ]
    monkeypatch.setattr(copilot_macos_keychain.sys, "platform", "darwin")
