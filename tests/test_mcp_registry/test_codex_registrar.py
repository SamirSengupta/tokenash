...
from tokenash.mcp_registry.base import RegisterStatus, ServerSpec
from tokenash.mcp_registry.codex import CodexRegistrar
...
def test_register_uses_codex_home_env(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    assert "[mcp_servers.tokenash]" in text
...
def test_get_server_returns_none_when_config_missing(tmp_path: Path) -> None:
    assert _make_registrar(tmp_path).get_server("tokenash") is None
...
def test_get_server_returns_spec_when_table_present(tmp_path: Path) -> None:
    ...
    got = _make_registrar(tmp_path).get_server("tokenash")
    assert got is not None
    assert got.command == "tokenash"
    assert got.args == ("mcp", "serve")
    assert got.env == {"TOKENASH_PROXY_URL": "http://127.0.0.1:9000"}
...
def test_register_creates_config_when_missing(tmp_path: Path) -> None:
    ...
    assert cfg.exists()
    text = cfg.read_text()
    assert "# --- Tokenash MCP server ---" in text
    assert "[mcp_servers.tokenash]" in text
    parsed = tomllib.loads(text)
    assert parsed["mcp_servers"]["tokenash"]["command"] == "tokenash"
    assert parsed["mcp_servers"]["tokenash"]["args"] == ["mcp", "serve"]
...
def test_register_includes_env_subtable(tmp_path: Path) -> None:
    spec = _spec(env={"TOKENASH_PROXY_URL": "http://127.0.0.1:9000"})
    _make_registrar(tmp_path).register_server(spec)
    text = _config_path(tmp_path).read_text()
    assert "[mcp_servers.tokenash.env]" in text
    parsed = tomllib.loads(text)
    assert parsed["mcp_servers"]["tokenash"]["env"] == {
        "TOKENASH_PROXY_URL": "http://127.0.0.1:9000"
    }


def test_register_tokenash_and_serena_coexist(tmp_path: Path) -> None:
    reg = _make_registrar(tmp_path)
    ...
    assert "# --- Tokenash MCP server ---" in text
    assert "# --- Tokenash MCP server: serena ---" in text
...
def test_register_omits_env_subtable_when_env_empty(tmp_path: Path) -> None:
    ...
    text = _config_path(tmp_path).read_text()
    assert "[mcp_servers.tokenash.env]" not in text
...
def test_register_mismatch_when_block_differs_no_force(tmp_path: Path) -> None:
    reg = _make_registrar(tmp_path)
    reg.register_server(_spec(env={"TOKENASH_PROXY_URL": "http://127.0.0.1:9999"}))
    text_before = _config_path(tmp_path).read_text()
...
def test_register_force_preserves_user_managed_entry(tmp_path: Path) -> None:
    ...
    cfg.parent.mkdir()
    cfg.write_text(
        '[mcp_servers.tokenash]\ncommand = "/usr/local/bin/custom-tokenash"\nargs = ["serve"]\n'
    )
    ...
    assert "/usr/local/bin/custom-tokenash" in cfg.read_text()
    assert cfg.read_text().count("[mcp_servers.tokenash]") == 1


def test_register_mismatch_when_user_managed_outside_markers(tmp_path: Path) -> None:
    cfg = _config_path(tmp_path)
    cfg.parent.mkdir()
    # User has manually put [mcp_servers.tokenash] with different config — no markers.
    cfg.write_text(
        '[mcp_servers.tokenash]\ncommand = "/usr/local/bin/custom-tokenash"\nargs = ["serve"]\n'
    )
    result = _make_registrar(tmp_path).register_server(_spec())
...
def test_unregister_removes_marker_block(tmp_path: Path) -> None:
    ...
    assert "[mcp_servers.tokenash]" in cfg.read_text()

    assert reg.unregister_server("tokenash") is True
    text = cfg.read_text()
    assert "[mcp_servers.tokenash]" not in text
    assert "# --- Tokenash MCP server ---" not in text
    # Surrounding content survives.
    assert "[other_section]" in text


def test_unregister_serena_preserves_tokenash_block(tmp_path: Path) -> None:
    reg = _make_registrar(tmp_path)
    reg.register_server(_spec())
    ...
    assert "# --- Tokenash MCP server ---" in text
    assert "# --- Tokenash MCP server: serena ---" not in text
...
def test_unregister_returns_false_when_no_block(tmp_path: Path) -> None:
    ...
    cfg.parent.mkdir()
    cfg.write_text('model = "gpt-4o"\n')
    assert _make_registrar(tmp_path).unregister_server("tokenash") is False


def test_unregister_preserves_user_managed_entry(tmp_path: Path) -> None:
    """User-managed [mcp_servers.tokenash] without our markers stays put."""
    cfg = _config_path(tmp_path)
    cfg.parent.mkdir()
    cfg.write_text('[mcp_servers.tokenash]\ncommand = "/custom/tokenash"\n')
    # No markers => unregister is a no-op.
    assert _make_registrar(tmp_path).unregister_server("tokenash") is False
    assert "/custom/tokenash" in cfg.read_text()
...
@pytest.mark.parametrize(
    "spec",
    [
        ServerSpec(name="tokenash", command="tokenash", args=("mcp", "serve")),
        ServerSpec(
            name="tokenash",
            command="tokenash",
            args=("mcp", "serve"),
            env={"TOKENASH_PROXY_URL": "http://127.0.0.1:9000"},
        ),
        ServerSpec(name="tokenash", command="/usr/bin/tokenash", args=()),
    ],
)
def test_round_trip(tmp_path: Path, spec: ServerSpec) -> None:
    reg = _make_registrar(tmp_path)
    reg.register_server(spec)
    got = reg.get_server("tokenash")
    assert got is not None
