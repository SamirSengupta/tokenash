...
from tokenash.mcp_registry.base import (
    MCPRegistrar,
    RegisterResult,
    RegisterStatus,
    ServerSpec,
...
def test_build_spec_default_proxy_no_env() -> None:
    spec = build_tokenash_spec()
    assert spec.name == "tokenash"
    assert spec.command == "tokenash"
    assert spec.args == ("mcp", "serve")
    assert spec.env == {}


def test_build_spec_custom_proxy_sets_env() -> None:
    spec = build_tokenash_spec("http://127.0.0.1:9999")
    assert spec.env == {"TOKENASH_PROXY_URL": "http://127.0.0.1:9999"}


def test_build_spec_default_url_omits_env() -> None:
    spec = build_tokenash_spec(DEFAULT_PROXY_URL)
    assert spec.env == {}
...
def test_build_serena_spec_disables_dashboard_popup_by_default() -> None:
    # Tokenash installs Serena by default; the dashboard browser tab must not
...
def test_install_everywhere_passes_proxy_url_into_spec() -> None:
    ...
    assert captured[0].env == {"TOKENASH_PROXY_URL": "http://localhost:9000"}
