...
* ``StreamingMixin._stream_openai_via_backend`` (Azure/LiteLLM/AnyLLM OpenAI
  ...
  for this path, so ``tokenash perf`` shows 0 cache writes for every
...
Both surface to the user as "Cache write: 0 tokens" in ``tokenash perf``.
"""
...
from tokenash.backends.base import StreamEvent  # noqa: E402
...
class _ListHandler(logging.Handler):
    ...
    ``caplog`` attaches to root; ``tokenash.proxy.helpers._setup_file_logging``
    flips ``logging.getLogger("tokenash").propagate = False`` once a proxy
    instance is constructed in the test, after which root-attached handlers
    stop receiving tokenash-namespaced records. Attaching directly to
    ``tokenash.proxy`` sidesteps that.
    """
...
def test_openai_backend_streaming_emits_perf_with_cache_read_and_inferred_write() -> None:
    ...
    ``tokenash perf`` must report both.
    """
    config = ProxyConfig(
        optimize=False,
        cache_enabled=False,
    ...
    try:
        with patch("tokenash.proxy.server.AnyLLMBackend", return_value=backend):
...
def test_openai_backend_streaming_perf_zeros_when_upstream_omits_usage() -> None:
    ...
    final usage chunk) the PERF line still has to emit so ``tokenash perf``
    counts the request.
    """
    config = ProxyConfig(
        optimize=False,
...
def test_streaming_perf_log_has_no_hardcoded_cache_zeros() -> None:
    ...
    src = Path(__file__).resolve().parents[1] / "tokenash" / "proxy" / "handlers" / "streaming.py"
    text = src.read_text()
