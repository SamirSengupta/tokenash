...
`x-tokenash-*` request headers are internal control flags consumed by the
...
for bypass gating, `_extract_tags` reading `x-tokenash-*`) keep working
because they never depended on the local outbound-bound dict.

Operator opt-in `TOKENASH_STRIP_INTERNAL_HEADERS=disabled` keeps the
...
from tokenash.proxy.server import ProxyConfig, create_app
...
def test_strip_returns_new_dict_does_not_mutate_caller() -> None:
    ...
    original = {
        ...
        "x-tokenash-bypass": "true",
    }
    out = _strip_internal_headers(original)
    assert out is not original
    assert "x-tokenash-bypass" in original
    assert "x-tokenash-bypass" not in out
    assert out["authorization"] == "Bearer x"


def test_strip_x_tokenash_bypass_removed() -> None:
    out = _strip_internal_headers({"x-tokenash-bypass": "true", "k": "v"})
    assert "x-tokenash-bypass" not in out
    assert out["k"] == "v"


def test_strip_x_tokenash_mode_removed() -> None:
    out = _strip_internal_headers({"x-tokenash-mode": "passthrough", "k": "v"})
    assert "x-tokenash-mode" not in out
    assert out["k"] == "v"


def test_strip_x_tokenash_user_id_removed() -> None:
    out = _strip_internal_headers({"x-tokenash-user-id": "u1", "k": "v"})
    assert "x-tokenash-user-id" not in out
    assert out["k"] == "v"


def test_strip_x_tokenash_stack_removed() -> None:
    out = _strip_internal_headers({"x-tokenash-stack": "engineer", "k": "v"})
    assert "x-tokenash-stack" not in out
    assert out["k"] == "v"


def test_strip_x_tokenash_base_url_removed() -> None:
    out = _strip_internal_headers({"x-tokenash-base-url": "http://x", "k": "v"})
    assert "x-tokenash-base-url" not in out
    assert out["k"] == "v"


def test_strip_case_insensitive_prefix_match() -> None:
    """Mixed-case `X-Tokenash-Foo`, `x-Tokenash-Bar`, `X-TOKENASH-BAZ` all stripped."""
    ...
    assert "X-Tokenash-Foo" not in out
    assert "x-Tokenash-Bar" not in out
    assert "X-TOKENASH-BAZ" not in out
    assert out["Authorization"] == "Bearer x"
...
def test_strip_disabled_mode_passes_internal_headers_through(
    ...
    """`TOKENASH_STRIP_INTERNAL_HEADERS=disabled` is operator opt-in for diag."""
    monkeypatch.setenv("TOKENASH_STRIP_INTERNAL_HEADERS", "disabled")
    out = _strip_internal_headers({"x-tokenash-bypass": "true", "authorization": "Bearer x"})
    assert out["x-tokenash-bypass"] == "true"
    assert out["authorization"] == "Bearer x"
...
# ---------------------------------------------------------------------------
# End-to-end: x-tokenash-* never reaches the upstream
# ---------------------------------------------------------------------------
...
def test_x_tokenash_bypass_not_forwarded() -> None:
    ...
    assert "x-tokenash-bypass" not in upstream
...
def test_x_tokenash_mode_not_forwarded() -> None:
    client, transport = _make_anthropic_app()
    resp = client.post(
        "/v1/messages",
        headers={
    ...
    assert transport.captured_headers is not None
    upstream = {k.lower(): v for k, v in transport.captured_headers.items()}
    assert "x-tokenash-mode" not in upstream


def test_x_tokenash_user_id_not_forwarded() -> None:
    client, transport = _make_anthropic_app()
    resp = client.post(
        "/v1/messages",
        headers={
    ...
    assert transport.captured_headers is not None
    upstream = {k.lower(): v for k, v in transport.captured_headers.items()}
    assert "x-tokenash-user-id" not in upstream


def test_x_tokenash_stack_not_forwarded() -> None:
    client, transport = _make_anthropic_app()
    resp = client.post(
        "/v1/messages",
        headers={
    ...
    assert transport.captured_headers is not None
    upstream = {k.lower(): v for k, v in transport.captured_headers.items()}
    assert "x-tokenash-stack" not in upstream


def test_x_tokenash_base_url_not_forwarded() -> None:
    client, transport = _make_anthropic_app()
    resp = client.post(
        "/v1/messages",
        headers={
            ...
            "anthropic-version": "2023-06-01",
            "content-type": "application/json",
            "x-tokenash-base-url": "https://override.example",
        },
        json={
            "model": "claude-sonnet-4-6",
            "max_tokens": 16,
    ...
    assert transport.captured_headers is not None
    upstream = {k.lower(): v for k, v in transport.captured_headers.items()}
    assert "x-tokenash-base-url" not in upstream
...
def test_case_insensitive_prefix_match_e2e() -> None:
    ...
    assert "x-tokenash-foo" not in upstream_lower
    assert "x-tokenash-bar" not in upstream_lower
    assert "x-tokenash-baz" not in upstream_lower
...
def test_inbound_read_path_still_reads_x_tokenash_bypass() -> None:
    """Bypass header still gates compression even though it's stripped from upstream.

    The handler reads `request.headers.get('x-tokenash-bypass')` directly.
    ...
    #   2. The upstream did NOT receive the `x-tokenash-bypass` header.
    resp = client.post(
        "/v1/messages",
        headers={
            "x-api-key": "test-key",
...
def test_disabled_mode_passes_through_e2e(
    ...
    """`TOKENASH_STRIP_INTERNAL_HEADERS=disabled` lets internal headers through."""
    monkeypatch.setenv("TOKENASH_STRIP_INTERNAL_HEADERS", "disabled")
    client, transport = _make_anthropic_app()
    resp = client.post(
        "/v1/messages",
    ...
    assert upstream.get("x-tokenash-mode") == "passthrough"
...
def test_openai_chat_x_tokenash_bypass_not_forwarded() -> None:
    """OpenAI handler also strips x-tokenash-* before upstream call."""
    config = ProxyConfig(
        optimize=False,
        cache_enabled=False,
        rate_limit_enabled=False,
    ...
    assert "x-tokenash-bypass" not in sent_headers
    assert "x-tokenash-user-id" not in sent_headers
