...
**Goal:** Add `deepseek-v4-flash` and `deepseek-v4-pro` pricing to Tokenash so cost estimation works when routing through `--anthropic-api-url https://api.deepseek.com/anthropic`.
...
- Vendored JSON entries at `crates/tokenash-proxy/data/model_prices_and_context_window.json`
...
**Files:**
- Create: `tokenash/pricing/deepseek_prices.py`
- Modify: `tokenash/pricing/__init__.py`
- Test: `tests/test_providers/test_deepseek.py` (first test class)

**Interfaces:**
- Consumes: `ModelPricing`, `PricingRegistry` from `tokenash.pricing.registry`
- Produces: `DEEPSEEK_PRICES: dict[str, ModelPricing]`, `get_deepseek_registry() -> PricingRegistry`, exported via `tokenash.pricing`
...
from tokenash.pricing.deepseek_prices import (
    DEEPSEEK_PRICES,
    get_deepseek_registry,
)
from tokenash.pricing.registry import PricingRegistry
...
```bash
cd D:/tokenash && python -m pytest tests/test_providers/test_deepseek.py::TestDeepSeekPricingModule -v
```
...
Edit `tokenash/pricing/__init__.py` — add these imports after the existing OpenAI imports:
...
```bash
git add tokenash/pricing/deepseek_prices.py tokenash/pricing/__init__.py tests/test_providers/test_deepseek.py
...
**Files:**
- Modify: `tokenash/pricing/litellm_pricing.py` (add injection after `LITELLM_AVAILABLE` block)
...
class TestDeepSeekLiteLLMInjection:
    ...
    def test_deepseek_v4_models_in_litellm_model_cost(self):
        from tokenash.pricing.litellm_pricing import litellm, LITELLM_AVAILABLE
    ...
    def test_injection_does_not_overwrite_existing_upstream_entries(self):
        ...
        import importlib
        import tokenash.pricing.litellm_pricing as lp
        importlib.reload(lp)
...
```bash
cd D:/tokenash && python -m pytest tests/test_providers/test_deepseek.py::TestDeepSeekLiteLLMInjection -v
```
...
At the end of `tokenash/pricing/litellm_pricing.py`, before the `_resolved_model_cache` and function definitions, add:
...
```bash
git add tokenash/pricing/litellm_pricing.py tests/test_providers/test_deepseek.py
...
**Files:**
- Modify: `tokenash/providers/anthropic.py` (add `_get_deepseek_pricing()` helper and call in `_get_pricing()`)
...
class TestDeepSeekAnthropicProviderFallback:
    ...
    def test_deepseek_v4_flash_fallback(self):
        from tokenash.providers.anthropic import AnthropicProvider
...
```bash
cd D:/tokenash && python -m pytest tests/test_providers/test_deepseek.py::TestDeepSeekAnthropicProviderFallback -v
```
...
In `tokenash/providers/anthropic.py`, add a new helper function after the `_UNKNOWN_CLAUDE_DEFAULT` dict (around line 151) and modify `_get_pricing()`:
...
```bash
cd D:/tokenash && python -m pytest tests/test_providers/test_anthropic.py tests/test_providers/test_deepseek.py tests/test_pricing.py tests/test_pricing_litellm.py tests/test_litellm_optional.py -v
```
...
```bash
git add tokenash/providers/anthropic.py tests/test_providers/test_deepseek.py
...
**Files:**
- Modify: `crates/tokenash-proxy/data/model_prices_and_context_window.json`
...
Open `crates/tokenash-proxy/data/model_prices_and_context_window.json` and insert the following after the `"deepseek/deepseek-v3.2"` entry (after line 12516):
...
```bash
cd D:/tokenash && python -m json.tool crates/tokenash-proxy/data/model_prices_and_context_window.json > /dev/null && echo "Valid JSON"
```
...
```bash
cd D:/tokenash && python -m pytest tests/test_pricing.py tests/test_pricing_litellm.py tests/test_providers/test_deepseek.py -v
```
...
```bash
git add crates/tokenash-proxy/data/model_prices_and_context_window.json
...
```bash
cd D:/tokenash && python -m pytest tests/test_pricing.py tests/test_pricing_litellm.py tests/test_litellm_optional.py tests/test_providers/test_anthropic.py tests/test_providers/test_openai.py tests/test_providers/test_deepseek.py tests/test_providers/test_universal.py tests/test_cost_tracker_counterfactual.py tests/test_proxy_savings_history.py tests/test_provider_model_fallback.py -v
```
...
```bash
cd D:/tokenash && python -c "from tokenash.pricing import DEEPSEEK_PRICES, get_deepseek_registry; print('Direct import OK:', list(DEEPSEEK_PRICES.keys()))"
cd D:/tokenash && python -c "from tokenash.pricing.litellm_pricing import get_model_pricing; p = get_model_pricing('deepseek-v4-flash'); print('LiteLLM pricing OK:', p.input_cost_per_1m if p else 'None')"
cd D:/tokenash && python -c "from tokenash.providers.anthropic import AnthropicProvider; p = AnthropicProvider(); print('Anthropic fallback OK:', p._get_pricing('deepseek-v4-flash'))"
```
