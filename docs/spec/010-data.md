...
|-------|----------|------|---------|
| SQLite | `tokenash.db` (cwd or `~/.tokenash/`) | Local file | Metrics, sessions, compression store |
| Memory DB | `~/.tokenash/tokenash_memory.db` | Local file | Memory system (optional) |
| Vector Index | `~/.tokenash/tokenash_memory_vectors.db` | Local file | Semantic search (optional, requires sqlite-vec) |
| Graph Store | `~/.tokenash/tokenash_memory_graph.db` | Local file | Memory relationships |
| Compression cache | `~/.tokenash/cache/` | Directory | Semantic + summary cache |

**Note:** Default store URL is `sqlite:///tokenash.db` (relative to working directory).
...
|----------|---------|-------------|
| `TOKENASH_WORKSPACE_DIR` | `~/.tokenash` | Workspace root; all DBs live under this directory |
| `TOKENASH_CONFIG_DIR` | `~/.tokenash/config` | Config root (read-mostly: models.json, per-plugin config) |
...
| `TOKENASH_WORKSPACE_DIR` | `~/.tokenash` | Workspace root for proxy state, logs, memory, and savings |
| `TOKENASH_STATELESS` | `false` | Disable filesystem writes and keep runtime state in memory |
...
```bash
tar -czf tokenash-backup.tar.gz ~/.tokenash/
```
...
```bash
export TOKENASH_WORKSPACE_DIR=/mnt/state
```
