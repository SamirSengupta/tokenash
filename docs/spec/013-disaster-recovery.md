...
```bash
# Full backup
tar -czf tokenash-backup-$(date +%Y%m%d).tar.gz ~/.tokenash/

# Incremental (last 24h)
sqlite3 ~/.tokenash/tokenash_memory.db ".backup /tmp/tokenash_incremental.db"
```
...
0 2 * * * tar -czf /backup/tokenash-$(date +\%Y\%m\%d).tar.gz ~/.tokenash/
```

### External Storage

> **Note:** External PostgreSQL/Redis storage is not yet implemented. Tokenash uses SQLite at `~/.tokenash/` (configurable via `TOKENASH_WORKSPACE_DIR`). The `TOKENASH_DB_URL` and `TOKENASH_CACHE_BACKEND` vars do not exist.
...
```bash
# Docker
docker-compose restart tokenash

# Native
pkill tokenash && tokenash proxy &
```
...
# Restore SQLite
rm ~/.tokenash/tokenash_memory.db
tar -xzf tokenash-backup-20260416.tar.gz -C ~/
