...
Tokenash is a **context compression proxy** for AI provider APIs. It sits between AI coding tools (Claude Code, Copilot, Codex, etc.) and provider APIs (OpenAI, Anthropic, Google, Cohere), compressing context before it reaches the provider to reduce token usage and costs.
...
### What Tokenash Is Not

- A model provider — Tokenash does not host or run AI models
...
**Principle:** Tokenash works alongside existing tools without requiring workflow changes.

**Implications:**
- Proxy mode: route traffic through Tokenash
...
| **Plugin Authors** | Extend Tokenash via plugin ABI |
