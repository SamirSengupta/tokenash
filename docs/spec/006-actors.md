...
The developer using an AI coding agent with Tokenash.

**Interactions:**
- Configures Tokenash via environment variables or config file
...
The person deploying and managing Tokenash in production.

**Interactions:**
- Deploys Tokenash (Docker, native, or embedded)
...
- Plugins auto-discovered from `tokenash/learn/plugins/` directory
- Contributes to Tokenash
...
**Plugin Template:**
```python
from tokenash.learn.base import LearnPlugin, ConversationScanner
...
The person assessing Tokenash for organizational adoption.
...
TOKENASH_TELEMETRY=off
TOKENASH_STATELESS=true
tokenash proxy --no-cache --no-optimize
```
...
**Flow:**
1. User's AI agent sends request to Tokenash proxy
2. Tokenash compresses context
...
2. Plugins are auto-discovered from `tokenash/learn/plugins/` directory
3. Writes unit tests
