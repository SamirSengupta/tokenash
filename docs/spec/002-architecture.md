...
Tokenash is a context compression proxy for LLM applications, featuring intelligent transforms, semantic caching, and CCR (Compress-Cache-Retrieve) architecture.
...
### Proxy Server (`tokenash/proxy/server.py`)

**`TokenashProxy` class** — Main proxy server (default port 8787):
...
**`ProxyConfig` dataclass** (from `tokenash/models/config.py`):
```python
@dataclass
class ProxyConfig:
    store_url: str = "sqlite:///tokenash.db"
    default_mode: TokenashMode = TokenashMode.AUDIT
...
**`TokenashMode` enum** (actual modes):
```python
class TokenashMode(str, Enum):
...
### Python SDK (`tokenash/client.py`)

**`TokenashClient` class:**
```python
...
### CCR MCP Server (`tokenash/ccr/mcp_server.py`)
...
**MCP Tools:**
- `tokenash_compress` — Compress messages
- `tokenash_retrieve` — Retrieve cached content
- `tokenash_stats` — Get statistics
...
## Compression Layer (`tokenash/transforms/`)

### SmartCrusher (`tokenash/transforms/smart_crusher.py`)
...
### CacheAligner (`tokenash/transforms/cache_aligner.py`)
...
### RollingWindow (`tokenash/transforms/rolling_window.py`)
...
### ContentRouter (`tokenash/transforms/content_router.py`)
...
## TOIN (`tokenash/telemetry/toin.py`)
...
```
Client → Tokenash Proxy → [ContentRouter] → [SmartCrusher/CacheAligner/RollingWindow]
              │                    │              │
              │              [CCR Store] ←───────────────────────┘
              │
              │         [Telemetry/Metrics]
