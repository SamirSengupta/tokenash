# Loop weighting in Tokenash Learn + RTK-loop eval

**Status:** proposed (branch `purva/rtk-loop-evals`).
**Context:** Tejas asked for (1) an eval that reproduces the RTK loop, runs it
through Tokenash Learn, and checks the generated rule prevents re-triggering,
and (2) a change so Tokenash Learn gives loops more weight.

## The gap

Before this change, `tokenash learn` ranked every recommendation by a single
...
A new module `tokenash/learn/loops.py`:
...
`--real` — via an API key or an installed CLI backend (`TOKENASH_LEARN_CLI=claude`).
