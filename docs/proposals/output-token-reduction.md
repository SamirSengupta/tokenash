...
Tokenash's entire transform pipeline compresses what goes **into** the model.
...
`tokenash/proxy/output_shaper.py` — a request-body rewriter invoked in
...
`x-tokenash-bypass` header as compression. Opt-in via `TOKENASH_OUTPUT_SHAPER=1`.
...
4. **Respect `x-tokenash-bypass`** — sub-agent calls that opt out of
...
|---|---|---|
| `TOKENASH_OUTPUT_SHAPER` | off | master switch (`1`/`true`/`yes`) |
| `TOKENASH_VERBOSITY_LEVEL` | `2` | 0–4 (clamped) |
| `TOKENASH_EFFORT_ROUTER` | on | set `0` to disable effort routing |
| `TOKENASH_MECHANICAL_EFFORT` | `low` | floor effort for mechanical turns |
...
The right verbosity is **per-user**, not global. A fixed `TOKENASH_VERBOSITY_LEVEL`
is a guess. We can do better: mine the user's own past sessions to infer what
they actually tolerate — exactly the philosophy of `tokenash learn`, which
...
### 4.3 Design — `tokenash learn --verbosity`

Slots into the existing `learn` architecture. `tokenash/learn/plugins/claude.py`
...
```
Verbosity analysis — /Users/tcms/demo/tokenash (24 sessions, 210 turns)
...
**(b) A persisted setting** written to `~/.tokenash/` (alongside the existing
savings tracker) — per-project verbosity level + confidence:
```json
{"project": "/Users/tcms/demo/tokenash",
...
gains a fallback: if `TOKENASH_VERBOSITY_LEVEL` is unset, load the learned
...
1. **Removes the guess.** Today you set `TOKENASH_VERBOSITY_LEVEL=2` by hand.
...
| File | Status | Purpose |
|---|---|---|
| `tokenash/proxy/output_shaper.py` | ✅ built | the shaper (steering + effort routing) |
| `tokenash/proxy/handlers/anthropic.py` | ✅ wired | invoke shaper after body mutations |
...
| `tokenash/learn/plugins/claude.py` (+ analyzer) | 🔜 extend | `--verbosity` analysis path |
| `~/.tokenash/verbosity.json` | 🔜 | persisted per-project learned level |
...
Input compression is a **pure function**: Tokenash takes a request, shrinks it,
...
**Tier 2 — Measured (A/B holdout).** Set `TOKENASH_OUTPUT_HOLDOUT=0.1` and 10%
...
proxy request (TOKENASH_OUTPUT_SHAPER=1)
...
tokenash output-savings   /   dashboard "Output Tokens Saved" card
...
- Runtime AIMD upward-ratcheting is gated off by default: we can reliably detect
  ...
  behind `TOKENASH_VERBOSITY_AUTOTUNE` until both directions are trustworthy.
