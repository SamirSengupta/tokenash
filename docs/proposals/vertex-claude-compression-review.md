# Claude Code + Vertex AI + Tokenash compression — does it work?
...
copy-paste runbook: [`docs/claude-code-vertex-tokenash.md`](../claude-code-vertex-tokenash.md).**
...
>   proxy never receives a byte. Not a Tokenash bug — the native `:rawPredict`
...
>   path.** Verified: Claude Code → Tokenash → LiteLLM → Vertex (`global`), real
...
There is still **no `tokenash wrap claude` turnkey** for Vertex, and the one backend
...
client is pointed at Tokenash, one mis-named backend, a missing pip extra, a
default-off compressor, and a few env vars Tokenash never sets for you.
...
Vertex AI** (not the direct Anthropic API). They want **Tokenash** in the middle so
...
1. **The client's traffic must actually reach Tokenash** (the proxy must be in the path).
2. **Tokenash must compress it.**
3. **Tokenash must forward it to Vertex correctly** (right URL, right auth, right body shape) and translate the answer back so Claude Code understands it.
...
|---|---|---|---|
| **Python proxy, native Vertex `:rawPredict`** (publisher = `anthropic`) | Client sends a real Vertex request to Tokenash | ✅ **Yes** | Runs the full Anthropic compression pipeline, keeps the Vertex body shape, forwards the client's own Google token. `proxy_routes.py:648` |
| **Python proxy, `--backend litellm-vertex_ai`** | Client speaks plain Anthropic; Tokenash translates to Vertex | ✅ **Yes** (correct string only) | Full Anthropic↔Vertex translation incl. streaming + tools. **`litellm-vertex` is broken — must use `litellm-vertex_ai`.** |
| **Rust proxy, native Vertex `:rawPredict`** | A separate `tokenash-proxy` binary | ✅ **Yes** | Correct and well-built — **but never run by `tokenash proxy`/`wrap`.** Dead code for normal users. |
...
**Through `tokenash wrap claude` with zero extra setup: no.** `wrap claude` only
sets `ANTHROPIC_BASE_URL`. If Claude Code is in Vertex mode it ignores that and
talks straight to Google — Tokenash is never in the path. And `wrap claude` has no
...
You just tell it "send Vertex requests to Tokenash instead of straight to Google,"
and you tell Tokenash where the real Vertex endpoint is.

```bash
# 1) Run Tokenash, telling it the real Vertex endpoint (match your region!)
tokenash proxy --port 8787 \
  --vertex-api-url https://us-east5-aiplatform.googleapis.com   # use YOUR region

# 2) Run Claude Code in Vertex mode, but point its Vertex base URL at Tokenash
...
What happens: Claude Code → `ANTHROPIC_VERTEX_BASE_URL` (Tokenash) → Tokenash
...
## Setup B — Claude Code in normal Anthropic mode; Tokenash talks to Vertex

Idea: Claude Code thinks it's talking to plain Anthropic. Tokenash holds the Google
credentials and is the one that actually talks to Vertex.

```bash
# Tokenash does the Vertex talking — note the backend name carefully
export TOKENASH_BACKEND=litellm-vertex_ai        # NOT "litellm-vertex" (that's broken — Issue #1)
export TOKENASH_REGION=us-east5                  # becomes the Vertex location
export VERTEXAI_PROJECT=<your-gcp-project>        # Tokenash does NOT set this for you (Issue #4)
...
What happens: Claude Code → Tokenash (plain Anthropic `/v1/messages`) →
...
2. Or check response headers on a request: `x-tokenash-tokens-before`,
   `x-tokenash-tokens-after`, `x-tokenash-tokens-saved`.
...
on `vertex_ai`, not `vertex`. So Tokenash falls back to a generic "unknown
...
### 2. GAP — `tokenash wrap claude` has no Vertex support
...
### 3. GAP — Vertex-mode Claude Code bypasses the proxy, and Tokenash never sets the fix
...
— that points Claude Code's Vertex traffic at a gateway. But Tokenash never sets it
...
For Setup B, Tokenash passes the region to LiteLLM but never the project. You must
...
Tokenash pins the Vertex host to one region (default `us-central1`) but throws away
the region in the client's request path. If your client targets, say,
`europe-west1` while Tokenash is on the default host, the request goes to the wrong
...
There's a second, Rust proxy (`crates/tokenash-proxy/src/vertex/`) that compresses
Vertex traffic correctly. But `tokenash proxy` and `tokenash wrap` run the **Python**
...
*Where:* `crates/tokenash-proxy/Cargo.toml` (`[[bin]]`), no Python→Rust bridge to it.
...
`crates/tokenash-proxy/src/vertex/mod.rs:42-47` describes a "synthetic model
...
After #1–#3, the honest customer message becomes: *"`tokenash wrap claude` works
