...
**Owner:** Tokenash realignment.

## TL;DR

**RTK is a wrap-CLI hook, not a proxy-side compressor.** The Tokenash
...
wrap-CLI tail — `tokenash wrap claude`, `tokenash wrap codex`, etc.
...
The Rust proxy already has a `crates/tokenash-core/src/transforms/log_compressor.rs`
...
Every `tokenash wrap <agent>` subcommand:

1. Ensures the RTK binary is installed via `_ensure_rtk_binary()`.
2. Injects the `<!-- tokenash:rtk-instructions -->` block into the
...
See `tokenash/cli/wrap/` for the per-agent shims.
...
- `tokenash/cli/wrap/` — wrap-CLI implementation.
- `crates/tokenash-core/src/transforms/log_compressor.rs` — the
