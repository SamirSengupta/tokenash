# Cortex Code + Tokenash — Integration Guide

Tokenash compresses the context Cortex Code (CoCo) sends to `claude-sonnet-4-6`
...
response — not tokenash's local estimate.
...
CoCo (cortex CLI)
  ...
  ▼
Tokenash proxy  (local, your data never leaves your machine)
...
Tokenash's **SmartCrusher** targets the large JSON blobs that CoCo produces:
...
tokenash wrap cortex-code          # starts proxy + prints the env var to set
```

`tokenash wrap cortex-code` starts the local proxy and prints:

```
  Tokenash proxy is running. Configure Cortex Code (CoCo):
...
Cortex Code authenticates using your Snowflake connection. Tokenash sits
...
Use `tokenash wrap cortex-code --project <name>` to attribute savings to a
specific project in the tokenash dashboard:

```bash
tokenash wrap cortex-code --project my-dbt-project
```
...
After a CoCo session, check what tokenash saved:

```bash
tokenash perf          # token savings for the last session
tokenash perf --hours 24  # last 24 hours
```
...
Cortex Code routes through tokenash's OpenAI-compatible pipeline. The provider
slice lives in `tokenash/providers/cortex_code/`:
...
Registered in `tokenash/providers/install_registry.py` under the key
`"cortex-code"`, which is what `tokenash wrap cortex-code` resolves to.
...
- The tokenash proxy cannot rewrite the Cortex inference path
  (`/api/v2/cortex/inference:complete` ≠ `/v1/chat/completions`), so
  **library mode** (`from tokenash import compress`) is required when calling
...
- Output-token reduction (`TOKENASH_OUTPUT_SHAPER=1`) is supported in proxy
...
- [Claude Code + Vertex](claude-code-vertex-tokenash.md)
