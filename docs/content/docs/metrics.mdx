...
description: Monitor compression performance, cost savings, and system health with Tokenash's built-in metrics, Prometheus endpoint, and SDK APIs.
---

Tokenash provides comprehensive metrics for monitoring compression performance, cost savings, and system health through both the proxy server and the SDK.
...
Persistent savings are stored at `~/.tokenash/proxy_savings.json` and survive proxy restarts. Override the path with `TOKENASH_SAVINGS_PATH`.
...
```
# HELP tokenash_requests_total Total requests processed
tokenash_requests_total{mode="optimize"} 1234

# HELP tokenash_tokens_saved_total Total tokens saved
tokenash_tokens_saved_total 5678900

# HELP tokenash_compression_ratio Compression ratio histogram
tokenash_compression_ratio_bucket{le="0.5"} 890
tokenash_compression_ratio_bucket{le="0.7"} 1100
tokenash_compression_ratio_bucket{le="0.9"} 1200

# HELP tokenash_latency_seconds Request latency histogram
tokenash_latency_seconds_bucket{le="0.01"} 800
tokenash_latency_seconds_bucket{le="0.1"} 1150

# HELP tokenash_cache_hits_total Cache hit counter
...
| Panel | PromQL |
|-------|--------|
| Tokens Saved | `tokenash_tokens_saved_total` |
| Compression Ratio (median) | `histogram_quantile(0.5, tokenash_compression_ratio_bucket)` |
| Request Latency (p99) | `histogram_quantile(0.99, tokenash_latency_seconds_bucket)` |
| Cache Hit Rate | `tokenash_cache_hits_total / (tokenash_cache_hits_total + tokenash_cache_misses_total)` |
