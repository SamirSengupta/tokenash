---
title: OpenAI SDK
description: Auto-compress messages in the OpenAI Node.js SDK with a single withTokenash() wrapper.
---

Tokenash wraps the OpenAI Node.js SDK to automatically compress messages before every `chat.completions.create()` call. All other methods (embeddings, images, audio) pass through unchanged.

## Installation

```bash
npm install tokenash openai
```
...
`withTokenash()` returns a proxy around your OpenAI client that intercepts `chat.completions.create()`:

1. Extracts `messages` from the request params
2. Sends them to the Tokenash proxy's `/v1/compress` endpoint
