...
When agents hand off to each other, context gets replayed in full. SharedContext compresses what moves between agents using Tokenash's compression pipeline, typically saving **~80% of tokens** on agent handoffs.
...
Under the hood, `put()` calls `tokenash.compress()` -- the same pipeline used by the Tokenash proxy -- and stores the original in memory. `get()` returns the compressed version. `get(full=True)` returns the original.
