...
SmartCrusher is Tokenash's compressor for JSON tool outputs. It analyzes arrays statistically, keeps the important items (errors, anomalies, relevant matches), and drops the rest. This is the compressor that fires automatically when ContentRouter detects JSON arrays.
...
// Configure via the Tokenash proxy or TokenashClient
const result = await compress(messages, {
  model: "gpt-4o",
