---
title: Architecture
description: How Tokenash's three-stage compression pipeline works, from message parsing through transform execution to provider cache optimization.
---

Tokenash sits between your application and the LLM provider. It intercepts messages, compresses them intelligently, and forwards the optimized request. The response comes back unchanged.
...
Tokenash can be used in three ways, all feeding into the same pipeline:

| Entry Point | How It Works | Code Changes |
|-------------|-------------|--------------|
| **SDK Mode** | Wrap your LLM client with `TokenashClient` | Minimal -- swap client constructor |
| **Proxy Mode** | Run `tokenash proxy` and point your client at it | Zero -- just change the base URL |
...
After the pipeline, Tokenash applies provider-specific cache hints:
