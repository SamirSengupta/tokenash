...
description: How to catch and handle Tokenash errors in Python and TypeScript. Error hierarchy, proxy error mapping, and safety guarantees.
---

Tokenash provides explicit exceptions for debugging, with a core safety guarantee: **compression failures never break your LLM calls**. If compression fails, the original content passes through unchanged.
...
```
TokenashError (base class)
  +-- TokenashConnectionError   # Cannot reach proxy
  +-- TokenashAuthError         # 401 from proxy
  +-- TokenashCompressError     # Compression failed (with statusCode)
...
```ts twoslash
import { compress, TokenashConnectionError, TokenashAuthError, TokenashCompressError, TokenashError } from 'tokenash';

try {
  const result = await compress(messages, { model: 'gpt-4o' });
} catch (e) {
  if (e instanceof TokenashConnectionError) {
    console.error('Cannot reach proxy:', e.message);
  } else if (e instanceof TokenashAuthError) {
    console.error('Auth failed:', e.message);
  } else if (e instanceof TokenashCompressError) {
    console.error(`Compress failed (${e.statusCode}):`, e.message);
  } else if (e instanceof TokenashError) {
    console.error('Tokenash error:', e.message, e.details);
  }
}
```
...
<Tab value="TypeScript">
```ts twoslash
import { ConfigurationError } from 'tokenash';
...
### TokenashConnectionError (TypeScript)

Raised when the TypeScript SDK cannot connect to the Tokenash proxy.

```ts twoslash
import { compress, TokenashConnectionError } from 'tokenash';

try {
  await compress(messages, { model: 'gpt-4o' });
} catch (e) {
  if (e instanceof TokenashConnectionError) {
    console.error('Is the proxy running? Start with: tokenash proxy');
  }
}
```
...
| 4xx/5xx | (other) | `TokenashCompressError` |

The `mapProxyError()` function handles this mapping:

```ts twoslash
import { mapProxyError } from 'tokenash';
...
All Tokenash exceptions include a `details` dict/object with additional context:

<Tabs groupId="lang" items={['TypeScript', 'Python']}>
<Tab value="TypeScript">
```ts twoslash
import { TokenashError } from 'tokenash';

// TokenashError.details is Record<string, any> | undefined
// TokenashCompressError also has .statusCode and .errorType
```
</Tab>
<Tab value="Python">
```python
...
If compression fails, the original content passes through unchanged. Your LLM calls never fail due to Tokenash:
