---
title: API Reference
description: Complete API reference for the Tokenash Python and TypeScript SDKs. Core client, configuration types, result types, errors, and utilities.
---

Complete API reference for the Tokenash Python and TypeScript SDKs.

## Core

### TokenashClient

The main entry point for the Tokenash SDK.

<Tabs groupId="lang" items={['TypeScript', 'Python']}>
<Tab value="TypeScript">

<TypeTable type={{
  baseUrl: { type: 'string', description: 'Base URL for the Tokenash proxy' },
...
Accepts all standard OpenAI/Anthropic parameters plus Tokenash-specific overrides:

<TypeTable type={{
  tokenash_mode: { type: '"audit" | "optimize" | "simulate"', description: 'Override mode for this request', default: 'Client default' },
  tokenash_query: { type: 'str', description: 'Query for relevance scoring', default: 'None' },
  tokenash_output_buffer_tokens: { type: 'int', description: 'Reserve tokens for output', default: '4000' },
  tokenash_keep_turns: { type: 'int', description: 'Keep last N turns uncompressed', default: '2' },
  tokenash_tool_profiles: { type: 'dict', description: 'Per-tool compression overrides', default: '{}' },
}} />
...
Top-level function to compress messages via the Tokenash proxy.
...
<TypeTable type={{
  ...
  fallback: { type: 'boolean', description: 'Return original messages on failure instead of throwing' },
  retries: { type: 'number', description: 'Number of retry attempts on failure' },
  client: { type: 'TokenashClientInterface', description: 'Pre-configured client instance to use' },
...
Context management is now automatic. Tokenash handles rolling window behavior internally based on the model's context limit and the `tokenash_keep_turns` / `tokenash_output_buffer_tokens` per-request overrides. The `RollingWindowConfig`, `IntelligentContextConfig`, and `ScoringWeights` classes no longer exist as public API.
...
<TypeTable type={{
  ...
  defaultMode: { type: 'TokenashMode', description: 'Default compression mode' },
...
# Note: rolling_window was removed in 0.9.x — use tokenash_keep_turns per-request instead
```
...
Semantic similarity scoring (requires `tokenash[relevance]`):
...
| Exception | Meaning |
|-----------|---------|
| `TokenashError` | Base class for all errors |
| `TokenashConnectionError` | Cannot reach proxy |
| `TokenashAuthError` | 401 from proxy |
| `TokenashCompressError` | Compression failed (includes `statusCode`, `errorType`) |
...
| Exception | Meaning |
|-----------|---------|
| `TokenashError` | Base class for all Tokenash errors |
