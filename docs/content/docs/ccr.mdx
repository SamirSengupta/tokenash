...
Tokenash's CCR (Compress-Cache-Retrieve) architecture makes compression **reversible**. When content is compressed, the original data is cached locally. If the LLM needs the full data, it retrieves it instantly.
...
The client never sees CCR tool calls -- they are handled transparently by Tokenash.
...
import type { CCRConfig } from "tokenash";
...
const ccrConfig: CCRConfig = {
  enabled: true,
  injectTool: true,             // Inject tokenash_retrieve tool
...
# The LLM calls tokenash_retrieve when it needs more data.
...
agent runs, set `TOKENASH_CCR_TTL_SECONDS` before starting the proxy:

```bash
TOKENASH_CCR_TTL_SECONDS=7200 tokenash proxy
```
