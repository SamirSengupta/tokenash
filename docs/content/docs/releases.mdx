...
Tokenash uses `release-please` to maintain a release PR from conventional commits on `main`. Merging that release PR creates the release tag and GitHub Release, which triggers `.github/workflows/release.yml` to publish all packages, build version-matched Docker images, and attach release assets.
...
|---------|------|----------|----------------------|
| `tokenash` | Python | PyPI | `PYPI_PACKAGE` |
| `tokenash` | TypeScript SDK | npmjs.org | `NPM_SDK_PACKAGE` |
| `tokenash-openclaw` | TypeScript plugin | npmjs.org | `NPM_OPENCLAW_PACKAGE` |
| `@{owner}/tokenash` | TypeScript SDK | GitHub Package Registry | — |
| `@{owner}/tokenash-openclaw` | TypeScript plugin | GitHub Package Registry | — |
| `tokenash-{version}.tar.gz` / `tokenash_ai-{version}-py3-none-any.whl` | Python package distributions | GitHub Release (`{owner}/tokenash`) | — |
| `tokenash-{version}.tgz` / `tokenash-openclaw-{version}.tgz` | Node release assets | GitHub Release (`{owner}/tokenash`) | — |
| `ghcr.io/{owner}/tokenash` | Docker image | GitHub Container Registry | — |
...
- `tokenash/_version.py` - `__version__`, synced at build time
...
### smoke-import-wheels
Installs the built wheels into representative customer environments and imports `tokenash._core`. This blocks publishing if a wheel builds successfully but cannot import on its promised platform floor.
...
- `sdk/typescript/` as `tokenash`
- `plugins/openclaw/` as `tokenash-openclaw`
...
- `sdk/typescript/` as `@{owner}/tokenash`
- `plugins/openclaw/` as `@{owner}/tokenash-openclaw`
...
```yaml
env:
  PYPI_PACKAGE: tokenash
  PYPI_ENVIRONMENT: pypi
  NPM_REGISTRY_URL: https://registry.npmjs.org
  NPM_SDK_PACKAGE: tokenash
  NPM_OPENCLAW_PACKAGE: tokenash-openclaw
