...
description: Solutions for common Tokenash issues including proxy startup, connection errors, no token savings, high latency, and installation problems.
---
...
**Fix**: Tokenash pins `ORT_DYLIB_PATH` automatically at import time to the
DLL inside the `onnxruntime` pip package (included in `tokenash[proxy]`).
...
`TOKENASH_MAGIKA_INIT_TIMEOUT_SECS` (default `5`) bounds the init as a safety
...
**Symptom**: After pointing Claude Code at Tokenash (`ANTHROPIC_BASE_URL`), `/context all`
...
```bash
# Easiest: `tokenash wrap claude` sets ENABLE_TOOL_SEARCH=true automatically.
...
tokenash wrap claude --tool-search auto
...
See [issue #746](https://github.com/SamirSengupta/tokenash/issues/746) for the full analysis.
...
### pipx installs an older Tokenash version

**Symptom**: PyPI shows a newer `tokenash` release, but `pipx install` or
...
environment uses a Python version that Tokenash does not publish wheels for yet,
...
If you already have Tokenash installed under `pipx`, uninstall it first or
...
4. File an issue at [github.com/SamirSengupta/tokenash](https://github.com/SamirSengupta/tokenash/issues) with your Tokenash version, Python version, provider, debug log output, and minimal reproduction code
