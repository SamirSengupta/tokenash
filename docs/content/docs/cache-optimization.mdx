...
LLM providers cache prompt prefixes to avoid reprocessing identical input on repeated calls. Tokenash's **CacheAligner** stabilizes your message prefixes so these caches actually hit, and then applies provider-specific strategies to maximize savings.
...
Each LLM provider implements caching differently. Tokenash applies the optimal strategy for each.
...
Tokenash automatically inserts `cache_control` breakpoints at the right positions in your messages so that stable prefixes (system prompts, early conversation turns) are cached across requests.
...
Tokenash can manage CachedContent lifecycle automatically, creating and refreshing cached content objects as needed.
...
<Tab value="Python">
```python
from tokenash import TokenashClient, OpenAIProvider, AnthropicProvider, GoogleProvider
...
CacheAligner and provider caching work together with Tokenash's compression transforms:
