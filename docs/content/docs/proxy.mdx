---
title: Proxy Server
description: Run the Tokenash proxy to compress LLM traffic for any client — Claude Code, Cursor, OpenAI SDK, or custom apps.
---

The Tokenash proxy is a standalone HTTP server that compresses all LLM traffic passing through it. Point any client at the proxy and get automatic context optimization.
...
tokenash proxy --host 0.0.0.0 --port 8080
...
Telemetry is **off by default** (opt-in). Opt in with `TOKENASH_TELEMETRY=on` or `--telemetry`.
...
| `--code-aware` / `--no-code-aware` | disabled | Enable or disable AST-based code compression. Requires `tokenash[code]`. |
...
| `--memory-db-path` | `{cwd}/.tokenash/memory.db` | Override the memory SQLite path |
...
tokenash proxy --codex-wire-debug
```

<Callout type="info" title="LLMLingua removed from the proxy CLI">
The old LLMLingua proxy toggles are no longer part of the CLI. Tokenash's proxy compression path uses ContentRouter plus the current built-in compressors, including Kompress where applicable.
</Callout>
...
Live session statistics plus durable `persistent_savings` totals. Stored at `~/.tokenash/proxy_savings.json` (override with `TOKENASH_SAVINGS_PATH`).
...
```
tokenash_requests_total{mode="optimize"} 1234
tokenash_tokens_saved_total 5678900
tokenash_compression_ratio_bucket{le="0.5"} 890
tokenash_latency_seconds_bucket{le="0.01"} 800
tokenash_cache_hits_total 456
```
...
Set `x-tokenash-bypass: true` to skip compression.

## Agent wrapping

Use `tokenash wrap` to transparently proxy any CLI tool:
...
`--bedrock-api-url` lets Tokenash sit in that chain. It registers passthrough routes for `/model/{id}/invoke` and `/model/{id}/invoke-with-response-stream`, compresses the request body with the same pipeline as `/v1/messages`, and forwards to the gateway:

```bash
tokenash proxy --bedrock-api-url http://127.0.0.1:4000
# then point the client's Bedrock endpoint at Tokenash:
...
`build-essential` is required at install time because `tokenash` includes `hnswlib`, a C++ extension compiled from source. It is removed after installation to keep the image slim.
</Callout>
