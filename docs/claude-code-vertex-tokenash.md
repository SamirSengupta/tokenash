# Claude Code + Google Vertex AI, with Tokenash compression

*Validated end-to-end on 2026-06-19 (Claude Code 2.1.181, Tokenash 0.27.0).*

This is the **working, tested** way to run **Claude Code** against **Claude models on
Google Vertex AI** with **Tokenash compressing the context** in the middle.

## TL;DR

Run Claude Code in **normal Anthropic mode** (NOT Vertex mode) pointed at a local
Tokenash proxy, and let **Tokenash** be the thing that talks to Vertex:

```
Claude Code  ──ANTHROPIC_BASE_URL──▶  Tokenash proxy  ──LiteLLM (vertex_ai)──▶  Vertex AI
...
## Why not "just point Claude Code's Vertex URL at Tokenash"?
...
receives a byte. This is a Claude Code limitation, not a Tokenash bug. The native
`:rawPredict` passthrough in Tokenash is correct and compresses (verified by direct
...
- **Tokenash ML extra** for compression: `pip install "google-cloud-aiplatform>=1.38"`
  plus the Kompress ML stack (`torch`, `transformers`, `onnxruntime` — the
  `tokenash[ml]` extra). The `kompress-v2-base` model downloads from Hugging Face
  on first use.

## Terminal 1 — start the Tokenash proxy (Vertex backend)
...
export ANTHROPIC_MODEL=claude-sonnet-4-6             # sent to Tokenash, mapped to vertex_ai/claude-sonnet-4-6
...
Claude Code now talks plain Anthropic `/v1/messages` to Tokenash; Tokenash compresses
...
if the model needs the exact bytes it can pull them back via the `tokenash_retrieve`
tool — compression is reversible, not destructive.

**What it does NOT touch:**

- **Your files on disk.** Tokenash is a network proxy: it only rewrites the *request
...
- Tokenash maps clean ids to Vertex publisher ids: `claude-sonnet-4-6` →
  `vertex_ai/claude-sonnet-4-6` (see `tokenash/backends/litellm.py`). Newer models use
...
| `tokens_saved: 0` everywhere, `/debug/warmup` shows `kompress: not installed` | ML extra missing | install `tokenash[ml]` (torch/transformers/onnxruntime) |
