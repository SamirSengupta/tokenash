...
Tokenash normally shrinks the prompt you **send**. This feature shrinks what the
...
You don't change any code. It runs in the Tokenash proxy.

## Turn it on

```bash
export TOKENASH_OUTPUT_SHAPER=1     # off by default
tokenash proxy --port 8787
```

> **If a proxy is already running** (e.g. `tokenash wrap claude` attaches to one
...
> it. `tokenash wrap` handles this for you: it hot-syncs your current output
...
2. **Effort routing** — if a turn is just the model continuing after a tool ran
   (e.g. it read a file and there were no errors), Tokenash turns the model's
...
```bash
export TOKENASH_VERBOSITY_LEVEL=3
```

Or — better — let Tokenash learn it from your habits (next section).

## Let Tokenash pick the level for you

People rarely *say* "be brief." They *show* it: they interrupt long answers, or
reply so fast they couldn't have read the whole thing. `tokenash learn
--verbosity` reads your past sessions and picks a level from those signals.

```bash
# Preview what it found (doesn't change anything)
tokenash learn --verbosity

# Save it — the proxy uses this level from now on
tokenash learn --verbosity --apply
```
...
see what the model *would* have written without our nudge. So Tokenash reports an
**estimate with a confidence range**, never a fake exact number:

```bash
tokenash output-savings
```
...
```bash
export TOKENASH_OUTPUT_HOLDOUT=0.1     # 10% of conversations stay unshaped
```

Now `tokenash output-savings` compares shaped vs unshaped directly and reports a
...
Unset `TOKENASH_OUTPUT_SHAPER` (or set it to `0`) and restart the proxy. You can
also send `x-tokenash-bypass: true` on a request to skip it for that call.
