# Tokenash Rust build targets. `just` is not installed on dev boxes; this
...
help:
	@echo "Tokenash Rust targets:"
	@echo "  make test               - cargo test --workspace"
	@echo "  make test-parity        - maturin develop + parity-run against fixtures"
	@echo "  make bench              - cargo bench --workspace"
	@echo "  make build-proxy        - release build + strip tokenash-proxy, print size"
	@echo "  make build-wheel        - release wheel for tokenash-py"
	@echo "  make verify-rust-core   - build + install + import-verify tokenash._core"
...
test-parity:
	...
	$(MATURIN) develop -m crates/tokenash-py/Cargo.toml
	$(CARGO) run -p tokenash-parity -- run --fixtures $(FIXTURES)

bench:
	$(CARGO) bench --workspace

build-proxy:
	$(CARGO) build --release -p tokenash-proxy
	@BIN=target/release/tokenash-proxy; \
	if command -v strip >/dev/null 2>&1; then strip "$$BIN" || true; fi; \
	SIZE=$$(wc -c < "$$BIN"); \
	printf 'tokenash-proxy: %s bytes (%.1f MiB)\n' "$$SIZE" "$$(echo "$$SIZE / 1048576" | bc -l)"

build-wheel:
	$(MATURIN) build --release -m crates/tokenash-py/Cargo.toml
...
# mode (Finding #2 in TOKENASH_PROXY_LOG_FINDINGS_2026_05_03.md). The
...
# darwin wheel, tokenash._core not built in test-extras + smoke-test,
...
# instantiate `SmartCrusher`, which hard-imports `tokenash._core`.
ci-precheck-python:
...
build-e2e-wrap:
	docker build --platform linux/amd64 -f e2e/wrap/Dockerfile -t tokenash-wrap-e2e .

run-e2e-wrap: build-e2e-wrap
	docker run --rm tokenash-wrap-e2e
