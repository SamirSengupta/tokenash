...
//!    Python `tokenash.cli.toin_publish` CLI emits.
...
use tokenash_core::transforms::recommendations::{
    AuthMode, RecommendationStore, RecommendationsError,
};

/// Minimal tempdir helper. Project convention is to avoid a
/// dev-dependency on `tempfile` (see `crates/tokenash-parity` for the
...
fn tempdir() -> TempDir {
    ...
    let p = std::env::temp_dir().join(format!(
        "tokenash-recommendations-{nanos}-{:?}",
