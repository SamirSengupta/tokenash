...
//!   `User-Agent`, never inject `X-Tokenash-*`, never strip
//!   `accept-encoding`).
//!
...
pub enum AuthMode {
    ...
    /// `X-Tokenash-*`; never mutate `User-Agent`.
    Subscription,
}
...
/// `crates/tokenash-core/benches/auth_mode.rs` asserts <10us / call.
...
mod inline_tests {
    ...
    //! tokenash-core --lib` exercises the helper without pulling in
    //! the integration-test binary. The exhaustive test matrix lives
    //! in `crates/tokenash-core/tests/auth_mode.rs`.
