...
//! and maps its 200+ labels onto Tokenash's existing
...
///
/// The real fix is `tokenash/_ort.py`, which pins `ORT_DYLIB_PATH` to the
...
/// Override with `TOKENASH_MAGIKA_INIT_TIMEOUT_SECS`.
const MAGIKA_INIT_TIMEOUT_SECS_DEFAULT: u64 = 5;

fn magika_init_timeout() -> Duration {
    let secs = std::env::var("TOKENASH_MAGIKA_INIT_TIMEOUT_SECS")
        .ok()
...
/// Classify `content` and return the mapped Tokenash [`ContentType`].
///
...
/// Map a magika label string to Tokenash's [`ContentType`] enum.
///
