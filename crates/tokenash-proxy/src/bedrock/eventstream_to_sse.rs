...
//! CRC-checksummed framing format. Most clients of Tokenash (Claude
//! Code, the Anthropic SDK in non-Bedrock mode, the Tokenash Python
    ...
    match v {
        HeaderValue::String(s) => {
            if s.len() <= 64 {
                ...
                // was only stabilised in Rust 1.91; tokenash's MSRV is 1.80.)
                let mut end = 64;
