...
/// Policy for stripping internal `x-tokenash-*` headers from upstream-bound
/// requests (PR-A5, fixes P5-49).
///
/// When `enabled` (default), every header whose name starts with
/// `x-tokenash-` is dropped before the upstream call. Stops fingerprinting
/// of the proxy via subscription-revocation flags (`x-tokenash-bypass`,
/// `x-tokenash-mode`, etc.) and prevents leakage of internal user-id /
...
///
/// Source priority: CLI flag → `TOKENASH_PROXY_STRIP_INTERNAL_HEADERS`
/// env var → default (`enabled`).
#[derive(Debug, Clone, Copy, PartialEq, Eq, ValueEnum)]
#[clap(rename_all = "snake_case")]
pub enum StripInternalHeaders {
    /// Strip every `x-tokenash-*` header from upstream-bound requests.
    /// Default. Operationally safe.
    Enabled,
    /// Forward `x-tokenash-*` to upstream verbatim. Diagnostic-only;
...
///
/// Source priority: CLI flag → `TOKENASH_PROXY_CACHE_CONTROL_AUTO_FROZEN`
/// env var → default (`enabled`).
#[derive(Debug, Clone, Copy, PartialEq, Eq, ValueEnum)]
#[clap(rename_all = "snake_case")]
...
/// `TOKENASH_PROXY_AUTH_MODE_POLICY_ENFORCEMENT` env var →
/// default (`disabled`).
#[derive(Debug, Clone, Copy, PartialEq, Eq, ValueEnum)]
#[clap(rename_all = "snake_case")]
...
#[command(
    name = "tokenash-proxy",
    version,
    about = "Tokenash transparent reverse proxy"
)]
pub struct CliArgs {
    /// Address the proxy listens on (e.g. 0.0.0.0:8787).
    #[arg(long, env = "TOKENASH_PROXY_LISTEN", default_value = "0.0.0.0:8787")]
    ...
    #[arg(long, env = "TOKENASH_PROXY_UPSTREAM")]
    pub upstream: Url,
    ...
    /// Enable Tokenash compression on LLM-shaped requests
    ...
    /// `--compression` (or set `TOKENASH_PROXY_COMPRESSION=1`).
    #[arg(
        long = "compression",
        env = "TOKENASH_PROXY_COMPRESSION",
    ...
    ///
    /// Source priority: CLI flag → `TOKENASH_PROXY_COMPRESSION_MODE`
    /// env var → default (`off`).
    #[arg(
        long = "compression-mode",
        env = "TOKENASH_PROXY_COMPRESSION_MODE",
        value_enum,
    ...
    #[arg(
        ...
        env = "TOKENASH_PROXY_CACHE_CONTROL_AUTO_FROZEN",
        value_enum,
    ...
    #[arg(
        ...
        env = "TOKENASH_PROXY_STRIP_INTERNAL_HEADERS",
        value_enum,
    ...
    ///
    /// Source priority: CLI flag → `TOKENASH_PROXY_ENABLE_RESPONSES_STREAMING`
    /// env var → default (`true`).
    #[arg(
        long = "enable-responses-streaming",
        env = "TOKENASH_PROXY_ENABLE_RESPONSES_STREAMING",
    ...
    ///
    /// Source priority: CLI flag → `TOKENASH_PROXY_ENABLE_CONVERSATIONS_PASSTHROUGH`
    /// env var → default (`true`).
    #[arg(
        long = "enable-conversations-passthrough",
        env = "TOKENASH_PROXY_ENABLE_CONVERSATIONS_PASSTHROUGH",
        default_value_t = true,
        action = clap::ArgAction::Set,
    )]
    ...
    ///
    /// Source priority: CLI flag → `TOKENASH_PROXY_ENABLE_BEDROCK_NATIVE`
    /// env var → default (`true`).
    #[arg(
        long = "enable-bedrock-native",
        env = "TOKENASH_PROXY_ENABLE_BEDROCK_NATIVE",
        default_value_t = true,
        action = clap::ArgAction::Set,
    )]
    ...
    ///
    /// Source priority: CLI flag → `TOKENASH_PROXY_BEDROCK_REGION`
    /// env var → `AWS_REGION` env var → default (`us-east-1`).
    #[arg(
        long = "bedrock-region",
        env = "TOKENASH_PROXY_BEDROCK_REGION",
    ...
    ///
    /// Source priority: CLI flag → `TOKENASH_PROXY_BEDROCK_ENDPOINT`
    /// env var → derived-from-region.
    #[arg(long = "bedrock-endpoint", env = "TOKENASH_PROXY_BEDROCK_ENDPOINT")]
    ...
    ///
    /// Source priority: CLI flag → `TOKENASH_PROXY_AWS_PROFILE`
    /// env var → `AWS_PROFILE` env var → default chain.
    #[arg(long = "aws-profile", env = "TOKENASH_PROXY_AWS_PROFILE")]
    ...
    ///
    /// Source priority: CLI flag → `TOKENASH_PROXY_BEDROCK_VALIDATE_EVENTSTREAM_CRC`
    /// env var → default (`true`).
    #[arg(
        long = "bedrock-validate-eventstream-crc",
        env = "TOKENASH_PROXY_BEDROCK_VALIDATE_EVENTSTREAM_CRC",
        default_value_t = true,
        action = clap::ArgAction::Set,
    )]
    ...
    ///
    /// Source priority: CLI flag → `TOKENASH_PROXY_VERTEX_REGION`
    /// env var → default (`us-central1`).
    #[arg(
        long = "vertex-region",
        env = "TOKENASH_PROXY_VERTEX_REGION",
    ...
    ///
    /// Source priority: CLI flag → `TOKENASH_PROXY_VERTEX_ADC_SCOPE`
    /// env var → default (`cloud-platform`).
    #[arg(
        long = "vertex-adc-scope",
        env = "TOKENASH_PROXY_VERTEX_ADC_SCOPE",
...
pub struct Config {
    ...
    pub auth_mode_policy_enforcement: AuthModePolicyEnforcement,
    /// Whether to strip internal `x-tokenash-*` headers from
...
impl Config {
    ...
    pub fn for_test(upstream: Url) -> Self {
        Self {
            ...
            // Production default: strip internal `x-tokenash-*` headers
