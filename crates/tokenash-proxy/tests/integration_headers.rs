...
//! internal `x-tokenash-*` strip (PR-A5, fixes P5-49).

mod common;

use common::{start_proxy, start_proxy_with};
use tokenash_proxy::config::StripInternalHeaders;
...
#[tokio::test]
async fn x_tokenash_request_headers_stripped() {
    let upstream = MockServer::start().await;
    Mock::given(method("POST"))
        .and(path("/v1/messages"))
        .respond_with(move |req: &wiremock::Request| {
            // PR-A5: internal x-tokenash-* must NOT reach upstream.
            assert!(
                req.headers.get("x-tokenash-bypass").is_none(),
                "x-tokenash-bypass leaked upstream"
            );
            assert!(
                req.headers.get("x-tokenash-mode").is_none(),
                "x-tokenash-mode leaked upstream"
            );
            assert!(
                req.headers.get("x-tokenash-user-id").is_none(),
                "x-tokenash-user-id leaked upstream"
            );
    ...
    let resp = reqwest::Client::new()
        ...
        .header("x-tokenash-bypass", "true")
        .header("x-tokenash-mode", "passthrough")
        .header("x-tokenash-user-id", "alice")
        .body("{}")
        .send()
        .await
        .unwrap();
...
#[tokio::test]
async fn x_tokenash_case_insensitive_stripped() {
    let upstream = MockServer::start().await;
    Mock::given(method("POST"))
        .and(path("/v1/messages"))
        .respond_with(move |req: &wiremock::Request| {
    ...
    let resp = reqwest::Client::new()
        .post(format!("{}/v1/messages", proxy.url()))
        .header("X-Tokenash-Foo", "1")
        .header("x-Tokenash-Bar", "2")
        .header("X-TOKENASH-BAZ", "3")
        .body("{}")
        .send()
        .await
        .unwrap();
...
async fn legitimate_headers_passthrough_with_strip_enabled() {
    ...
    Mock::given(method("POST"))
        ...
        .respond_with(move |req: &wiremock::Request| {
            ...
            assert!(req.headers.get("x-tokenash-bypass").is_none());
            ResponseTemplate::new(200)
        })
        .mount(&upstream)
        .await;
...
async fn disabled_mode_passes_internal_headers_through() {
    ...
    Mock::given(method("POST"))
        ...
        .respond_with(move |req: &wiremock::Request| {
            ...
            assert_eq!(req.headers.get("x-tokenash-bypass").unwrap(), "true");
            assert_eq!(req.headers.get("x-tokenash-mode").unwrap(), "passthrough");
            ResponseTemplate::new(200).set_body_string("{}")
        })
        .mount(&upstream)
        .await;
