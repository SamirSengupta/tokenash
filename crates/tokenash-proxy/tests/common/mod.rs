...
use tokenash_proxy::vertex::TokenSource;
use tokenash_proxy::{build_app, AppState, Config};
...
pub fn install_static_token_source(mut state: AppState, bearer: &str) -> AppState {
    state.vertex_token_source = Arc::new(tokenash_proxy::vertex::StaticTokenSource::new(
        bearer.to_string(),
