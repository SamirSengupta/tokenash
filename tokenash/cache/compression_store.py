...
DEFAULT_CCR_TTL_SECONDS = 1800  # session-scale; override via TOKENASH_CCR_TTL_SECONDS
CCR_TTL_SECONDS_ENV = "TOKENASH_CCR_TTL_SECONDS"
...
CCR_MISS_MESSAGE = (
    ...
    "(default 30 minutes; configurable via TOKENASH_CCR_TTL_SECONDS)."
)
...
class CompressionStore:
    ...
    def _search_items_from_original(self, original_content: str) -> list[Any]:
        ...
        else so `tokenash_retrieve(hash, query=...)` can find plain-text
        originals.
        """
...
_request_ccr_store: ContextVar[CompressionStore | None] = ContextVar(
    "tokenash_request_ccr_store", default=None
)
...
def _create_default_ccr_backend() -> CompressionStoreBackend | None:
    """Create a CCR backend from env (e.g. TOKENASH_CCR_BACKEND=redis).

    Default (env unset or "sqlite"): SQLiteBackend at
    ~/.tokenash/ccr_store.db — restart-safe and shared across worker
    processes, which the session-scale 30-minute TTL assumes.
    "memory" opts back into the in-process dict. Other values load
    adapters via setuptools entry point 'tokenash.ccr_backend'.
    Returns None to use InMemoryBackend.
    """
    backend_type = (os.environ.get("TOKENASH_CCR_BACKEND") or "").strip().lower()
    ...
    try:
        ...
        all_eps = entry_points(group="tokenash.ccr_backend")
        ep = next((e for e in all_eps if e.name == backend_type), None)
        if ep is None:
            logger.warning(
                "TOKENASH_CCR_BACKEND=%s but no entry point tokenash.ccr_backend[%s]",
                backend_type,
                backend_type,
            )
            return None
        fn = ep.load()
        kwargs = {
            "url": os.environ.get("TOKENASH_REDIS_URL", ""),
            "tenant_prefix": os.environ.get("TOKENASH_CCR_TENANT_PREFIX", ""),
        }
...
def get_compression_store(
    ...
    explicitly or created from env (TOKENASH_CCR_BACKEND) when building the global.
    ...
    Args:
        ...
        default_ttl: Default TTL (only used on first call for global store).
            When omitted, TOKENASH_CCR_TTL_SECONDS overrides the 1800-second default.
