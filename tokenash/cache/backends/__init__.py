...
- ``get_compression_store()`` (the proxy path) defaults to SQLite
  (restart-safe, shared across workers); ``TOKENASH_CCR_BACKEND=memory``
...
Usage:
    from tokenash.cache.backends import SQLiteBackend, CompressionStoreBackend
    from tokenash.cache.compression_store import CompressionStore, get_compression_store

    # Env-driven default (SQLite at ~/.tokenash/ccr_store.db)
