...
- **Multi-worker sharing.** The database file (WAL mode) is shared
  across worker processes, so a `tokenash_retrieve` call served by a
...
Set ``TOKENASH_CCR_BACKEND=memory`` to opt back into the in-memory
backend, or ``TOKENASH_CCR_SQLITE_PATH`` to relocate the database file
(default ``~/.tokenash/ccr_store.db``).
"""
...
def default_db_path() -> Path:
    """Resolve the database path (env override or ~/.tokenash/)."""
    env = os.environ.get("TOKENASH_CCR_SQLITE_PATH", "").strip()
    if env:
        return Path(env).expanduser()
    return Path.home() / ".tokenash" / "ccr_store.db"
