...
cache_control breakpoints, growing-prefix strategy). If Tokenash compresses
...
# tokenash.cache.anthropic.ANTHROPIC_CACHE_TTL_SECONDS); the others are best-
...
class SessionTrackerStore:
    ...
    Keyed by session ID (from x-tokenash-session-id header or computed hash).
    ...
    def compute_session_id(
        ...
        Priority:
        1. x-tokenash-session-id header (explicit)
        2. Hash of (model + system prompt) — stable per conversation
        """
        # Check for explicit session header
        if hasattr(request, "headers"):
            session_header = request.headers.get("x-tokenash-session-id")
            if session_header:
