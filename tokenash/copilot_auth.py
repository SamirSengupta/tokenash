...
from tokenash.copilot_linux_secret import read_copilot_oauth_token as read_linux_secret_token
from tokenash.copilot_macos_keychain import read_copilot_oauth_token as read_macos_keychain_token
...
def tokenash_copilot_auth_path() -> Path:
    """Return the path where Tokenash stores its Copilot OAuth token."""

    override = os.environ.get("TOKENASH_COPILOT_AUTH_FILE", "").strip()
    if override:
...
def read_tokenash_copilot_oauth_token() -> str | None:
    """Return Tokenash's saved Copilot OAuth token, if one is available."""

    try:
        payload = json.loads(tokenash_copilot_auth_path().read_text(encoding="utf-8"))
    except FileNotFoundError:
        return None
    except Exception as exc:
        logger.debug("Unable to read Tokenash Copilot auth file: %s", exc)
        return None
...
def save_tokenash_copilot_oauth_token(
    token: str,
    *,
    ...
    path = tokenash_copilot_auth_path()
...
def iter_oauth_token_candidates() -> list[CopilotTokenCandidate]:
    ...
    tokenash_copilot_token = read_tokenash_copilot_oauth_token()
    if tokenash_copilot_token:
        candidates.append(
            CopilotTokenCandidate(
                token=tokenash_copilot_token,
                source=f"tokenash-copilot-auth:{tokenash_copilot_auth_path()}",
