"""Best-effort "is a newer Tokenash released?" check.
...
Opt out with ``TOKENASH_UPDATE_CHECK=off``. Also skipped in ``--stateless``
...
def is_update_check_enabled() -> bool:
    ...
    Disabled by ``TOKENASH_UPDATE_CHECK=off``, stateless mode
    (``TOKENASH_STATELESS=true``/``1``/``yes``/``on``, matching the proxy's own
    parsing), or any CI environment (``CI`` set).
    """
    if _env_off("TOKENASH_UPDATE_CHECK"):
        return False
    if _env_on("TOKENASH_STATELESS"):
        return False
...
def installed_version() -> str | None:
    ...
    ``tokenash._version.get_version()`` — the latter computes a synthetic
...
def _cache_path() -> Path:
    from tokenash.paths import workspace_dir
...
def write_cache(latest_version: str, *, now: float | None = None) -> None:
    ...
    try:
        from tokenash.paths import ensure_workspace_dir
...
def fetch_latest_version(*, allow_pre: bool = False, timeout: float = 4.0) -> str | None:
    ...
    try:
        req = urllib.request.Request(
            _PYPI_JSON_URL,
            headers={"Accept": "application/json", "User-Agent": "tokenash-update-check"},
        )
...
def run_check(*, allow_pre: bool = False, now: float | None = None) -> str | None:
    ...
    Synchronous — used directly by ``tokenash update`` and indirectly by
...
def maybe_check_async() -> threading.Thread | None:
    ...
    try:
        ...
        thread = threading.Thread(target=_worker, name="tokenash-update-check", daemon=True)
        thread.start()
        return thread
    except Exception:
...
def format_update_notice() -> str | None:
    ...
    try:
        ...
        return f"Update available: Tokenash {latest} (you have {current}) - run: tokenash update"
    except Exception:
