"""Relevance scoring module for Tokenash SDK.
...
1. HybridScorer (DEFAULT - recommended)
   ...
   - Install for full support: pip install tokenash[relevance]
...
3. EmbeddingScorer (requires sentence-transformers)
   ...
   - Install: pip install tokenash[relevance]
...
Example usage:
    from tokenash.relevance import HybridScorer, create_scorer
    ...
    if tier == "bm25":
        ...
        if not EmbeddingScorer.is_available():
            raise RuntimeError(
                ...
                "Install with: pip install tokenash[relevance]"
            )
