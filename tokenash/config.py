"""Configuration models for Tokenash SDK."""
...
from tokenash.models.config import ML_MODEL_DEFAULTS


class TokenashMode(str, Enum):
    """Operating modes for Tokenash."""
...
class RelevanceScorerConfig:
    ...
    For full hybrid support, install: pip install tokenash[relevance]
...
class ReadMaturationConfig:
    ...
    Motivation (measured by `tokenash audit-reads`): the median Read stays
...
class CacheOptimizerConfig:
    ...
    auto_detect_provider: bool = True  # Auto-detect from TokenashClient provider
...
class CCRConfig:
    ...
    4. RETRIEVE: If LLM needs more, it calls tokenash_retrieve(hash, query)
    ...
    # with Rust DEFAULT_TTL (crates/tokenash-core/src/ccr/mod.rs) and
    ...
    inject_tool: bool = True  # Inject tokenash_retrieve tool into tools array
...
class PrefixFreezeConfig:
    ...
    Tokenash from invalidating the provider's prefix cache (which would replace
...
@dataclass
class TokenashConfig:
    """Main configuration for TokenashClient."""
    ...
    # Env var TOKENASH_INTERCEPT_ENABLED=1 also enables (for CLI `--intercept-tool-results`).
...
class DiffArtifact:
    ...
    Opt-in via TokenashConfig.generate_diff_artifact = True.
