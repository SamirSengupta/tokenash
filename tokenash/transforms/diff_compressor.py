...
diff compression now goes through `tokenash._core.DiffCompressor` (built
from `crates/tokenash-py`). The byte-equality of the two implementations
was verified against 27 recorded fixtures before the Python source was
removed; the Rust crate has its own test coverage in `crates/tokenash-core/`.
...
class DiffCompressor:
    """Rust-backed `DiffCompressor` (via PyO3 / `tokenash._core`).
