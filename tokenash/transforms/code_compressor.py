...
def _get_parser(language: str) -> Any:
    ...
    from any thread other than its creator.  Because Tokenash runs
    ...
    if not _tree_sitter_importable():
        raise ImportError(
            "tree-sitter is not installed. Install with: pip install tokenash[code]\n"
...
class CodeAwareCompressor(Transform):
    ...
    def compress(
        ...
        if not _check_tree_sitter_available():
            logger.warning("tree-sitter not available. Install with: pip install tokenash[code]")
            if self.config.fallback_to_kompress:
                return self._fallback_compress(code, original_tokens)
            return CodeCompressionResult(
                compressed=code,
    ...
    def _verify_syntax(self, code: str, language: CodeLanguage) -> bool:
        ...
        try:
            if language == CodeLanguage.PYTHON:
                ...
                ast.parse(code)
                compile(code, "<tokenash-compressed>", "exec")
    ...
    def apply(
        ...
        **kwargs: Any,
        ...
        if not _check_tree_sitter_available():
            warnings.append(
                "tree-sitter not installed. Install with: pip install tokenash[code]"
            )
