...
Requires the [ml] extra: pip install tokenash[ml]

Usage:
    >>> from tokenash.transforms.kompress_compressor import KompressCompressor
...
KOMPRESS_BACKEND_ENV = "TOKENASH_KOMPRESS_BACKEND"
KOMPRESS_ONNX_FILENAME_ENV = "TOKENASH_KOMPRESS_ONNX_FILENAME"
...
#
# An operator can pin an exact file via TOKENASH_KOMPRESS_ONNX_FILENAME.
_DEFAULT_ONNX_FILENAMES = (
    "onnx/kompress-int8-wo.onnx",
    "onnx/kompress-fp32.onnx",
    "onnx/kompress-int8.onnx",
)
KOMPRESS_ONNX_INTRA_THREADS_ENV = "TOKENASH_KOMPRESS_ONNX_INTRA_THREADS"
KOMPRESS_ONNX_INTER_THREADS_ENV = "TOKENASH_KOMPRESS_ONNX_INTER_THREADS"
KOMPRESS_COREML_CACHE_DIR_ENV = "TOKENASH_KOMPRESS_COREML_CACHE_DIR"
KOMPRESS_MAX_CONCURRENT_ENV = "TOKENASH_KOMPRESS_MAX_CONCURRENT"
KOMPRESS_BATCH_SIZE_ENV = "TOKENASH_KOMPRESS_BATCH_SIZE"
...
def _get_model_class() -> type:
    """Return the TokenashCompressorModel class, importing torch on demand."""
    import torch
    import torch.nn as nn
    from transformers import AutoModel

    class TokenashCompressorModel(nn.Module):
    ...
    return TokenashCompressorModel
...
def _load_kompress_pytorch(
    ...
    with _kompress_lock:
        ...
        TokenashCompressorModel = _get_model_class()
        model = TokenashCompressorModel()
...
def _load_kompress(
    ...
    # allowing opt-in MPS/CoreML experiments via TOKENASH_KOMPRESS_BACKEND.
    if _is_onnx_available():
        try:
            return _load_kompress_onnx(model_id, use_coreml=False, allow_download=allow_download)
        except KompressModelNotCached:
    ...
    raise ImportError(
        "Kompress requires onnxruntime or torch. Install with: pip install tokenash[proxy]"
    )
...
# compression timeout (TOKENASH_COMPRESSION_TIMEOUT_SECONDS, default 30s). The
...
class KompressCompressor(Transform):
    ...
    def compress(
        ...
        *,
        ...
        # disables. Env TOKENASH_COMPRESSION_DEADLINE_MS overrides (default 20s).
        ...
        if deadline_s is None:
            try:
                deadline_s = max(
                    0.0,
                    float(os.environ.get("TOKENASH_COMPRESSION_DEADLINE_MS", "20000")) / 1000.0,
                )
            except ValueError:
                deadline_s = 20.0
