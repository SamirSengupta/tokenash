...
All array compression now goes through `tokenash._core.SmartCrusher`
(built from `crates/tokenash-py`). Byte-equality of the two
...
removed; the Rust crate has its own coverage in `crates/tokenash-core/`
...
The `tokenash._core` extension is a hard import: there is no Python
...
# `crates/tokenash-core/.../compaction/mod.rs`.
...
class SmartCrusherConfig:
    ...
    # crates/tokenash-core/src/transforms/smart_crusher/compaction/compactor.rs).
...
class SmartCrusher(Transform):
    """Rust-backed `SmartCrusher` (via PyO3 / `tokenash._core`).
    ...
    def __init__(
        ...
        # `tokenash.config.SmartCrusherConfig`, which also carries it.
        self._lossless_only = (
            bool(getattr(cfg, "lossless_only", False))
            if lossless_only is None
            else bool(lossless_only)
        )
        # `observer`: see `tokenash.transforms.observability`. The
        ...
        # back (`tokenash.proxy.server` does). Both `enabled=False` and
        ...
        self._rust_cfg_kwargs = {
            ...
            # `tokenash.config.SmartCrusherConfig` (MCP server, SDK) or a
        ...
        # TOKENASH_COMPACTION_FORMAT env var when the kwarg is None.
        # Ignored when with_compaction=False.
        resolved_format = compaction_format or os.environ.get(
            "TOKENASH_COMPACTION_FORMAT", "csv-schema"
        )
    ...
    def _mirror_single_hash_to_python_store(
        ...
        # the Rust store's `DEFAULT_TTL` (see crates/tokenash-core/src/
    ...
    def _notify_observer(self, original_tokens: int, compressed_tokens: int) -> None:
        ...
        `CompressionObserver` (see `tokenash.transforms.observability`).
