...
Hand-mirrored port of `tokenash_core::compression_policy::CompressionPolicy`
...
See `crates/tokenash-core/src/compression_policy.rs` for the canonical
...
# ``crates/tokenash-core/src/compression_policy.rs``. Centralised here so
...
def policy_default_payg() -> CompressionPolicy:
    ...
    ``TOKENASH_PROXY_AUTH_MODE_POLICY_ENFORCEMENT`` flag is disabled
...
_ENFORCEMENT_ENV = "TOKENASH_PROXY_AUTH_MODE_POLICY_ENFORCEMENT"
...
def is_enforcement_enabled() -> bool:
    ...
    # (`tokenash/telemetry/beacon.py::_OFF_VALUES`) so operators don't
