"""Strands SDK model wrapper for Tokenash optimization.

This module provides TokenashStrandsModel, which wraps any Strands model
...
from tokenash import TokenashConfig  # noqa: E402
from tokenash.providers import OpenAIProvider  # noqa: E402
from tokenash.transforms import TransformPipeline  # noqa: E402

from .providers import get_tokenash_provider, get_model_name_from_strands  # noqa: E402
...
class TokenashStrandsModel(Model):  # type: ignore[misc]
    """Strands model wrapper that applies Tokenash optimizations.
    ...
    Example:
        ...
        optimized = TokenashStrandsModel(wrapped_model=model)
        ...
        optimized = TokenashStrandsModel(wrapped_model=model, config=config)
    ...
    def __init__(
        ...
        """Initialize TokenashStrandsModel.

        Args:
            wrapped_model: The Strands model to wrap (e.g., BedrockModel, OpenAIModel)
            config: Optional TokenashConfig for optimization settings
            auto_detect_provider: Whether to auto-detect the Tokenash provider
        ...
        self.tokenash_config = config or TokenashConfig()
        ...
        self._tokenash_provider: Any = None
    ...
    def _convert_messages_to_openai(self, messages: list[Any]) -> list[dict[str, Any]]:
        """Convert Strands messages to OpenAI format for Tokenash.
    ...
    async def stream(
        ...
        **kwargs: Any,
        ...
        logger.info(
            f"Tokenash optimized (stream): {metrics.tokens_before} -> "
    ...
    async def structured_output(
        ...
        **kwargs: Any,
        ...
        """Generate structured output with Tokenash optimization.
        ...
        logger.info(
            f"Tokenash optimized (structured_output): {metrics.tokens_before} -> "
            f"{metrics.tokens_after} tokens ({metrics.savings_percent:.1f}% saved)"
        )
...
def optimize_messages(
    ...
    Example:
        from tokenash.integrations.strands import optimize_messages
