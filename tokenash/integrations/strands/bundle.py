"""TokenashBundle — single-helper MCP wiring for a Strands Agent.

The cleanest production setup for Strands is the same kit
``tokenash wrap claude`` installs for Claude Code, restated as
Strands-native primitives:

* **Tokenash MCP** (``tokenash mcp serve``) — exposes
  ``tokenash_retrieve`` / ``tokenash_compress`` / ``tokenash_stats``
  via stdio. The proxy emits ``Retrieve original: hash=...`` markers
  in compressed content; the LLM calls ``tokenash_retrieve`` when it
...
* **TokenashHookProvider** — the RTK-equivalent for Strands.
...
.. code-block:: python
    ...
    from tokenash.integrations.strands import TokenashBundle
    ...
    bundle = TokenashBundle(proxy_url="http://127.0.0.1:8787")
    agent = Agent(
        model=model,
...
``tokenash proxy``. This keeps the bundle stateless and lets the
...
from tokenash.mcp_registry.install import (
    DEFAULT_PROXY_URL,
    build_tokenash_spec,
    build_serena_spec,
)
...
#: code-aware agent loop (the same context ``tokenash wrap claude`` uses).
DEFAULT_SERENA_CONTEXT = "ide-assistant"


def _make_tokenash_client(proxy_url: str) -> MCPClient:
...
@dataclass
class TokenashBundle:
    """Single helper that hands a Strands Agent every Tokenash integration.

    Attributes:
        proxy_url: HTTP URL the Tokenash MCP server should contact for
            retrieval. Default :data:`DEFAULT_PROXY_URL`
            (``http://127.0.0.1:8787``).
        serena_context: Serena context label. Default ``"ide-assistant"``.
        enable_tokenash_mcp: Include the Tokenash MCP server. Default True.
        enable_serena_mcp: Include the Serena MCP server. Default True.
            Disabling skips the ``uvx`` first-launch download entirely.
        enable_hooks: Include :class:`TokenashHookProvider` for in-place
            tool-output compression (the RTK-equivalent for Strands).
            Default True.
        config: Optional :class:`TokenashConfig` passed to
            :class:`TokenashHookProvider`. Default uses framework
            defaults.
    ...
    # (TokenashHookProvider) is an optimisation for memory/network when
    ...
    _tokenash_mcp: MCPClient | None = field(default=None, init=False, repr=False, compare=False)
    _serena_mcp: MCPClient | None = field(default=None, init=False, repr=False, compare=False)
    _hook: TokenashHookProvider | None = field(default=None, init=False, repr=False, compare=False)

    def __post_init__(self) -> None:
        if self.enable_tokenash_mcp:
            self._tokenash_mcp = _make_tokenash_client(self.proxy_url)
            logger.info(
                "TokenashBundle: Tokenash MCP client constructed (proxy_url=%s)",
                self.proxy_url,
            )
        if self.enable_serena_mcp:
            self._serena_mcp = _make_serena_client(self.serena_context)
            logger.info(
                "TokenashBundle: Serena MCP client constructed (context=%s)",
                self.serena_context,
            )
        if self.enable_hooks:
            self._hook = TokenashHookProvider(config=self.config)
            logger.info("TokenashBundle: TokenashHookProvider attached")
    ...
    def tools(self) -> list[Any]:
        ...
        """
        out: list[Any] = []
        if self._tokenash_mcp is not None:
            out.append(self._tokenash_mcp)
    ...
    @property
    def tokenash_mcp(self) -> MCPClient | None:
        """Direct handle to the Tokenash MCPClient (for advanced callers)."""
        return self._tokenash_mcp
