...
Automatically detects the correct Tokenash provider based on the Strands model type.
"""
...
# Mapping from Strands model class names to Tokenash providers
...
def get_tokenash_provider(model: Any) -> Provider:
    """Get the appropriate Tokenash provider for a Strands model.
    ...
    Returns:
        Appropriate Tokenash Provider instance.

    Example:
        from strands.models import BedrockModel
        from tokenash.integrations.strands.providers import get_tokenash_provider

        model = BedrockModel(model_id="anthropic.claude-3-5-sonnet-20241022-v2:0")
        provider = get_tokenash_provider(model)  # Returns AnthropicProvider
    """
