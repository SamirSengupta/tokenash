"""Strands SDK hook provider for Tokenash tool output compression.

This module provides TokenashHookProvider, which implements Strands' HookProvider
interface to intercept tool outputs and compress them using Tokenash's SmartCrusher.
...
from tokenash.transforms.smart_crusher import SmartCrusher, SmartCrusherConfig
...
@dataclass
class TokenashHookProvider(HookProvider):  # type: ignore[misc]
    """Strands HookProvider that compresses tool outputs using Tokenash.

    This hook provider intercepts tool call results via AfterToolCallEvent
    and applies Tokenash's SmartCrusher to compress large outputs, reducing
    ...
    Example:
        ...
        hook = TokenashHookProvider(min_tokens_to_compress=50)
    ...
    config: TokenashConfig | None = field(default=None)
    ...
    def __post_init__(self) -> None:
        ...
        if self.config is None:
            self.config = TokenashConfig()

        self._initialized = True
        logger.debug(
            "TokenashHookProvider initialized: compress=%s, min_tokens=%d, preserve_errors=%s",
    ...
    def crusher(self) -> SmartCrusher:
        ...
        if self._crusher is None:
            with self._lock:
                ...
                if self._crusher is None:
                    # Use config from TokenashConfig if available
    ...
    def register_hooks(self, registry: HookRegistry, **kwargs: Any) -> None:
        ...
        logger.info(
            "TokenashHookProvider registered: compressing tool outputs >= %d tokens",
            self.min_tokens_to_compress,
        )
    ...
    def reset(self) -> None:
        ...
        logger.debug("TokenashHookProvider metrics reset")
