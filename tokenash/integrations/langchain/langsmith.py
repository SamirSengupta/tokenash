"""LangSmith integration for Tokenash compression metrics.

This module provides TokenashLangSmithCallbackHandler, a LangChain callback
handler that adds Tokenash compression metrics to LangSmith traces.

When used with TokenashChatModel, it automatically captures:
...
Example:
    ...
    # Create handler
    handler = TokenashLangSmithCallbackHandler()

    # Use with TokenashChatModel
    llm = TokenashChatModel(
        ChatOpenAI(model="gpt-4o"),
        callbacks=[handler],
    )

    # Traces will include tokenash.* metadata
...
class TokenashLangSmithCallbackHandler(BaseCallbackHandler):
    """Callback handler that adds Tokenash metrics to LangSmith traces.
    ...
    2. Used as a callback with TokenashChatModel
    ...
    Example:
        ...
        # - tokenash.savings_percent
        # - tokenash.transforms_applied
    ...
    def __init__(
        ...
        """Initialize TokenashLangSmithCallbackHandler.
        ...
        Args:
            ...
            auto_update_runs: If True, automatically updates LangSmith
                runs with Tokenash metadata. Default True.
        """
    ...
    def set_tokenash_metrics(
        ...
        """Set Tokenash metrics for a run.

        Call this from TokenashChatModel after optimization to attach
        ...
        logger.debug(
            f"Tokenash metrics set for run {run_id_str}: "
    ...
    def on_llm_end(
        ...
        **kwargs: Any,
        ...
        Attaches pending Tokenash metrics to the LangSmith run.
        """
        run_id_str = str(run_id)
    ...
    def _attach_metrics_to_run(self, run_id: str, metrics: PendingMetrics) -> None:
        """Attach Tokenash metrics to a LangSmith run.
        ...
        """
        metadata = {
            "tokenash.tokens_before": metrics.tokens_before,
            "tokenash.tokens_after": metrics.tokens_after,
            "tokenash.tokens_saved": metrics.tokens_saved,
            "tokenash.savings_percent": round(metrics.savings_percent, 2),
            "tokenash.transforms_applied": metrics.transforms_applied,
            "tokenash.optimization_timestamp": metrics.timestamp.isoformat(),
        }
        ...
        if self._client and self._auto_update:
            try:
                ...
                logger.debug(f"Updated LangSmith run {run_id} with Tokenash metrics")
            except Exception as e:
                logger.debug(f"Could not update LangSmith run: {e}")

    def get_run_metrics(self, run_id: str | UUID) -> dict[str, Any]:
        """Get Tokenash metrics for a specific run.

        Args:
            run_id: The run ID.

        Returns:
            Dictionary of tokenash.* metrics for the run.
        """
    ...
    def get_summary(self) -> dict[str, Any]:
        ...
        total_saved = sum(m.get("tokenash.tokens_saved", 0) for m in self._run_metrics.values())
        savings_percents = [
            m.get("tokenash.savings_percent", 0) for m in self._run_metrics.values()
        ]
