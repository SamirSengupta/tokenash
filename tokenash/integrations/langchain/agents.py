...
This module provides TokenashToolWrapper and wrap_tools_with_tokenash
...
Example:
    ...
    # Wrap with Tokenash compression
    wrapped_tools = wrap_tools_with_tokenash(tools)
...
class TokenashToolWrapper:
    ...
    Example:
        from langchain.tools import Tool
        from tokenash.integrations import TokenashToolWrapper
        ...
        wrapped = TokenashToolWrapper(search_tool)
    ...
    def __init__(
        ...
        """Initialize TokenashToolWrapper.
    ...
    def _record_metrics(self, original: str, compressed: str, was_compressed: bool) -> None:
        ...
        if was_compressed and chars_saved > 0:
            logger.info(
                f"TokenashToolWrapper[{self.name}]: {chars_before} -> {chars_after} chars "
...
def wrap_tools_with_tokenash(
    tools: list[BaseTool],
    min_chars_to_compress: int = 1000,
    metrics_collector: ToolMetricsCollector | None = None,
) -> list[StructuredTool]:
    """Wrap multiple LangChain tools with Tokenash compression.
