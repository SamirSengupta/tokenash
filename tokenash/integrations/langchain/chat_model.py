"""LangChain integration for Tokenash SDK.
...
Components:
1. TokenashChatModel - Wraps any BaseChatModel to apply Tokenash transforms
2. TokenashCallbackHandler - Tracks metrics and token usage (observability only)
3. TokenashRunnable - LCEL-compatible Runnable for chain composition
...
Example:
    ...
    optimized_llm = TokenashChatModel(llm)

    # Use normally - Tokenash automatically optimizes context
...
from tokenash.transforms import TransformPipeline

from .providers import get_tokenash_provider, get_model_name_from_langchain
...
def _check_langchain_available() -> None:
    ...
    if not LANGCHAIN_AVAILABLE:
        raise ImportError(
            ...
            "Install with: pip install tokenash[langchain] "
...
class TokenashChatModel(BaseChatModel):
    """LangChain chat model wrapper that applies Tokenash optimizations.

    Wraps any LangChain BaseChatModel and automatically optimizes the context
    before each API call. This is the recommended way to use Tokenash with
    LangChain because:
    ...
    Example:
        ...
        # Basic usage
        llm = ChatOpenAI(model="gpt-4o")
        optimized = TokenashChatModel(llm)
        response = optimized.invoke([HumanMessage("Hello!")])

        # With custom config
        from tokenash import TokenashConfig, TokenashMode
        config = TokenashConfig(default_mode=TokenashMode.OPTIMIZE)
        optimized = TokenashChatModel(llm, config=config)
    ...
    Attributes:
        ...
        tokenash_client: TokenashClient instance for optimization
    ...
    tokenash_config: Any = Field(default=None, description="Tokenash configuration")
    mode: TokenashMode = Field(default=TokenashMode.OPTIMIZE, description="Tokenash mode")
    ...
    def __init__(
        ...
        mode: TokenashMode = TokenashMode.OPTIMIZE,
        auto_detect_provider: bool = True,
        **kwargs: Any,
    ) -> None:
        """Initialize TokenashChatModel.

        Args:
            wrapped_model: Any LangChain BaseChatModel to wrap
            config: TokenashConfig for optimization settings
            mode: TokenashMode (AUDIT, OPTIMIZE, or SIMULATE)
            auto_detect_provider: Auto-detect provider from wrapped model.
                ...
                Tokenash provider for accurate token counting.
        ...
        super().__init__(  # type: ignore[call-arg]
            ...
            tokenash_config=config or TokenashConfig(),
            mode=mode,
    ...
    def _llm_type(self) -> str:
        ...
        return f"tokenash-{self.wrapped_model._llm_type}"
    ...
    def pipeline(self) -> TransformPipeline:
        ...
        if self._pipeline is None:
            if self.auto_detect_provider:
                self._provider = get_tokenash_provider(self.wrapped_model)
                logger.debug(f"Auto-detected provider: {self._provider.__class__.__name__}")
            else:
                self._provider = OpenAIProvider()
            self._pipeline = TransformPipeline(
                config=self.tokenash_config,
    ...
    def _convert_messages_to_openai(self, messages: list[BaseMessage]) -> list[dict[str, Any]]:
        """Convert LangChain messages to OpenAI format for Tokenash."""
        result = []
        for msg in messages:
    ...
    def _optimize_messages(
        ...
        """Apply Tokenash optimization to messages."""
        ...
        # Apply Tokenash transforms via pipeline
    ...
    def _generate(
        ...
        **kwargs: Any,
        ...
        """Generate response with Tokenash optimization.
        ...
        logger.info(
            f"Tokenash optimized: {metrics.tokens_before} -> {metrics.tokens_after} tokens "
    ...
    def _stream(
        ...
        **kwargs: Any,
        ...
        """Stream response with Tokenash optimization."""
        # Optimize messages
        optimized_messages, metrics = self._optimize_messages(messages)

        logger.info(
            f"Tokenash optimized (streaming): {metrics.tokens_before} -> "
    ...
    async def _agenerate(
        ...
        **kwargs: Any,
        ...
        """Async generate response with Tokenash optimization.
        ...
        logger.info(
            f"Tokenash optimized (async): {metrics.tokens_before} -> {metrics.tokens_after} tokens "
            f"({metrics.savings_percent:.1f}% saved)"
        )
        ...
        # same TokenashChatModel instance. (GitHub #1285, review feedback)
    ...
    async def _astream(
        ...
        **kwargs: Any,
        ...
        """Async stream response with Tokenash optimization.
        ...
        logger.info(
            f"Tokenash optimized (async streaming): {metrics.tokens_before} -> "
            f"{metrics.tokens_after} tokens"
        )
    ...
    def bind_tools(self, tools: Sequence[Any], **kwargs: Any) -> TokenashChatModel:
        """Bind tools to the wrapped model."""
        new_wrapped = self.wrapped_model.bind_tools(tools, **kwargs)
        return TokenashChatModel(
...
class TokenashCallbackHandler(BaseCallbackHandler):
    """LangChain callback handler for Tokenash metrics and observability.

    NOTE: Callbacks CANNOT modify messages in LangChain (by design).
    Use TokenashChatModel for actual optimization. This handler is for:
    ...
    Example:
        from langchain_openai import ChatOpenAI
        from tokenash.integrations import TokenashCallbackHandler

        handler = TokenashCallbackHandler(
            log_level="INFO",
...
class TokenashRunnable:
    """LCEL-compatible Runnable for Tokenash optimization.

    Use this to add Tokenash optimization to any LangChain chain using LCEL.

    Example:
        from langchain_openai import ChatOpenAI
        from langchain_core.prompts import ChatPromptTemplate
        from tokenash.integrations import TokenashRunnable
        ...
        # Add Tokenash optimization to chain
        chain = prompt | TokenashRunnable() | llm
    ...
    def __init__(
        ...
        self.config = config or TokenashConfig()
        self.mode = mode
...
def optimize_messages(
    ...
    Example:
        ...
        from tokenash.integrations import optimize_messages
    ...
    config = config or TokenashConfig()
