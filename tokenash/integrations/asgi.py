"""ASGI Middleware — add Tokenash compression to any Python proxy.
...
Cloud mode (managed CCR, TOIN, analytics via Tokenash Cloud):
...
_DEFAULT_CLOUD_URL = "https://api.SamirSengupta.ai"
...
class CompressionMiddleware:
    ...
    Two modes:
    - Local (default): Compresses in-process using tokenash.compress().
    - Cloud (api_key set): Calls Tokenash Cloud API for managed compression
      with org-scoped CCR, TOIN learning, and analytics dashboards.

    Response headers include compression metrics:
    - x-tokenash-tokens-before: original token count
    - x-tokenash-tokens-after: compressed token count
    - x-tokenash-tokens-saved: tokens removed
    - x-tokenash-compressed: "true" if compression occurred
    """
    ...
    def __init__(
        ...
        # Cloud mode: if api_key is set, compress via Tokenash Cloud API
        self._api_key = api_key or os.environ.get("TOKENASH_API_KEY", "").strip() or None
        self._api_url = (
            api_url or os.environ.get("TOKENASH_API_URL", "").strip() or _DEFAULT_CLOUD_URL
        ).rstrip("/")
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        try:
            ...
            if messages:
                ...
                if result and result.get("tokens_saved", 0) > 0 and "messages" in result:
                    ...
                    logger.info(
                        "Tokenash%s: %d→%d tokens (saved %d, %.0f%%)",
            ...
            logger.debug("Tokenash middleware: skipping non-JSON request: %s", e)
        ...
        async def metrics_send(message: MutableMapping[str, Any]) -> None:
            if message["type"] == "http.response.start" and tokens_saved > 0:
                ...
                headers.append((b"x-tokenash-compressed", b"true"))
                headers.append((b"x-tokenash-tokens-before", str(tokens_before).encode()))
                headers.append((b"x-tokenash-tokens-after", str(tokens_after).encode()))
                headers.append((b"x-tokenash-tokens-saved", str(tokens_saved).encode()))
    ...
    def _local_compress(self, messages: list[dict], model: str) -> dict[str, Any] | None:
        """Compress locally using tokenash.compress()."""
    ...
    async def _cloud_compress(self, messages: list[dict], model: str) -> dict[str, Any] | None:
        """Compress via Tokenash Cloud API (managed CCR, TOIN, analytics)."""
        if self._client is None:
            try:
                import httpx
            except ImportError as e:
                raise ImportError(
                    "httpx is required for Tokenash Cloud mode: pip install httpx"
                ) from e
        ...
        if resp.status_code != 200:
            logger.warning("Tokenash Cloud API error: %d %s", resp.status_code, resp.text[:200])
            return None
