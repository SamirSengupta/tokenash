"""LiteLLM callback — add Tokenash compression to LiteLLM with one line.
    ...
    litellm.callbacks = [TokenashCallback(api_key="hdr_xxx")]
...
class TokenashCallback:
    ...
    def __init__(
        ...
        # Falls back to TOKENASH_API_KEY env var
        import os
    ...
    async def async_pre_call_hook(
        ...
        try:
            ...
            if result and result.get("tokens_saved", 0) > 0 and "messages" in result:
                ...
                logger.info(
                    "Tokenash%s: %d→%d tokens (saved %d, %.0f%%) [total saved: %d]",
            ...
            logger.warning("Tokenash compression failed, using original messages: %s", e)
