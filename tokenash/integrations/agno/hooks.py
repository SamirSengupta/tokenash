"""Agno hooks for Tokenash integration.

This module provides pre_hooks and post_hooks that can be used with
Agno agents to apply Tokenash optimization at the agent level.
"""
...
@dataclass
class HookMetrics:
    """Metrics collected by Tokenash pre-hooks.

    Note: These metrics track request counts and timing, not token savings.
    For actual token optimization metrics, use TokenashAgnoModel which
    ...
    # Use TokenashAgnoModel for actual token optimization
    tokens_before: int = 0
    tokens_after: int = 0
    tokens_saved: int = 0
...
class TokenashPreHook:
    ...
    optimization, use TokenashAgnoModel to wrap your model.
    ...
    using TokenashAgnoModel.

    Example:
        from agno.agent import Agent
        from agno.models.openai import OpenAIChat
        from tokenash.integrations.agno import TokenashPreHook, TokenashAgnoModel
    ...
    def __init__(
        ...
        """Initialize TokenashPreHook.

        Args:
            config: TokenashConfig for optimization settings (stored for future use)
            mode: TokenashMode (stored for future use)
    ...
    def __call__(self, run_input: Any, **kwargs: Any) -> Any:
        ...
        logger.debug(f"TokenashPreHook tracking request {request_id}")
        ...
        # Return input unchanged (use TokenashAgnoModel for actual optimization)
        return run_input
...
def create_tokenash_hooks(
    config: TokenashConfig | None = None,
    mode: TokenashMode = TokenashMode.OPTIMIZE,
    model: str = "gpt-4o",
    log_level: str = "INFO",
    token_alert_threshold: int | None = None,
) -> tuple[TokenashPreHook, TokenashPostHook]:
    """Create a pair of Tokenash hooks for Agno agents.
    ...
    """
    pre_hook = TokenashPreHook(config=config, mode=mode, model=model)
    post_hook = TokenashPostHook(
        log_level=log_level,
