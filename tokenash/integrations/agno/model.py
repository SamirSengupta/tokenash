"""Agno model wrapper for Tokenash optimization.

This module provides TokenashAgnoModel, which wraps any Agno model
to apply Tokenash context optimization before API calls.
"""
...
from tokenash.parser import _coerce_tool_call_to_dict
...
@dataclass
class TokenashAgnoModel(Model):  # type: ignore[misc]
    """Agno model wrapper that applies Tokenash optimizations.
    ...
    Example:
        ...
        optimized = TokenashAgnoModel(wrapped_model=model)
        ...
        optimized = TokenashAgnoModel(wrapped_model=model, tokenash_config=config)

        # Agno reasoning with TokenashAgnoModel
        model = OpenAIChat(id="gpt-4o")
        wrapped = TokenashAgnoModel(wrapped_model=model)
        agent = Agent(
            model=wrapped,
            reasoning=True,
    ...
    id: str = field(default="tokenash-wrapper")
    name: str | None = field(default=None)
    provider: str | None = field(default=None)

    # TokenashAgnoModel specific fields
    wrapped_model: Any = field(default=None)
    tokenash_config: TokenashConfig | None = field(default=None)
    tokenash_mode: TokenashMode | None = field(default=None)
    ...
    _tokenash_provider: Any = field(default=None, repr=False, compare=False)
    _lock: threading.Lock = field(default_factory=threading.Lock, repr=False, compare=False)
    _initialized: bool = field(default=False, repr=False, compare=False)

    def __post_init__(self) -> None:
        """Initialize TokenashAgnoModel after dataclass construction."""
        ...
        if hasattr(self.wrapped_model, "id"):
            self.id = f"tokenash:{self.wrapped_model.id}"
        ...
        # Initialize config
        if self.tokenash_config is None:
            self.tokenash_config = TokenashConfig()

        # Handle deprecated mode parameter
        if self.tokenash_mode is not None:
            warnings.warn(
                "The 'tokenash_mode' parameter is deprecated. Use TokenashConfig(default_mode=...) instead.",
                DeprecationWarning,
                stacklevel=2,
            )
    ...
    def underlying_model(self) -> Any:
        ...
        Example:
            wrapped = TokenashAgnoModel(wrapped_model=Claude(...))
            ...
            with self._lock:
                ...
                if self._pipeline is None:
                    if self.auto_detect_provider:
                        self._tokenash_provider = get_tokenash_provider(self.wrapped_model)
                        logger.debug(
                            f"Auto-detected provider: {self._tokenash_provider.__class__.__name__}"
                        )
                    else:
                        self._tokenash_provider = OpenAIProvider()
                    self._pipeline = TransformPipeline(
                        config=self.tokenash_config,
                        provider=self._tokenash_provider,
                    )
    ...
    def _convert_messages_to_openai(self, messages: list[Any]) -> list[dict[str, Any]]:
        """Convert Agno messages to OpenAI format for Tokenash.
        ...
        for msg in messages:
            ...
            if hasattr(msg, "role") and hasattr(msg, "content"):
                ...
                # Tokenash pipeline + Agno's own re-serialization both call
    ...
    def _optimize_messages(self, messages: list[Any]) -> tuple[list[Any], OptimizationMetrics]:
        ...
        if self._has_thinking_blocks(openai_messages):
            logger.info("Skipping Tokenash optimization: messages contain extended thinking blocks")
        ...
        model_limit = (
            self._tokenash_provider.get_context_limit(model) if self._tokenash_provider else 128000
        )
        ...
        try:
            ...
            logger.warning(
                f"Tokenash optimization failed, using original messages: {type(e).__name__}: {e}"
            )
    ...
    def response(self, messages: list[Any], **kwargs: Any) -> Any:  # type: ignore[override]
        ...
        applies Tokenash optimization before delegating to wrapped_model.invoke().
    ...
    # These delegate to the wrapped model after applying Tokenash optimization
    # =========================================================================
    ...
    def invoke(self, messages: list[Any], **kwargs: Any) -> Any:
        ...
        logger.info(
            f"Tokenash optimized (invoke): {metrics.tokens_before} -> {metrics.tokens_after} tokens "
    ...
    async def ainvoke(self, messages: list[Any], **kwargs: Any) -> Any:
        ...
        logger.info(
            f"Tokenash optimized (ainvoke): {metrics.tokens_before} -> {metrics.tokens_after} tokens "
            f"({metrics.savings_percent:.1f}% saved)"
        )
    ...
    def invoke_stream(self, messages: list[Any], **kwargs: Any) -> Iterator[Any]:
        ...
        logger.info(
            f"Tokenash optimized (invoke_stream): {metrics.tokens_before} -> {metrics.tokens_after} tokens "
            f"({metrics.savings_percent:.1f}% saved)"
        )
    ...
    async def ainvoke_stream(self, messages: list[Any], **kwargs: Any) -> AsyncIterator[Any]:
        ...
        logger.info(
            f"Tokenash optimized (ainvoke_stream): {metrics.tokens_before} -> {metrics.tokens_after} tokens "
            f"({metrics.savings_percent:.1f}% saved)"
        )
