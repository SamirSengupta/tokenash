"""Tokenash MCP integration helpers for compressing tool outputs.

This module currently provides these MCP integration surfaces:

1. TokenashMCPCompressor - core compression logic for MCP tool results
2. compress_tool_result() - standalone helper for host applications
3. TokenashMCPClientWrapper - client wrapper that compresses tool outputs
4. create_tokenash_mcp_proxy() - configuration helper for custom proxy setups

The key insight: MCP tool outputs are the PERFECT use case for Tokenash.
...
Example - Custom proxy configuration:
    ...
    proxy_config = create_tokenash_mcp_proxy(
        upstream_servers=[
            ("slack", slack_server),
            ("database", db_server),
...
Example - Middleware (for MCP client libraries):
    ...
    middleware = TokenashMCPMiddleware(config)
...
from tokenash.config import TokenashConfig, SmartCrusherConfig
from tokenash.providers.openai import OpenAIProvider
from tokenash.transforms.smart_crusher import SmartCrusher
...
class TokenashMCPCompressor:
    ...
    def compress(
        ...
        # Remove any Tokenash markers for clean output
        compressed_content = re.sub(r"\n<tokenash:[^>]+>", "", compressed_content)
...
def compress_tool_result(
    ...
    This is the simplest way to use Tokenash with MCP in your host application.
    ...
    Args:
        ...
        config: Optional Tokenash configuration.
    ...
    Example:
        ...
        ```
    """
    compressor = TokenashMCPCompressor(config=config)
...
class TokenashMCPClientWrapper:
    ...
    Example:
        ...
        from tokenash.integrations.mcp import TokenashMCPClientWrapper

        # Wrap your MCP client
        base_client = Client(transport)
        client = TokenashMCPClientWrapper(base_client)
    ...
    def __init__(
        ...
        """
        self._client = client
        self._compressor = TokenashMCPCompressor(config=config)
...
def create_tokenash_mcp_proxy(
    upstream_servers: list[tuple[str, MCPServer]],
    config: TokenashConfig | None = None,
) -> dict[str, Any]:
    """Create configuration for a custom Tokenash MCP proxy/server wrapper.

    This returns the compressor and upstream-server mapping needed by an
    application-defined MCP proxy/server wrapper. Tokenash does not yet ship
    a ready-to-run ``TokenashMCPProxy`` server implementation.
