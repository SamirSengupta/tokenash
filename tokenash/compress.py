"""One-function compression API for Tokenash.

The simplest way to use Tokenash — no proxy, no config, just compress:
...
class CompressConfig:
    ...
    Pass to ``compress()`` or any integration that uses tokenash.
...
def compress(
    ...
    **kwargs: Any,
    ...
    """Compress messages using Tokenash's full compression pipeline.

    This is the simplest way to use Tokenash. No proxy, no config needed.
...
def compress_spreadsheet(
    ...
    **kwargs: Any,
    ...
    (``pip install tokenash[spreadsheet]``).
    ...
    """
    from tokenash.transforms.spreadsheet_ingest import load_spreadsheet
...
def _get_pipeline() -> Any:
    ...
    with _pipeline_lock:
        ...
        logger.debug("Tokenash compression pipeline initialized")
        return _pipeline
