...
External packages hook into the Tokenash proxy at startup by declaring an
entry point in the ``tokenash.proxy_extension`` group in their ``pyproject.toml``:

    [project.entry-points."tokenash.proxy_extension"]
...
Enabling extensions:
  ...
  * CLI:  ``tokenash proxy --proxy-extension shield_enterprise,mypkg``
  * Env:  ``TOKENASH_PROXY_EXTENSIONS=shield_enterprise,mypkg``
...
ENTRY_POINT_GROUP = "tokenash.proxy_extension"
ENV_VAR = "TOKENASH_PROXY_EXTENSIONS"
...
def _resolve_enabled(enabled: Iterable[str] | None) -> set[str]:
    ...
    Precedence: explicit ``enabled`` argument > ``TOKENASH_PROXY_EXTENSIONS``
