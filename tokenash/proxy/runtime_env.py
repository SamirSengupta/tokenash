...
Most Tokenash settings are read once at proxy startup into ``Config`` and are
...
process environment, so a *reused* proxy — one ``tokenash wrap`` attaches to
...
process-global override store. ``tokenash wrap`` pushes the values it would
...
(``TOKENASH_TARGET_RATIO`` etc.) do not belong here — a fresh proxy already
...
RUNTIME_ENV_KNOBS: tuple[Knob, ...] = (
    Knob("TOKENASH_OUTPUT_SHAPER", "bool", "Master switch for output-token shaping."),
    Knob(
        "TOKENASH_VERBOSITY_LEVEL", "int", "Verbosity steering level 0-4 (unset = learned/default)."
    ),
    Knob("TOKENASH_EFFORT_ROUTER", "bool", "Lower effort on mechanical tool-result continuations."),
    Knob("TOKENASH_MECHANICAL_EFFORT", "str", "Effort value used on mechanical continuations."),
    Knob("TOKENASH_VERBOSITY_AUTOTUNE", "bool", "Use the AIMD verbosity controller state."),
    Knob(
        "TOKENASH_OUTPUT_HOLDOUT",
        "float",
        "Fraction of conversations held out for A/B measurement.",
    ),
    Knob(
        "TOKENASH_INTERCEPT_READ_MIN_CHARS",
        "int",
...
def explicit_env(environ: Mapping[str, str] | None = None) -> dict[str, str]:
    ...
    a default on a shared proxy, set it explicitly (e.g. ``TOKENASH_OUTPUT_SHAPER=0``).
    """
