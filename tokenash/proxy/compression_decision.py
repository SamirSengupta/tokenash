...
This is the input-side analog of :class:`tokenash.proxy.outcome.RequestOutcome`.
...
``x-tokenash-bypass: true`` requests were silently ignored on every
...
class CompressionDecision:
    ...
    #   * ``"bypass_header"``      — user set x-tokenash-bypass or
    ...
    def decide(
        ...
        *,
        ...
        headers
            ...
            Both ``x-tokenash-bypass: true`` and
            ``x-tokenash-mode: passthrough`` trigger bypass — semantics
            mirrored from :func:`tokenash.proxy.helpers._tokenash_bypass_enabled`.
        config
            ``TokenashConfig``-shaped object; only ``optimize: bool``
            is read.
        usage_reporter
