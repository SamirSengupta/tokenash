...
_MODE_ALIASES = {
    ...
    "token_tokenash": PROXY_MODE_TOKEN,
...
def normalize_proxy_mode(mode: str | None, *, default: str = PROXY_MODE_TOKEN) -> str:
    ...
    if normalized is None:
        logger.warning("Unknown TOKENASH_MODE '%s', falling back to '%s'", mode, default)
        return default

    if key != normalized:
        logger.info("TOKENASH_MODE alias '%s' normalized to '%s'", mode, normalized)
    return normalized
