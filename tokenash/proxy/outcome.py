...
:meth:`TokenashProxy._record_request_outcome`), which owns the four
...
class RequestOutcome:
    ...
    # Response-cache hit (Tokenash's own semantic cache served the
    ...
    #     via :func:`tokenash.proxy.auth_mode.classify_client`. The
    ...
    def cache_hit(self) -> bool:
        ...
        was served from Tokenash's own response cache.
...
async def emit_request_outcome(handler: Any, outcome: RequestOutcome) -> None:
    ...
    Owns the four downstream effects in canonical order:
      ...
      4. structured PERF log line — consumed by ``tokenash perf``

    Takes the handler as a free argument rather than ``self`` so this
    function is callable from:
    * ``TokenashProxy._record_request_outcome`` (production)
    ...
    * any provider handler mixin
    ...
    """
    from tokenash.proxy.cost import _summarize_transforms
    from tokenash.proxy.models import RequestLog
    from tokenash.proxy.project_context import get_current_project
    ...
    # HTTP middleware / WS accept captured from ``X-Tokenash-Project``.
    ...
    #    line unchanged, and gives ``tokenash perf --client X``
