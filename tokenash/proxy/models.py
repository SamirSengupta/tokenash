"""Data models for the Tokenash proxy.
...
from tokenash.providers.registry import ProviderApiOverrides
...
class RequestLog:
    ...
    # See ``tokenash.proxy.helpers.compute_turn_id`` for the derivation. None
...
class ProxyConfig:
    ...
    # CLI: --disable-kompress; env: TOKENASH_DISABLE_KOMPRESS=1.
    ...
    # CLI: --disable-kompress-fallback; env: TOKENASH_DISABLE_KOMPRESS_FALLBACK=1.
    ...
    # (and -openai); env: TOKENASH_DISABLE_KOMPRESS_ANTHROPIC / _OPENAI
    # (1 = disable, 0 = enable).
    ...
    # CLI: --compress-user-messages; env: TOKENASH_COMPRESS_USER_MESSAGES=1.
    compress_user_messages: bool = False
    # Named savings policy shared across Claude/Codex/Cursor proxy handlers.
    # CLI/env: TOKENASH_SAVINGS_PROFILE=agent-90.
    ...
    # CLI: --exclude-tools <name1,name2>; env: TOKENASH_EXCLUDE_TOOLS=<name1,name2>
    ...
    # env: TOKENASH_READ_MATURATION=1
    ...
    # env: TOKENASH_READ_MATURATION_QUIESCE_TURNS / _MAX_HOLD_TURNS / _MIN_SIZE_BYTES.
    ...
    # to enable from the `tokenash.proxy_extension` group, or `["*"]` for
    # wildcard. Empty/None means no extensions run, even if installed.
    # CLI: --proxy-extension <name1,name2>; env: TOKENASH_PROXY_EXTENSIONS.
    ...
    memory_db_path: str = ""  # Empty = auto: {cwd}/.tokenash/memory.db
    ...
    # x-tokenash-user-id only. ``global`` keeps the pre-fix single-DB
    ...
    # Qdrant connection (defaults resolve from TOKENASH_QDRANT_* env vars)
    ...
    license_cloud_url: str = "https://app.SamirSengupta.ai"
    ...
    # Env: TOKENASH_PERIODIC_TOIN_STATS=0.
    ...
    # ``TOKENASH_COMPRESSION_MAX_WORKERS``.
    #
    ...
    # ``TokenashProxy._run_compression_in_executor``.
