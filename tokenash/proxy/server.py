"""Tokenash Proxy Server - Production Ready.
...
Usage:
    python -m tokenash.proxy.server --port 8787
...
from tokenash._version import __version__
from tokenash.agent_savings import proxy_pipeline_kwargs
from tokenash.cache.compression_feedback import get_compression_feedback
from tokenash.cache.compression_store import format_retrieval_miss_detail, get_compression_store
from tokenash.ccr import (
    CCR_TOOL_NAME,
    # Batch processing
    CCRResponseHandler,
    CCRToolInjector,
...
from tokenash.config import (
    DEFAULT_EXCLUDE_TOOLS,
    CacheAlignerConfig,
    ReadLifecycleConfig,
)
from tokenash.dashboard import get_dashboard_html
...
from tokenash.pipeline import PipelineExtensionManager, PipelineStage
from tokenash.providers.proxy_routes import register_provider_routes
from tokenash.providers.registry import (
...
from tokenash.proxy import runtime_env
from tokenash.proxy.auth_mode import should_stamp_codex_client
from tokenash.proxy.background_compression import BackgroundCompressor

# =============================================================================
# Extracted modules (re-exported for backward compatibility)
# =============================================================================
from tokenash.proxy.cost import (
...
from tokenash.proxy.helpers import (
...
from tokenash.proxy.memory_handler import MemoryConfig, MemoryHandler

# Data models (extracted to tokenash/proxy/models.py for maintainability)
from tokenash.proxy.models import CacheEntry, ProxyConfig, RateLimitState, RequestLog  # noqa: F401
from tokenash.proxy.modes import (
    PROXY_MODE_CACHE,
    PROXY_MODE_TOKEN,
    is_token_mode,
    normalize_proxy_mode,
)
from tokenash.proxy.probe_recorder import probe_recorder_from_env
from tokenash.proxy.project_context import (
    classify_project,
    set_current_project,
    strip_project_path_prefix,
)
from tokenash.proxy.prometheus_metrics import PrometheusMetrics  # noqa: F401
from tokenash.proxy.rate_limiter import TokenBucketRateLimiter  # noqa: F401
from tokenash.proxy.request_logger import RequestLogger  # noqa: F401
from tokenash.proxy.semantic_cache import SemanticCache  # noqa: F401
from tokenash.proxy.ssl_context import build_httpx_verify
from tokenash.proxy.warmup import WarmupRegistry
from tokenash.proxy.ws_session_registry import WebSocketSessionRegistry
from tokenash.subscription.base import get_quota_registry, reset_quota_registry
from tokenash.subscription.codex_rate_limits import get_codex_rate_limit_state
from tokenash.subscription.copilot_quota import get_copilot_quota_tracker
from tokenash.subscription.tracker import (
    configure_subscription_tracker,
    get_subscription_tracker,
)
from tokenash.telemetry import get_telemetry_collector
from tokenash.telemetry.beacon import is_telemetry_enabled
from tokenash.telemetry.toin import get_toin
from tokenash.transforms import (
    CacheAligner,
    CodeAwareCompressor,
...
def _classify_agent_from_log(entry: dict[str, Any]) -> tuple[str, str, str]:
    ...
    for source, candidate in (
        ...
        ("stack", tags.get("stack") or tags.get("tokenash-stack")),
    ):
...
# PyTorch is optional in tokenash; the warning is not actionable for operators.
os.environ.setdefault("TRANSFORMERS_VERBOSITY", "error")

logging.basicConfig(
    level=logging.INFO, format="%(asctime)s - %(name)s - %(levelname)s - %(message)s"
)
logger = logging.getLogger("tokenash.proxy")

_MULTI_WORKER_CONFIG_ENV = "TOKENASH_PROXY_CONFIG_JSON"

# Env var that opts out of the Rust core deployment smoke test (Hotfix-A0).
# Default behavior: hard-fail at startup if `tokenash._core` is unimportable
# (Finding #2 in TOKENASH_PROXY_LOG_FINDINGS_2026_05_03.md — production
...
# fail-loud behavior.
_RUST_CORE_REQUIRED_ENV = "TOKENASH_REQUIRE_RUST_CORE"
...
def _check_rust_core() -> tuple[str, str | None]:
    """Verify the Rust extension `tokenash._core` is loadable at startup.

    Returns a `(status, error)` tuple:
      - ``("loaded", None)``     — `tokenash._core.hello()` returned the
        expected sentinel.
    ...
    Behavior is gated by the ``TOKENASH_REQUIRE_RUST_CORE`` env var:
    ...
    try:
        from tokenash._core import hello as _rust_hello
        ...
        msg = (
            f"FATAL: Rust extension `tokenash._core` not loadable.\n"
            f"    error: {reason}\n"
            f"    fix:   `make build-wheel && pip install --force-reinstall "
            f"target/wheels/tokenash_*.whl`\n"
    ...
    if marker != "tokenash-core":
        ...
        if not require:
            ...
            return ("disabled", reason)
        msg = (
            f"FATAL: Rust extension `tokenash._core` is loaded but the "
            f"marker function returned {marker!r}; expected 'tokenash-core'.\n"
            f"    fix:   rebuild: `make build-wheel && pip install "
            f"--force-reinstall target/wheels/tokenash_*.whl`\n"
        )
...
from tokenash.proxy.handlers import (  # noqa: E402
...
class TokenashProxy(
    ...
    """Production-ready Tokenash optimization proxy."""
    ...
    def __init__(self, config: ProxyConfig):
        ...
        TokenashProxy.ANTHROPIC_API_URL = api_targets.anthropic
        TokenashProxy.OPENAI_API_URL = api_targets.openai
        TokenashProxy.GEMINI_API_URL = api_targets.gemini
        TokenashProxy.CLOUDCODE_API_URL = api_targets.cloudcode
        TokenashProxy.VERTEX_API_URL = api_targets.vertex
        ...
        # `TOKENASH_LOSSLESS_ONLY=1` routes SmartCrusher through strict
        ...
        if "TOKENASH_LOSSLESS_ONLY" in os.environ:
            router_config.smart_crusher_lossless_only = _get_env_bool(
                "TOKENASH_LOSSLESS_ONLY", False
            )
        ...
        from tokenash.cache.prefix_tracker import PrefixFreezeConfig, SessionTrackerStore
        ...
        self._compression_executor = concurrent.futures.ThreadPoolExecutor(
            ...
            thread_name_prefix="tokenash-compress",
        )
        ...
        self._background_compression_enabled: bool = os.environ.get(
            "TOKENASH_BACKGROUND_COMPRESSION", ""
        ).strip().lower() in ("1", "true", "yes", "on")
        try:
            self._background_compression_min_tokens: int = int(
                os.environ.get("TOKENASH_BACKGROUND_COMPRESSION_MIN_TOKENS", "50000")
            )
        except ValueError:
        ...
        self._background_compression_executor = concurrent.futures.ThreadPoolExecutor(
            max_workers=1, thread_name_prefix="tokenash-bg-compress"
        )
        ...
        if config.memory_enabled:
            ...
            from tokenash.proxy.memory_handler import MemoryMode
            ...
            from tokenash.memory.storage_router import MemoryStorageMode
        ...
        if config.license_key:
            from tokenash.telemetry.reporter import UsageReporter
        ...
        if config.traffic_learning_enabled:
            from tokenash.memory.traffic_learner import TrafficLearner

            self.traffic_learner = TrafficLearner(
                user_id=os.environ.get("TOKENASH_USER_ID", os.environ.get("USER", "default")),
                agent_type=config.traffic_learning_agent_type,
        ...
        if config.code_graph_watcher:
            from tokenash.graph.watcher import CodeGraphWatcher
    ...
    def _get_compression_cache(self, session_id: str) -> CompressionCache:
        ...
        with self._compression_caches_lock:
            if session_id not in self._compression_caches:
                from tokenash.cache.compression_cache import CompressionCache
    ...
    async def startup(self):
        ...
        # TOKENASH_TLS_STRICT=0, else httpx's default strict verification.
        ...
        logger.info("Tokenash Proxy started")
        ...
        if self._kompress_status == "enabled":
            ...
            logger.info("Kompress: not installed (pip install tokenash[ml] for ML compression)")
        ...
        if self._code_aware_status == "enabled":
            ...
            logger.info("Code-Aware: not installed (pip install tokenash[code])")
        ...
        if is_telemetry_enabled():
            logger.info(
                ...
                "Opt out: TOKENASH_TELEMETRY=off or --no-telemetry"
            )
        else:
            logger.info(
                "Anonymous telemetry: DISABLED (off by default — opt in: "
                "TOKENASH_TELEMETRY=on or --telemetry)"
            )
    ...
    async def shutdown(self):
        ...
        with contextlib.suppress(Exception):
            from tokenash.models.ml_models import MLModelRegistry
    ...
    def _print_summary(self):
        ...
        m = self.metrics
        logger.info("=" * 70)
        logger.info("TOKENASH PROXY SESSION SUMMARY")
        logger.info("=" * 70)
    ...
    async def _record_request_outcome(self, outcome: RequestOutcome) -> None:
        ...
        Thin wrapper around :func:`tokenash.proxy.outcome.emit_request_outcome`
        ...
        """
        from tokenash.proxy.outcome import emit_request_outcome
    ...
    def _extract_tags(self, headers: dict) -> dict[str, str]:
        ...
        """
        from tokenash.proxy.helpers import extract_tags
    ...
    async def _retry_request(
        ...
        *,
        ...
        Byte-faithful forwarding (PR-A3, fixes P0-2):
          ...
          * ``TOKENASH_PROXY_PYTHON_FORWARDER_MODE=legacy_json_kwarg`` is an
...
def _register_memory_components(proxy: TokenashProxy, tracker: MemoryTracker) -> None:
    ...
    Args:
        proxy: The TokenashProxy instance.
...
def _request_is_loopback(request: Request) -> bool:
    ...
    """
    from tokenash.proxy.loopback_guard import is_loopback_host, is_loopback_host_header
...
def create_app(config: ProxyConfig | None = None) -> FastAPI:
    ...
    # Always-on file logging to ~/.tokenash/logs/ for `tokenash perf` analysis.
    # Installed here (not at module import) so importing tokenash.proxy.server
    # in tests or library contexts does not silently attach a RotatingFileHandler
    # to the user's live proxy.log.
    _setup_file_logging()

    config = config or ProxyConfig()
    proxy = TokenashProxy(config)

    # cc-switch reconciler (opt-in: TOKENASH_CC_SWITCH_RECONCILE=1).
    # Keeps Tokenash in the request path while cc-switch overwrites
    # ~/.claude/settings.json on every provider switch. See
    # tokenash/proxy/cc_switch_reconciler.py for the full rationale.
    from tokenash.proxy.cc_switch_reconciler import (
        CCSwitchReconciler,
        reconciler_enabled,
    )
    ...
    if reconciler_enabled():
        ...
        def _set_anthropic_upstream(url: str) -> None:
            from tokenash.providers.registry import _normalize_api_url

            TokenashProxy.ANTHROPIC_API_URL = _normalize_api_url(
    ...
    from tokenash.telemetry.beacon import TelemetryBeacon

    _beacon = TelemetryBeacon(
        port=config.port if hasattr(config, "port") else 8787,
        sdk=os.environ.get("TOKENASH_SDK", "proxy").strip() or "proxy",
        backend=config.backend if hasattr(config, "backend") else "anthropic",
    )
    from tokenash import paths as _hr_paths
    ...
    async def lifespan(app: FastAPI):  # type: ignore[no-untyped-def]
        ...
        # explicitly opted out with TOKENASH_REQUIRE_RUST_CORE=false. See
        # Finding #2 in TOKENASH_PROXY_LOG_FINDINGS_2026_05_03.md.
        ...
        configure_otel_metrics(OTelMetricsConfig.from_env(default_service_name="tokenash-proxy"))
        configure_langfuse_tracing(
            LangfuseTracingConfig.from_env(default_service_name="tokenash-proxy")
        )
        ...
        try:
            try:
                ...
                # TokenashProxy.ANTHROPIC_API_URL, so workers could disagree on
    ...
    def _health_checks() -> dict[str, dict[str, Any]]:
        ...
        return {
            ...
            "upstream": _component_health(
                enabled=os.environ.get("TOKENASH_SKIP_UPSTREAM_CHECK", "").strip() != "1",
    ...
    def _health_payload(*, include_config: bool) -> dict[str, Any]:
        ...
        deployment_profile = os.environ.get("TOKENASH_DEPLOYMENT_PROFILE")
        if deployment_profile:
            payload["deployment"] = {
                "profile": deployment_profile,
                "preset": os.environ.get("TOKENASH_DEPLOYMENT_PRESET"),
                "runtime": os.environ.get("TOKENASH_DEPLOYMENT_RUNTIME"),
                "supervisor": os.environ.get("TOKENASH_DEPLOYMENT_SUPERVISOR"),
                "scope": os.environ.get("TOKENASH_DEPLOYMENT_SCOPE"),
            }
        if include_config:
            profile_kwargs = proxy_pipeline_kwargs(config)
            ...
            payload["config"] = {
                ...
                # Surfaced so `tokenash wrap` can see what a reused proxy is
    ...
    # every /readyz poll.  Set TOKENASH_SKIP_UPSTREAM_CHECK=1 to opt out (e.g.
    ...
    async def _check_upstream() -> None:
        ...
        """
        if os.environ.get("TOKENASH_SKIP_UPSTREAM_CHECK", "").strip() == "1":
    ...
    # layer, not in ProxyConfig). Set TOKENASH_CORS_ORIGINS (comma-separated)
    ...
    _cors_origins_env = os.environ.get("TOKENASH_CORS_ORIGINS", "").strip()
    ...
    # X-Tokenash-Stack: SDK adapters (TS openai/anthropic/etc.) tag their
    ...
    @app.middleware("http")
    async def _record_tokenash_stack(request, call_next):
        started = time.perf_counter()
        inbound_id = f"inbound-{time.time_ns()}"
        # Project attribution: an explicit X-Tokenash-Project header wins
        ...
        try:
            from tokenash.proxy.helpers import redact_for_wire_debug
        ...
        if request.url.path.startswith("/v1/"):
            stack = request.headers.get("x-tokenash-stack")
            if stack:
                try:
    ...
    # the `tokenash.proxy_extension` entry-point group, but **opt-in only**:
    # only names listed in config.proxy_extensions (CLI: --proxy-extension,
    # env: TOKENASH_PROXY_EXTENSIONS) actually get installed. Discovery alone
    ...
    from tokenash.proxy.extensions import install_all as _install_extensions
    ...
    from tokenash.proxy.debug_introspection import (
        collect_tasks as _collect_tasks,
    )
    from tokenash.proxy.loopback_guard import require_loopback as _require_loopback
    ...
    async def get_upstream():
        ...
        """
        return {
            "anthropic": TokenashProxy.ANTHROPIC_API_URL,
    ...
    async def admin_runtime_env(request: Request):
        ...
        that ``tokenash wrap`` reused — rather than started — never sees values
    ...
    async def dashboard():
        """Serve the Tokenash dashboard UI."""
    ...
    async def _build_stats_payload() -> dict[str, Any]:
        ...
        async with _throughput_cache_lock:
            ...
            if _throughput_cache["expires_at"] < now or _throughput_cache["value"] is None:
                ...
                def _compute_throughput():
                    from tokenash.perf.analyzer import build_perf_summary, parse_log_files
        ...
        # Calculate Tokenash overhead (optimization time only, excludes pass-through requests)
        avg_overhead_ms = (
        ...
        # Calculate total tokens before Tokenash-side reduction. Proxy
        ...
        try:
            from tokenash.proxy.output_savings import get_recorder
            ...
            "savings": {
                ...
                "by_layer": {
                    ...
                    "compression": {
                        ...
                        "description": (
                            "Tokens removed by Tokenash proxy compression. "
                    ...
                    "prefix_cache": {
                        ...
                        "description": (
                            ...
                            "Tokenash's CacheAligner improves hit rates; "
    ...
    async def stats_history(
        ...
        if format == "csv":
            filename = f"tokenash-stats-history-{series}.csv"
            return Response(
    ...
    async def subscription_window():
        """Current Anthropic subscription window utilisation and Tokenash contribution.
    ...
    # CCR Tool Call Handler - for agent frameworks to call when LLM uses tokenash_retrieve
...
def _proxy_config_from_env() -> ProxyConfig:
    ...
    if raw_config:
        try:
            ...
            logger.warning(
                "Invalid %s; falling back to TOKENASH_* env vars", _MULTI_WORKER_CONFIG_ENV
            )

    return ProxyConfig(
        host=_get_env_str("TOKENASH_HOST", "127.0.0.1"),
        port=_get_env_int("TOKENASH_PORT", 8787),
        ...
        anthropic_buffered_request_timeout_seconds=_get_env_int(
            "TOKENASH_ANTHROPIC_BUFFERED_REQUEST_TIMEOUT_SECONDS",
            600,
            min_value=1,
        ),
        vertex_api_url=os.environ.get("VERTEX_TARGET_API_URL"),
        backend=_get_env_str("TOKENASH_BACKEND", "anthropic"),
        bedrock_region=_get_env_str("TOKENASH_BEDROCK_REGION", "us-west-2"),
        bedrock_profile=os.environ.get("AWS_PROFILE"),
        bedrock_api_url=os.environ.get("BEDROCK_TARGET_API_URL"),
        anyllm_provider=_get_env_str("TOKENASH_ANYLLM_PROVIDER", "openai"),
        disable_kompress=_get_env_bool("TOKENASH_DISABLE_KOMPRESS", False),
        disable_kompress_fallback=_get_env_bool("TOKENASH_DISABLE_KOMPRESS_FALLBACK", False),
        disable_kompress_anthropic=_get_env_optional_bool("TOKENASH_DISABLE_KOMPRESS_ANTHROPIC"),
        disable_kompress_openai=_get_env_optional_bool("TOKENASH_DISABLE_KOMPRESS_OPENAI"),
        max_connections=_get_env_int("TOKENASH_MAX_CONNECTIONS", 500),
        max_keepalive_connections=_get_env_int("TOKENASH_MAX_KEEPALIVE", 100),
        keepalive_expiry=_get_env_float("TOKENASH_KEEPALIVE_EXPIRY", 90.0),
        http2=_get_env_bool("TOKENASH_HTTP2", True),
        periodic_toin_stats_enabled=_get_env_bool("TOKENASH_PERIODIC_TOIN_STATS", True),
        mode=normalize_proxy_mode(_get_env_str("TOKENASH_MODE", PROXY_MODE_TOKEN)),
        read_maturation=_get_env_bool("TOKENASH_READ_MATURATION", False),
        read_maturation_quiesce_turns=_get_env_int("TOKENASH_READ_MATURATION_QUIESCE_TURNS", 5),
        read_maturation_max_hold_turns=_get_env_int("TOKENASH_READ_MATURATION_MAX_HOLD_TURNS", 25),
        read_maturation_min_size_bytes=_get_env_int(
            "TOKENASH_READ_MATURATION_MIN_SIZE_BYTES", 2048
        ),
    )
...
def _get_code_aware_banner_status(config: ProxyConfig) -> str:
    ...
    if config.code_aware_enabled:
        if is_tree_sitter_available():
            ...
            return "NOT INSTALLED (pip install tokenash[code])"
    else:
        if is_tree_sitter_available():
            return "DISABLED (--code-aware or TOKENASH_CODE_AWARE_ENABLED=1 to enable)"
        return "DISABLED  (install tokenash[code] to enable)"
...
def run_server(
    ...
    Args:
        ...
        print_banner: When False, skip the legacy ASCII banner. The
            Click CLI (`tokenash proxy`) prints its own startup banner
            before calling this — printing a second banner here is the
            "dual banner" UX issue. Direct `python -m tokenash.proxy.server`
...
""")
    ...
    if workers > 1:
        ...
        # cross-worker backend is configured via TOKENASH_CCR_BACKEND.
        # See RUST_DEV.md -> "Multi-worker deployment -- CCR fragmentation".
        if os.environ.get("TOKENASH_CCR_BACKEND", "").strip():
            logger.warning(
                "Tokenash is running with workers=%d. Compression cache, "
            ...
            logger.warning(
                "Tokenash is running with workers=%d. The in-memory CCR store, "
                ...
                "sessions land on different workers. Set TOKENASH_CCR_BACKEND=sqlite for a "
        ...
        app_target = "tokenash.proxy.server:create_app_from_env"
...
def _parse_exclude_tools(cli_excludes: str | None) -> set[str]:
    ...
    Both --exclude-tools and TOKENASH_EXCLUDE_TOOLS are comma-separated
    ...
    """
    raw = ",".join(s for s in (cli_excludes, os.environ.get("TOKENASH_EXCLUDE_TOOLS")) if s)
    names: set[str] = set()
...
def _parse_tool_profiles(cli_profiles: list[str]) -> dict[str, Any]:
    """Parse tool profiles from CLI args and TOKENASH_TOOL_PROFILES env var.
    ...
    """
    from tokenash.config import PROFILE_PRESETS, CompressionProfile

    profiles: dict[str, CompressionProfile] = {}
    raw_entries: list[str] = list(cli_profiles)

    # Also check env var
    env_val = os.environ.get("TOKENASH_TOOL_PROFILES", "")
    if env_val:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Tokenash Proxy Server")
    ...
    parser.add_argument(
        ...
        "Can be specified multiple times. Also settable via TOKENASH_TOOL_PROFILES env var.",
    )
    parser.add_argument(
        ...
        help=(
            ...
            "Also settable via TOKENASH_COMPRESS_USER_MESSAGES=1."
        ),
    )
    parser.add_argument(
        "--disable-kompress",
        ...
        help=(
            ...
            "Also settable via TOKENASH_DISABLE_KOMPRESS=1."
        ),
    )
    parser.add_argument(
        ...
        help=(
            ...
            "Also settable via TOKENASH_DISABLE_KOMPRESS_FALLBACK=1."
        ),
    )
    parser.add_argument(
        ...
        help=(
            ...
            "Also settable via TOKENASH_DISABLE_KOMPRESS_ANTHROPIC=1."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Also settable via TOKENASH_DISABLE_KOMPRESS_OPENAI=1."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        "Also settable via TOKENASH_EXCLUDE_TOOLS env var.",
    )
    ...
    parser.add_argument(
        "--code-aware",
        action="store_true",
        help="Enable AST-based code compression (requires: pip install tokenash[code])",
    )
    parser.add_argument(
        "--no-code-aware",
        action="store_true",
    ...
    # Environment variable defaults (TOKENASH_* prefix)
    # CLI args override env vars, env vars override ProxyConfig defaults
    env_code_aware = _get_env_bool("TOKENASH_CODE_AWARE_ENABLED", True)
    env_optimize = _get_env_bool("TOKENASH_OPTIMIZE", True)
    env_cache = _get_env_bool("TOKENASH_CACHE_ENABLED", True)
    env_rate_limit = _get_env_bool("TOKENASH_RATE_LIMIT_ENABLED", True)
    ...
    config = ProxyConfig(
        host=_get_env_str("TOKENASH_HOST", args.host),
        port=_get_env_int("TOKENASH_PORT", args.port),
        ...
        # Backend settings
        backend=_get_env_str("TOKENASH_BACKEND", args.backend),  # type: ignore[arg-type]
        bedrock_region=_get_env_str("TOKENASH_BEDROCK_REGION", args.bedrock_region),
        ...
        anyllm_provider=_get_env_str("TOKENASH_ANYLLM_PROVIDER", args.anyllm_provider),
        optimize=optimize,
        min_tokens_to_crush=_get_env_int("TOKENASH_MIN_TOKENS", args.min_tokens),
        max_items_after_crush=_get_env_int("TOKENASH_MAX_ITEMS", args.max_items),
        smart_crusher_with_compaction=(
            _get_env_bool("TOKENASH_SMART_CRUSHER_COMPACTION", False)
            if "TOKENASH_SMART_CRUSHER_COMPACTION" in os.environ
            else None
        ),
        cache_enabled=cache_enabled,
        cache_ttl_seconds=_get_env_int("TOKENASH_CACHE_TTL", args.cache_ttl),
        rate_limit_enabled=rate_limit_enabled,
        rate_limit_requests_per_minute=_get_env_int("TOKENASH_RPM", args.rpm),
        rate_limit_tokens_per_minute=_get_env_int("TOKENASH_TPM", args.tpm),
        budget_limit_usd=args.budget,
        budget_period=args.budget_period,
        log_file=_get_env_str("TOKENASH_LOG_FILE", args.log_file)
        if args.log_file
        else os.environ.get("TOKENASH_LOG_FILE"),
        log_full_messages=args.log_messages or _get_env_bool("TOKENASH_LOG_MESSAGES", False),
        ...
        max_connections=_get_env_int("TOKENASH_MAX_CONNECTIONS", args.max_connections),
        max_keepalive_connections=_get_env_int("TOKENASH_MAX_KEEPALIVE", args.max_keepalive),
        keepalive_expiry=_get_env_float("TOKENASH_KEEPALIVE_EXPIRY", args.keepalive_expiry),
        http2=not args.no_http2 and _get_env_bool("TOKENASH_HTTP2", True),
        read_maturation=_get_env_bool("TOKENASH_READ_MATURATION", False),
        read_maturation_quiesce_turns=_get_env_int("TOKENASH_READ_MATURATION_QUIESCE_TURNS", 5),
        read_maturation_max_hold_turns=_get_env_int("TOKENASH_READ_MATURATION_MAX_HOLD_TURNS", 25),
        read_maturation_min_size_bytes=_get_env_int(
        ...
        or _get_env_bool("TOKENASH_COMPRESS_USER_MESSAGES", False),
        savings_profile=os.environ.get("TOKENASH_SAVINGS_PROFILE") or None,
        # Default 0.4 keep-ratio so the Kompress text (prose/code) path compresses
        # meaningfully out of the box; TOKENASH_TARGET_RATIO overrides.
        target_ratio=(
            float(os.environ["TOKENASH_TARGET_RATIO"])
            if os.environ.get("TOKENASH_TARGET_RATIO")
            else 0.4
        ),
        compress_system_messages=(
            _get_env_bool("TOKENASH_COMPRESS_SYSTEM_MESSAGES", False)
            if "TOKENASH_COMPRESS_SYSTEM_MESSAGES" in os.environ
            else None
        ),
        protect_recent=(
            int(os.environ["TOKENASH_PROTECT_RECENT"])
            if os.environ.get("TOKENASH_PROTECT_RECENT")
            else None
        ),
        protect_analysis_context=(
            _get_env_bool("TOKENASH_PROTECT_ANALYSIS_CONTEXT", False)
            if "TOKENASH_PROTECT_ANALYSIS_CONTEXT" in os.environ
            else None
        ),
        accuracy_guard=os.environ.get("TOKENASH_ACCURACY_GUARD") or None,
    )

    # Get worker and concurrency settings
    workers = _get_env_int("TOKENASH_WORKERS", args.workers)
    limit_concurrency = _get_env_int("TOKENASH_LIMIT_CONCURRENCY", args.limit_concurrency)
