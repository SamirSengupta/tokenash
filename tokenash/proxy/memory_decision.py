...
OpenAI chat, Gemini) never gated on ``x-tokenash-bypass``, so
...
Precedence (highest first):
  ...
  4. ``mode_disabled`` — operator TOKENASH_MEMORY_INJECTION_MODE=disabled
  5. ``mode_tool`` — operator TOKENASH_MEMORY_INJECTION_MODE=tool
...
class MemoryDecision:
    ...
    #   * "bypass_header"  — user set x-tokenash-bypass/x-tokenash-mode
    #   * "no_handler"     — no memory backend configured on the proxy
    #   * "no_user_id"     — per-request user_id missing
    #   * "mode_disabled"  — operator TOKENASH_MEMORY_INJECTION_MODE=disabled
    #   * "mode_tool"      — operator TOKENASH_MEMORY_INJECTION_MODE=tool
    ...
    def decide(
        ...
        *,
        ...
        mode_name
            ...
            ``TOKENASH_MEMORY_INJECTION_MODE``.
        """
        bypass = _tokenash_bypass_enabled(headers)
