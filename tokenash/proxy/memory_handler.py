...
This module provides memory capabilities for the Tokenash proxy:
...
if TYPE_CHECKING:
    from tokenash.memory.backends.local import LocalBackend
...
# Memory tool names for detection (Tokenash's custom tools)
MEMORY_TOOL_NAMES = {
    "memory_save",
    "memory_search",
    "memory_update",
...
class MemoryConfig:
    ...
    Qdrant connection fields default to values read from ``TOKENASH_QDRANT_*``
    environment variables (see :mod:`tokenash.memory.qdrant_env`). Passing an
    explicit value to the constructor always wins over the environment.
    """

    enabled: bool = False
    backend: Literal["local", "qdrant-neo4j"] = "local"
    db_path: str = "tokenash_memory.db"
    ...
    native_memory_dir: str = ""  # Directory for native memory files (default: ~/.tokenash/memories)
    # Qdrant+Neo4j config (Qdrant defaults resolve from TOKENASH_QDRANT_* env vars)
    ...
    # Memory Bridge (bidirectional markdown <-> Tokenash sync)
...
class MemoryHandler:
    ...
    Supports two modes:
    - Custom tools: Tokenash's memory_save, memory_search, etc. (default)
    ...
    def _init_native_memory_dir(self) -> None:
        ...
        if self.config.native_memory_dir:
            ...
            # Default: workspace memories directory (respects TOKENASH_WORKSPACE_DIR)
    ...
    async def _init_backend_locked(self) -> None:
        ...
        if self.config.backend == "local":
            from tokenash.memory.backends.local import LocalBackend, LocalBackendConfig
            ...
            # Opt-in GPU offload: TOKENASH_EMBEDDER_RUNTIME=pytorch_mps routes embedding
            ...
            if os.environ.get("TOKENASH_EMBEDDER_RUNTIME", "").strip().lower() == "pytorch_mps":
                try:
                    ...
                    if torch.backends.mps.is_available():
                        ...
                        logger.info(
                            "Memory: TOKENASH_EMBEDDER_RUNTIME=pytorch_mps → "
                            "torch embedder on Apple GPU (MPS)"
                        )
                    else:
                        logger.warning(
                            "Memory: TOKENASH_EMBEDDER_RUNTIME=pytorch_mps requested but "
            ...
            try:
                from tokenash.memory.backends.direct_mem0 import (
                    DirectMem0Adapter,
                    Mem0Config,
                )
                ...
                logger.error(
                    ...
                    "Install with: pip install 'tokenash[memory-stack]'"
                )
                raise
        else:
    ...
    async def _init_and_import_bridge(self) -> None:
        ...
        if self._bridge is not None:
            return
        try:
            from tokenash.memory.bridge import MemoryBridge
            from tokenash.memory.bridge_config import BridgeConfig, MarkdownFormat
    ...
    def _get_memory_tools(self) -> list[dict[str, Any]]:
        ...
        if self._memory_tools is None:
            from tokenash.memory.tools import get_memory_tools_optimized
        ...
        *,
        ...
        Args:
            user_id: User identifier for memory scoping (the base user
                id, derived from ``x-tokenash-user-id`` upstream).
            ...
            ranker: Optional :class:`~tokenash.proxy.memory_ranker.MemoryRanker`
        ...
        """
        from tokenash.proxy.memory_injection import MemoryInjectionBudget
        ...
        # but no x-tokenash-project-id / x-tokenash-cwd / system-prompt
        ...
        try:
            ...
            if ranker is not None:
                from tokenash.proxy.memory_ranker import MemoryCandidate
    ...
    def _append_to_latest_user_tail(
        ...
        *,
        ...
        if provider == "anthropic":
            ...
            # tokenash.proxy.handlers.anthropic which imports MemoryHandler.
        ...
        if provider == "openai":
            from tokenash.proxy.helpers import append_text_to_latest_user_chat_message
