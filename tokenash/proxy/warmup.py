...
``TokenashProxy.startup`` so that request-time use shares one in-process
singleton and concurrent first-use callers do not trigger N parallel loads.

The registry is populated by ``TokenashProxy.startup`` and exposed as
...
class WarmupRegistry:
    """Shared preloaded asset registry populated by ``TokenashProxy.startup``.

    One instance per ``TokenashProxy``. All writes happen on the startup
