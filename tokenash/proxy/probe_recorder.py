...
``tokenash evals probes`` can measure what compression removed from real
proxied sessions. Activated only when ``TOKENASH_PROBE_RECORD_DIR`` is set;
...
from tokenash.pipeline import PipelineEvent, PipelineStage

logger = logging.getLogger(__name__)

RECORD_DIR_ENV = "TOKENASH_PROBE_RECORD_DIR"
...
def probe_recorder_from_env() -> CompressionEventRecorder | None:
    """Build a recorder when ``TOKENASH_PROBE_RECORD_DIR`` is set, else None.
