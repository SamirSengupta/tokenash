"""Streaming handler mixin for TokenashProxy.
...
from tokenash.proxy.helpers import jitter_delay_ms, retry_after_ms
...
from tokenash.copilot_auth import apply_copilot_api_auth
...
class StreamingMixin:
    """Mixin providing streaming response methods for TokenashProxy."""
    ...
    def _parse_sse_usage(self, chunk: bytes, provider: str) -> dict[str, int] | None:
        ...
        """
        from tokenash.proxy.helpers import parse_sse_events_from_byte_buffer
    ...
    def _record_ccr_feedback_from_response(
        ...
        """Extract tokenash_retrieve tool calls from a response and record feedback.
        ...
        for block in content:
            ...
            if block.get("type") != "tool_use":
                continue
            if block.get("name") != "tokenash_retrieve":
                continue
    ...
    def _record_ccr_feedback_from_openai_sse(self, full_sse_data: str, request_id: str) -> None:
        """Record tokenash_retrieve feedback from OpenAI Chat Completions SSE.
        ...
        ``tokenash_retrieve`` invocation as a no-op store call for the
        ...
        for slot in tool_calls.values():
            if slot["name"] != "tokenash_retrieve":
                continue
            try:
        ...
        *,
        ...
        async def generate():
            ...
            try:
                ...
                # CCR Feedback: Record tokenash_retrieve tool calls for TOIN learning.
        ...
        async def generate():
            try:
                ...
                async for event in self.anthropic_backend.stream_message(body, headers):
                    ...
                    # the token count Tokenash actually sent upstream
    ...
    async def _stream_openai_via_backend(
        ...
        from tokenash.proxy.handlers.openai import _infer_openai_cache_write_tokens
        from tokenash.proxy.outcome import RequestOutcome
        ...
        async def generate():
            ...
            try:
                ...
                # CCR Feedback: record tokenash_retrieve tool calls so
