"""Reconcile Codex thread provider tags across the Tokenash proxy boundary.

Codex stamps every thread with the ``model_provider`` it ran under and filters
its history/projects menu by the active provider set.  When Tokenash rewrites
Codex's config to route through the custom ``tokenash`` provider (see
:mod:`tokenash.providers.codex.install`), threads created through Tokenash are
tagged ``tokenash`` while native threads keep ``openai`` -- so the two sets never
appear in the same menu, and connecting Tokenash appears to "lose" history.

To keep the menu whole we retag threads to match whichever provider is active:
``openai -> tokenash`` when Tokenash is enabled, ``tokenash -> openai`` when it is
...
def retag_to_tokenash(codex_home: Path) -> None:
    """Pull existing native threads into the tokenash-provider menu (on enable)."""
    retag_thread_providers(codex_home, frm=NATIVE_PROVIDER, to=TOKENASH_PROVIDER)


def retag_to_native(codex_home: Path) -> None:
    """Hand threads back to the native-provider menu (on revert)."""
    retag_thread_providers(codex_home, frm=TOKENASH_PROVIDER, to=NATIVE_PROVIDER)
