...
from tokenash.install.paths import opencode_config_path

# Tokenash-managed JSON marker comments for idempotent block injection.
_PROVIDER_MARKER_START = "// --- Tokenash proxy provider ---"
_PROVIDER_MARKER_END = "// --- end Tokenash proxy provider ---"
_MCP_MARKER_START = "// --- Tokenash MCP server ---"
_MCP_MARKER_END = "// --- end Tokenash MCP server ---"

# Regex to strip tokenash blocks (including the marker comments).
...
TOKENASH_OPENCODE_PLUGIN = "tokenash-opencode"
...
def snapshot_opencode_config_if_unwrapped(config_file: Path, backup_file: Path) -> None:
    ...
    Guarantees that ``tokenash unwrap opencode`` can restore the user's
...
def strip_opencode_tokenash_blocks(content: str, *, remove_mcp: bool = True) -> str:
    """Remove all Tokenash-managed blocks from opencode JSON text.
...
def _render_provider_block(port: int) -> str:
    """Render a Tokenash provider block as a JSON comment-wrapped snippet."""
    provider = {
        "tokenash": {
...
def _render_mcp_block(port: int) -> str:
    """Render a Tokenash MCP block as a JSON comment-wrapped snippet."""
    mcp = {
        "tokenash": {
            "type": "remote",
...
def append_tokenash_plugin(config: dict[str, object]) -> bool:
    """Append the optional OpenCode plugin entry if it is not already present."""
    plugin = config.get("plugin")
    if plugin is None:
        config["plugin"] = [TOKENASH_OPENCODE_PLUGIN]
        return True
    ...
    for entry in plugin:
        if entry == TOKENASH_OPENCODE_PLUGIN:
            return False
        if isinstance(entry, list) and entry and entry[0] == TOKENASH_OPENCODE_PLUGIN:
            return False

    plugin.append(TOKENASH_OPENCODE_PLUGIN)
    return True


def inject_opencode_provider_config(port: int) -> None:
    """Inject a Tokenash model provider into OpenCode's config file.
    ...
    ``opencode.json.tokenash-backup`` so ``tokenash unwrap opencode``
    ...
    try:
        ...
        # Strip any prior Tokenash-managed blocks before re-injecting.
        if _PROVIDER_MARKER_START in content:
            content = strip_opencode_tokenash_blocks(content)
            data = _parse_json_loose(content)
