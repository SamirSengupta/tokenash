"""Anthropic provider implementation for Tokenash SDK.
...
def _load_custom_model_config() -> dict[str, Any]:
    ...
    Checks (in order):
    1. TOKENASH_MODEL_LIMITS environment variable (JSON string or file path)
    2. ~/.tokenash/models.json config file
    ...
    env_config = os.environ.get("TOKENASH_MODEL_LIMITS", "")
    if env_config:
        try:
            ...
            logger.debug(f"Loaded custom model config from TOKENASH_MODEL_LIMITS: {loaded}")
        except (json.JSONDecodeError, OSError) as e:
            logger.warning(f"Failed to load TOKENASH_MODEL_LIMITS: {e}")
...
class AnthropicTokenCounter(TokenCounter):
    ...
    def __init__(self, model: str, client: Any = None, warn: bool = True):
        ...
        try:
            from tokenash.tokenizers.tiktoken_counter import (
                TiktokenLoadError,
                load_encoding,
            )
...
class AnthropicProvider(Provider):
    ...
    Custom Model Configuration:
        ...
        1. Environment variable (JSON string):
           export TOKENASH_MODEL_LIMITS='{"context_limits": {"my-model": 200000}}'

        2. Environment variable (file path):
           export TOKENASH_MODEL_LIMITS=/path/to/models.json

        3. Config file (~/.tokenash/models.json):
           {
             "anthropic": {
    ...
    def get_context_limit(self, model: str) -> int:
        ...
        2. TOKENASH_MODEL_LIMITS environment variable
        3. ~/.tokenash/models.json config file
    ...
    def _warn_unknown_model(self, model: str, limit: int, reason: str) -> None:
        ...
        if model not in _UNKNOWN_MODEL_WARNINGS:
            ...
            logger.warning(
                ...
                f"To configure explicitly, set TOKENASH_MODEL_LIMITS env var or "
                f"add to ~/.tokenash/models.json"
            )
