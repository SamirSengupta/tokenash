...
from tokenash.install.models import DeploymentManifest, ManagedMutation, ToolTarget
from tokenash.install.paths import openclaw_config_path
from tokenash.install.runtime import resolve_tokenash_command
...
def revert_provider_scope(mutation: ManagedMutation, manifest: DeploymentManifest) -> None:
    ...
    if not shutil_which("openclaw"):
        return
    command = [*resolve_tokenash_command(), "unwrap", "openclaw"]
    _invoke_openclaw(command)
