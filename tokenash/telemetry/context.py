...
* ``tokenash_stack`` — how Tokenash is being invoked
...
# from unbounded label explosion when clients send arbitrary X-Tokenash-Stack
# header values.
...
def detect_install_mode(port: int) -> str:
    ...
    1. ``TOKENASH_AGENT_TYPE`` env var set → ``wrapped`` (spawned by ``tokenash wrap``).
    ...
    try:
        if os.environ.get("TOKENASH_AGENT_TYPE"):
            return "wrapped"
...
def detect_stack(stats: dict[str, Any] | None = None) -> str:
    """Classify how Tokenash is being invoked.

    Resolution order:

    1. ``TOKENASH_STACK`` env var set → use that slug verbatim.
    2. ``TOKENASH_AGENT_TYPE`` env var set → ``wrap_<agent>``.
    ...
    try:
        explicit = normalize_stack(os.environ.get("TOKENASH_STACK"))
        if explicit:
            return explicit

        agent_type = os.environ.get("TOKENASH_AGENT_TYPE")
        if agent_type:
