...
def get_telemetry_collector(
    ...
    if _telemetry_collector is None:
        with _collector_lock:
            if _telemetry_collector is None:
                # Honour TOKENASH_TELEMETRY (the documented opt-out var,
                # also used by the Supabase beacon at telemetry/beacon.py).
                # Pre-#390 this only checked TOKENASH_TELEMETRY_DISABLED,
                # so users who set TOKENASH_TELEMETRY=off (the value in
                # the docs) still saw /v1/telemetry report enabled=true.
                # TOKENASH_TELEMETRY_DISABLED stays accepted for back-compat.
                from tokenash.telemetry.beacon import is_telemetry_enabled

                disabled_legacy = os.environ.get("TOKENASH_TELEMETRY_DISABLED", "").lower() in (
                    "1",
                    "true",
                )
