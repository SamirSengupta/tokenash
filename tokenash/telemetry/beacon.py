"""Anonymous usage telemetry beacon for Tokenash.

Sends aggregate-only stats (tokens saved, compression ratios, cache hit rates,
performance overhead) to help improve Tokenash.  No prompts, no content, no PII.

Off by default (opt-in). Nothing is collected or sent unless you opt in with:
    TOKENASH_TELEMETRY=on tokenash proxy
    tokenash proxy --telemetry
"""
...
from tokenash.telemetry.context import detect_install_mode, detect_stack
...
def is_telemetry_enabled() -> bool:
    ...
    Fail-closed: telemetry is only enabled when TOKENASH_TELEMETRY is set to an
    ...
    """
    val = os.environ.get("TOKENASH_TELEMETRY", "").lower().strip()
...
def is_telemetry_warn_enabled() -> bool:
    ...
    Set TOKENASH_TELEMETRY_WARN=off to suppress startup/wrap notices.
    ...
    """
    val = os.environ.get("TOKENASH_TELEMETRY_WARN", "on").lower().strip()
...
def format_telemetry_notice(*, prefix: str = "") -> str:
    ...
    return (
        ...
        "Disable: TOKENASH_TELEMETRY=off or --no-telemetry"
    )
...
class TelemetryBeacon:
    ...
    async def start(self) -> None:
        ...
        if not is_telemetry_enabled():
            logger.debug("Telemetry disabled (TOKENASH_TELEMETRY=off)")
            return
        self._task = asyncio.create_task(self._loop())
        logger.info(
            "Telemetry: ENABLED (anonymous aggregate stats, opt out: TOKENASH_TELEMETRY=off)"
        )
    ...
    async def _report(self) -> None:
        ...
        try:
            from tokenash._version import __version__ as tokenash_version
        except Exception:
            tokenash_version = "unknown"

        # Core identity (always present)
        payload: dict = {
            "session_id": self._session_id,
            "instance_id": self._instance_id,
            "tokenash_version": tokenash_version,
            "python_version": (
            ...
            "tokenash_stack": detect_stack(stats),
        }
        ...
        # --- Performance overhead (how much latency Tokenash adds) ---
        try:
        ...
        try:
            ...
            # Tokenash continues working perfectly without telemetry.
