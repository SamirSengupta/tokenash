...
statistics to the Tokenash cloud for billing. Designed to be non-intrusive:
...
logger = logging.getLogger("tokenash.telemetry.reporter")

# Grace period: if the cloud API is unreachable, use cached license for up to 7 days
GRACE_PERIOD_SECONDS = 7 * 24 * 3600  # 7 days

# Default cache location (workspace bucket, respects TOKENASH_WORKSPACE_DIR).
...
class UsageReporter:
    ...
    usage stats to the Tokenash cloud. If the cloud is unreachable, the proxy
    ...
    def __init__(
        self,
        license_key: str,
        cloud_url: str = "https://app.SamirSengupta.ai",
        ...
        self._proxy: TokenashProxy | None = None
    ...
    async def start(self, proxy: TokenashProxy) -> None:
    ...
    async def _get_client(self) -> httpx.AsyncClient:
        if self._http_client is None:
            self._http_client = httpx.AsyncClient(
                ...
                headers={"User-Agent": "tokenash-proxy"},
            )
