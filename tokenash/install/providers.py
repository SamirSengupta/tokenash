...
from tokenash.providers.install_registry import (
...
_ENV_MARKER_START = "# >>> tokenash persistent env >>>"
_ENV_MARKER_END = "# <<< tokenash persistent env <<<"
    ...
    for name, value in merged.items():
        previous = run(
            [
                ...
                "if ($null -eq $value) { '__TOKENASH_UNSET__' } else { $value }",
            ],
            capture_output=True,
            text=True,
            check=True,
        ...
        mutations.append(
            ManagedMutation(
                ...
                data={
                    "name": name,
                    "scope": scope_name,
                    "previous": None if previous == "__TOKENASH_UNSET__" else previous,
                },
            )
        )
    return mutations
