...
from tokenash.providers.install_registry import build_install_target_envs
...
def build_manifest(
    *,
    ...
    base_env = {
        "TOKENASH_PORT": str(port),
        "TOKENASH_HOST": "127.0.0.1",
        "TOKENASH_MODE": proxy_mode,
        "TOKENASH_BACKEND": backend,
    }
    if anyllm_provider:
        base_env["TOKENASH_ANYLLM_PROVIDER"] = anyllm_provider
    if region:
        base_env["TOKENASH_REGION"] = region
    ...
    base_env["TOKENASH_TELEMETRY"] = "on" if telemetry_enabled else "off"
    if memory_enabled:
        base_env["TOKENASH_MEMORY_ENABLED"] = "1"
    ...
    container_name = f"tokenash-{normalized_profile}"
    return DeploymentManifest(
        ...
        image=image,
        service_name=f"tokenash-{normalized_profile}",
