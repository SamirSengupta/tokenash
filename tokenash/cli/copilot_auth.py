...
from tokenash.copilot_auth import (
    DEFAULT_GITHUB_HOST,
    tokenash_copilot_auth_path,
    poll_copilot_device_authorization,
    read_tokenash_copilot_oauth_token,
    save_tokenash_copilot_oauth_token,
...
def copilot_auth() -> None:
    """Manage Tokenash's GitHub Copilot OAuth token."""
...
def login(domain: str) -> None:
    ...
    path = save_tokenash_copilot_oauth_token(token, domain=domain)
...
def status() -> None:
    """Show whether Tokenash has a saved Copilot OAuth token."""

    token = read_tokenash_copilot_oauth_token()
