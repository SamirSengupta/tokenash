...
from tokenash.install.models import ConfigScope, InstallPreset, RuntimeKind, SupervisorKind
from tokenash.install.paths import claude_settings_path, codex_config_path, validate_profile_name
from tokenash.install.planner import build_manifest
from tokenash.install.providers import _apply_unix_env_scope, _apply_windows_env_scope
...
from tokenash.install.state import load_manifest, save_manifest
from tokenash.install.supervisors import start_supervisor
from tokenash.providers.claude import TOOL_SEARCH_DEFAULT, TOOL_SEARCH_ENV
...
_VERBOSE_HANDLER_ATTR = "_tokenash_init_verbose_handler"

_GLOBAL_PROFILE = "init-user"
_CLAUDE_HOOK_MARKER = "tokenash-init-claude"
_COPILOT_HOOK_MARKER = "tokenash-init-copilot"
_CODEX_HOOK_MARKER = "tokenash-init-codex"
_CODEX_PROVIDER_MARKER_START = "# --- Tokenash init provider ---"
_CODEX_PROVIDER_MARKER_END = "# --- end Tokenash init provider ---"
_CODEX_FEATURE_MARKER_START = "# --- Tokenash init features ---"
_CODEX_FEATURE_MARKER_END = "# --- end Tokenash init features ---"
...
def _hook_command(*parts: str) -> str:
    return _command_string([*resolve_tokenash_command(), "init", "hook", "ensure", *parts])
...
def _enable_verbose_logging() -> None:
    ...
    mutate stdout; all verbose output goes to stderr so ``tokenash init``
    ...
    handler.setFormatter(logging.Formatter("[tokenash init] %(message)s"))
...
def _strip_codex_init_block(content: str) -> str:
    """Remove all Tokenash init-managed blocks and orphan keys from a Codex config.toml string."""
    import re
    ...
    # Strip any orphaned [model_providers.tokenash] table that is recognisably ours.
...
def _marketplace_source() -> str:
    override = os.environ.get("TOKENASH_MARKETPLACE_SOURCE")
    if override:
        return override
    repo_root = Path(__file__).resolve().parents[2]
    if (repo_root / ".claude-plugin" / "marketplace.json").exists():
        return str(repo_root)
    return "SamirSengupta/tokenash"
...
def _install_claude_marketplace(scope: str) -> None:
    ...
    _run_checked(
        [claude_bin, "plugin", "install", "tokenash@tokenash-marketplace", "--scope", scope],
...
def _install_copilot_marketplace() -> None:
    ...
    _run_checked(
        [copilot_bin, "plugin", "install", "tokenash@tokenash-marketplace"],
...
def _format_empty_detection_error(global_scope: bool) -> str:
    ...
    lines: list[str] = [
        ...
        "",
        "Tokenash probed the following agents via shutil.which():",
    ]
    ...
    lines.extend(
        [
            ...
            "tokenash init just needs to know which agent to target.",
    ...
    for name, _path in probes:
        ...
        lines.append(f"  tokenash init{flag} {name}")

    lines.extend(
        [
            "",
            "Tip: run `tokenash init --help` to see all options.",
        ]
    )
    return "\n".join(lines)
...
def _init_claude(*, global_scope: bool, profile: str, port: int) -> None:
    ...
    click.echo("Restart Claude Code to activate Tokenash hooks and provider routing.")
...
def _init_copilot(*, global_scope: bool, profile: str, port: int, backend: str) -> None:
    ...
    click.echo("Restart Copilot CLI to activate Tokenash hooks and provider routing.")
...
def _init_codex(*, global_scope: bool, profile: str, port: int) -> None:
    ...
    click.echo("Restart Codex to activate Tokenash configuration.")
...
def _init_openclaw(*, global_scope: bool, port: int) -> None:
    ...
    command = [*resolve_tokenash_command(), "wrap", "openclaw", "--proxy-port", str(port)]
...
def _run_init_targets(
    *,
    ...
    # Register the tokenash MCP server with every targeted agent that has
    ...
    _install_tokenash_mcp_for_targets(targets=targets, port=port)


def _install_tokenash_mcp_for_targets(*, targets: list[str], port: int) -> None:
    """Install the tokenash MCP server into each detected target agent."""
    from tokenash.mcp_registry import format_results, install_everywhere
...
@click.option("--port", default=8787, type=int, show_default=True, help="Tokenash proxy port.")
...
def init(
    ...
    """Install durable Tokenash integrations for supported agents."""
    if verbose:
...
def init_openclaw(ctx: click.Context) -> None:
    """Install the durable OpenClaw Tokenash plugin."""
    _run_init_targets(
        targets=["openclaw"],
        global_scope=bool(_ctx_value(ctx, "global_scope")),
        port=int(_ctx_value(ctx, "port") or 8787),
