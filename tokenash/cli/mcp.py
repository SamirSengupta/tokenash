...
Provides commands to configure and run the Tokenash MCP server, enabling
...
def get_tokenash_command() -> list[str]:
    """Get the command to run tokenash MCP server.

    Returns the CLI invocation used by Claude Code config.
    """
    return ["tokenash", "mcp", "serve"]
...
def mcp() -> None:
    ...
    \b
    The MCP server exposes tokenash_retrieve as a tool that Claude Code
    ...
    \b
    Quick Start:
        tokenash mcp install    # Configure Claude Code
        tokenash proxy          # Start the proxy (in another terminal)
    ...
    How it works:
        ...
        4. When Claude needs full details, it calls tokenash_retrieve
        5. The MCP server fetches original content from the proxy

    \b
    Note on tool naming: MCP clients display tools as
    `mcp__<server>__<tool>`. Our server is named "tokenash" and our
    tools are named tokenash_retrieve / tokenash_compress / etc., so
    Claude Code shows them as `mcp__tokenash__tokenash_retrieve`. The
    "tokenash" doubling is normal MCP namespacing — not a bug. The
    proxy's compression markers (and any docs/prompts) reference the
    bare tool name `tokenash_retrieve`.
    """
    pass
...
@click.option(
    ...
    help=f"Tokenash proxy URL (default: {DEFAULT_PROXY_URL})",
)
@click.option(
    "--agent",
    "agents",
...
@click.option(
    "--force",
    is_flag=True,
    help="Overwrite existing tokenash config in case of mismatch.",
)
def mcp_install(proxy_url: str, agents: tuple[str, ...], force: bool) -> None:
    """Install the Tokenash MCP server into every detected coding agent.
    ...
    \b
    Examples:
        tokenash mcp install                            # every detected agent
        tokenash mcp install --agent claude             # Claude Code only
        tokenash mcp install --proxy-url http://localhost:9000
    """
    try:
        import mcp  # noqa: F401
    except ImportError:
        click.echo("Error: MCP SDK not installed.", err=True)
        click.echo("Install with: pip install 'tokenash[mcp]'", err=True)
        raise SystemExit(1) from None

    from tokenash.mcp_registry import any_succeeded, format_results, install_everywhere
    ...
    click.echo("Installing Tokenash MCP server...")
    ...
    click.echo(
        f"\nNext steps:\n"
        f"  1. Start the Tokenash proxy (if not running): tokenash proxy\n"
...
def mcp_uninstall() -> None:
    """Remove Tokenash MCP server from Claude Code config.

    \b
    Removes tokenash from both the claude CLI registry (Claude Code CLI >=2.x)
    ...
    if claude_cli:
        check = subprocess.run(
            [claude_cli, "mcp", "get", "tokenash"],
            capture_output=True,
        )
        if check.returncode == 0:
            rm = run(
                [claude_cli, "mcp", "remove", "tokenash", "-s", "user"],
                capture_output=True,
                text=True,
            )
            if rm.returncode == 0:
                click.echo("✓ Tokenash MCP server removed (via claude mcp remove)")
                removed = True
            else:
                click.echo(
    ...
    if MCP_CONFIG_PATH.exists():
        ...
        changed = False
        for server_name in ("tokenash", "codebase-memory-mcp"):
    ...
    if not removed:
        if MCP_CONFIG_PATH.exists():
            click.echo("Tokenash MCP is not configured. Nothing to uninstall.")
        else:
...
@mcp.command("status")
def mcp_status() -> None:
    """Check Tokenash MCP configuration status.

    \b
    Shows whether tokenash is configured in Claude Code and if
    the proxy is reachable.
    """
    click.echo("Tokenash MCP Status")
    click.echo("=" * 40)
    ...
    try:
        ...
        click.echo("                pip install 'tokenash[mcp]'")

    # Check config
    if MCP_CONFIG_PATH.exists():
        config = load_mcp_config()
        if "tokenash" in config.get("mcpServers", {}):
            server_config = config["mcpServers"]["tokenash"]
            click.echo("Claude Config:  ✓ Configured")
            click.echo(f"                {MCP_CONFIG_PATH}")

            # Show proxy URL
            env = server_config.get("env", {})
            proxy_url = env.get("TOKENASH_PROXY_URL", DEFAULT_PROXY_URL)
            click.echo(f"Proxy URL:      {proxy_url}")
        else:
            click.echo("Claude Config:  ✗ Not configured")
            click.echo("                Run: tokenash mcp install")
    else:
    ...
    try:
        ...
        config = load_mcp_config()
        env = config.get("mcpServers", {}).get("tokenash", {}).get("env", {})
        proxy_url = env.get("TOKENASH_PROXY_URL", DEFAULT_PROXY_URL)
...
def mcp_serve(proxy_url: str | None, direct: bool, debug: bool) -> None:
    ...
    # Check for MCP SDK
    try:
        from tokenash.ccr.mcp_server import create_ccr_mcp_server
