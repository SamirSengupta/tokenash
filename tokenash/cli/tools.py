...
Exposes:
    ...
    tokenash sg …           ->  ast-grep (from the ast-grep-cli PyPI wheel)
    tokenash diff A B …     ->  difftastic
    tokenash loc [PATH] …   ->  scc
    tokenash tools install  ->  pre-fetch all bundled binaries
    tokenash tools doctor   ->  print a status table
    tokenash tools list     ->  show the registry
...
tool without any Tokenash-specific protocol.
"""
...
def _exec_tool(tool: str, argv: Sequence[str]) -> None:
    try:
        ...
        click.secho(
            f"error: {e}\nHint: run `tokenash tools install` on a networked machine, "
    ...
    # the parent `tokenash` process, not these thin passthroughs).
    cmd = [str(path), *argv]
    if not _is_windows():
    ...
    for name in selected:
        ...
        if binaries._is_pypi_tool(name):  # noqa: SLF001
            ...
            if on_path:
                ...
                click.secho(
                    f"{name}: not on PATH — `pip install tokenash` should provide it",
                    fg="yellow",
                )
                exit_code = 1
            continue
