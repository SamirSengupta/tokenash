...
def memory(ctx: click.Context) -> None:
    ...
    \b
    Examples:
        tokenash memory list                     List all stored memories
        tokenash memory list --limit 10          List the 10 most recent memories
        tokenash memory list --scope USER        List only USER-level memories
        tokenash memory list --since 7d          List memories from the last 7 days
        tokenash memory show <id>                Show full details of a memory
        tokenash memory stats                    Show memory statistics
        tokenash memory edit <id> --content ...  Edit a memory's content
        tokenash memory delete <id>              Delete a memory
        tokenash memory prune --older-than 30d   Delete memories older than 30 days
        tokenash memory purge --confirm          Delete ALL memories
        tokenash memory export --output file.json  Export all memories to JSON
        tokenash memory import file.json         Import memories from JSON
    """
    pass
...
def list_memories(
    ...
    \b
    Examples:
        tokenash memory list                  List most recent memories
        tokenash memory list --limit 10       Show only 10 memories
        tokenash memory list --scope USER     Show only USER-level memories
        tokenash memory list --since 7d       Show memories from the last 7 days
        tokenash memory list --search "api"   Search for memories containing "api"
    """
...
def show_memory(
    ...
    \b
    Examples:
        tokenash memory show abc123            Show memory details
        tokenash memory show abc123 --json     Output as raw JSON
    """
    store = get_store(db_path)
    ...
    try:
        ...
        if old_count > 0:
            console.print(
                ...
                f"Consider running: tokenash memory prune --older-than 30d[/dim]"
            )
...
def edit_memory(
    ...
    \b
    Examples:
        tokenash memory edit abc123 --content "Updated content"
        tokenash memory edit abc123 --importance 0.8
        tokenash memory edit abc123 -c "New content" -i 0.9
    """
...
def delete_memories(
    ...
    \b
    Examples:
        tokenash memory delete abc123           Delete one memory
        tokenash memory delete abc123 def456    Delete multiple memories
        tokenash memory delete abc123 --force   Skip confirmation
    """
    store = get_store(db_path)
...
def prune_memories(
    ...
    Examples:
        tokenash memory prune --older-than 30d              Delete memories older than 30 days
        tokenash memory prune --scope TURN                  Delete all TURN-level memories
        tokenash memory prune --low-importance 0.3          Delete low-importance memories
        tokenash memory prune --older-than 7d --scope TURN  Combine filters
        tokenash memory prune --older-than 30d --dry-run    Preview what would be deleted
    """
...
def export_memories(ctx: click.Context, db_path: str, output: str | None) -> None:
    ...
    \b
    Examples:
        tokenash memory export                       Output to stdout
        tokenash memory export --output backup.json  Save to file
        tokenash memory export -o backup.json        Save to file (short form)
    """
    store = get_store(db_path)
...
def import_memories(ctx: click.Context, db_path: str, file: str, force: bool) -> None:
    ...
    \b
    Examples:
        tokenash memory import backup.json          Import from file
        tokenash memory import backup.json --force  Skip confirmation
    """
    file_path = Path(file)
