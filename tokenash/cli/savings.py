...
Reads the append-only savings ledger (``~/.tokenash/savings_events.jsonl``,
...
def savings(as_json: bool, days: int, reset: bool) -> None:
    ...
    if calls == 0:
        ...
        click.echo(
            "Compress via the Tokenash MCP tool or route traffic through the "
            "proxy, then re-run `tokenash savings`."
        )
