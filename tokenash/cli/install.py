...
from tokenash.install.health import probe_json, probe_ready
from tokenash.install.models import (
    ConfigScope,
    DeploymentManifest,
    InstallPreset,
...
from tokenash.install.providers import apply_mutations, revert_mutations
...
from tokenash.install.state import delete_manifest, load_manifest, save_manifest
...
def _reject_task_lifecycle(manifest: DeploymentManifest, action: str) -> None:
    if manifest.supervisor_kind == SupervisorKind.TASK.value:
        raise click.ClickException(
            ...
            f"`tokenash install {action}` is not supported for task deployments."
        )
...
@click.option(
    ...
    show_default=True,
    help="Runtime used to execute Tokenash for service/task modes.",
)
@click.option(
    "--scope",
...
@click.option(
    "--image",
    default="ghcr.io/samirsengupta/tokenash:latest",
    show_default=True,
