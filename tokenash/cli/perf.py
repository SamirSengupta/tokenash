...
def perf(hours: float, raw: bool, output_format: str) -> None:
    ...
    \b
    Reads logs from ~/.tokenash/logs/proxy.log and shows:
    ...
    \b
    Examples:
        tokenash perf                      Analyze last 7 days
        tokenash perf --hours 24           Analyze last 24 hours
        tokenash perf --raw                Show raw parsed records
        tokenash perf --format json        Aggregated report as JSON
        tokenash perf --format csv --hours 24 > last-24h.csv
        tokenash perf --format json --raw  Raw records as a JSON array
    """
    ...
    if raw:
        ...
        if not report.perf_records:
            click.echo("No PERF records found. Run the proxy first: tokenash proxy")
    else:
