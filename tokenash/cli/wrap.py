"""Wrap CLI commands to run through Tokenash proxy.

Usage:
    tokenash wrap claude                    # Start proxy + context tool + claude
    tokenash wrap copilot -- --model ...    # Start proxy + launch GitHub Copilot CLI
    tokenash wrap codex                     # Start proxy + OpenAI Codex CLI
    tokenash wrap aider                     # Start proxy + aider
    tokenash wrap vibe                      # Start proxy + Mistral Vibe
    tokenash wrap cursor                    # Start proxy + print Cursor config instructions
    tokenash wrap openclaw                  # Install + configure OpenClaw plugin
    tokenash wrap claude --no-context-tool  # Without CLI context-tool setup
    tokenash wrap claude --port 9999        # Custom proxy port
    tokenash wrap claude -- --model opus    # Pass args to claude
"""
...
from tokenash._subprocess import run
...
from tokenash._version import __version__ as _TOKENASH_VERSION
from tokenash.agent_savings import (
    apply_agent_savings_env_defaults,
)
from tokenash.copilot_auth import (
    has_oauth_auth,
    resolve_client_bearer_token,
    resolve_copilot_api_url,
    resolve_subscription_bearer_token_details,
)
from tokenash.providers.aider import build_launch_env as _build_aider_launch_env
from tokenash.providers.claude import (
    TOOL_SEARCH_DEFAULT,
    TOOL_SEARCH_ENV,
)
from tokenash.providers.claude import (
...
from tokenash.providers.codex import build_launch_env as _build_codex_launch_env
from tokenash.providers.codex.install import codex_uses_chatgpt_auth
from tokenash.providers.codex.threads import retag_to_tokenash, retag_to_native
from tokenash.providers.copilot import (
...
from tokenash.providers.cursor import render_setup_lines as _render_cursor_setup_lines
from tokenash.providers.mistral_vibe import build_launch_env as _build_mistral_vibe_launch_env
from tokenash.providers.openclaw import (
...
from tokenash.providers.opencode import build_launch_env as _build_opencode_launch_env
from tokenash.providers.opencode.config import (
    ...
    strip_opencode_tokenash_blocks,
)
from tokenash.proxy.project_context import with_project_prefix as _with_project_prefix

from .main import main

_CONTEXT_TOOL_ENV = "TOKENASH_CONTEXT_TOOL"
...
_WRAP_PROXY_TIMEOUT_ENV = "TOKENASH_WRAP_PROXY_TIMEOUT"
...
def _selected_context_tool() -> str:
    ...
    ``TOKENASH_CONTEXT_TOOL=lean-ctx`` to let lean-ctx configure the supported
...
def _wrap_agent_savings_profile(agent_type: str) -> str | None:
    ...
    if agent_type not in _AGENT_SAVINGS_WRAP_AGENTS:
        return None
    return os.environ.get("TOKENASH_SAVINGS_PROFILE") or None
...
def _print_telemetry_notice() -> None:
    ...
    Respects the TOKENASH_TELEMETRY and TOKENASH_TELEMETRY_WARN feature flags.
    Does nothing when telemetry or warnings are disabled.
    """
    from tokenash.telemetry.beacon import format_telemetry_notice
...
def _check_proxy(port: int) -> bool:
    """Check if Tokenash proxy is running on given port."""
    try:
...
def _format_unbindable_port_error(port: int, error: OSError, agent_type: str) -> str:
    ...
    command = "tokenash proxy"
    if agent_type != "unknown":
        command = f"tokenash wrap {agent_type}"
...
def _get_log_path() -> Path:
    ...
    from tokenash import paths as _paths
...
def _start_proxy(
    ...
    *,
    ...
    """Start Tokenash proxy as a background subprocess.

    Stdout and stderr are written to a dedicated sibling file, usually
    `~/.tokenash/logs/proxy-stdio.log`, to avoid pipe deadlock risk without
    competing with the rotating `proxy.log` runtime log.
    """
    cmd = [sys.executable, "-m", "tokenash.cli", "proxy", "--port", str(port)]

    # Forward TOKENASH_MODE env var so the proxy respects the user's mode choice
    tokenash_mode = os.environ.get("TOKENASH_MODE")
    if tokenash_mode:
        cmd.extend(["--mode", tokenash_mode])
    ...
    _backend = backend or os.environ.get("TOKENASH_BACKEND")
    if _backend:
        cmd.extend(["--backend", _backend])

    _anyllm = anyllm_provider or os.environ.get("TOKENASH_ANYLLM_PROVIDER")
    if _anyllm:
        cmd.extend(["--anyllm-provider", _anyllm])

    _region = region or os.environ.get("TOKENASH_REGION")
    if _region:
    ...
    if agent_type != "unknown":
        proxy_env["TOKENASH_AGENT_TYPE"] = agent_type
        proxy_env.setdefault("TOKENASH_STACK", f"wrap_{agent_type}")
...
def _setup_rtk(verbose: bool = False) -> Path | None:
    ...
    from tokenash.rtk import get_rtk_path
    from tokenash.rtk.installer import ensure_rtk, register_claude_hooks
...
def _setup_lean_ctx_agent(agent: str, verbose: bool = False) -> Path | None:
    ...
    from tokenash.lean_ctx import get_lean_ctx_path
    from tokenash.lean_ctx.installer import ensure_lean_ctx
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="tokenash-lean-ctx-") as setup_cwd:
...
# Hook-command markers Tokenash manages in Claude settings.json. unwrap drops
# any hook entry whose command contains one of these.
_TOKENASH_HOOK_MARKERS = ("rtk-rewrite", "tokenash-init-claude")

# Env vars Tokenash's init/wrap inject into Claude settings.json; unwrap removes
# them. ENABLE_TOOL_SEARCH keeps Claude Code's tool deferral on behind the proxy
# (GH #746), paired with init/wrap setting it.
_TOKENASH_ENV_KEYS = ("ANTHROPIC_BASE_URL", "ENABLE_TOOL_SEARCH")


def _remove_claude_rtk_hooks(settings_path: Path | None = None) -> bool:
    """Remove Tokenash-managed entries from Claude settings.json.

    Reverses what ``tokenash init claude`` and ``rtk init --auto-patch`` add:
      * PreToolUse / SessionStart hooks whose command contains a Tokenash marker
        (``rtk-rewrite`` or ``tokenash-init-claude``), and
                ...
                retained_hooks = [
                    ...
                    if not (
                        ...
                        and any(
                            ...
                            for marker in _TOKENASH_HOOK_MARKERS
                        )
                    )
                ]
    ...
    if isinstance(env, dict):
        removed_keys = [k for k in _TOKENASH_ENV_KEYS if env.pop(k, None) is not None]
        if removed_keys:
            changed = True
            if env:
                payload["env"] = env
...
def _write_claude_wrap_base_url(
    ...
    *,
    ...
    routes through the Tokenash proxy without touching the global user settings
...
def _setup_tokenash_mcp(
    registrar: Any, port: int, *, verbose: bool = False, force: bool = False
) -> None:
    """Register the tokenash MCP server with the given agent (idempotent).
    ...
    nothing — the agent has no ``tokenash_retrieve`` tool to call.
    ...
    """
    from tokenash.mcp_registry import build_tokenash_spec, format_result
    ...
    spec = build_tokenash_spec(proxy_url)
    ...
    line = format_result(
        ...
        verbose=verbose,
        overwrite_hint=f"tokenash mcp install --proxy-url {proxy_url} --force",
...
def _setup_serena_mcp(
    ...
    A prior ``tokenash wrap`` may have persisted a Serena entry built from an
    ...
    config is one Tokenash installed, force-update it to the current spec. A
    user-managed Serena (absent from our ledger) is left untouched and the
    mismatch is reported as before.
    """
    from tokenash.mcp_registry import build_serena_spec, format_result
    from tokenash.mcp_registry.base import RegisterStatus
    from tokenash.mcp_registry.ledger import tokenash_installed_matching, record_install
    ...
    # Migrate a stale Tokenash-installed entry. register_server won't overwrite
    # a differing spec without force, so an older Tokenash Serena entry would
    # otherwise persist across re-wraps. Force-update it only when the ledger
    # proves Tokenash installed the entry that's currently on disk — never a
    # user-managed Serena.
    if (
        result.status == RegisterStatus.MISMATCH
        and not force
        and tokenash_installed_matching(registrar.name, registrar.get_server("serena"))
    ):
    ...
    line = format_result(
        ...
        label="Serena MCP",
        verbose=verbose,
        overwrite_hint="update or remove the existing serena MCP entry, then rerun tokenash wrap",
        restart_hint=f"restart {registrar.display_name} if it was already running",
    )
    if line is not None:
        click.echo(line)
...
def _remove_tokenash_installed_serena_mcp(registrar: Any) -> str:
    """Remove Serena MCP only if the ledger proves Tokenash installed it."""
    from tokenash.mcp_registry.ledger import clear_install, tokenash_installed_matching

    current = registrar.get_server("serena")
    if not tokenash_installed_matching(registrar.name, current):
        return "not_tokenash_owned"
...
def _disable_serena_mcp(registrar: Any, *, verbose: bool = False) -> None:
    ...
    Serena is registered by default, so a prior ``tokenash wrap`` persists a
    ...
    remove the entry Tokenash installed. A user-managed Serena (absent from
    ...
    status = _remove_tokenash_installed_serena_mcp(registrar)
    if status == "removed":
    ...
    elif status == "not_tokenash_owned":
        click.echo(
            "  Serena MCP is present but user-managed — leaving it in place "
            "(--no-serena only removes entries Tokenash installed)"
        )
    else:  # "failed"
        click.echo(
...
def _setup_code_graph(verbose: bool = False) -> bool:
    ...
    """
    from tokenash.graph.installer import ensure_cbm, get_cbm_path
...
_RTK_MARKER = "<!-- tokenash:rtk-instructions -->"

# Memory MCP markers
_MEMORY_MCP_MARKER = "# --- Tokenash memory MCP (auto-injected) ---"
_MEMORY_MCP_END = "# --- end Tokenash memory ---"
_MEMORY_AGENTS_MARKER = "<!-- tokenash:memory-instructions -->"

# Codex config injection markers
_CODEX_TOP_LEVEL_MARKER = "# --- Tokenash proxy (auto-injected by tokenash wrap codex) ---"
_CODEX_END_MARKER = "# --- end Tokenash ---"
_CODEX_MCP_MARKER = "# --- Tokenash MCP server ---"
_CODEX_MCP_END = "# --- end Tokenash MCP server ---"
# File name used for the pre-wrap snapshot of the Codex config file.  The
# snapshot lets `tokenash unwrap codex` restore the exact prior state, even
# if the user had their own `model_provider` / `[model_providers.*]` config
# before running wrap.
_CODEX_CONFIG_BACKUP_SUFFIX = ".tokenash-backup"
...
def _strip_codex_tokenash_blocks(
    content: str,
    *,
    remove_mcp: bool = False,
    remove_named_mcp: bool = True,
) -> str:
    """Remove all Tokenash-managed blocks from a Codex ``config.toml`` string.
    ...
    if remove_mcp:
        # Remove Tokenash-managed MCP blocks written by `wrap codex`.
        content = _remove_marker_span(content, _CODEX_MCP_MARKER, _CODEX_MCP_END)
        if remove_named_mcp:
            content = re.sub(
                r"(?ms)^# --- Tokenash MCP server: [^\n]+ ---\n.*?"
                r"^# --- end Tokenash MCP server: [^\n]+ ---\n?",
                "",
                content,
            )
    ...
    content = re.sub(r'(?m)^[ \t]*model_provider[ \t]*=[ \t]*"tokenash"[ \t]*\r?\n', "", content)
    content = re.sub(
    ...
    # Strip any orphaned `[model_providers.tokenash]` table with the fields we
    # write.  We only remove it if the table is recognisably ours (base_url
    # mentions localhost and a Tokenash proxy port).  This protects users who
    # happen to have a differently configured `tokenash` provider.
    orphan_tokenash_table = re.compile(
        r"(?ms)^\[model_providers\.tokenash\][^\[]*?"
        r'base_url[ \t]*=[ \t]*"http://127\.0\.0\.1:\d+/v1"[^\[]*?'
        r"(?=^\[|\Z)"
    )
    content = orphan_tokenash_table.sub("", content)

    return content.lstrip("\n").rstrip() + "\n" if content.strip() else ""


# Top-level bare keys we redirect to tokenash values when the user already
...
def _redirect_existing_top_level_keys(content: str, port: int) -> str:
    ...
    which would break ``codex`` startup after ``tokenash wrap codex`` runs
    ...
    the value with the tokenash one and append ``# was: <original-value>``
    ...
    pre-wrap file can be restored byte-for-byte on ``tokenash unwrap
    codex``.
...
def _codex_config_has_tokenash_markers(content: str) -> bool:
...
def _snapshot_codex_config_if_unwrapped(config_file: Path, backup_file: Path) -> None:
    ...
    Called as the first step of every Tokenash injection into Codex's
    ``config.toml``.  Guarantees that ``tokenash unwrap codex`` can restore the
    ...
    * If the config already contains any Tokenash-managed Codex marker, a wrap
    ...
    try:
        ...
        return
    if _codex_config_has_tokenash_markers(content):
        return
...
def _ensure_rtk_binary(verbose: bool = False) -> Path | None:
    ...
    from tokenash.rtk import get_rtk_path
    from tokenash.rtk.installer import ensure_rtk
...
# case-insensitively by tokenash.proxy.project_context.PROJECT_HEADER).
_PROJECT_HEADER_NAME = "X-Tokenash-Project"


def _project_name_from_cwd() -> str | None:
    """Project label for X-Tokenash-Project: basename of the launch directory.
...
def _apply_project_header_env(env: dict[str, str]) -> None:
    """Inject X-Tokenash-Project into ``ANTHROPIC_CUSTOM_HEADERS``.
    ...
    Tokenash proxy uses the X-Tokenash-Project header for per-project
    savings attribution.  An existing user-supplied x-tokenash-project
...
def _inject_codex_provider_config(port: int) -> None:
    """Inject a Tokenash model provider into Codex's config.toml.

    Two keys need to be in effect for the proxy to route all traffic:

    * ``model_provider = "tokenash"`` — selects the custom provider for
    ...
    * ``openai_base_url = "http://127.0.0.1:{port}/v1"`` — overrides the
    ...
    in place to the tokenash values (with the previous value kept in a
    ...
    ``config.toml.tokenash-backup`` so ``tokenash unwrap codex``
    ...
    provider_section = (
        ...
        "[model_providers.tokenash]\n"
        'name = "OpenAI via Tokenash proxy"\n'
        f'base_url = "http://127.0.0.1:{port}/v1"\n'
        f"supports_websockets = true\n"
        f"{requires_openai_auth}"
        # Per-project savings: Codex sends the header only when the mapped
        # env var (TOKENASH_PROJECT, set by `tokenash wrap codex`) exists at
        # Codex runtime.  Inline table keeps the key inside this section so
        # _strip_codex_tokenash_blocks removes it with the rest of the block.
        f'env_http_headers = {{ "{_PROJECT_HEADER_NAME}" = "TOKENASH_PROJECT" }}\n'
        f"{_CODEX_END_MARKER}\n"
    )

    # The two redirectable keys and their tokenash target values.
    _REDIRECT_TARGETS = {
        "model_provider": "tokenash",
    ...
    try:
        ...
        if config_file.exists():
            content = config_file.read_text()
            # Remove any prior Tokenash-managed blocks before re-injecting so
            # the operation is idempotent and supports port changes.
            content = _strip_codex_tokenash_blocks(content)
            ...
            # to the tokenash values; for keys the user has not declared,
        ...
        click.echo(f"  Codex config: injected Tokenash provider (WS + HTTP) into {config_file}")
        # Pull existing native threads into the tokenash-provider menu so Codex's
        # history list stays whole once it routes through Tokenash. Best-effort.
        retag_to_tokenash(_codex_home_dir())
    except Exception as e:
...
def _restore_codex_provider_config() -> tuple[str, Path]:
    ...
    * ``"cleaned"``  — no backup existed, but the Tokenash-managed block was
      found and stripped out (preserving surrounding user content).
    * ``"removed"``  — the config file only contained Tokenash-managed
      content (created by wrap) and has been deleted.
    * ``"noop"``     — nothing to undo; no Tokenash marker and no backup.
    """
    config_file, backup_file = _codex_config_paths()
    ...
    if config_file.exists():
        ...
        if _codex_config_has_tokenash_markers(original):
            ...
            cleaned = _strip_codex_tokenash_blocks(
                original,
                remove_mcp=True,
                remove_named_mcp=remove_named_mcp,
            )
            if not cleaned.strip():
                # Nothing left but Tokenash content — remove the file entirely
...
def _emit_wrap_interrupted(agent: str, marker_path: Path | None) -> None:
    ...
    re-running the same ``tokenash wrap <agent>`` command is idempotent and
    safe.
    """
    if marker_path is not None:
        click.echo(
            f"\n  Wrap was interrupted; marker file at {marker_path} is on "
            f"disk. Rerun `tokenash wrap {agent}` to retry — it's idempotent."
        )
    else:
        click.echo(
            f"\n  Wrap was interrupted before any on-disk changes. Rerun "
            f"`tokenash wrap {agent}` to retry — it's idempotent."
        )
...
def _print_wrap_banner(agent: str) -> None:
    """Print a centered ``TOKENASH WRAP: <AGENT>`` banner.
    ...
    """
    title = f"TOKENASH WRAP: {agent.upper()}"
...
def _remove_rtk_instructions(file_path: Path) -> bool:
    """Remove Tokenash's marker-fenced rtk guidance from an instruction file."""
    if not file_path.exists():
        return False

    content = file_path.read_text(encoding="utf-8")
    end_marker = "<!-- /tokenash:rtk-instructions -->"
...
def _inject_memory_mcp_config(user_id: str) -> None:
    """Register tokenash memory as an MCP server in Codex's config.toml.
    ...
    mcp_section = (
        f"\n{_MEMORY_MCP_MARKER}\n"
        f"[mcp_servers.tokenash_memory]\n"
        f'command = "{python_bin}"\n'
        f'args = ["-m", "tokenash.memory.mcp_server", "--user", "{user_id}"]\n'
...
def _inject_memory_agents_md(file_path: Path) -> bool:
    ...
    memory_block = (
        ...
        "## Memory\n\n"
        "Use the `tokenash_memory` MCP server for persistent cross-session knowledge.\n\n"
...
def _inject_continue_rtk_systemmessage(config_file: Path, verbose: bool = False) -> bool:
    ...
    * each ``models[i].systemMessage`` where ``models[i]`` is a dict
    ...
    The RTK marker (``<!-- tokenash:rtk-instructions -->``) is the idempotency
...
def _proxy_health_config(payload: dict[str, Any] | None) -> dict[str, Any] | None:
    """Extract the config block from a Tokenash /health payload."""
    if payload is None:
        return None
...
def _agent_savings_config_mismatches(
    ...
    checks: tuple[tuple[str, str, str, str], ...] = (
        ("TOKENASH_SAVINGS_PROFILE", "savings_profile", "savings-profile", "str"),
        ("TOKENASH_TARGET_RATIO", "target_ratio", "target-ratio", "float"),
        (
            "TOKENASH_COMPRESS_USER_MESSAGES",
            "compress_user_messages",
            "compress-user-messages",
            "bool",
        ),
        (
            "TOKENASH_COMPRESS_SYSTEM_MESSAGES",
            "compress_system_messages",
            "compress-system-messages",
            "bool",
        ),
        ("TOKENASH_PROTECT_RECENT", "protect_recent", "protect-recent", "int"),
        (
            "TOKENASH_PROTECT_ANALYSIS_CONTEXT",
            "protect_analysis_context",
            "protect-analysis-context",
            "bool",
        ),
        ("TOKENASH_MIN_TOKENS", "min_tokens_to_crush", "min-tokens", "int"),
        ("TOKENASH_MAX_ITEMS", "max_items_after_crush", "max-items", "int"),
        (
            "TOKENASH_SMART_CRUSHER_COMPACTION",
            "smart_crusher_with_compaction",
            "smart-crusher-compaction",
            "bool",
        ),
        ("TOKENASH_ACCURACY_GUARD", "accuracy_guard", "accuracy-guard", "str"),
    )
...
def _proxy_needs_version_restart(payload: dict[str, Any] | None) -> bool:
    """Return True when a running Tokenash proxy uses a different package version."""
    running_version = _proxy_version(payload)
    return (
        running_version is not None
        and running_version != "unknown"
        and _TOKENASH_VERSION != "unknown"
        and running_version != _TOKENASH_VERSION
    )
...
def _stop_local_proxy_for_unwrap(port: int) -> str:
    """Stop a local Tokenash proxy for durable unwrap commands.

    Returns a status string:
      * ``"stopped"``: a Tokenash proxy was identified and stopped.
      * ``"not_running"``: nothing is listening on the requested port.
      * ``"unidentified"``: something is listening, but it did not expose
        Tokenash's health/config payload, so we did not kill it.
      * ``"no_pid"``: the service looked like Tokenash but did not expose a PID.
...
def _echo_unwrap_proxy_stop_status(status: str, port: int) -> None:
    ...
    if status == "stopped":
        click.echo(f"  Stopped local Tokenash proxy on port {port}.")
    elif status == "not_running":
        click.echo(f"  No local Tokenash proxy detected on port {port}.")
    elif status == "unidentified":
        click.echo(
            f"  Warning: port {port} is in use, but it did not look like Tokenash; left it running."
        )
    elif status == "no_pid":
        click.echo(
            f"  Warning: Tokenash proxy on port {port} did not expose a PID; left it running."
        )
    else:
        click.echo(f"  Warning: failed to stop Tokenash proxy on port {port}; stop it manually.")


def _find_persistent_manifest(port: int) -> Any:
    """Return a matching persistent deployment manifest for the requested port."""
    from tokenash.install.state import list_manifests
...
def _recover_persistent_proxy(port: int) -> bool:
    ...
    from tokenash.install.health import probe_ready
    from tokenash.install.models import InstallPreset, SupervisorKind
    from tokenash.install.runtime import start_detached_agent, start_persistent_docker, wait_ready
    from tokenash.install.supervisors import start_supervisor
...
def _restart_persistent_proxy(manifest: Any, port: int) -> bool:
    ...
    click.echo(
        ...
        f"with Tokenash {_TOKENASH_VERSION}..."
    )
    try:
        if manifest.preset == InstallPreset.PERSISTENT_DOCKER.value:
...
def _should_use_copilot_oauth(
    *,
    ...
    effective_backend = backend or os.environ.get("TOKENASH_BACKEND")
...
def _push_runtime_env(port: int, no_proxy: bool) -> None:
    ...
    if no_proxy:
        return
    from tokenash.proxy import runtime_env as _rt
    ...
    if not no_proxy:
            ...
            if probe_ready(manifest.health_url):
                ...
                if helpers._proxy_needs_version_restart(health_payload):
                    ...
                    if active_sessions > 0 or other_wrappers:
                        ...
                        click.echo(
                            f"  Proxy on port {port} is running Tokenash {running_version}; "
                            f"current CLI is {_TOKENASH_VERSION}."
                        )
                        click.echo(
                    ...
                    raise click.ClickException(
                        ...
                        f"is running stale Tokenash {running_version} and could not be restarted."
                    )
        ...
        if helpers._check_proxy(port):
            ...
            if helpers._proxy_needs_version_restart(health_payload):
                ...
                click.echo(
                    f"  Proxy on port {port} is running Tokenash {running_version}; "
                    f"restarting with {_TOKENASH_VERSION}..."
                )
        ...
        click.echo(f"  Starting Tokenash proxy on port {port}...")
        try:
            proc = cast(
...
def _make_cleanup(proxy_proc_holder: list, port: int = 8787) -> Any:
    ...
    Only kills the proxy when no other live tokenash-wrapped clients remain,
...
def _launch_tool(
    ...
    *,
    ...
    try:
        click.echo()
        padded = f"TOKENASH WRAP: {tool_label}".center(47)
        click.echo("  ╔═══════════════════════════════════════════════╗")
        click.echo(f"  ║{padded}║")
        click.echo("  ╚═══════════════════════════════════════════════╝")
        click.echo()
        ...
        click.echo()
        click.echo(f"  Launching {tool_label} (API routed through Tokenash)...")
...
def _build_openclaw_plugin_entry(
    *,
    ...
    """Merge managed Tokenash plugin settings with any existing entry payload."""
...
def _write_openclaw_plugin_entry(openclaw_bin: str, entry: dict[str, object]) -> None:
    """Persist the Tokenash plugin config entry."""
    _run_checked(
        [
            openclaw_bin,
            "config",
            "set",
            "plugins.entries.tokenash",
            json.dumps(entry, separators=(",", ":")),
            "--strict-json",
        ],
        action="openclaw config set plugins.entries.tokenash",
    )
...
def _copy_openclaw_plugin_into_extensions(
    *,
    ...
    target_dir = extensions_dir / "tokenash"
...
@main.group()
def wrap() -> None:
    """Wrap CLI tools to run through Tokenash.

    \b
    Starts a Tokenash proxy, configures the environment, and launches
    the target tool so all API calls route through Tokenash automatically.

    \b
    Supported tools (one Click subcommand per tool):
        tokenash wrap claude              # Claude Code (Anthropic)
        tokenash wrap codex               # OpenAI Codex CLI
        tokenash wrap copilot -- --model claude-sonnet-4-20250514
        tokenash wrap aider               # Aider
        tokenash wrap vibe                # Mistral Vibe
        tokenash wrap cursor              # Cursor (prints config instructions)
        tokenash wrap cline               # Cline (VS Code; prints config instructions)
        tokenash wrap continue            # Continue (VS Code/JetBrains; injects systemMessage)
        tokenash wrap goose               # Goose (Block) CLI
        tokenash wrap openhands           # OpenHands CLI
        tokenash wrap openclaw            # OpenClaw plugin bootstrap
        tokenash wrap opencode            # OpenCode CLI

    \b
    `wrap` vs `proxy`:
        - `tokenash wrap <tool>` — convenience: starts the proxy for you,
          sets the right env vars, and launches the wrapped CLI.
        - `tokenash proxy` — just the proxy. Use this with any
...
@main.group()
def unwrap() -> None:
    """Undo durable Tokenash wrapping for supported tools."""
...
@click.option(
    "--no-mcp",
    is_flag=True,
    help="Skip tokenash MCP server registration (compression markers will be unactionable)",
)
...
@click.option(
    ...
    "(env: TOKENASH_BACKEND). For Vertex, prefer CLAUDE_CODE_USE_VERTEX=1 (native, "
    "keeps your GCP auth) over a litellm backend.",
)
@click.option(
    "--region",
    default=None,
    help="Cloud region for Vertex/Bedrock backends (env: TOKENASH_REGION).",
)
...
def claude(
    ...
    """Launch Claude Code through Tokenash proxy.

    \b
    Sets ANTHROPIC_BASE_URL to route all Anthropic API calls through Tokenash.
    All unknown flags are passed through to claude (e.g. --resume, --model).

    \b
    Examples:
        tokenash wrap claude                    # Start everything
        tokenash wrap claude --memory           # With persistent memory
        tokenash wrap claude --resume <id>      # Resume a session
        tokenash wrap claude -- -p              # Claude in print mode
        tokenash wrap claude --code-graph        # With code graph intelligence
        tokenash wrap claude --no-context-tool  # Skip CLI context-tool setup
        tokenash wrap claude --no-mcp           # Skip MCP retrieve tool registration
        tokenash wrap claude --no-serena        # Skip Serena MCP registration
    """
    if prepare_only:
        if not no_rtk:
            if _selected_context_tool() == _CONTEXT_TOOL_LEAN_CTX:
    ...
    # Memory sync BEFORE proxy startup — sync tokenash DB ↔ Claude's files
    if memory:
        try:
            mem_dir = Path.cwd() / ".tokenash"
    ...
    try:
        click.echo()
        click.echo("  ╔═══════════════════════════════════════════════╗")
        click.echo("  ║            TOKENASH WRAP: CLAUDE              ║")
        click.echo("  ╚═══════════════════════════════════════════════╝")
        click.echo()
        ...
        # Tokenash and the proxy compresses the request, then forwards to the
        ...
        # by Tokenash. This is the turnkey Vertex compression path.
        ...
        if not no_mcp:
            from tokenash.mcp_registry import ClaudeRegistrar

            _setup_tokenash_mcp(ClaudeRegistrar(), port, verbose=verbose)
        elif verbose:
        ...
        click.echo()
        click.echo("  Launching Claude Code (API routed through Tokenash)...")
        if use_vertex:
            click.echo(
        ...
        if use_vertex:
            ...
            # we only redirect its Vertex endpoint to Tokenash.
        ...
        # daemon's environment) also route through Tokenash.
        ...
        # directory's name via X-Tokenash-Project (user override wins).
...
@click.option("--port", "-p", default=8787, type=int, help="Proxy port (default: 8787)")
@click.option("--no-stop-proxy", is_flag=True, help="Do not stop the local Tokenash proxy")
@click.option("--keep-mcp", is_flag=True, help="Keep Tokenash MCP registrations")
...
def unwrap_claude(
    ...
    """Undo durable setup from ``tokenash wrap claude``."""
    click.echo()
    click.echo("  ╔═══════════════════════════════════════════════╗")
    click.echo("  ║          TOKENASH UNWRAP: CLAUDE              ║")
    click.echo("  ╚═══════════════════════════════════════════════╝")
    click.echo()
    ...
    if not keep_mcp:
        ...
        if registrar.detect():
            removed_tokenash = registrar.unregister_server("tokenash")
            removed_code_graph = registrar.unregister_server(_CBM_MCP_SERVER_NAME)
            serena_status = _remove_tokenash_installed_serena_mcp(registrar)
            if removed_tokenash:
                click.echo("  Removed Tokenash MCP retrieve tool from Claude.")
            else:
                click.echo("  Tokenash MCP retrieve tool was not registered in Claude.")
            if removed_code_graph:
                click.echo("  Removed code graph MCP server from Claude.")
            if serena_status == "removed":
                click.echo("  Removed Tokenash-installed Serena MCP server from Claude.")
            elif serena_status == "failed":
                click.echo("  Serena MCP server matched Tokenash ledger but could not be removed.")
        else:
    ...
    click.echo()
    click.echo("✓ Claude is no longer durably wrapped by Tokenash.")
...
@click.option(
    "--backend",
    default=None,
    help="API backend for the proxy: 'anthropic', 'anyllm', 'litellm-vertex', etc. (env: TOKENASH_BACKEND)",
)
@click.option(
    "--anyllm-provider",
    default=None,
    help="Provider for any-llm backend: openai, mistral, groq, etc. (env: TOKENASH_ANYLLM_PROVIDER)",
)
@click.option(
    "--region", default=None, help="Cloud region for Bedrock/Vertex (env: TOKENASH_REGION)"
)
@click.option(
    "--provider-type",
...
@click.option(
    ...
    is_flag=True,
    help=(
        "Experimental: route GitHub-authenticated Copilot CLI traffic through Tokenash "
...
def copilot(
    ...
    """Launch GitHub Copilot CLI through Tokenash proxy.

    \b
    Configures Copilot CLI BYOK provider variables so Copilot routes through
    the local Tokenash proxy. In auto mode, the wrapper uses Anthropic-style
    ...
    Examples:
        tokenash wrap copilot -- --model claude-sonnet-4-20250514
        tokenash wrap copilot --backend anyllm --anyllm-provider groq -- --model gpt-4o
        tokenash wrap copilot --provider-type openai --wire-api responses -- --model gpt-5.4
        tokenash wrap copilot --subscription -- --model gpt-4.1
        tokenash wrap copilot --no-context-tool -- --prompt "explain this file"
    ...
    if _should_use_copilot_oauth(
        ...
        if not client_bearer:
            raise click.ClickException(
                ...
                "token, but none could be resolved. Run `tokenash copilot-auth login` first, or set "
        ...
        if not env.get("COPILOT_PROVIDER_API_KEY"):
            ...
            click.echo(
                ...
                f"  `tokenash wrap copilot` uses Copilot's BYOK mode, which bypasses GitHub's\n"
                f"  Copilot API and routes requests directly to the model provider through the\n"
                f"  Tokenash proxy. A GitHub Copilot subscription alone is not sufficient.\n\n"
                f"  Set one of:\n"
    ...
    if not subscription and not _copilot_model_configured(copilot_args, env):
        ...
        if _is_auto_model(raw_model):
            click.echo(
                ...
                "      tokenash wrap copilot --subscription -- --model auto"
            )
            raise SystemExit(1)
        else:
            click.echo(
...
def unwrap_copilot(port: int, no_stop_proxy: bool) -> None:
    """Undo durable setup from ``tokenash wrap copilot``."""
    instructions = Path.cwd() / ".github" / "copilot-instructions.md"
    if _remove_rtk_instructions(instructions):
        click.echo("  Removed Tokenash rtk instructions from Copilot.")
    else:
        click.echo("  No Tokenash rtk instructions found for Copilot.")
...
def codex(
    ...
    """Launch OpenAI Codex CLI through Tokenash proxy.

    \b
    Sets OPENAI_BASE_URL to route all OpenAI API calls through Tokenash.
    Sets up the selected CLI context tool so Codex uses token-optimized
    commands (60-90% savings on shell output). Also
    registers the tokenash MCP server in the active Codex config file
    so Codex can call ``tokenash_retrieve`` on compression markers.

    \b
    Examples:
        tokenash wrap codex                         # Start proxy + context tool + mcp + codex
        tokenash wrap codex -- "fix the bug"        # Pass prompt to codex
        tokenash wrap codex --no-context-tool       # Skip CLI context-tool setup
        tokenash wrap codex --no-mcp                # Skip MCP retrieve tool registration
        tokenash wrap codex --no-serena             # Skip Serena MCP registration
        tokenash wrap codex --port 9999             # Custom proxy port
        tokenash wrap codex --backend anyllm --anyllm-provider groq
    """
    # Snapshot Codex config.toml BEFORE any wrap-time mutation so
    # `tokenash unwrap codex` can restore the user's pre-wrap state
    # byte-for-byte. The snapshot is a no-op if the backup already exists
    # or if the file already has Tokenash markers, so this is safe to
    ...
    # Register tokenash MCP server in Codex config.toml so Codex can
    # call tokenash_retrieve on compression markers from the proxy.
    if not no_mcp:
        from tokenash.mcp_registry import CodexRegistrar
        ...
        _setup_tokenash_mcp(CodexRegistrar(), port, verbose=verbose, force=True)
    elif verbose:
        click.echo("  Skipping MCP retrieve tool (--no-mcp)")
    ...
    if memory:
        ...
        try:
            ...
            from tokenash.memory.sync import _build_sync_backend, sync_import
            from tokenash.memory.sync_adapters.claude_code import (
                ClaudeCodeAdapter,
    ...
    # X-Tokenash-Project header to TOKENASH_PROJECT via env_http_headers, so
    # Codex sends it only when this var is set.  A user-set value wins.
    _codex_project = _project_name_from_cwd()
    if _codex_project and "TOKENASH_PROJECT" not in env:
        env["TOKENASH_PROJECT"] = _codex_project

    # Inject Tokenash provider into Codex config so WebSocket traffic also
...
def aider(
    ...
    """Launch aider through Tokenash proxy.

    \b
    Sets OPENAI_API_BASE to route all API calls through Tokenash.
    Sets up the selected CLI context tool so aider uses token-optimized commands.

    \b
    Examples:
        tokenash wrap aider                              # Start proxy + context tool + aider
        tokenash wrap aider -- --model gpt-4o            # Use GPT-4o
        tokenash wrap aider -- --model claude-sonnet-4   # Use Claude
        tokenash wrap aider --no-context-tool            # Skip CLI context-tool setup
        tokenash wrap aider --backend litellm-vertex --region us-central1
    """
...
def vibe(
    ...
    """Launch Mistral Vibe through Tokenash proxy.

    \b
    Sets VIBE_PROVIDERS to route all Mistral API calls through Tokenash.

    \b
    Examples:
        tokenash wrap vibe                         # Start proxy + vibe
        tokenash wrap vibe -- "fix the bug"        # Pass prompt to vibe
        tokenash wrap vibe --port 9999             # Custom proxy port
        tokenash wrap vibe --no-context-tool       # Skip CLI context-tool setup
    """
    if prepare_only:
        return
...
def cursor(
    ...
    """Start Tokenash proxy for use with Cursor.
    ...
    \b
    Example:
        tokenash wrap cursor                # Start proxy + context-tool instructions
        tokenash wrap cursor --no-context-tool  # Proxy only, no CLI context tool
        tokenash wrap cursor --port 9999    # Custom proxy port
    """
...
def cline(
    ...
    """Start Tokenash proxy for use with Cline (VS Code extension).
    ...
    the API Base URL to point at the local Tokenash proxy.

    \b
    Uninstall: there is no ``tokenash unwrap cline`` subcommand. To remove the
    injected guidance, hand-edit ``.clinerules`` at the project root and
    delete everything between ``<!-- tokenash:rtk-instructions -->`` and
    ``<!-- /tokenash:rtk-instructions -->`` (inclusive). If ``lean-ctx`` mode
    ...
    \b
    Examples:
        tokenash wrap cline                  # Start proxy + .clinerules instructions
        tokenash wrap cline --no-context-tool # Proxy only, no CLI context tool
        tokenash wrap cline --port 9999      # Custom proxy port
    """
...
def continue_dev(
    ...
    """Start Tokenash proxy for use with Continue (VS Code / JetBrains).
    ...
    \b
    Uninstall: there is no ``tokenash unwrap continue`` subcommand. To remove
    the injected guidance, hand-edit ``.continue/config.json`` and delete
    everything between ``<!-- tokenash:rtk-instructions -->`` and
    ``<!-- /tokenash:rtk-instructions -->`` (inclusive) from every
    ...
    \b
    Examples:
        tokenash wrap continue                # Start proxy + inject systemMessage
        tokenash wrap continue --no-context-tool   # Proxy only
        tokenash wrap continue --port 9999    # Custom proxy port
        tokenash wrap continue --config path/to/config.json
    """
...
def goose(
    ...
    """Launch Goose (Block) CLI through Tokenash proxy.

    \b
    Sets OPENAI_BASE_URL and ANTHROPIC_BASE_URL to route Goose's API calls
    through Tokenash. Sets up the selected CLI context tool by injecting RTK
    guidance into .goosehints at the project root (Goose reads this file as
    extra system context).

    \b
    Uninstall: there is no ``tokenash unwrap goose`` subcommand. To remove the
    ...
    \b
    Examples:
        tokenash wrap goose                          # Start proxy + context tool + goose
        tokenash wrap goose -- session               # Start a Goose session
        tokenash wrap goose -- --provider anthropic  # Pass args to goose
        tokenash wrap goose --no-context-tool        # Skip CLI context-tool setup
    """
...
def openhands(
    ...
    """Launch OpenHands CLI through Tokenash proxy.

    \b
    Sets OPENAI_BASE_URL / ANTHROPIC_BASE_URL to route OpenHands' API calls
    through Tokenash. Instructions are injected via the
    ...
    ``<!-- tokenash:rtk-instructions -->`` marker. To uninstall, simply do not
    ...
    \b
    Examples:
        tokenash wrap openhands                # Start proxy + context tool + openhands
        tokenash wrap openhands -- --task ...  # Pass args to openhands
        tokenash wrap openhands --no-context-tool
    """
...
@click.option("--proxy-port", default=8787, type=int, help="Tokenash proxy port")
@click.option("--startup-timeout-ms", default=20000, type=int, help="Proxy startup timeout")
@click.option(
    "--gateway-provider-id",
    "gateway_provider_ids",
    multiple=True,
    help="OpenClaw provider id to route through Tokenash (repeatable; default: openai-codex)",
)
@click.option(
    "--python-path",
    default=None,
...
@click.option(
    "--no-auto-start",
    is_flag=True,
    help="Disable plugin auto-start of local tokenash proxy",
)
@click.option(
    "--no-restart",
    is_flag=True,
...
def openclaw(
    ...
    """Install and configure Tokenash OpenClaw plugin in one command.
    ...
    \b
    Example:
      tokenash wrap openclaw
      tokenash wrap openclaw --plugin-path C:\\git\\tokenash\\plugins\\openclaw
    """
    if prepare_only:
    ...
    click.echo()
    click.echo("  ╔═══════════════════════════════════════════════╗")
    click.echo("  ║           TOKENASH WRAP: OPENCLAW             ║")
    click.echo("  ╚═══════════════════════════════════════════════╝")
    click.echo()
    ...
    existing_entry = _read_openclaw_config_value(openclaw_bin, "plugins.entries.tokenash")
    entry = _build_openclaw_plugin_entry(
        existing_entry=existing_entry,
        proxy_port=proxy_port,
        startup_timeout_ms=startup_timeout_ms,
    ...
    _set_openclaw_context_engine_slot(openclaw_bin, "tokenash")
    _run_checked(
    ...
    inspect_result = _run_checked(
        [openclaw_bin, "plugins", "inspect", "tokenash"],
        action="openclaw plugins inspect tokenash",
    )
    if verbose and inspect_result.stdout.strip():
        click.echo(inspect_result.stdout.strip())

    click.echo()
    click.echo("✓ OpenClaw is configured to use Tokenash context compression.")
    click.echo("  Plugin: tokenash")
    click.echo("  Slot:   plugins.slots.contextEngine = tokenash")
    click.echo()
...
@click.option("--no-mcp", is_flag=True, help="Skip tokenash MCP server registration")
@click.option("--no-serena", is_flag=True, help="Skip Serena MCP server registration")
@click.option(
    "--code-graph",
    is_flag=True,
...
def opencode(
    ...
    """Launch OpenCode through Tokenash proxy.

    \b
    Sets OPENCODE_CONFIG_CONTENT to route all OpenCode API calls through
    Tokenash. Configures a tokenash provider via @ai-sdk/openai-compatible.
    Also sets OPENAI_BASE_URL and ANTHROPIC_BASE_URL as fallbacks.

    \b
    Examples:
        tokenash wrap opencode                         # Start proxy + context tool + opencode
        tokenash wrap opencode -- "fix the bug"        # Pass prompt to opencode
        tokenash wrap opencode --no-context-tool       # Skip CLI context-tool setup
        tokenash wrap opencode --no-mcp                # Skip MCP retrieve tool registration
        tokenash wrap opencode --no-serena             # Skip Serena MCP registration
        tokenash wrap opencode --port 9999             # Custom proxy port
        tokenash wrap opencode --backend anyllm --anyllm-provider groq
    """
    # Snapshot OpenCode config.json BEFORE any wrap-time mutation so
    # `tokenash unwrap opencode` can restore the user's pre-wrap state.
    ...
    # Register tokenash MCP server in OpenCode config so OpenCode can
    # call tokenash_retrieve on compression markers from the proxy.
    if not no_mcp:
        from tokenash.mcp_registry import OpencodeRegistrar

        _setup_tokenash_mcp(OpencodeRegistrar(), port, verbose=verbose, force=True)
    elif verbose:
        click.echo("  Skipping MCP retrieve tool (--no-mcp)")
    ...
    # Inject Tokenash provider into OpenCode config so traffic routes through proxy.
    inject_opencode_provider_config(port)
    if memory:
        mem_dir = Path.cwd() / ".tokenash"
...
def unwrap_opencode(port: int, no_stop_proxy: bool) -> None:
    """Undo ``tokenash wrap opencode`` edits to the active OpenCode config file.

    Behaviour:

    * If a pre-wrap backup (``opencode.json.tokenash-backup``) exists, the
      original file is restored byte-for-byte and the backup is removed.
    * Otherwise, if the config file still contains the Tokenash-managed
      block, that block is stripped out and the rest of the file is
      preserved.
    * If the config only ever contained Tokenash-written content, the file
      is removed entirely so OpenCode falls back to its defaults.
    * If neither a backup nor a Tokenash block is present, this is a safe
      no-op.
    """
    click.echo()
    click.echo("  ╔═══════════════════════════════════════════════╗")
    click.echo("  ║         TOKENASH UNWRAP: OPENCODE             ║")
    click.echo("  ╚═══════════════════════════════════════════════╝")
    click.echo()
    ...
    if backup_file.exists():
        ...
        if _PROVIDER_MARKER_START in content or _MCP_MARKER_START in content:
            cleaned = strip_opencode_tokenash_blocks(content)
            if cleaned.strip():
                config_file.write_text(cleaned + "\n", encoding="utf-8")
                click.echo(f"  Removed Tokenash block from {config_file}; other content preserved.")
                status = "cleaned"
            else:
                config_file.unlink()
                click.echo(f"  Removed {config_file} (contained only Tokenash-written config).")
                status = "removed"
        else:
            click.echo(f"  Nothing to undo: {config_file} has no Tokenash wrap markers.")
            status = "noop"
    else:
        click.echo(f"  Nothing to undo: {config_file} does not exist.")
        status = "noop"

    # Remove Serena MCP if it was installed by Tokenash.
    # Also remove the tokenash MCP server itself.
    from tokenash.mcp_registry import OpencodeRegistrar

    opencode_registrar = OpencodeRegistrar()
    if opencode_registrar.detect():
        if opencode_registrar.unregister_server("tokenash"):
            click.echo("  Removed Tokenash MCP server from OpenCode.")
        serena_status = _remove_tokenash_installed_serena_mcp(opencode_registrar)
        if serena_status == "removed":
            click.echo("  Removed Tokenash-installed Serena MCP server from OpenCode.")
        elif serena_status == "failed":
            click.echo("  Serena MCP server matched Tokenash ledger but could not be removed.")

    click.echo()
    click.echo("✓ OpenCode is no longer routed through the Tokenash proxy.")
...
def unwrap_openclaw(
    ...
    """Disable the Tokenash OpenClaw plugin and restore the legacy engine slot."""
    if prepare_only:
        click.echo(
            json.dumps(
    ...
    click.echo()
    click.echo("  ╔═══════════════════════════════════════════════╗")
    click.echo("  ║          TOKENASH UNWRAP: OPENCLAW            ║")
    click.echo("  ╚═══════════════════════════════════════════════╝")
    click.echo()
    click.echo("  Disabling Tokenash plugin and removing engine mapping...")
    ...
    click.echo()
    click.echo("✓ OpenClaw Tokenash wrap removed.")
    click.echo("  Plugin: tokenash (installed, disabled)")
...
def unwrap_codex(port: int, no_stop_proxy: bool) -> None:
    """Undo ``tokenash wrap codex`` edits to the active Codex config file.

    Behaviour:

    * If a pre-wrap backup (``config.toml.tokenash-backup``) exists, the
      original file is restored byte-for-byte and the backup is removed.
    * Otherwise, if the config file still contains the Tokenash-managed
      block, that block is stripped out and the rest of the file is
      preserved.
    ...
    * If neither a backup nor a Tokenash block is present, this is a safe
      ...
      it). When ``CODEX_HOME`` is unset, print a warning hint because Tokenash
      may be looking at the default config while Codex was wrapped with a
      custom home.
    """
    click.echo()
    click.echo("  ╔═══════════════════════════════════════════════╗")
    click.echo("  ║           TOKENASH UNWRAP: CODEX              ║")
    click.echo("  ╚═══════════════════════════════════════════════╝")
    click.echo()
    ...
    if status == "restored":
        ...
        if not os.environ.get("CODEX_HOME"):
            click.echo(
                "  Warning: found no Tokenash wrap markers in the default Codex config. "
                "If you wrapped Codex with CODEX_HOME, rerun unwrap with the same "
                "environment variable, e.g. CODEX_HOME=/path/to/codex-home "
                "tokenash unwrap codex."
            )
        click.echo(f"  Nothing to undo: {config_file} has no Tokenash wrap markers.")

    # Serena is written as its own [mcp_servers.serena] table with Tokenash
    ...
    if codex_registrar.detect():
        serena_status = _remove_tokenash_installed_serena_mcp(codex_registrar)
        if serena_status == "removed":
            click.echo("  Removed Tokenash-installed Serena MCP server from Codex.")
        elif serena_status == "failed":
            click.echo("  Serena MCP server matched Tokenash ledger but could not be removed.")

    if status in {"restored", "cleaned", "removed"}:
        # Hand the threads back to the native-provider menu so the full history
        # stays visible once Codex no longer routes through Tokenash. Best-effort.
        retag_to_native(_codex_home_dir())

    click.echo()
    click.echo("✓ Codex is no longer routed through the Tokenash proxy.")
    if not no_stop_proxy and status != "noop":
        _echo_unwrap_proxy_stop_status(_stop_local_proxy_for_unwrap(port), port)
    click.echo()
