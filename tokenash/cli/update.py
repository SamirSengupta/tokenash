"""`tokenash update` — self-update across the supported install methods.

Detects how Tokenash was installed and runs the matching upgrade command, or
...
from tokenash.update_check import (
    PACKAGE_NAME,
...
class InstallMethod:
    """How Tokenash is installed and how (or whether) to upgrade it."""
...
def _is_source_checkout() -> bool:
    try:
        from tokenash._version import _source_root
...
def _in_docker() -> bool:
    try:
        ...
        return Path("/.dockerenv").exists() or bool(
            os.environ.get("TOKENASH_IN_DOCKER", "").strip()
        )
    except Exception:
        return False
...
def _package_location() -> str | None:
    """Normalized base directory the tokenash distribution is installed in."""
    try:
        from importlib.metadata import distribution
...
def _managed_env_guidance() -> str:
    if sys.platform == "darwin":
        hint = "`brew upgrade tokenash` (if installed via Homebrew), or reinstall with pipx"
    elif sys.platform.startswith("win"):
        hint = "reinstall with pipx (`pipx install tokenash`) or use a virtualenv"
    else:
        hint = "use your distro package manager, or reinstall with pipx / a virtualenv"
    return (
        "Tokenash is installed in an externally-managed system Python (PEP 668). "
...
def detect_install_method(extras: str | None = None) -> InstallMethod:
    ...
    if _in_docker():
        return InstallMethod(
            ...
            can_self_update=False,
            guidance=(
                "Running inside a container — pull a newer Tokenash image instead of self-updating."
            ),
        )
...
def update(check_only: bool, assume_yes: bool, allow_pre: bool, extras: str | None) -> None:
    """Update Tokenash to the latest release.
    ...
    click.echo("Checking PyPI for the latest Tokenash release...")
    ...
    if current:
        ...
        try:
            if Version(latest) <= Version(current):
                click.echo(f"Tokenash is up to date ({current}).")
                return
        except InvalidVersion:
            pass
        click.echo(f"Update available: {current} → {latest}")
    else:
        click.echo(f"Latest Tokenash release: {latest}")
    ...
    click.echo(f"Tokenash upgraded to {latest}.")
    click.echo("Restart any running `tokenash proxy` to pick up the new version.")
