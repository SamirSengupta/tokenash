"""CLI commands for Tokenash Learn — offline failure learning."""
...
External plugins register via 'tokenash.learn_plugin' entry point.
...
def learn(
    ...
    be installed via pip (entry point: tokenash.learn_plugin).

    \b
    Examples:
        tokenash learn                        # Auto-detect agent & model
        tokenash learn --apply                # Write recommendations
        tokenash learn --model gpt-4o         # Use GPT-4o for analysis
        tokenash learn --all                  # Analyze all projects
        tokenash learn --agent codex --all    # Analyze all Codex sessions
        tokenash learn --target CLAUDE.md     # Write to the team-shared file
    """
    import os
    ...
    for agent_name, plugin in agent_configs:
        ...
        if analyze_all:
            ...
            if not targets and len(agent_configs) == 1:
                ...
                click.echo("Try: tokenash learn --all  or  tokenash learn --project <path>")
    ...
    for proj in targets:
        ...
        if apply:
            ...
            click.echo(
                ...
                "TOKENASH_OUTPUT_SHAPER=1 and TOKENASH_VERBOSITY_LEVEL is unset."
            )
        else:
