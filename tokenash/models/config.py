"""Central configuration for all ML models used in Tokenash.
...
Usage:
    ...
    # TOKENASH_SENTENCE_TRANSFORMER=intfloat/e5-small-v2
    # TOKENASH_SIGLIP=google/siglip-base-patch16-224
"""
...
class MLModelConfig:
    ...
    All model names are defined here. Components throughout Tokenash
    import these defaults, so changing a value here changes it everywhere.

    Environment variables can override any default:
    - TOKENASH_SENTENCE_TRANSFORMER
    - TOKENASH_SIGLIP
    - TOKENASH_SPACY
    - TOKENASH_TECHNIQUE_ROUTER
    ...
    Attributes:
        ...
        technique_router: Model for image optimization routing.
            Default: SamirSengupta/technique-router (~100MB)
    """

    # Text Embeddings (SentenceTransformer)
    sentence_transformer: str = field(
        default_factory=lambda: os.environ.get("TOKENASH_SENTENCE_TRANSFORMER", "all-MiniLM-L6-v2")
    )
    sentence_transformer_dim: int = 384

    # Image Embeddings (SIGLIP)
    siglip: str = field(
        default_factory=lambda: os.environ.get("TOKENASH_SIGLIP", "google/siglip-base-patch16-224")
    )

    # Named Entity Recognition (spaCy)
    spacy: str = field(default_factory=lambda: os.environ.get("TOKENASH_SPACY", "en_core_web_sm"))

    # Image Technique Router
    technique_router: str = field(
        default_factory=lambda: os.environ.get(
            "TOKENASH_TECHNIQUE_ROUTER", "SamirSengupta/technique-router"
        )
    )
    ...
    _memory_estimates: dict[str, int] = field(
        default_factory=lambda: {
            ...
            "en_core_web_lg": 560,
            # Technique Router
            "SamirSengupta/technique-router": 100,
        }
    )
