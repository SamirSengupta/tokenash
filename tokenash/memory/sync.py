...
Provides bidirectional sync between tokenash's memory DB and any
...
Usage:
    from tokenash.memory.sync import sync, SyncResult
    from tokenash.memory.sync_adapters.claude_code import ClaudeCodeAdapter
...
logger = logging.getLogger("tokenash.memory.sync")

# State file for fast no-op detection (workspace bucket, respects
# TOKENASH_WORKSPACE_DIR). Resolved at import time, matching prior behavior.
    ...
    async def write_memories(self, memories: list[dict[str, Any]]) -> int:
        ...
        Args:
            memories: List of dicts with keys: content, category, importance,
                      tokenash_id, source_agent, content_hash.
...
async def sync(
    ...
    """Bidirectional sync between tokenash DB and an agent's memory.
...
# ---------------------------------------------------------------------------
# CLI entry point: python -m tokenash.memory.sync --db ... --user ... --agent ...
# ---------------------------------------------------------------------------
...
def _build_sync_backend(db_path: str) -> Any:
    ...
    Match the proxy MCP server (see ``tokenash/memory/mcp_server.py``): use the
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Tokenash memory sync")
    ...
    async def _run() -> None:
        ...
        if args.agent == "claude":
            ...
            from tokenash.memory.sync_adapters.codex_agent import CodexAdapter
