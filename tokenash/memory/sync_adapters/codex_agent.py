...
Syncs memories to/from a tokenash-managed section in AGENTS.md.
...
from tokenash.memory.sync import AgentMemory, AgentMemoryAdapter

_MARKER_START = "<!-- tokenash:memory:start -->"
_MARKER_END = "<!-- tokenash:memory:end -->"
...
class CodexAdapter(AgentMemoryAdapter):
    ...
    async def read_memories(self) -> list[AgentMemory]:
        """Read memories from the tokenash section of AGENTS.md."""
    ...
    async def write_memories(self, memories: list[dict[str, Any]]) -> int:
        """Write memories into the tokenash section of AGENTS.md."""
        if not memories:
            return 0

        # Build section content
        lines = ["## Tokenash Shared Memory", ""]
        for mem in memories:
