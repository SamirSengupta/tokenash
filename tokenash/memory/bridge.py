"""Memory Bridge: bidirectional bridge between markdown files and Tokenash memory.

Imports markdown memory files (Claude Code MEMORY.md, ChatGPT facts, etc.)
into Tokenash's semantic memory system, and exports Tokenash memories back
to organized markdown files. Supports bidirectional sync with hash-based
change detection.

Usage:
    from tokenash.memory.bridge import MemoryBridge
    from tokenash.memory.bridge_config import BridgeConfig
    ...
    # Import markdown -> Tokenash
    stats = await bridge.import_from_markdown()

    # Export Tokenash -> markdown
...
from tokenash.memory.bridge_parsers import (
    ParsedFile,
    ParsedSection,
...
if TYPE_CHECKING:
    ...
    from tokenash.memory.models import Memory
...
class MemoryBridge:
    """Bidirectional bridge between markdown memory files and Tokenash's
    semantic memory system.

    Supports import (md -> Tokenash), export (Tokenash -> md), and
    ...
    async def import_from_markdown(
        ...
        """Import markdown memory files into Tokenash's vector store.
    ...
    async def _import_section(
        ...
        """Import a single parsed section into Tokenash.
    ...
    async def export_to_markdown(
        ...
        """Export Tokenash memories to a markdown file.
    ...
    async def sync(
        ...
        """Bidirectional sync between markdown files and Tokenash.

        1. Import new/changed sections from md files.
        2. Export new organic Tokenash memories back to md files.
        """
        paths = paths or self._config.md_paths
        user_id = user_id or self._config.user_id
        stats = SyncStats()
