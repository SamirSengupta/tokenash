...
This module provides the easiest way to use Tokenash's memory system.
...
if TYPE_CHECKING:
    from tokenash.memory.ports import MemorySearchResult
...
class Memory:
    ...
    Args:
        ...
        db_path: Path for local database (only for "local" backend).
            Defaults to ~/.tokenash/memory.db
    ...
    Examples:
        ...
        # Hosted Qdrant via URL + API key (or set TOKENASH_QDRANT_URL /
        # TOKENASH_QDRANT_API_KEY in the environment)
        memory = Memory(
    ...
    def __init__(
        ...
        if db_path is None:
            # Default: workspace memory.db (respects TOKENASH_WORKSPACE_DIR)
        ...
        # ``None`` sentinels fall back to TOKENASH_QDRANT_* env vars so that
