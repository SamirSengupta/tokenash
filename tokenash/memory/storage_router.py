...
* ``USER``: one DB per ``x-tokenash-user-id`` (no project axis).
...
class ProjectResolver:
    ...
    def resolve(self, ctx: RequestContext) -> tuple[str, str] | None:
        ...
        explicit = self._first_nonempty_header(ctx.headers, "x-tokenash-project-id")
        if explicit:
        ...
        explicit_cwd = self._first_nonempty_header(ctx.headers, "x-tokenash-cwd")
        if explicit_cwd:
        ...
        if ident is None:
            ...
            if fallback == "empty":
                ...
                logger.warning(
                    ...
                    "hint='set x-tokenash-project-id or x-tokenash-cwd header, "
