...
Provides a single source of truth for ``TOKENASH_QDRANT_*`` env vars so that
...
- ``TOKENASH_QDRANT_URL``        Full URL (e.g. ``https://xyz.cloud.qdrant.io:6333``).
                                  When set, takes precedence over host/port.
- ``TOKENASH_QDRANT_HOST``       Hostname. Default: ``localhost``.
- ``TOKENASH_QDRANT_PORT``       HTTP port. Default: ``6333``.
- ``TOKENASH_QDRANT_API_KEY``    API key for hosted Qdrant (e.g. Qdrant Cloud).
- ``TOKENASH_QDRANT_HTTPS``      ``true``/``false``. Forces HTTPS on/off.
- ``TOKENASH_QDRANT_PREFER_GRPC````true``/``false``. Use gRPC instead of HTTP.
- ``TOKENASH_QDRANT_GRPC_PORT``  gRPC port. Default: ``6334``.
...
def qdrant_env_url() -> str | None:
    """Return ``TOKENASH_QDRANT_URL`` or ``None`` if unset."""
    return _strip_env("TOKENASH_QDRANT_URL")


def qdrant_env_host() -> str:
    """Return ``TOKENASH_QDRANT_HOST`` or the ``localhost`` default."""
    return _strip_env("TOKENASH_QDRANT_HOST") or DEFAULT_QDRANT_HOST


def qdrant_env_port() -> int:
    """Return ``TOKENASH_QDRANT_PORT`` or the ``6333`` default."""
    return (
        _parse_port(_strip_env("TOKENASH_QDRANT_PORT"), "TOKENASH_QDRANT_PORT")
        or DEFAULT_QDRANT_PORT
    )


def qdrant_env_api_key() -> str | None:
    """Return ``TOKENASH_QDRANT_API_KEY`` or ``None`` if unset."""
    return _strip_env("TOKENASH_QDRANT_API_KEY")


def qdrant_env_https() -> bool | None:
    """Return ``TOKENASH_QDRANT_HTTPS`` parsed as bool, or ``None`` if unset."""
    return _parse_bool(_strip_env("TOKENASH_QDRANT_HTTPS"))


def qdrant_env_prefer_grpc() -> bool:
    """Return ``TOKENASH_QDRANT_PREFER_GRPC`` parsed as bool. Default: ``False``."""
    return _parse_bool(_strip_env("TOKENASH_QDRANT_PREFER_GRPC")) or False


def qdrant_env_grpc_port() -> int:
    """Return ``TOKENASH_QDRANT_GRPC_PORT`` or the ``6334`` default."""
    return (
        _parse_port(_strip_env("TOKENASH_QDRANT_GRPC_PORT"), "TOKENASH_QDRANT_GRPC_PORT")
