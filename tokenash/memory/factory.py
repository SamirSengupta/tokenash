...
if TYPE_CHECKING:
    from tokenash.memory.ports import Embedder, MemoryCache, MemoryStore, TextIndex, VectorIndex


# Extension groups for memory backends registered via setuptools entry points.
_MEMORY_STORE_GROUP = "tokenash.memory_store"
_MEMORY_VECTOR_GROUP = "tokenash.memory_vector"
_MEMORY_TEXT_GROUP = "tokenash.memory_text"
...
def _load_external_backend(
    ...
    `tokenash.cache.compression_store._create_default_ccr_backend`.
    """
    if not name:
        raise ValueError(
...
def _create_store(config: MemoryConfig) -> MemoryStore:
    ...
    if config.store_backend == StoreBackend.SQLITE:
        from tokenash.memory.adapters.sqlite import SQLiteMemoryStore
...
def _create_embedder(config: MemoryConfig) -> Embedder:
    ...
    with _EMBEDDER_CACHE_LOCK:
        ...
        if config.embedder_backend == EmbedderBackend.LOCAL:
            ...
            from tokenash.memory.adapters.embedders import OllamaEmbedder
...
def _create_vector_index(config: MemoryConfig) -> VectorIndex:
    ...
    if backend == VectorBackend.AUTO:
        from tokenash.memory.adapters import HNSW_AVAILABLE, SQLITE_VEC_AVAILABLE
        ...
        if SQLITE_VEC_AVAILABLE:
            ...
            raise ValueError(
                ...
                "Or install the full proxy bundle: pip install tokenash[proxy]"
            )

    if backend == VectorBackend.SQLITE_VEC:
        from tokenash.memory.adapters import SQLITE_VEC_AVAILABLE
        ...
        from tokenash.memory.adapters.sqlite_vector import SQLiteVectorIndex
    ...
    if backend == VectorBackend.HNSW:
        from tokenash.memory.adapters import HNSW_AVAILABLE
        ...
        from tokenash.memory.adapters.hnsw import HNSWVectorIndex
...
def _create_text_index(config: MemoryConfig) -> TextIndex:
    ...
    if config.text_backend == TextBackend.FTS5:
        from tokenash.memory.adapters.fts5 import FTS5TextIndex
...
def _create_cache(config: MemoryConfig) -> MemoryCache:
    ...
    """
    from tokenash.memory.adapters.cache import LRUMemoryCache
