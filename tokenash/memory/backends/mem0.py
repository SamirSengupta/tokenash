"""Mem0 backend adapter for Tokenash's hierarchical memory system.
...
from tokenash.memory.ports import MemoryFilter, VectorFilter, VectorSearchResult
...
class Mem0Config:
    ...
    # Qdrant settings (defaults resolve from TOKENASH_QDRANT_* env vars)
    ...
    # Collection settings
    collection_name: str = "tokenash_memories"


class Mem0Backend:
    """
    Mem0 backend implementation for Tokenash memory system.
    ...
    This adapter maps Mem0's API to Tokenash's MemoryBackend interface:
    ...
    async def _ensure_client(self) -> Any:
        ...
        if self._client is None:
            try:
                ...
                raise ImportError(
                    "mem0 package not installed. Install with: pip install 'tokenash[memory-stack]'"
                ) from None
    ...
    def _mem0_result_to_memory(self, result: dict[str, Any]) -> Memory:
        ...
        memory_id = metadata.get("tokenash_id", result.get("id", str(uuid.uuid4())))
