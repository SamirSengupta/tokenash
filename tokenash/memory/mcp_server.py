"""Tokenash Memory MCP Server.

A stdio MCP server that exposes tokenash's memory backend as tools
...
Usage:
    ...
    python -m tokenash.memory.mcp_server --db /path/to/.tokenash/memory.db

    # Registered in Codex config.toml (done by `tokenash wrap codex --memory`):
    [mcp_servers.tokenash_memory]
    command = "python"
    args = ["-m", "tokenash.memory.mcp_server", "--user", "alice"]
    # When --db is omitted, the server resolves .tokenash/memory.db from cwd.
"""
...
logger = logging.getLogger("tokenash.memory.mcp")
...
def create_memory_server(db_path: str, user_id: str = "default") -> Server:
    """Create an MCP server backed by tokenash's local memory."""

    server = Server("tokenash-memory")
...
def _memory_mcp_startup_context(
    ...
    active_project_db = (cwd / ".tokenash" / "memory.db").resolve(strict=False)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Tokenash Memory MCP Server")
    parser.add_argument(
        "--db",
        default=str(Path.cwd() / ".tokenash" / "memory.db"),
