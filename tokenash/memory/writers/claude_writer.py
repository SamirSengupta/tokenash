...
from tokenash.memory.sync_adapters.claude_code import encode_claude_project_path
from tokenash.memory.writers.base import AgentWriter, MemoryEntry
...
class ClaudeCodeMemoryWriter(AgentWriter):
    ...
    def format_memories(self, memories: list[MemoryEntry]) -> str:
        ...
        lines = [
            "## Tokenash Learned Context",
            "*Auto-maintained by Tokenash — do not edit manually*",
            "",
        ]
    ...
    def export_topics(
        ...
        for category, entries in grouped.items():
            ...
            filename = f"tokenash_{category}.md"
            lines = [
                "---",
                f"name: tokenash-{category}",
                f"description: Tokenash-learned {category.replace('_', ' ')} patterns",
                "type: reference",
                "---",
                "",
