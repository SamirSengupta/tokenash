...
class CursorMemoryWriter(AgentWriter):
    ...
    def default_path(self) -> Path:
        """Default: .cursor/rules/tokenash-memory.mdc in project root."""
        return self._project_path / ".cursor" / "rules" / "tokenash-memory.mdc"
    ...
    def export(
        ...
        if target.exists():
            ...
            full_content = (
                "---\n"
                "description: Tokenash-learned patterns from proxy traffic\n"
                "alwaysApply: true\n"
                "---\n"
                "\n"
