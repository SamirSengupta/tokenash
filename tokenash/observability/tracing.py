"""OTEL tracing helpers for Tokenash and Langfuse."""
...
from .metrics import _tokenash_version, _parse_bool, _parse_key_value_pairs
...
_tracing_lock = Lock()
_global_tracer: TokenashTracer | None = None
...
class LangfuseTracingConfig:
    """Configuration for Tokenash-managed Langfuse OTLP trace export."""
    ...
    @classmethod
    def from_env(cls, *, default_service_name: str = "tokenash") -> LangfuseTracingConfig:
        public_key = os.environ.get("LANGFUSE_PUBLIC_KEY", "").strip()
        secret_key = os.environ.get("LANGFUSE_SECRET_KEY", "").strip()

        return cls(
            enabled=_parse_bool(
                os.environ.get("TOKENASH_LANGFUSE_ENABLED"),
                default=False,
            ),
            ...
            service_name=os.environ.get(
                "TOKENASH_LANGFUSE_SERVICE_NAME", default_service_name
            ).strip()
            or default_service_name,
            resource_attributes=_parse_key_value_pairs(
                os.environ.get("TOKENASH_LANGFUSE_RESOURCE_ATTRIBUTES")
            ),
        )
...
class TokenashTracer:
    """Tracer facade used by shared Tokenash compression paths."""

    def __init__(self, tracer_provider: Any | None = None):
        if tracer_provider is None:
            self._tracer = trace.get_tracer(_SCOPE_NAME, _tokenash_version())
        else:
            self._tracer = tracer_provider.get_tracer(_SCOPE_NAME, _tokenash_version())
...
def get_tokenash_tracer() -> TokenashTracer:
    global _global_tracer

    if _global_tracer is None:
        with _tracing_lock:
            if _global_tracer is None:
                _global_tracer = TokenashTracer()

    return _global_tracer


def set_tokenash_tracer(tokenash_tracer: TokenashTracer) -> TokenashTracer:
    global _global_tracer
    with _tracing_lock:
        _global_tracer = tokenash_tracer
    return tokenash_tracer
...
def configure_langfuse_tracing(
    ...
    try:
        ...
        logger.warning(
            ...
            "Install tokenash[otel] to enable Langfuse OTLP tracing."
        )
        return get_tokenash_tracer()
    ...
    tokenash_tracer = TokenashTracer(tracer_provider=tracer_provider)
...
def get_langfuse_tracing_status() -> dict[str, Any]:
    ...
    return LangfuseTracingConfig.from_env(default_service_name="tokenash-proxy").status()


def shutdown_tokenash_tracing() -> None:
    global _global_tracer
    global _owned_tracer_provider
    global _owned_langfuse_config
...
def reset_tokenash_tracing() -> None:
