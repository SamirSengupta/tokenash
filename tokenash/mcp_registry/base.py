...
The MCP protocol itself is universal, and the tokenash MCP server
(``tokenash mcp serve``) is a single stdio binary that any compliant client
...
The orchestrator in :mod:`tokenash.mcp_registry.install` calls a fleet of
registrars to install tokenash across every detected agent.
"""
