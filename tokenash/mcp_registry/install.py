"""Top-level orchestration: register Tokenash MCP across detected agents."""
...
def build_tokenash_spec(proxy_url: str = DEFAULT_PROXY_URL) -> ServerSpec:
    """Construct the canonical :class:`ServerSpec` for the tokenash server.
    ...
    if proxy_url and proxy_url != DEFAULT_PROXY_URL:
        env["TOKENASH_PROXY_URL"] = proxy_url
    return ServerSpec(
        name="tokenash",
        command="tokenash",
        args=("mcp", "serve"),
...
def build_serena_spec(context: str) -> ServerSpec:
    ...
    startup. Tokenash installs Serena by default, so without this flag every
...
def install_everywhere(
    ...
    *,
    ...
    """Install the tokenash MCP server into every detected agent.
