"""Tokenash-owned MCP install ledger.

The ledger tracks MCP servers that Tokenash registered on the user's behalf
when the target agent config cannot carry Tokenash-specific ownership markers.
It lets unwrap remove only entries still matching the spec Tokenash installed,
...
def ledger_path() -> Path:
    """Return the Tokenash MCP install ledger path."""
...
def record_install(agent: str, spec: ServerSpec, *, path: Path | None = None) -> None:
    """Record that Tokenash installed ``spec`` for ``agent``."""
...
def tokenash_installed_matching(
    agent: str,
    current_spec: ServerSpec | None,
    *,
    path: Path | None = None,
) -> bool:
    """Return True when the ledger says Tokenash installed ``current_spec``."""
