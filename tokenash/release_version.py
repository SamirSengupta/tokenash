...
# Load the UTF-8-forcing subprocess wrapper WITHOUT importing the `tokenash`
# package. The release workflow runs this file as a bare script
# (`python tokenash/release_version.py`), where `sys.path[0]` is `tokenash/`
# rather than the repo root, so `from tokenash._subprocess import run` fails
# with `ModuleNotFoundError: No module named 'tokenash'` — and even when it
# resolves, it drags in `tokenash/__init__.py` (the Rust `_core` import), which
...
try:  # normal package context (tests import `tokenash.release_version`)
    from tokenash._subprocess import run
except ModuleNotFoundError:  # bare-script context (the release workflow)
    import importlib.util as _ilu

    _spec = _ilu.spec_from_file_location(
        "_tokenash_subprocess", Path(__file__).resolve().parent / "_subprocess.py"
    )
