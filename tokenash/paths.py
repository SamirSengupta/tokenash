"""Canonical filesystem contract for Tokenash.

This module defines the single source of truth for where Tokenash reads and
writes files. It introduces two canonical roots:

* ``TOKENASH_CONFIG_DIR`` -- read-mostly configuration (defaults to
  ``~/.tokenash/config``). Holds model catalogs, plugin settings, and other
  configuration that users or admins edit.
* ``TOKENASH_WORKSPACE_DIR`` -- read-write state (defaults to ``~/.tokenash``).
...
per-resource override (``TOKENASH_SAVINGS_PATH``, ``TOKENASH_TOIN_PATH``,
...
TOKENASH_CONFIG_DIR_ENV = "TOKENASH_CONFIG_DIR"
TOKENASH_WORKSPACE_DIR_ENV = "TOKENASH_WORKSPACE_DIR"

# ---------------------------------------------------------------------------
# Legacy per-resource env vars (kept for backward compatibility)
# ---------------------------------------------------------------------------

TOKENASH_SAVINGS_PATH_ENV = "TOKENASH_SAVINGS_PATH"
TOKENASH_SAVINGS_EVENTS_PATH_ENV = "TOKENASH_SAVINGS_EVENTS_PATH"
TOKENASH_TOIN_PATH_ENV = "TOKENASH_TOIN_PATH"
TOKENASH_SUBSCRIPTION_STATE_PATH_ENV = "TOKENASH_SUBSCRIPTION_STATE_PATH"

# ---------------------------------------------------------------------------
# Default sub-path fragments
# ---------------------------------------------------------------------------

_WORKSPACE_DIR_DEFAULT = ".tokenash"
...
def workspace_dir() -> Path:
    ...
    1. ``$TOKENASH_WORKSPACE_DIR`` (trimmed, tilde-expanded) if set.
    2. ``~/.tokenash`` otherwise.
    """

    env_value = _env(TOKENASH_WORKSPACE_DIR_ENV)
    if env_value:
        return Path(env_value).expanduser()
...
def config_dir() -> Path:
    ...
    1. ``$TOKENASH_CONFIG_DIR`` (trimmed, tilde-expanded) if set.
    2. ``$TOKENASH_WORKSPACE_DIR/config`` when the workspace env var is set
       so that a single override relocates both roots coherently.
    3. ``~/.tokenash/config`` otherwise.
    """

    env_value = _env(TOKENASH_CONFIG_DIR_ENV)
    if env_value:
        return Path(env_value).expanduser()
    workspace_env = _env(TOKENASH_WORKSPACE_DIR_ENV)
    if workspace_env:
...
def toin_path(explicit: str | os.PathLike[str] | None = None) -> Path:
    ...
    ``~/.tokenash/toin.json`` to preserve byte-for-byte backward compat.
    """
...
def savings_events_path(explicit: str | os.PathLike[str] | None = None) -> Path:
    ...
    MCP processes, and is the source of truth for ``tokenash savings``.
    """

    return _resolve(
        explicit,
        TOKENASH_SAVINGS_EVENTS_PATH_ENV,
...
def log_dir() -> Path:
    """Return the directory for Tokenash log files."""
...
def bin_dir() -> Path:
    """Return the directory where Tokenash ships vendored binaries."""
...
def models_config_path() -> Path:
    ...
    Note: the ``TOKENASH_MODEL_LIMITS`` env var is a *content* override
    ...
    location and deliberately ignores ``TOKENASH_MODEL_LIMITS``.
    """
