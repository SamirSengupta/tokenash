"""Plugin registry for tokenash learn.

Discovers built-in plugins from tokenash.learn.plugins.* and external
plugins registered via the ``tokenash.learn_plugin`` entry point group.

Follows the same pattern as tokenash.storage_backend (storage/__init__.py).
"""
...
def _discover() -> dict[str, LearnPlugin]:
    ...
    # 1. Built-in: scan tokenash.learn.plugins.* submodules
    from tokenash.learn import plugins as plugins_pkg

    for _, mod_name, _ in pkgutil.iter_modules(plugins_pkg.__path__):
        try:
            mod = importlib.import_module(f"tokenash.learn.plugins.{mod_name}")
    ...
    # 2. External: entry_points(group="tokenash.learn_plugin")
    try:
        from importlib.metadata import entry_points

        for ep in entry_points(group="tokenash.learn_plugin"):
            try:
                obj = ep.load()
