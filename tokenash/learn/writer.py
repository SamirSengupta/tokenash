...
# Marker delimiters for Tokenash-managed sections
_MARKER_START = "<!-- tokenash:learn:start -->"
_MARKER_END = "<!-- tokenash:learn:end -->"
...
def _build_section(recommendations: list[Recommendation]) -> str:
    ...
    lines = [
        ...
        f"*Auto-generated by `tokenash learn` on {now} — do not edit manually*",
        "",
    ]
...
def extract_marker_block(file_content: str) -> str | None:
    """Return the raw text of the tokenash:learn marker block, or None.
...
def _strip_marker_block(content: str) -> str:
    """Remove the tokenash:learn marker block from text, tidying blank lines.
...
class ClaudeCodeWriter(ContextWriter):
    ...
    def write(
        ...
        if context_recs:
            ...
            # tokenash versions into the new target, then strip it from CLAUDE.md
    ...
    def _migrate_legacy_block(
        ...
        """Move a stale tokenash block out of CLAUDE.md into the new target.
        ...
        if cleaned:
            ...
            result.warnings.append(
                f"Moved Tokenash learnings out of {legacy_path} into {target_path}: "
            ...
            # CLAUDE.md held nothing but the Tokenash block — remove the husk.
            result.warnings.append(
                f"Removed {legacy_path} (it contained only Tokenash learnings) and "
