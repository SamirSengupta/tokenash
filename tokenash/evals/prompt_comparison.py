...
This module provides tools to verify that Tokenash preserves the semantic
meaning of prompts by comparing prompts before and after processing.

This is CRITICAL for proving that Tokenash doesn't change the meaning of prompts
when they go through the proxy.

Usage:
    from tokenash.evals.prompt_comparison import compare_prompts, PromptComparer

    # Simple comparison
    result = compare_prompts(
        original_prompt="What is the capital of France?",
        tokenash_modified_prompt="What is the capital of France?",
    )
...
class PromptComparisonResult:
    ...
    Attributes:
        original: The original prompt before Tokenash processing.
        tokenash_modified: The prompt after Tokenash processing.
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "tokenash_modified": self.tokenash_modified,
    ...
    def from_dict(cls, data: dict[str, Any]) -> PromptComparisonResult:
        ...
        return cls(
            ...
            tokenash_modified=data["tokenash_modified"],
...
def compare_prompts(
    ...
    Args:
        original_prompt: The original prompt before Tokenash processing.
        tokenash_modified_prompt: The prompt after Tokenash processing.
    ...
    Example:
        result = compare_prompts(
            ...
            tokenash_modified_prompt="Explain quantum computing in simple terms.",
        )
    ...
    judge_prompt = SEMANTIC_EQUIVALENCE_JUDGE_PROMPT.format(
        ...
        modified_prompt=tokenash_modified_prompt,
    )
    ...
    return PromptComparisonResult(
        ...
        tokenash_modified=tokenash_modified_prompt,
...
class PromptComparer:
    ...
    def compare(
        ...
        result = compare_prompts(
            original_prompt=original_prompt,
            tokenash_modified_prompt=tokenash_modified_prompt,
            judge_model=self.judge_model,
    ...
    def compare_and_log(
        ...
        Returns:
            PromptComparisonResult with the comparison outcome.
        """
        result = self.compare(original_prompt, tokenash_modified_prompt, metadata)
...
def compare_messages(
    ...
    Args:
        original_messages: Original message list before Tokenash.
        modified_messages: Modified message list after Tokenash.
        judge_model: The OpenAI model to use as judge.
    ...
    result = compare_prompts(
        ...
        tokenash_modified_prompt=modified_str,
        judge_model=judge_model,
        api_key=api_key,
        metadata={
...
def batch_compare_prompts(
    ...
    def compare_pair(
        ...
        result = compare_prompts(
            ...
            tokenash_modified_prompt=modified,
            judge_model=judge_model,
            api_key=api_key,
...
def verify_tokenash_preservation(
    original_messages: list[dict[str, Any]],
    tokenash_messages: list[dict[str, Any]],
    judge_model: str = "gpt-4o",
    api_key: str | None = None,
    fail_on_difference: bool = False,
) -> PromptComparisonResult:
    """Verify that Tokenash preserves the semantic meaning of a request.

    This is the main entry point for verifying Tokenash's accuracy preservation.
    It compares the original messages with those modified by Tokenash and
    raises an error if differences are found (when fail_on_difference=True).

    Args:
        original_messages: Messages before Tokenash processing.
        tokenash_messages: Messages after Tokenash processing.
        judge_model: The model to use for judging.
        api_key: OpenAI API key.
    ...
    Example:
        # Capture messages before and after Tokenash
        original = [{"role": "user", "content": "Hello, how are you?"}]
        after_tokenash = [{"role": "user", "content": "Hello, how are you?"}]

        result = verify_tokenash_preservation(
            original, after_tokenash, fail_on_difference=True
        )
    """
    result = compare_messages(
        original_messages=original_messages,
        modified_messages=tokenash_messages,
        judge_model=judge_model,
        api_key=api_key,
    )
    ...
    if fail_on_difference and not result.are_equivalent:
        raise ValueError(
            f"Tokenash modified prompt semantics! "
