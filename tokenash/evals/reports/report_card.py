...
baseline vs Tokenash accuracy + token savings.
"""
...
class BenchmarkRunResult:
    ...
    # For lm-eval (standard benchmarks): baseline vs tokenash scores
    baseline_score: float | None = None
    tokenash_score: float | None = None
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        if self.tokenash_score is not None:
            d["tokenash_score"] = round(self.tokenash_score, 4)
...
class SuiteResult:
    ...
    def standard_benchmarks(self) -> list[BenchmarkRunResult]:
        """Benchmarks with baseline vs tokenash comparison."""
    ...
    if standard:
        lines.extend(
            [
                ...
                "",
                "| Benchmark | Category | N | Baseline | Tokenash | Delta | Tokens Saved | Status |",
                "|-----------|----------|---|----------|----------|-------|--------------|--------|",
            ]
        )
        for b in standard:
            ...
            tokenash_str = f"{b.tokenash_score:.3f}" if b.tokenash_score is not None else "N/A"
            ...
            lines.append(
                ...
                f"{tokenash_str} | {delta_str} | {comp_str} | {status} |"
            )
        lines.append("")
...
def generate_html(result: SuiteResult) -> str:
    ...
    for b in standard:
        ...
        standard_rows += f"""        <tr>
            ...
            <td>{baseline_str}</td><td>{tokenash_str}</td><td>{delta_str}</td>
            <td>{comp_str}</td>
...
<html>
<head>
    <title>Tokenash Accuracy Report Card</title>
    <style>
        body {{
...
<body>
    <h1>Tokenash Accuracy Report Card</h1>
    <div class="meta">
    ...
    {"<table><tr><th>Benchmark</th><th>Category</th><th>N</th><th>Baseline</th><th>Tokenash</th><th>Delta</th><th>Saved</th><th>Status</th></tr>" if standard else ""}
    {standard_rows}
    ...
    <footer>Generated by <strong>Tokenash Evaluation Framework</strong> | <code>pip install tokenash[evals]</code></footer>
</body>
</html>"""
