...
batch API requests through the Tokenash proxy.
...
Usage:
    >>> from tokenash.evals.batch_compression_eval import (
...
CLI:
    python -m tokenash.evals.batch_compression_eval --provider anthropic --samples 10
"""
...
from tokenash.evals.metrics import (
    compute_exact_match,
    compute_f1,
    compute_rouge_l,
...
Context:
The Tokenash SDK is a context optimization layer for LLM applications. It was created
...
Question: Which repository has the most stars? Answer with just the repository name.""",
            ...
            context_facts=["tokenash", "1250", "llm-cache", "890", "prompt-optimizer", "2100"],
        ),
        BatchTestCase(
            id="json_003",
            category=TestCategory.JSON_EXTRACTION,
...
def evaluate_token_counting_accuracy(
    ...
    This verifies that Tokenash's token counting matches the provider's
