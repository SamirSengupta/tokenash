...
This module runs industry-standard benchmarks to prove Tokenash preserves accuracy.

Approach:
1. Run benchmarks directly against LLM provider (baseline)
2. Run same benchmarks through Tokenash proxy
...
Usage:
    ...
    python -m tokenash.evals.comprehensive_benchmark --quick

    # Full benchmark
    python -m tokenash.evals.comprehensive_benchmark --tasks mmlu,hellaswag

    # Compare with baseline
    python -m tokenash.evals.comprehensive_benchmark --compare
"""
...
class ComparisonResult:
    """Comparison between baseline and Tokenash results."""

    task: str
    metric: str
    baseline_score: float
    tokenash_score: float

    @property
    def delta(self) -> float:
        return self.tokenash_score - self.baseline_score

    @property
    def accuracy_preserved(self) -> bool:
        """True if Tokenash score is within 2% of baseline."""
        return self.tokenash_score >= self.baseline_score - 0.02

    @property
    def accuracy_improved(self) -> bool:
        """True if Tokenash score exceeds baseline."""
        return self.tokenash_score > self.baseline_score
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "tokenash": round(self.tokenash_score, 4),
...
class BenchmarkSuiteResult:
    ...
    tokenash_results: list[BenchmarkResult] = field(default_factory=list)
    ...
    def avg_delta(self) -> float:
        """Average score delta (positive = Tokenash better)."""
        if not self.comparisons:
            return 0.0
    ...
    def summary(self) -> dict[str, Any]:
        return {
            ...
            "tokenash": [r.to_dict() for r in self.tokenash_results],
    ...
    def print_summary(self) -> None:
        ...
        print("\n" + "=" * 70)
        print("TOKENASH COMPREHENSIVE BENCHMARK RESULTS")
        print("=" * 70)
        ...
        if self.comparisons:
            print(f"{'Task':<20} {'Baseline':>10} {'Tokenash':>10} {'Delta':>10} {'Status':>10}")
            print("-" * 70)
            for c in self.comparisons:
                status = "PASS" if c.accuracy_preserved else "FAIL"
                delta_str = f"{c.delta:+.4f}"
                print(
                    f"{c.task:<20} {c.baseline_score:>10.4f} {c.tokenash_score:>10.4f} {delta_str:>10} {status:>10}"
                )
            print("-" * 70)
            ...
            print("Tokenash results only (no baseline comparison):")
            print(f"{'Task':<20} {'Score':>10} {'Metric':<15}")
            print("-" * 50)
            for r in self.tokenash_results:
...
def run_lm_eval(
    ...
    Args:
        ...
        base_url: Base URL for API (for Tokenash proxy)
...
def run_tokenash_benchmark(
    model: str = "gpt-4o-mini",
    tasks: list[str] | None = None,
    limit: int | None = None,
    tokenash_port: int = 8787,
) -> list[BenchmarkResult]:
    """Run benchmark through Tokenash proxy."""
    logger.info(f"Running Tokenash benchmark with {model} through proxy...")

    # lm_eval expects base_url to be the full path to chat/completions
    base_url = f"http://localhost:{tokenash_port}/v1/chat/completions"
...
def compare_results(
    ...
    tokenash: list[BenchmarkResult],
) -> list[ComparisonResult]:
    """Compare baseline and Tokenash results."""
    comparisons = []

    # Match results by task
    baseline_by_task = {r.task: r for r in baseline}
    tokenash_by_task = {r.task: r for r in tokenash}

    for task in baseline_by_task:
        if task in tokenash_by_task:
            b = baseline_by_task[task]
            h = tokenash_by_task[task]
            comparisons.append(
                ComparisonResult(
                    task=task,
                    metric=b.metric,
                    baseline_score=b.score,
                    tokenash_score=h.score,
                )
            )
...
def run_comprehensive_benchmark(
    ...
    Args:
        ...
        tokenash_port: Port where Tokenash proxy is running
    ...
    # This ensures a fair comparison - any speedup from Tokenash is real, not from warm cache
    if compare_baseline:
    ...
    # Run Tokenash benchmark (after baseline warms things up)
    suite.tokenash_results = run_tokenash_benchmark(
        model=model,
        tasks=tasks,
        limit=limit,
        tokenash_port=tokenash_port,
    )

    # Compare results
    if compare_baseline:
        suite.comparisons = compare_results(suite.baseline_results, suite.tokenash_results)

    suite.total_duration_seconds = time.time() - start_time
    return suite


def check_tokenash_proxy(port: int = 8787) -> bool:
    """Check if Tokenash proxy is running."""
    import socket
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Run comprehensive LLM benchmarks to verify Tokenash accuracy"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--compare", "-c", action="store_true", help="Compare with baseline (run without Tokenash)"
    )
    parser.add_argument(
        "--port", "-p", type=int, default=8787, help="Tokenash proxy port (default: 8787)"
    )
    parser.add_argument("--output", "-o", default=None, help="Output file for results JSON")
    parser.add_argument(
        "--tokenash-only", action="store_true", help="Only run through Tokenash (skip baseline)"
    )
    ...
    if not check_tokenash_proxy(args.port):
        print(f"ERROR: Tokenash proxy not running on port {args.port}")
        print(f"Start it with: tokenash proxy --port {args.port}")
        sys.exit(1)

    # Run benchmarks
    compare = args.compare and not args.tokenash_only
