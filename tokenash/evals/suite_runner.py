...
if TYPE_CHECKING:
    from tokenash.evals.reports.report_card import SuiteResult
...
class BenchmarkSpec:
    ...
    pass_threshold: float = 0.98  # Tokenash score >= baseline - (1-threshold)
...
BENCHMARK_SUITE: list[BenchmarkSpec] = [
    ...
    # -----------------------------------------------------------------------
    # Standard benchmarks via lm-eval harness (through Tokenash proxy)
    BenchmarkSpec(
        name="GSM8K",
        category="reasoning",
        tier=1,
...
def _start_proxy(port: int) -> subprocess.Popen | None:
    """Start Tokenash proxy as a subprocess. Returns process handle."""
    logger.info(f"Starting Tokenash proxy on port {port}...")
    try:
        proc = subprocess.Popen(
            [sys.executable, "-m", "tokenash.proxy.server", "--port", str(port)],
        ...
        for _ in range(30):
            ...
            if _check_proxy(port):
                logger.info(f"Tokenash proxy ready on port {port}")
                return proc
...
class SuiteRunner:
    ...
    def __init__(
        ...
        self.tokenash_port = tokenash_port
    ...
    def _ensure_proxy(self) -> bool:
        """Ensure the Tokenash proxy is running (needed for lm-eval benchmarks)."""
        if _check_proxy(self.tokenash_port):
            return True
        if self.auto_start_proxy:
            self._proxy_proc = _start_proxy(self.tokenash_port)
    ...
    def _cleanup_proxy(self) -> None:
        ...
        if self._proxy_proc:
            logger.info("Stopping Tokenash proxy...")
    ...
    def _run_lm_eval_benchmark(
        ...
        from tokenash.evals.comprehensive_benchmark import (
            compare_results,
            run_baseline_benchmark,
            run_tokenash_benchmark,
        )
        ...
        # Run through Tokenash proxy
        print("    Running through Tokenash proxy...")
        tokenash_results = run_tokenash_benchmark(
            model=spec.model or self.model,
            tasks=tasks,
            limit=limit,
            tokenash_port=self.tokenash_port,
        )

        # Compare
        comparisons = compare_results(baseline_results, tokenash_results)
        ...
        if comparisons:
            ...
            tokenash_score = c.tokenash_score
            delta = c.delta
            passed = c.accuracy_preserved

        return {
            "baseline_score": baseline_score,
            "tokenash_score": tokenash_score,
            "delta": delta,
            "passed": passed,
            "n_samples": limit,
        }
    ...
    def _run_before_after_benchmark(
        ...
        from tokenash.evals.core import EvalMode
        from tokenash.evals.datasets import load_dataset_by_name, load_tool_output_samples
        ...
        proxy_url = (
            f"http://localhost:{self.tokenash_port}" if _check_proxy(self.tokenash_port) else None
        )
    ...
    def run(self) -> SuiteResult:
        ...
        from tokenash.evals.cost_tracker import CostTracker
        from tokenash.evals.reports.report_card import BenchmarkRunResult, SuiteResult
        ...
        if has_lm_eval:
            ...
            if not proxy_available:
                print("WARNING: Tokenash proxy not available. Skipping lm-eval benchmarks.")
                print("  Start with: tokenash proxy --port 8787")
                    ...
                    if spec.runner_type == "lm_eval":
                        ...
                        results.append(
                            BenchmarkRunResult(
                                ...
                                tokenash_score=raw.get("tokenash_score"),
                                delta=raw.get("delta"),
    ...
    def _print_summary(self, result: SuiteResult, tracker: Any) -> None:
        ...
        from tokenash.evals.reports.report_card import generate_markdown
