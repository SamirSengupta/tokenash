...
| Benchmark | Category | N | Baseline | Tokenash | Delta |
|-----------|----------|---|----------|----------|-------|
...
```bash
pip install "tokenash[all]"    # Everything including evals (recommended)
pip install "tokenash[evals]"  # Evaluation framework only
```
...
python -m tokenash.evals quick -n 8 --provider openai --model gpt-4o-mini

# Full Tier 1 suite (~$3, ~15 min) — requires proxy running
python -m tokenash.evals suite --tier 1 -o eval_results/

# Extended suite (Tiers 1+2, ~$8, ~1 hr)
python -m tokenash.evals suite --tier 2 -o eval_results/

# CI mode — exit 1 on any regression
python -m tokenash.evals suite --tier 1 --ci

# List all available datasets
python -m tokenash.evals list
```

### Running with the Proxy (Recommended)

For the most accurate evaluation, run through the Tokenash proxy which provides
...
TOKENASH_PROBE_RECORD_DIR=~/.tokenash/probe-recordings tokenash proxy start

# 2. Use your agent through the proxy as normal, then score retention:
tokenash evals probes --recordings ~/.tokenash/probe-recordings
```
...
When the Tokenash proxy is running, the "compressed" path goes through the full
...
on:
  pull_request:
    paths: ['tokenash/transforms/**', 'tokenash/evals/**']
  schedule:
    - cron: '0 6 * * 1'  # Weekly
    ...
    steps:
      ...
      - name: CCR Round-trip (zero cost)
        run: |
          ...
          "
      - name: Quick eval
        run: python -m tokenash.evals quick -n 8 --provider openai --model gpt-4o-mini
        env:
