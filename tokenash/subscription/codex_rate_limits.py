...
(``x-codex-primary-used-percent`` etc.) and tokenash captured those headers
...
CODEX_USAGE_URL = (
    os.environ.get("TOKENASH_CODEX_USAGE_URL", "https://chatgpt.com/backend-api/wham/usage").strip()
...
class CodexRateLimitState(QuotaTracker):
    ...
    be registered with the :class:`~tokenash.subscription.base.QuotaTrackerRegistry`.
    ...
    tokenash, so :meth:`start` and :meth:`stop` are no-ops.
    """
