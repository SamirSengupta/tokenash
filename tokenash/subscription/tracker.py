...
- Thread-safe state updates via threading.Lock (consistent with tokenash patterns)
...
token breakdowns per window that enable:
  - Tokenash efficiency metrics (tokens saved = raw - what proxy sent)
...
from tokenash.subscription.base import QuotaTracker
from tokenash.subscription.client import SubscriptionClient
from tokenash.subscription.models import (
    TokenashContribution,
    RateLimitWindow,
...
_PERSIST_FILE_ENV = _paths.TOKENASH_SUBSCRIPTION_STATE_PATH_ENV
_DEFAULT_PERSIST_DIR = ".tokenash"
...
# without uninstalling the binary or unsetting ``TOKENASH_CONTEXT_TOOL``.
# Used for diagnostics and for environments where RTK is intentionally
# excluded from tokenash accounting (e.g. shadow tests).
#
...
_RTK_WIRING_ENV = "TOKENASH_RTK_WIRING"
...
# ``tokenash/proxy/server.py``: a non-blocking ``fcntl.flock`` on
# ``TOKENASH_RTK_POLL_LOCK`` (default under the workspace dir). Only the
...
_RTK_POLL_LOCK_ENV = "TOKENASH_RTK_POLL_LOCK"
...
def _validate_rtk_env_at_startup() -> None:
    ...
    Raises ``ValueError`` loudly if ``TOKENASH_RTK_WIRING`` is set to an
...
class SubscriptionTracker(QuotaTracker):
    ...
    Implements :class:`~tokenash.subscription.base.QuotaTracker` so it can
    be registered with :func:`~tokenash.subscription.base.get_quota_registry`
    ...
    def __init__(
        ...
        # :func:`~tokenash.proxy.helpers._get_context_tool_stats`) so it
        ...
        # entire pre-Tokenash RTK history as one fake delta. Switching to
    ...
    def update_contribution(
        ...
        *,
        ...
        """Update tokenash contribution counters for the current session window.
        ...
        :func:`tokenash.proxy.helpers._get_rtk_stats`) when the caller does
        ...
        reads 0 instead of the entire pre-Tokenash RTK history.
    ...
    def _poll_rtk_delta(self) -> int:
        ...
        :func:`tokenash.proxy.helpers._get_rtk_stats` and reads the
        ...
        - ``TOKENASH_RTK_WIRING=disabled`` — operator opt-out.
        ...
        PR-G2 remediation (H1): ``TOKENASH_RTK_WIRING`` is now validated at
        ...
        try:
            ...
            from tokenash.proxy.helpers import _get_rtk_stats
        ...
        # the pre-Tokenash RTK history. Falls back to the top-level
    ...
    def _try_acquire_rtk_poll_lock(self) -> bool:
        ...
        ``tokenash/proxy/server.py`` (fcntl.flock, LOCK_EX | LOCK_NB).
        """
    ...
    def _compute_used_since_reset(self, window: RateLimitWindow | None) -> int | None:
        ...
        if now < window.resets_at:
            return None
        try:
            from tokenash.subscription import session_tracking
    ...
    async def _maybe_poll(self) -> None:
        ...
        if not is_active:
            ...
            from tokenash.subscription.client import read_cached_oauth_token
        ...
        try:
            from tokenash.observability.metrics import get_otel_metrics
    ...
    def _maybe_reset_contribution(self, snapshot: SubscriptionSnapshot) -> None:
        ...
        if (
            ...
            logger.info("5h window rolled over; resetting tokenash contribution counters")
            self._state.contribution = TokenashContribution()
...
def configure_subscription_tracker(
    ...
    a typo (``TOKENASH_RTK_WIRING=enabld``) crashes the proxy at startup
