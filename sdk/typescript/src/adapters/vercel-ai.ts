 ...
 * ```
 */
export function tokenashMiddleware(options: CompressOptions = {}) {
  return {
...
/**
 * Wrap a Vercel AI SDK language model with Tokenash compression.
 * Convenience wrapper around `wrapLanguageModel` + `tokenashMiddleware`.
 *
 * @example
 * ```typescript
 ...
 * ```
 */
export function withTokenash<T extends LanguageModel>(
  model: T,
  options: CompressOptions = {},
): T {
  ...
  try {
    ...
    throw new Error(
      'withTokenash() requires the "ai" package. Install it with: npm install ai',
    );
  }

  return wrapLanguageModel({
    model,
    middleware: tokenashMiddleware(options),
  });
}
