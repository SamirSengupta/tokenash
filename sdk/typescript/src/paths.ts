/**
 * Canonical filesystem contract for Tokenash — parity shell for the npm SDK.
 *
 * The TypeScript SDK is an HTTP client today and does not touch the
 * filesystem directly. This module mirrors `tokenash/paths.py` so that
 * future local features (e.g. cache/log co-location with the Python
 * proxy) land on the same contract.
 *
 * Two canonical roots:
 *   - TOKENASH_CONFIG_DIR     — read-mostly configuration
 *                               (default: ~/.tokenash/config)
 *   - TOKENASH_WORKSPACE_DIR  — read-write state
 *                               (default: ~/.tokenash)
 *
 ...
 * shape of the API matches Python's `tokenash.paths` module.
 */
...
export const TOKENASH_CONFIG_DIR_ENV = "TOKENASH_CONFIG_DIR";
export const TOKENASH_WORKSPACE_DIR_ENV = "TOKENASH_WORKSPACE_DIR";

export const TOKENASH_SAVINGS_PATH_ENV = "TOKENASH_SAVINGS_PATH";
export const TOKENASH_TOIN_PATH_ENV = "TOKENASH_TOIN_PATH";
export const TOKENASH_SUBSCRIPTION_STATE_PATH_ENV =
...
export function workspaceDir(): string {
  if (!isNode()) return "";
  const envValue = getEnv(TOKENASH_WORKSPACE_DIR_ENV);
  if (envValue) return expandTilde(envValue);
  const home = homeDir();
  if (!home) return "";
  return joinPath(home, ".tokenash");
}

export function configDir(): string {
  if (!isNode()) return "";
  const envValue = getEnv(TOKENASH_CONFIG_DIR_ENV);
  if (envValue) return expandTilde(envValue);
  const workspaceEnv = getEnv(TOKENASH_WORKSPACE_DIR_ENV);
  if (workspaceEnv) {
    return joinPath(expandTilde(workspaceEnv), "config");
  }
  const home = homeDir();
  if (!home) return "";
  return joinPath(home, ".tokenash", "config");
}
