/**
 * TokenashClient — HTTP client for the Tokenash compression proxy.
 *
 * Supports:
...
import { mapProxyError, TokenashConnectionError, TokenashAuthError, TokenashCompressError } from "./errors.js";
import { deepCamelCase, deepSnakeCase } from "./utils/case.js";
import { parseSSE } from "./utils/stream.js";
import type { TokenashConfig, TokenashMode } from "./types/config.js";
import type {
  SimulationResult,
  RequestMetrics,
  MetricsSummary,
...
// --- Tokenash request params ---

export interface TokenashParams {
  tokenashMode?: TokenashMode;
  tokenashCachePrefixTokens?: number;
  tokenashOutputBufferTokens?: number;
  tokenashKeepTurns?: number;
  tokenashToolProfiles?: Record<string, Record<string, any>>;
}

// --- Sub-clients ---

class ChatCompletions {
  constructor(private client: TokenashClient) {}
   ...
   */
  async create(
    params: { model: string; messages: OpenAIMessage[]; stream?: boolean; [key: string]: any } & TokenashParams,
  ): Promise<any> {
    const { tokenashMode, tokenashCachePrefixTokens, tokenashOutputBufferTokens, tokenashKeepTurns, tokenashToolProfiles, ...apiParams } = params;

    const headers: Record<string, string> = {};
    if (tokenashMode) headers["x-tokenash-mode"] = tokenashMode;
   ...
   */
  async simulate(
    params: { model: string; messages: OpenAIMessage[] } & TokenashParams,
...
class Messages {
   ...
   */
  async create(
    params: { model: string; messages: any[]; max_tokens?: number; system?: string | any[]; stream?: boolean; [key: string]: any } & TokenashParams,
  ): Promise<any> {
    const { tokenashMode, tokenashCachePrefixTokens, tokenashOutputBufferTokens, tokenashKeepTurns, tokenashToolProfiles, ...apiParams } = params;
   ...
   */
  stream(
    params: { model: string; messages: any[]; max_tokens?: number; system?: string | any[]; [key: string]: any } & TokenashParams,
   ...
   */
  async simulate(
    params: { model: string; messages: any[] } & TokenashParams,
  ): Promise<SimulationResult> {
    const body = {
      messages: params.messages,
      model: params.model,
...
export interface ExtendedClientOptions extends TokenashClientOptions {
  providerApiKey?: string;
  defaultMode?: TokenashMode;
  config?: TokenashConfig;
}

export class TokenashClient implements TokenashClientInterface {
  private baseUrl: string;
  private apiKey: string | undefined;
  private timeout: number;
  private fallback: boolean;
  private retries: number;
  private config: TokenashConfig | undefined;
  ...
  constructor(options: ExtendedClientOptions = {}) {
    this.baseUrl = (
      options.baseUrl ??
      getEnv("TOKENASH_BASE_URL") ??
      DEFAULT_BASE_URL
    ).replace(/\/+$/, "");
    this.apiKey = options.apiKey ?? getEnv("TOKENASH_API_KEY");
  ...
  async compress(
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      try {
        ...
        lastError = error;
        if (error instanceof TokenashAuthError) throw error;
        if (
          error instanceof TokenashCompressError &&
    ...
    if (lastError instanceof TokenashConnectionError) throw lastError;
    if (lastError instanceof TokenashCompressError) throw lastError;
    throw new TokenashConnectionError(
  ...
  /** Handle an LLM tool call for tokenash_retrieve. */
  ...
  async rawFetch(
    ...
    if (this.stack && !headers["X-Tokenash-Stack"]) {
      headers["X-Tokenash-Stack"] = this.stack;
    }
    ...
    try {
      ...
      throw new TokenashConnectionError(
        `Failed to connect to Tokenash at ${this.baseUrl}: ${error}`,
      );
    }
