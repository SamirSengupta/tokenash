...
describe("TokenashClient", () => {
  ...
  it("sends POST to /v1/compress with messages and model", async () => {
    ...
    const client = new TokenashClient({ baseUrl: "http://localhost:8787" });
  ...
  it("throws TokenashAuthError on 401 (always, even with fallback)", async () => {
    mockFetch.mockResolvedValueOnce(
      errorResponse(401, {
    ...
    ).rejects.toThrow(TokenashAuthError);
  });

  it("throws TokenashCompressError on 400 (always, even with fallback)", async () => {
    mockFetch.mockResolvedValueOnce(
      errorResponse(400, {
    ...
    await expect(
      client.compress(sampleMessages, { model: "gpt-4o" }),
    ).rejects.toThrow(TokenashCompressError);
  });
  ...
  it("throws TokenashConnectionError on network error when fallback=false", async () => {
    mockFetch.mockRejectedValueOnce(new TypeError("fetch failed"));
    ...
    await expect(
      client.compress(sampleMessages, { model: "gpt-4o" }),
    ).rejects.toThrow(TokenashConnectionError);
  });
