/**
 * Tests for Gemini adapter — withTokenash for Google Generative AI.
 */
import { describe, it, expect, vi, beforeEach } from "vitest";
import { withTokenash } from "../../src/adapters/gemini.js";
...
describe("Gemini withTokenash", () => {
  ...
  it("wraps generateContent with compression", async () => {
    ...
    const wrapped = withTokenash(mockModel, { baseUrl: "http://test:8787" });
  ...
  it("uses model option for compression", async () => {
    ...
    const wrapped = withTokenash(mockModel, {
