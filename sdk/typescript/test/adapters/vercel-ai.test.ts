...
describe("tokenashMiddleware", () => {
  beforeEach(() => {
    mockFetch.mockReset();
  });

  it("returns object with transformParams function", () => {
    const mw = tokenashMiddleware({ baseUrl: "http://localhost:8787" });
...
describe("withTokenash", () => {
  ...
  it("returns a LanguageModelV3 with correct provider and modelId", () => {
    ...
    const wrapped = withTokenash(fakeModel as any, {
      baseUrl: "http://localhost:8787",
    });
  ...
  it("passes options through to tokenashMiddleware", async () => {
    mockFetch.mockResolvedValueOnce(
      mockCompressResponse([{ role: "user", content: "x" }]),
    );
