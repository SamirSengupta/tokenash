...
describe("canonical roots", () => {
  ...
  it("workspaceDir defaults to ~/.tokenash", () => {
    expect(workspaceDir()).toBe(path.join(os.homedir(), ".tokenash"));
  });

  it("workspaceDir honors TOKENASH_WORKSPACE_DIR env override", () => {
    process.env[TOKENASH_WORKSPACE_DIR_ENV] = "/tmp/alt_ws";
    expect(workspaceDir()).toBe("/tmp/alt_ws");
  });

  it("workspaceDir ignores blank env value", () => {
    process.env[TOKENASH_WORKSPACE_DIR_ENV] = "   ";
    expect(workspaceDir()).toBe(path.join(os.homedir(), ".tokenash"));
  });

  it("workspaceDir expands tilde", () => {
    process.env[TOKENASH_WORKSPACE_DIR_ENV] = "~/custom-ws";
    expect(workspaceDir()).toBe(path.join(os.homedir(), "custom-ws"));
  });

  it("configDir defaults to ~/.tokenash/config", () => {
    expect(configDir()).toBe(path.join(os.homedir(), ".tokenash", "config"));
  });

  it("configDir follows TOKENASH_WORKSPACE_DIR when only workspace set", () => {
    process.env[TOKENASH_WORKSPACE_DIR_ENV] = "/tmp/alt_ws";
    expect(configDir()).toBe(path.join("/tmp/alt_ws", "config"));
  });

  it("explicit TOKENASH_CONFIG_DIR env beats workspace env", () => {
    process.env[TOKENASH_WORKSPACE_DIR_ENV] = "/tmp/alt_ws";
    process.env[TOKENASH_CONFIG_DIR_ENV] = "/tmp/alt_cfg";
...
const RESOURCES: ResourceCase[] = [
  {
    name: "savingsPath",
    fn: savingsPath,
    envVar: TOKENASH_SAVINGS_PATH_ENV,
    filename: "proxy_savings.json",
  },
  {
    name: "toinPath",
    fn: toinPath,
    envVar: TOKENASH_TOIN_PATH_ENV,
    filename: "toin.json",
  },
  {
    name: "subscriptionStatePath",
    fn: subscriptionStatePath,
    envVar: TOKENASH_SUBSCRIPTION_STATE_PATH_ENV,
...
describe.each(RESOURCES)(
  ...
  ({ fn, envVar, filename }) => {
    ...
    it("default under ~/.tokenash", () => {
      expect(fn()).toBe(path.join(os.homedir(), ".tokenash", filename));
    });

    it("derived from TOKENASH_WORKSPACE_DIR when set", () => {
      process.env[TOKENASH_WORKSPACE_DIR_ENV] = "/tmp/state";
...
describe("derived-only resources", () => {
  ...
  it("memoryDbPath", () => {
    expect(memoryDbPath()).toBe(
      path.join(os.homedir(), ".tokenash", "memory.db"),
    );
  });

  it("nativeMemoryDir", () => {
    expect(nativeMemoryDir()).toBe(
      path.join(os.homedir(), ".tokenash", "memories"),
    );
  });

  it("licenseCachePath", () => {
    expect(licenseCachePath()).toBe(
      path.join(os.homedir(), ".tokenash", "license_cache.json"),
    );
  });

  it("sessionStatsPath", () => {
    expect(sessionStatsPath()).toBe(
      path.join(os.homedir(), ".tokenash", "session_stats.jsonl"),
    );
  });

  it("syncStatePath", () => {
    expect(syncStatePath()).toBe(
      path.join(os.homedir(), ".tokenash", "sync_state.json"),
    );
  });

  it("bridgeStatePath", () => {
    expect(bridgeStatePath()).toBe(
      path.join(os.homedir(), ".tokenash", "bridge_state.json"),
    );
  });

  it("logDir", () => {
    expect(logDir()).toBe(path.join(os.homedir(), ".tokenash", "logs"));
  });

  it("proxyLogPath", () => {
    expect(proxyLogPath()).toBe(
      path.join(os.homedir(), ".tokenash", "logs", "proxy.log"),
    );
  });

  it("debug400Dir", () => {
    expect(debug400Dir()).toBe(
      path.join(os.homedir(), ".tokenash", "logs", "debug_400"),
    );
  });

  it("binDir", () => {
    expect(binDir()).toBe(path.join(os.homedir(), ".tokenash", "bin"));
  });
...
// ---------------------------------------------------------------------------
// Derived-only helpers must follow TOKENASH_WORKSPACE_DIR end-to-end
// ---------------------------------------------------------------------------
...
describe("plugin dirs", () => {
  ...
  it("pluginConfigDir namespaced under configDir/plugins", () => {
    ...
    expect(a).not.toBe(b);
    expect(a).toBe(
      path.join(os.homedir(), ".tokenash", "config", "plugins", "alpha"),
    );
  });
  ...
  it("pluginWorkspaceDir namespaced under workspaceDir/plugins", () => {
    ...
    expect(a).not.toBe(b);
    expect(a).toBe(
      path.join(os.homedir(), ".tokenash", "plugins", "alpha"),
    );
  });
