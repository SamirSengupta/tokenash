...
Runnable examples showing how to use the Tokenash TypeScript SDK for context compression.

## Prerequisites

- Node.js 18+
- A running Tokenash proxy (`pip install "tokenash[proxy]" && tokenash proxy`)
...
| Example | Description |
|---------|-------------|
| [with-tokenash-vercel.ts](with-tokenash-vercel.ts) | One-liner `withTokenash(openai('gpt-4o'))` — simplest integration |
| [streaming-chat.ts](streaming-chat.ts) | `withTokenash` + `streamText` for real-time streaming |
...
| [middleware-composition.ts](middleware-composition.ts) | Stack `tokenashMiddleware` with other middlewares (`extractReasoningMiddleware`) |
...
| Example | Description |
|---------|-------------|
| [openai-anthropic-adapters.ts](openai-anthropic-adapters.ts) | `withTokenash` for native OpenAI and Anthropic SDKs (no Vercel AI SDK) |
