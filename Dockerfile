...
ARG TOKENASH_EXTRAS=proxy,code
RUN --mount=type=cache,target=/root/.cache/uv \
    --mount=type=cache,target=/root/.cargo/registry \
    --mount=type=cache,target=/build/target \
    uv pip install --system ".[${TOKENASH_EXTRAS}]"
...
# site-packages with /build/tokenash/ (which has no _core.so since
# maturin installed the .so into site-packages).
RUN cd /tmp && python -c "from tokenash._core import DiffCompressor, SmartCrusher; \
...
# native `tokenash-proxy` binary lets operators front the Python proxy with
...
RUN --mount=type=cache,target=/usr/local/cargo/registry \
    --mount=type=cache,target=/build/target \
    cargo build --release --locked --bin tokenash-proxy && \
    cp target/release/tokenash-proxy /usr/local/bin/tokenash-proxy
...
COPY --from=builder /usr/local/bin/tokenash /usr/local/bin/tokenash
# Native Rust reverse proxy binary (issue #976).
COPY --from=builder /usr/local/bin/tokenash-proxy /usr/local/bin/tokenash-proxy
...
RUN mkdir -p /home/nonroot /data && \
    if [ "$RUNTIME_USER" = "nonroot" ]; then \
      ...
      mkdir -p /home/nonroot/.tokenash && \
      chown -R nonroot:nonroot /data /home/nonroot; \
    else \
      mkdir -p /root/.tokenash; \
    fi
...
# Declare ~/.tokenash as a volume so Docker (and ACA) can attach persistent
...
ENTRYPOINT ["python3", "-m", "tokenash.cli", "proxy"]
CMD ["--host", "0.0.0.0", "--port", "8787"]
