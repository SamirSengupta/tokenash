...
Utility scripts bundled with the Tokenash repo. Most are one-off operator
...
against a local Tokenash proxy (default `http://127.0.0.1:8787`), as described
...
- Empirically tune the Unit 4 pre-upstream semaphore default
  (`TOKENASH_ANTHROPIC_PRE_UPSTREAM_CONCURRENCY`).
...
- `/livez p99` under threshold means the event loop is not starved during the
  ...
  (`TOKENASH_ANTHROPIC_PRE_UPSTREAM_CONCURRENCY=10000`) and drops back under
