...
IMAGE_DEFAULT="ghcr.io/samirsengupta/tokenash:latest"
INSTALL_IMAGE="${TOKENASH_DOCKER_IMAGE:-${IMAGE_DEFAULT}}"
...
if ((BASH_VERSINFO[0] < 4 || (BASH_VERSINFO[0] == 4 && BASH_VERSINFO[1] < 3))); then
  printf 'ERROR: Tokenash Docker-native install requires bash >= 4.3\n' >&2
  exit 1
fi
...
append_path_block() {
  local target_file="$1"
  local marker_start="# >>> tokenash docker-native >>>"
  local marker_end="# <<< tokenash docker-native <<<"
...
write_wrapper() {
  local wrapper_path="${INSTALL_DIR}/tokenash"

  {
    printf '#!%s\n\n' "${BASH_PATH}"
    printf 'TOKENASH_IMAGE_DEFAULT=%q\n' "${INSTALL_IMAGE}"
    cat <<'WRAPPER'

set -euo pipefail

TOKENASH_IMAGE="${TOKENASH_DOCKER_IMAGE:-${TOKENASH_IMAGE_DEFAULT}}"
TOKENASH_CONTAINER_HOME="${TOKENASH_CONTAINER_HOME:-/tmp/tokenash-home}"
TOKENASH_HOST_HOME="${HOME:?}"

if ((BASH_VERSINFO[0] < 4 || (BASH_VERSINFO[0] == 4 && BASH_VERSINFO[1] < 3))); then
  printf 'ERROR: Tokenash Docker-native wrapper requires bash >= 4.3\n' >&2
  exit 1
fi
...
ensure_host_dirs() {
  mkdir -p \
    "${TOKENASH_HOST_HOME}/.tokenash" \
    "${TOKENASH_HOST_HOME}/.claude" \
    "${TOKENASH_HOST_HOME}/.codex" \
    "${TOKENASH_HOST_HOME}/.gemini"
}
...
append_passthrough_envs() {
  ...
  for name in $(compgen -e); do
    case "${name}" in
      TOKENASH_*|ANTHROPIC_*|OPENAI_*|GEMINI_*|AWS_*|AZURE_*|VERTEX_*|GOOGLE_*|GOOGLE_CLOUD_*|MISTRAL_*|GROQ_*|OPENROUTER_*|XAI_*|TOGETHER_*|COHERE_*|OLLAMA_*|LITELLM_*|OTEL_*|SUPABASE_*|QDRANT_*|NEO4J_*|LANGSMITH_*)
        ref+=(--env "${name}")
        ;;
    esac
  done
...
append_common_container_args() {
  ...
  ensure_host_dirs
  ref+=(-w /workspace)
  ref+=(--env "HOME=${TOKENASH_CONTAINER_HOME}")
  ref+=(--env "PYTHONUNBUFFERED=1")
  # Canonical Tokenash filesystem contract (issue #175) — forward into the
  # container so the proxy resolves state/config to the bind-mounted path.
  ref+=(--env "TOKENASH_WORKSPACE_DIR=${TOKENASH_CONTAINER_HOME}/.tokenash")
  ref+=(--env "TOKENASH_CONFIG_DIR=${TOKENASH_CONTAINER_HOME}/.tokenash/config")
  ref+=(-v "${PWD}:/workspace")
  ref+=(-v "${TOKENASH_HOST_HOME}/.tokenash:${TOKENASH_CONTAINER_HOME}/.tokenash")
  ref+=(-v "${TOKENASH_HOST_HOME}/.claude:${TOKENASH_CONTAINER_HOME}/.claude")
  ref+=(-v "${TOKENASH_HOST_HOME}/.codex:${TOKENASH_CONTAINER_HOME}/.codex")
  ref+=(-v "${TOKENASH_HOST_HOME}/.gemini:${TOKENASH_CONTAINER_HOME}/.gemini")
...
run_tokenash() {
  ...
  args+=(--entrypoint tokenash "${TOKENASH_IMAGE}" "$@")
  "${args[@]}"
}
...
start_proxy_container() {
  ...
  local container_name="tokenash-proxy-${port}-$$"
  local args=()
  args=(docker run -d --rm --name "${container_name}" -p "${port}:${port}")
  append_common_container_args args
  args+=("${TOKENASH_IMAGE}" --host 0.0.0.0 --port "${port}" "$@")
  "${args[@]}" >/dev/null

  if ! wait_for_proxy "${container_name}" "${port}"; then
    docker stop "${container_name}" >/dev/null 2>&1 || true
    die "Tokenash proxy failed to start on port ${port}"
  fi
...
persistent_profile_root() {
  ...
  printf '%s/.tokenash/deploy/%s\n' "${TOKENASH_HOST_HOME}" "${profile}"
}
...
append_persistent_container_args() {
  ...
  ensure_host_dirs
  ref+=(--workdir "${TOKENASH_CONTAINER_HOME}")
  ref+=(--env "HOME=${TOKENASH_CONTAINER_HOME}")
  ref+=(--env "PYTHONUNBUFFERED=1")
  # Canonical Tokenash filesystem contract (issue #175).
  ref+=(--env "TOKENASH_WORKSPACE_DIR=${TOKENASH_CONTAINER_HOME}/.tokenash")
  ref+=(--env "TOKENASH_CONFIG_DIR=${TOKENASH_CONTAINER_HOME}/.tokenash/config")
  ref+=(-v "${TOKENASH_HOST_HOME}/.tokenash:${TOKENASH_CONTAINER_HOME}/.tokenash")
  ref+=(-v "${TOKENASH_HOST_HOME}/.claude:${TOKENASH_CONTAINER_HOME}/.claude")
...
build_manifest_proxy_args() {
  ...
  if [[ "${memory_enabled}" -eq 1 ]]; then
    out_args+=(--memory --memory-db-path "${TOKENASH_CONTAINER_HOME}/.tokenash/memory.db")
  fi
  if [[ -n "${anyllm}" ]]; then
  ...
  cat >"${manifest_path}" <<EOF
{
  ...
  "memory_db_path": "$(json_escape "${TOKENASH_CONTAINER_HOME}/.tokenash/memory.db")",
  "telemetry_enabled": ${telemetry_json},
  "image": "$(json_escape "${image}")",
  "service_name": "tokenash-$(json_escape "${profile}")",
  "container_name": "$(json_escape "$(persistent_container_name "${profile}")")",
  "health_url": "http://127.0.0.1:${port}/readyz",
  "base_env": {
    "TOKENASH_PORT": "${port}",
    "TOKENASH_HOST": "127.0.0.1",
    "TOKENASH_MODE": "$(json_escape "${proxy_mode}")",
    "TOKENASH_BACKEND": "$(json_escape "${backend}")"
  },
  "tool_envs": {},
...
start_persistent_docker_install() {
  ...
  args+=(
    --env "TOKENASH_DEPLOYMENT_PROFILE=${profile}"
    --env "TOKENASH_DEPLOYMENT_PRESET=persistent-docker"
    --env "TOKENASH_DEPLOYMENT_RUNTIME=docker"
    --env "TOKENASH_DEPLOYMENT_SUPERVISOR=none"
    --env "TOKENASH_DEPLOYMENT_SCOPE=user"
  )
  ...
  if ! wait_for_proxy "${container_name}" "${port}"; then
    docker rm -f "${container_name}" >/dev/null 2>&1 || true
    die "Tokenash persistent Docker deployment failed to start on port ${port}"
  fi
...
Usage: tokenash install [OPTIONS] COMMAND [ARGS]...
  ...
  Manage persistent Docker-native Tokenash deployments.

  The Docker-native wrapper currently supports the persistent-docker preset only.
  Use the Python-native `tokenash install` command for persistent-service and
...
Options:
  ...
  --image TEXT                  Docker image to use.  [default: TOKENASH_DOCKER_IMAGE or ghcr.io/samirsengupta/tokenash:latest]
  -?, --help                    Show this message and exit.
EOF
}
...
print_wrap_help() {
  cat <<'EOF'
Usage: tokenash wrap <COMMAND> [OPTIONS] [-- ARGS...]

  Launch supported host tools through a Docker-native Tokenash proxy.
  ...
  while (($#)); do
    case "$1" in
      ...
      --help|-?)
        ...
        ;;
      *)
        die "Unsupported option for 'tokenash install apply': $1"
        ;;
    esac
  done
  ...
  while (($#)); do
    case "$1" in
      ...
      --help|-?)
        ...
        ;;
      *)
        die "Unsupported option for 'tokenash install': $1"
        ;;
    esac
  done
}
...
run_claude_rtk_init() {
  local rtk_bin="${TOKENASH_HOST_HOME}/.tokenash/bin/rtk"
  if [[ ! -x "${rtk_bin}" ]]; then
...
selected_context_tool() {
  local value="${TOKENASH_CONTEXT_TOOL:-rtk}"
  value="${value,,}"
  value="${value//_/-}"
  if [[ -z "${value}" ]]; then
    value="rtk"
  ...
  case "${value}" in
    rtk|lean-ctx)
      ...
      ;;
    *)
      die "TOKENASH_CONTEXT_TOOL must be one of: lean-ctx, rtk"
      ;;
  esac
}
...
run_prepare_only() {
  ...
  append_tty_args args
  append_common_container_args args
  args+=(--env "TOKENASH_RTK_TARGET=$(detect_rtk_target)")
  args+=(--entrypoint tokenash "${TOKENASH_IMAGE}" wrap "${tool}" --prepare-only "$@")
  "${args[@]}"
}
...
parse_openclaw_wrap_args() {
  ...
  out_plugin_path=""
  out_plugin_spec="tokenash/openclaw"
  out_skip_build=0
  out_copy=0
  out_proxy_port=8787
  ...
  while (($#)); do
    case "$1" in
      ...
      --verbose|-v)
        ...
        ;;
      *)
        die "Unsupported option for 'tokenash wrap openclaw': $1"
        ;;
    esac
  done
}
  ...
  while (($#)); do
    case "$1" in
      ...
      --verbose|-v)
        ...
        ;;
      *)
        die "Unsupported option for 'tokenash unwrap openclaw': $1"
        ;;
    esac
  done
}
...
get_openclaw_existing_entry_json() {
  local output=""
  if output="$(openclaw config get plugins.entries.tokenash 2>/dev/null)"; then
    printf '%s' "${output}"
  fi
}
...
prepare_openclaw_entry_json() {
  ...
  args=(docker run --rm)
  append_common_container_args args
  args+=(--entrypoint tokenash "${TOKENASH_IMAGE}" wrap openclaw --prepare-only)
...
prepare_openclaw_unwrap_entry_json() {
  ...
  args=(docker run --rm)
  append_common_container_args args
  args+=(--entrypoint tokenash "${TOKENASH_IMAGE}" unwrap openclaw --prepare-only)
  if [[ -n "${existing_entry_json}" ]]; then
    args+=(--existing-entry-json "${existing_entry_json}")
  fi
  "${args[@]}"
...
copy_openclaw_plugin_into_extensions() {
  ...
  local target_dir="${extensions_dir}/tokenash"
  mkdir -p "${target_dir}"
...
wrap_openclaw_host() {
  ...
  run_openclaw_checked \
    "openclaw config set plugins.entries.tokenash" \
    openclaw config set plugins.entries.tokenash "${entry_json}" --strict-json >/dev/null
  ...
  run_openclaw_checked \
    ...
    openclaw config set plugins.slots.contextEngine '"tokenash"' --strict-json >/dev/null
  ...
  inspect_output="$(run_openclaw_checked "openclaw plugins inspect tokenash" openclaw plugins inspect tokenash)"
  if [[ "${verbose}" -eq 1 && -n "${inspect_output}" ]]; then
    printf '%s\n' "${inspect_output}"
  fi

  printf '\n✓ OpenClaw is configured to use Tokenash context compression.\n'
  printf '  Plugin: tokenash\n'
  printf '  Slot:   plugins.slots.contextEngine = tokenash\n\n'
}
...
unwrap_openclaw_host() {
  ...
  printf '\n  ╔═══════════════════════════════════════════════╗\n'
  printf '  ║          TOKENASH UNWRAP: OPENCLAW            ║\n'
  printf '  ╚═══════════════════════════════════════════════╝\n\n'
  printf '  Disabling Tokenash plugin and removing engine mapping...\n'
  ...
  printf '\n✓ OpenClaw Tokenash wrap removed.\n'
  printf '  Plugin: tokenash (installed, disabled)\n'
...
main() {
  ...
  case "$1" in
    ...
    wrap)
      ...
      (($# >= 2)) || die "Usage: tokenash wrap <claude|codex|aider|cursor|openclaw|opencode> [...]"
      local tool="$2"
      shift 2
      ...
      case "${tool}" in
        ...
        cursor)
          cat <<EOF
Tokenash proxy is running for Cursor.
...
EOF
    ...
    proxy)
      ...
      run_args+=(--entrypoint tokenash "${TOKENASH_IMAGE}" "${args[@]}")
      "${run_args[@]}"
      ;;
    *)
      run_tokenash "$@"
...
main() {
  ...
  if [[ -n "${TOKENASH_DOCKER_IMAGE:-}" ]]; then
    if docker image inspect "${INSTALL_IMAGE}" >/dev/null 2>&1; then
      info "Using existing TOKENASH_DOCKER_IMAGE=${INSTALL_IMAGE}"
    else
...
Tokenash Docker-native install complete.

Installed wrapper:
  ${INSTALL_DIR}/tokenash

Next steps:
  1. Restart your shell or run: export PATH="${INSTALL_DIR}:\$PATH"
  2. Try: tokenash proxy
  3. Docs: https://github.com/SamirSengupta/tokenash/blob/main/docs/docker-install.md
EOF
}
