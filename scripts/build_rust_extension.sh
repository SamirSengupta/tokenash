#!/usr/bin/env bash
# Build + install the Rust extension (tokenash._core) into the active venv.
#
...
# in-tree `tokenash/` directory because the dual-package layout left the
# .so in `crates/tokenash-py/python/tokenash/`. That dance is no longer
...
# wheel contains both the Python source and the compiled `tokenash/_core.so`,
...
log "verifying \`from tokenash._core import DiffCompressor, SmartCrusher\`"
python -c '
import sys
try:
    from tokenash._core import DiffCompressor, SmartCrusher
...
log "tokenash._core build + install + verify: OK"
