...
Sync now only runs when EITHER:
  ...
  * ``TOKENASH_SYNC_VERSIONS=1`` is set explicitly (release workflow)
...
try:
    from tokenash.release_version import (  # noqa: E402
    ...
    print("skip: tokenash deps not installed (run from a dev venv to enable)")
    sys.exit(0)
...
def _should_sync(root: Path) -> bool:
    ...
    Release workflow opts in via ``TOKENASH_SYNC_VERSIONS=1``; otherwise
    ...
    """
    if os.environ.get("TOKENASH_SYNC_VERSIONS") == "1":
        return True
...
def main() -> None:
    ...
    if not _should_sync(root):
        ...
        print(
            f"sync-plugin-versions: skipping on branch '{branch}' (set TOKENASH_SYNC_VERSIONS=1 to force)"
        )
        return
