#!/usr/bin/env python
"""Export a Kompress PyTorch checkpoint to ONNX INT8 for Tokenash's light path.

Why this exists
---------------
Tokenash's ``[proxy]`` extra ships ``onnxruntime`` but **not** torch — the
proxy runs Kompress text compression on ONNX Runtime alone. The loader
(``tokenash/transforms/kompress_compressor.py``) downloads
...
(``model.safetensors`` / ``merged.pt``) — no ONNX. So pointing Tokenash at v2
...
Requires
--------
    pip install tokenash[ml] onnxruntime    # torch + transformers + onnxruntime
...
def _build_core(model_id: str):
    """Instantiate TokenashCompressorModel and load the merged v2 weights.

    The v2 repo's ``model.safetensors`` is the *unmerged* PEFT structure
    (``encoder.base_model.model...`` with separate ``base_layer`` + LoRA
    adapters), which does not map onto ``TokenashCompressorModel``. The
    ...
    from tokenash.transforms.kompress_compressor import _get_model_class
...
def upload(model_id: str, out_path: Path) -> None:
    ...
    upload_file(
        ...
        repo_id=model_id,
        commit_message="Add ONNX export for Tokenash lightweight (no-torch) path",
    )
    logger.info("Uploaded. Tokenash's ONNX loader will now find it on next cold start.")
