...
$ImageDefault = 'ghcr.io/samirsengupta/tokenash:latest'
$InstallImage = if ($env:TOKENASH_DOCKER_IMAGE) { $env:TOKENASH_DOCKER_IMAGE } else { $ImageDefault }
...
function Ensure-ProfileBlock {
    ...
    $markerStart = '# >>> tokenash docker-native >>>'
    $markerEnd = '# <<< tokenash docker-native <<<'
...
function Write-Wrapper {
    ...
    $wrapperPath = Join-Path $TargetDir 'tokenash.ps1'
    $cmdPath = Join-Path $TargetDir 'tokenash.cmd'
    $resolvedInstallImage = $InstallImage.Replace("'", "''")

    $wrapper = @'
$ErrorActionPreference = 'Stop'

$TokenashImage = if ($env:TOKENASH_DOCKER_IMAGE) { $env:TOKENASH_DOCKER_IMAGE } else { '__TOKENASH_INSTALL_IMAGE__' }
$ContainerHome = if ($env:TOKENASH_CONTAINER_HOME) { $env:TOKENASH_CONTAINER_HOME } else { '/tmp/tokenash-home' }
$HostHome = $HOME
...
function Get-PassthroughEnvArgs {
    ...
    $prefixes = @(
        'TOKENASH_','ANTHROPIC_','OPENAI_','GEMINI_','AWS_','AZURE_','VERTEX_',
...
function Get-SharedDockerArgs {
    ...
    $args.Add('--env')
    $args.Add("TOKENASH_WORKSPACE_DIR=$ContainerHome/.tokenash")
    $args.Add('--env')
    $args.Add("TOKENASH_CONFIG_DIR=$ContainerHome/.tokenash/config")
    $args.Add('--volume')
    $args.Add("${PWD}:/workspace")
    $args.Add('--volume')
    $args.Add((Join-Path $HostHome '.tokenash') + ":$ContainerHome/.tokenash")
    $args.Add('--volume')
...
function Invoke-TokenashDocker {
    ...
    $dockerArgs.Add('tokenash')
    $dockerArgs.Add($TokenashImage)
...
function Wait-Proxy {
    ...
    throw "Tokenash proxy failed to start on port $Port"
}
...
function Start-ProxyContainer {
    ...
    $containerName = "tokenash-proxy-$Port-$PID"
    $dockerArgs = New-Object System.Collections.Generic.List[string]
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to start Tokenash proxy container"
    }
...
function Get-PersistentProfileRoot {
    ...
    return Join-Path (Join-Path $HostHome '.tokenash\deploy') $Profile
}
...
function Get-ManifestProxyArgs {
    ...
    if ($Memory) {
        $args.AddRange([string[]]@('--memory','--memory-db-path',"$ContainerHome/.tokenash/memory.db"))
    }
    if ($AnyllmProvider) {
...
function Write-PersistentManifest {
    ...
    $manifest = [ordered]@{
        ...
        proxy_mode = $Mode
        memory_enabled = $Memory
        memory_db_path = "$ContainerHome/.tokenash/memory.db"
        telemetry_enabled = $TelemetryEnabled
        image = $Image
        service_name = "tokenash-$Profile"
        container_name = Get-PersistentContainerName -Profile $Profile
        health_url = "http://127.0.0.1:$Port/readyz"
        base_env = $baseEnv
        tool_envs = @{}
...
function Parse-InstallApplyArgs {
    ...
    while ($i -lt $Arguments.Count) {
        ...
        switch -Regex ($arg) {
            ...
            default {
                Fail "Unsupported option for 'tokenash install apply': $arg"
            }
        }
    }
...
function Parse-InstallProfileArgs {
    ...
    while ($i -lt $Arguments.Count) {
        ...
        switch -Regex ($arg) {
            ...
            default {
                Fail "Unsupported option for 'tokenash install': $arg"
            }
        }
    }
...
function Invoke-ClaudeRtkInit {
    $rtkPath = Join-Path $HostHome '.tokenash\bin\rtk.exe'
...
function Get-ContextTool {
    $value = $env:TOKENASH_CONTEXT_TOOL
    ...
    if ($value -ne 'rtk' -and $value -ne 'lean-ctx') {
        Fail 'TOKENASH_CONTEXT_TOOL must be one of: lean-ctx, rtk'
    }
    return $value
}
...
function Parse-OpenClawWrapArgs {
    ...
    while ($i -lt $Arguments.Count) {
        ...
        switch -Regex ($arg) {
            ...
            default {
                Fail "Unsupported option for 'tokenash wrap openclaw': $arg"
            }
        }
    }
...
function Parse-OpenClawUnwrapArgs {
    ...
    while ($i -lt $Arguments.Count) {
        ...
        switch -Regex ($arg) {
            ...
            default {
                Fail "Unsupported option for 'tokenash unwrap openclaw': $arg"
            }
        }
    }
...
function Get-OpenClawExistingEntryJson {
    $output = (& openclaw config get plugins.entries.tokenash 2>$null | Out-String).Trim()
    if ($LASTEXITCODE -ne 0) {
        return $null
    }
    return $output
...
function Copy-OpenClawPluginIntoExtensions {
    ...
    $targetDir = Join-Path $extensionsDir 'tokenash'
...
function Invoke-OpenClawWrap {
    ...
    Write-Host ""
    Write-Host "  ╔═══════════════════════════════════════════════╗"
    Write-Host "  ║           TOKENASH WRAP: OPENCLAW             ║"
    Write-Host "  ╚═══════════════════════════════════════════════╝"
    Write-Host ""
    if ($parsed.PluginPath) {
    ...
    [void](Invoke-CapturedCommand -Action 'openclaw config set plugins.entries.tokenash' -Command 'openclaw' -Arguments @('config','set','plugins.entries.tokenash',$entryJson,'--strict-json'))
    Write-Host '  Installing OpenClaw plugin with required unsafe-install flag...'
    Install-OpenClawPlugin -Parsed $parsed
    [void](Invoke-CapturedCommand -Action 'openclaw config set plugins.slots.contextEngine' -Command 'openclaw' -Arguments @('config','set','plugins.slots.contextEngine','"tokenash"','--strict-json'))
    ...
    $inspectOutput = Invoke-CapturedCommand -Action 'openclaw plugins inspect tokenash' -Command 'openclaw' -Arguments @('plugins','inspect','tokenash')
    if ($parsed.Verbose -and $inspectOutput) {
        Write-Host $inspectOutput
    }

    Write-Host ""
    Write-Host "✓ OpenClaw is configured to use Tokenash context compression."
    Write-Host "  Plugin: tokenash"
    Write-Host "  Slot:   plugins.slots.contextEngine = tokenash"
    Write-Host ""
}
...
function Invoke-OpenClawUnwrap {
    ...
    Write-Host ""
    Write-Host "  ╔═══════════════════════════════════════════════╗"
    Write-Host "  ║          TOKENASH UNWRAP: OPENCLAW            ║"
    Write-Host "  ╚═══════════════════════════════════════════════╝"
    Write-Host ""
    Write-Host '  Disabling Tokenash plugin and removing engine mapping...'
    ...
    Write-Host ""
    Write-Host "✓ OpenClaw Tokenash wrap removed."
    Write-Host "  Plugin: tokenash (installed, disabled)"
...
function Invoke-PrepareOnly {
    ...
    $dockerArgs.AddRange((Get-SharedDockerArgs))
    $dockerArgs.Add('--env')
    $dockerArgs.Add("TOKENASH_RTK_TARGET=$(Get-RtkTarget)")
    $dockerArgs.Add('--entrypoint')
    $dockerArgs.Add('tokenash')
    $dockerArgs.Add($TokenashImage)
...
if ($args.Count -eq 0) {
    Invoke-TokenashDocker -Arguments @('--help')
    exit 0
}
...
switch ($args[0]) {
    ...
    'wrap' {
        ...
        if ($args.Count -lt 2) {
            Fail 'Usage: tokenash wrap <claude|codex|aider|cursor|openclaw|opencode> [...]'
        }
        ...
        if ($tool -eq 'openclaw') {
            if (Test-HelpFlag -Arguments $wrapArgs) {
                ...
                Invoke-TokenashDocker -Arguments $helpArgs
                exit 0
            }
        ...
        try {
            ...
            switch ($tool) {
                ...
                'cursor' {
                    Write-Host "Tokenash proxy is running for Cursor."
                    Write-Host ""
    ...
    'unwrap' {
        if ($args.Count -eq 1 -or $args[1] -eq '--help' -or $args[1] -eq '-?') {
            Invoke-TokenashDocker -Arguments @('unwrap','--help')
            exit 0
        }
        ...
        Invoke-TokenashDocker -Arguments $args
    }
    'proxy' {
        $port = 8787
...
'@
    ...
    $wrapper = $wrapper.Replace('__TOKENASH_INSTALL_IMAGE__', $resolvedInstallImage)

    $cmdWrapper = ([string][char]64) + "echo off`r`npowershell -NoLogo -NoProfile -ExecutionPolicy Bypass -File ""%~dp0tokenash.ps1"" %*`r`n"
...
if ($env:TOKENASH_DOCKER_IMAGE) {
    $null = docker image inspect $InstallImage 2>$null
    if ($LASTEXITCODE -eq 0) {
        Write-Info "Using existing TOKENASH_DOCKER_IMAGE=$InstallImage"
    } else {
...
Write-Host ""
Write-Host "Tokenash Docker-native install complete."
Write-Host ""
Write-Host "Installed wrappers:"
Write-Host "  $InstallDir\tokenash.ps1"
Write-Host "  $InstallDir\tokenash.cmd"
Write-Host ""
Write-Host "Next steps:"
Write-Host "  1. Restart PowerShell"
Write-Host "  2. Try: tokenash proxy"
Write-Host "  3. Docs: https://github.com/SamirSengupta/tokenash/blob/main/docs/docker-install.md"
