...
RTK_MARKER = "<!-- tokenash:rtk-instructions -->"
PROXY_PORT = 28887
CODEX_PORT = 28888
AIDER_PORT = 28889
CURSOR_PORT = 28890
...
def create_shims(shim_dir: Path) -> None:
    generic_shim = textwrap.dedent(
        ...
        log_dir = Path(os.environ["TOKENASH_E2E_LOG_DIR"])
    ...
    codex_shim = textwrap.dedent(
        ...
        if openai_base:
            ...
            chat_status, chat_body = request_json(
                ...
                payload={
                    ...
                    "messages": [
                        {
                            "role": "user",
                            "content": "Confirm Tokenash received this wrapped Codex message.",
                        }
                    ],
                },
                headers=auth_headers,
            ...
            record["tokenash_request_total"] = (
                requests.get("total") if isinstance(requests, dict) else None
            )
            record["tokenash_model_count"] = (
...
def start_proxy(port: int, env: dict[str, str]) -> subprocess.Popen[str]:
    log(f"Starting tokenash proxy on port {port}")
    proc = subprocess.Popen(
        ["tokenash", "proxy", "--host", "127.0.0.1", "--port", str(port)],
        env=env,
...
def verify_installs() -> None:
    ...
    for tool in ("tokenash", "codex", "aider", "openclaw"):
        assert_true(shutil.which(tool) is not None, f"Expected '{tool}' on PATH")
    run(["tokenash", "--help"], timeout=30)
...
def prepare_local_openclaw_plugin(base_env: dict[str, str], tmp_dir: Path) -> Path:
    ...
    package_json["dependencies"]["tokenash"] = f"file:{tarball_path.as_posix()}"
...
def verify_codex_wrap(
    ...
    port = CODEX_PORT
    run(
        ["tokenash", "wrap", "codex", "--port", str(port), "--", "--help"],
        env=base_env,
        cwd=project_dir,
        timeout=120,
    )
    ...
    assert_true(
        ...
        "Codex wrap should inject the tokenash provider base_url",
    )
    assert_true(
        'env_key = "OPENAI_API_KEY"' not in config,
        "Codex wrap should preserve OAuth and never inject env_key",
    )
    # Bug 3 (#406): requires_openai_auth must be absent from tokenash provider blocks.
    assert_true(
        "requires_openai_auth" not in config,
        "Codex wrap must NOT inject requires_openai_auth into the tokenash provider block",
    )
    assert_true(
    ...
    assert_true(
        ...
        "Codex shim should prove OPENAI_BASE_URL points at a live proxy and that Tokenash logged the wrapped message",
    )
    assert_true(
        entries[-1].get("chat_completion") == "mock completion from upstream",
        "Codex wrap should receive the mock upstream completion through Tokenash",
    )
    assert_true(
        entries[-1].get("tokenash_model_count", 0) >= 1,
        "Codex wrap should appear in Tokenash request stats",
    )
    assert_true(
        any(
            item["path"] == "/v1/chat/completions"
            and isinstance(item.get("body"), dict)
            and item["body"].get("model") == "tokenash-wrap-e2e"
            for item in mock_server.requests
        ),
        "Codex wrap should forward the wrapped message upstream through Tokenash",
    )
...
def verify_claude_wrap(base_env: dict[str, str], project_dir: Path, log_dir: Path) -> None:
    port = PROXY_PORT + 10
    run(
        ["tokenash", "wrap", "claude", "--port", str(port), "--", "--help"],
        env=base_env,
        cwd=project_dir,
        timeout=120,
    )
...
def verify_aider_wrap(base_env: dict[str, str], project_dir: Path, log_dir: Path) -> None:
    port = AIDER_PORT
    run(
        ["tokenash", "wrap", "aider", "--port", str(port), "--", "--help"],
        env=base_env,
        cwd=project_dir,
        timeout=120,
    )
...
def verify_cursor_wrap(base_env: dict[str, str], project_dir: Path) -> None:
    port = CURSOR_PORT
    proc = subprocess.Popen(
        ["tokenash", "wrap", "cursor", "--port", str(port)],
        env=base_env,
        cwd=str(project_dir),
        stdout=subprocess.PIPE,
        stderr=subprocess.STDOUT,
...
def verify_cline_wrap(base_env: dict[str, str], project_dir: Path) -> None:
    ...
    run(
        ["tokenash", "wrap", "cline", "--prepare-only", "--port", str(CLINE_PORT)],
        env=base_env,
        cwd=project_dir,
        timeout=60,
    )
...
def verify_continue_wrap(base_env: dict[str, str], project_dir: Path) -> None:
    ...
    run(
        ["tokenash", "wrap", "continue", "--prepare-only", "--port", str(CONTINUE_PORT)],
        env=base_env,
        cwd=project_dir,
        timeout=60,
    )
...
def verify_goose_wrap(base_env: dict[str, str], project_dir: Path) -> None:
    ...
    run(
        ["tokenash", "wrap", "goose", "--prepare-only", "--port", str(GOOSE_PORT)],
        env=base_env,
        cwd=project_dir,
        timeout=60,
    )
...
def verify_openhands_wrap(base_env: dict[str, str], project_dir: Path) -> None:
    ...
    """
    run(
        ["tokenash", "wrap", "openhands", "--prepare-only", "--port", str(OPENHANDS_PORT)],
        env=base_env,
        cwd=project_dir,
        timeout=60,
    )
...
def verify_openclaw_wrap(
    ...
    try:
        ...
        entry = state["plugins"]["entries"]["tokenash"]
        ...
        assert_true(
            state["plugins"]["slots"]["contextEngine"] == "tokenash",
    ...
    run(["tokenash", "unwrap", "openclaw"], env=base_env, cwd=project_dir, timeout=120)
    state = json.loads(config_path.read_text(encoding="utf-8"))
    assert_true(
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(
        prefix="tokenash-wrap-e2e-", ignore_cleanup_errors=True
    ) as tmp_dir_str:
        ...
        base_env.update(
            {
                ...
                "TOKENASH_E2E_LOG_DIR": str(log_dir),
...
def verify_opencode_wrap(base_env: dict[str, str], project_dir: Path, log_dir: Path) -> None:
    port = OPENCODE_PORT
    run(
        ["tokenash", "wrap", "opencode", "--port", str(port), "--", "--help"],
        env=base_env,
        cwd=project_dir,
        timeout=120,
    )
    ...
    assert_true(
        config["provider"]["tokenash"]["options"]["baseURL"] == f"http://127.0.0.1:{port}/v1",
        "Opencode wrap should inject tokenash provider baseURL",
    )

    run(
        ["tokenash", "unwrap", "opencode", "--port", str(port)],
        env=base_env,
        cwd=project_dir,
        timeout=120,
    )
    ...
    if config_path.exists():
        ...
        assert_true(
            "tokenash" not in content,
            "Opencode unwrap should remove tokenash provider from config",
        )
