"""Docker e2e cases for ``tokenash init``.
...
1. **existing sequence**: preserves the original scenario that exercised
   ``tokenash init claude`` (local) -> ``init -g copilot`` (global) ->
...
from tokenash.cli import init as init_cli  # noqa: E402
...
def _read_manifest(home: Path, profile: str) -> dict[str, object]:
    path = home / ".tokenash" / "deploy" / profile / "manifest.json"
    if not path.exists():
...
def _verify_claude_local(ctx: CaseContext) -> None:
    ...
    # `init` auto-registers the tokenash MCP server after the marketplace
    # install (see d9d8972 — keeps `[Retrieve more: hash=…]` markers from
    # being dead pointers for users who never ran `tokenash mcp install`).
    # The `-e TOKENASH_PROXY_URL=…` arg is only emitted when the proxy
    # port differs from the 8787 default; this case uses --port 9011.
    expected = [
        ["plugin", "marketplace", "add", str(REPO_ROOT_IN_CONTAINER)],
        ["plugin", "install", "tokenash@tokenash-marketplace", "--scope", "local"],
        [
            "mcp",
            "add",
            "tokenash",
            ...
            "user",
            "-e",
            "TOKENASH_PROXY_URL=http://127.0.0.1:9011",
            "--",
            "tokenash",
            "mcp",
            "serve",
...
def _verify_copilot_global(ctx: CaseContext) -> None:
    ...
    expected = [
        ["plugin", "marketplace", "add", str(REPO_ROOT_IN_CONTAINER)],
        ["plugin", "install", "tokenash@tokenash-marketplace"],
    ]
...
def _verify_codex_local(ctx: CaseContext) -> None:
    ...
    if "requires_openai_auth" in config:
        raise AssertionError(
            "Codex local init must NOT inject requires_openai_auth into the tokenash provider block"
        )
...
def _verify_codex_global(ctx: CaseContext) -> None:
    ...
    if "requires_openai_auth" in config:
        raise AssertionError(
            "Codex global init must NOT inject requires_openai_auth into the tokenash provider block"
        )
    if "supports_websockets = true" not in config:
...
def bare_init_g_cases() -> list[Case]:
    """Bare ``tokenash init -g`` — the direct coverage of issue #245."""
...
def per_subcommand_cases() -> list[Case]:
    """One case per ``tokenash init -g <agent>`` with only that agent's shim."""
    ...
    return [
        ...
        # openclaw delegates to `tokenash wrap openclaw` which has its own
