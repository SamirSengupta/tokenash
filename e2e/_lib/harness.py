...
* invoking the ``tokenash`` subprocess with the case's argv
...
class Case:
    ...
    Attributes:
        ...
        argv: Arguments passed to ``tokenash`` (e.g. ``["init", "-g", "claude"]``).
...
def _resolve_tokenash_bin(name: str) -> str:
    """Return the absolute path to the tokenash binary before PATH is scrubbed.

    ``with_clean_path`` intentionally narrows PATH so agent shims dominate;
    that would also hide the real ``tokenash`` binary (typically at
    ``/opt/*venv/bin/tokenash`` or similar). Resolving up-front lets the
...
def _run_single(case: Case, tokenash_bin: str = "tokenash") -> bool:
    """Execute one case. Return True on pass, False on fail."""

    with tempfile.TemporaryDirectory(prefix=f"tokenash-e2e-{case.name}-") as temp_raw:
        ...
        # Resolve tokenash to its absolute path BEFORE mutating PATH so the
        # shim dir can dominate PATH without losing the tokenash binary.
        resolved_bin = _resolve_tokenash_bin(tokenash_bin)

        with with_clean_path([shim_dir]) as env:
            env["HOME"] = str(home)
            env["USERPROFILE"] = str(home)
            env["TOKENASH_E2E_SHIM_LOG"] = str(shim_log)
...
def run_cases(
    ...
    *,
    ...
    failed = 0
    for case in cases:
        ok = _run_single(case, tokenash_bin=tokenash_bin)
        if ok:
            passed += 1
        else:
            failed += 1
...
def run_case_sequence(
    ...
    *,
    ...
    ``tokenash init`` accumulating targets in a shared manifest across
    successive calls).
    """

    passed = 0
    failed = 0
    with tempfile.TemporaryDirectory(prefix=f"tokenash-e2e-{label}-") as temp_raw:
        temp_root = Path(temp_raw)
        home = temp_root / "home"
        project = temp_root / "project"
        shim_dir = temp_root / "bin"
