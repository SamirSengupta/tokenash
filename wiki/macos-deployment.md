...
This guide covers deploying the tokenash proxy server as a background service on macOS using LaunchAgent. The service will start automatically on login and restart on crash.
...
- tokenash installed with proxy support
- Anthropic API key configured

### Installing Tokenash with Proxy Support
...
```bash
# Clone or navigate to tokenash repository
...
1. Detect your tokenash installation
...
```bash
cd examples/deployment/macos-launchagent
cp com.tokenash.proxy.plist.template ~/Library/LaunchAgents/com.tokenash.proxy.plist
```

Edit `~/Library/LaunchAgents/com.tokenash.proxy.plist`:

1. Replace `__TOKENASH_PATH__` with your tokenash path:

   ```bash
   command -v tokenash
   # Example output: /usr/local/bin/tokenash
   ```
...
```bash
launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/com.tokenash.proxy.plist
```
...
launchctl print gui/$(id -u)/com.tokenash.proxy
...
3. Update shell integration: `export TOKENASH_PORT=9000`

### Log Location

Logs are written to standard macOS locations:

- **Standard output**: `~/Library/Logs/tokenash/proxy.log`
- **Error output**: `~/Library/Logs/tokenash/proxy-error.log`
...
(`TOKENASH_COMPRESSION_PROVIDER=llmlingua`, `TOKENASH_LLMLINGUA_DEVICE`,
the `tokenash[llmlingua]` extra) were retired with the
...
source /path/to/tokenash/examples/deployment/macos-launchagent/shell-integration.sh
```
...
tail -f ~/Library/Logs/tokenash/proxy.log

# Tail error output
tail -f ~/Library/Logs/tokenash/proxy-error.log

# View last 50 lines
tail -n 50 ~/Library/Logs/tokenash/proxy-error.log
```
...
launchctl kickstart -k gui/$(id -u)/com.tokenash.proxy

# Manual stop/start
launchctl bootout gui/$(id -u)/com.tokenash.proxy
launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/com.tokenash.proxy.plist
```
...
launchctl disable gui/$(id -u)/com.tokenash.proxy

# Re-enable
launchctl enable gui/$(id -u)/com.tokenash.proxy
```
...
Should show tokenash listening on port 8787.
...
```bash
tail -n 20 ~/Library/Logs/tokenash/proxy-error.log
```
...
| `ModuleNotFoundError: No module named 'tokenash'` | Install: `pip install tokenash[proxy]` |
| `command not found: tokenash` | Update plist with correct path: `command -v tokenash` |
...
- Missing dependencies: `pip install tokenash[proxy]`
...
```bash
# Should be set to 1
echo $TOKENASH_SHELL_INTEGRATION_LOADED
```
...
```bash
grep -A1 RunAtLoad ~/Library/LaunchAgents/com.tokenash.proxy.plist
```
...
```bash
chmod 644 ~/Library/LaunchAgents/com.tokenash.proxy.plist
```

**Verify ownership:**

```bash
ls -l ~/Library/LaunchAgents/com.tokenash.proxy.plist
```
...
# Remove plist
rm ~/Library/LaunchAgents/com.tokenash.proxy.plist

# Remove logs (optional)
rm -rf ~/Library/Logs/tokenash
```
...
- [Architecture](ARCHITECTURE.md) - How Tokenash works internally
...
cp com.tokenash.proxy.plist.template ~/Library/LaunchAgents/com.tokenash.proxy-2.plist
# Edit: Change Label to com.tokenash.proxy-2, port to 8788
launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/com.tokenash.proxy-2.plist
```
...
```bash
pip install 'tokenash[pytorch-mps]'   # also works as [pytorch_mps]
export TOKENASH_EMBEDDER_RUNTIME=pytorch_mps
```
...
```xml
<key>TOKENASH_EMBEDDER_RUNTIME</key>
...
**Q: Why LaunchAgent instead of running `tokenash proxy` manually?**
...
A: Yes, fully compatible. ML compression (Kompress, opt-in via `tokenash[ml]`) auto-detects MPS on Apple Silicon.
