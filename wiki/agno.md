...
Tokenash integrates with [Agno](https://github.com/agno-agi/agno) (formerly Phidata) to provide automatic context optimization for AI agents. This guide covers model wrapping, observability hooks, and multi-provider support.
...
```bash
pip install "tokenash[agno]"
```

This installs Tokenash with Agno support. You'll also need Agno itself:
...
# Wrap your model
model = TokenashAgnoModel(OpenAIChat(id="gpt-4o"))
...
The simplest integration - wrap any Agno model with `TokenashAgnoModel`:
...
openai_model = TokenashAgnoModel(OpenAIChat(id="gpt-4o"))
claude_model = TokenashAgnoModel(Claude(id="claude-3-5-sonnet-20241022"))
gemini_model = TokenashAgnoModel(Gemini(id="gemini-2.0-flash"))

# Each automatically uses the correct provider for accurate token counting
```

**Why this matters**: Tokenash automatically detects the underlying provider and applies the correct tokenizer for accurate optimization metrics.
...
pre_hook = TokenashPreHook()
post_hook = TokenashPostHook(token_alert_threshold=10000)
...
Use `create_tokenash_hooks()` to create matched hook pairs:
...
Pass a `TokenashConfig` for fine-grained control:
...
fast_model = TokenashAgnoModel(OpenAIChat(id="gpt-4o-mini"))
powerful_model = TokenashAgnoModel(Claude(id="claude-3-5-sonnet-20241022"))
...
TokenashAgnoModel automatically detects the provider from the wrapped model:
...
TokenashAgnoModel optimizes messages at the LLM call boundary. This covers:
...
Contributions welcome! See [CONTRIBUTING.md](https://github.com/SamirSengupta/tokenash/blob/main/CONTRIBUTING.md).
...
| `config` | `TokenashConfig` | `None` | Custom configuration |
...
| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `config` | `TokenashConfig` | `None` | Configuration (for future use) |
...
| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `config` | `TokenashConfig` | `None` | Config for pre-hook |
...
Returns: `tuple[TokenashPreHook, TokenashPostHook]`
...
from tokenash.integrations.agno import get_tokenash_provider
from tokenash.integrations.agno import get_model_name_from_agno
...
```python
from tokenash.integrations.agno import get_tokenash_provider, get_model_name_from_agno

model = OpenAIChat(id="gpt-4o")
provider = get_tokenash_provider(model)
