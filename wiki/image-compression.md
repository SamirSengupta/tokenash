...
Tokenash automatically compresses images in your LLM requests, reducing token usage by **40-90%** while maintaining answer accuracy.
...
Tokenash's image compression uses a **trained ML router** to analyze your query and automatically select the optimal compression technique:
...
### With Tokenash Proxy (Zero Code Changes)
...
client = TokenashClient(provider="openai")
...
tokenash proxy --image-optimize

# Disable image compression
tokenash proxy --no-image-optimize
```
...
compressor = ImageCompressor(
    model_id="SamirSengupta/technique-router",  # HuggingFace model
...
- **Model**: `SamirSengupta/technique-router` on HuggingFace
- **Size**: ~128MB
...
class ImageCompressor:
    def __init__(
        self,
        model_id: str = "SamirSengupta/technique-router",
