...
You don't need to run the Tokenash proxy. Tokenash is a compression library that works with **any** LLM client, proxy, or framework.
...
| Claude Code / Cursor / Copilot CLI | [Tokenash proxy](#proxy) | 1 command or env var |
| Agno agents | [Agno integration](#agno) | Wrap model |
| LangChain | [LangChain integration](#langchain) | Wrap model |
| Non-Python app | [Tokenash proxy](#proxy) | HTTP |
| TypeScript SDK | [`compress()`](#typescript-sdk) | `npm install tokenash` |
| Vercel AI SDK | [`tokenashMiddleware()`](#typescript-sdk) | Middleware adapter |
| OpenAI Node SDK | [`withTokenash()`](#typescript-sdk) | Client wrapper |
| Anthropic TS SDK | [`withTokenash()`](#typescript-sdk) | Client wrapper |
...
If you're already using LiteLLM as your LLM gateway, add Tokenash as a callback:

```python
import litellm
from tokenash.integrations.litellm_callback import TokenashCallback

litellm.callbacks = [TokenashCallback()]
...
from tokenash.integrations.asgi import CompressionMiddleware
...
# litellm_config.yaml
litellm_settings:
  callbacks: ["tokenash.integrations.litellm_callback.TokenashCallback"]
```
...
- `x-tokenash-compressed: true` — compression was applied
- `x-tokenash-tokens-saved: 1234` — tokens removed

---

## Proxy

The Tokenash proxy is a standalone HTTP server. Best for non-Python apps or tools that only support base URL configuration (Claude Code, Cursor, GitHub Copilot CLI).
...
For translated backends, the Copilot wrapper can switch to Tokenash's OpenAI-compatible route:

```bash
tokenash wrap copilot --backend anyllm --anyllm-provider groq -- --model gpt-4o
```

By default, `tokenash wrap copilot` installs `rtk` and appends token-optimized shell guidance to `.github/copilot-instructions.md` so Copilot sessions reuse the same command-saving conventions as other wrapped agent CLIs. Use `--no-rtk` to skip that step.

For Copilot's **hosted** API (`--subscription` and the implicit OAuth path), Tokenash routes to the generic host `https://api.githubcopilot.com`, which serves the full model set. **Enterprise / data-residency** tenants on a dedicated Copilot host pin it with `GITHUB_COPILOT_API_URL` (e.g. `export GITHUB_COPILOT_API_URL=https://api.<your-host>.githubcopilot.com`); the override flows through to the upstream request. See [`TESTING-copilot-subscription.md`](https://github.com/SamirSengupta/tokenash/blob/main/TESTING-copilot-subscription.md).

### With Cloud Providers

```bash
# AWS Bedrock
tokenash proxy --backend bedrock --region us-east-1

# Google Vertex AI
tokenash proxy --backend vertex_ai --region us-central1

# Azure OpenAI
tokenash proxy --backend azure

# OpenRouter (400+ models)
OPENROUTER_API_KEY=sk-or-... tokenash proxy --backend openrouter
```
...
model = TokenashAgnoModel(Claude(id="claude-sonnet-4-20250514"))
...
```json
{ "plugins": { "slots": { "contextEngine": "tokenash" } } }
```
...
openclaw plugins install --dangerously-force-unsafe-install tokenash/openclaw
```

The plugin auto-detects a running Tokenash proxy or starts one. Compression happens in `assemble()` — zero changes to the agent's behavior.

See the [OpenClaw plugin documentation](https://github.com/SamirSengupta/tokenash/tree/main/plugins/openclaw) for full setup.

---

## Compression Hooks (Advanced)

Customize compression behavior without modifying Tokenash's code:

```python
from tokenash import compress, CompressionHooks, CompressContext
...
**Q: Does Tokenash change the response format?**
No. Your LLM returns the same response format. Tokenash only modifies the input messages.

**Q: What if compression removes something the LLM needs?**
Tokenash stores originals in CCR (Compress-Cache-Retrieve). The LLM can call `tokenash_retrieve` to get full uncompressed content. Compression summaries tell the LLM what's available.
