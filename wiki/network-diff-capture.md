...
Tokenash includes a containerized harness for comparing Claude Code traffic sent
directly to Anthropic with traffic sent through a Tokenash proxy. The harness
...
docker compose up --build mitm-direct mitm-tokenash-upstream tokenash-proxy mitm-tokenash-client
docker compose --profile run run --rm claude-direct
docker compose --profile run run --rm claude-tokenash
```

The primary captures are written to:

- `docker/differential-network-capture/captures/direct.jsonl`
- `docker/differential-network-capture/captures/tokenash-client.jsonl`

The Tokenash lane also writes
`docker/differential-network-capture/captures/tokenash-upstream.jsonl`, which is
the request Tokenash forwards to Anthropic after proxy processing.
...
```bash
tokenash capture network-diff \
  --direct docker/differential-network-capture/captures/direct.jsonl \
  --tokenash docker/differential-network-capture/captures/tokenash-client.jsonl \
...
`tools=0->N` in the Tokenash client lane is evidence that Claude Code eagerly
materialized tool schemas before the request reached Tokenash.
...
Use `CLAUDE_DIRECT_ARGS` and `CLAUDE_TOKENASH_ARGS` when each lane needs
different flags.
