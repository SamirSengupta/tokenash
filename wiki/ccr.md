...
Tokenash's CCR architecture makes compression **reversible**. When content is compressed, the original data is cached. If the LLM needs more data, it can retrieve it instantly.
...
│  Option B: LLM calls tokenash_retrieve(hash=abc123)             │
...
Tokenash injects a `tokenash_retrieve` tool into the LLM's available tools:

```json
{
  "name": "tokenash_retrieve",
  "description": "Retrieve original uncompressed data from Tokenash cache",
  "parameters": {
...
When the LLM calls `tokenash_retrieve`:
...
│            → Calls tokenash_retrieve(hash=def456)               │
...
| Feature | Description |
|---------|-------------|
| **Automatic Response Handling** | When LLM calls `tokenash_retrieve`, the proxy handles it automatically |
| **Multi-Turn Context Tracking** | Tracks compressed content across turns, proactively expands when relevant |
| **BM25 Search** | LLM can search within compressed data: `tokenash_retrieve(hash, query="errors")` |
