...
Tokenash supports Google Cloud Vertex AI publisher endpoints through the proxy
passthrough surface. Configure the proxy with a regional Vertex base URL, then
send normal Vertex REST requests through Tokenash.
...
```bash
tokenash proxy --vertex-api-url https://us-central1-aiplatform.googleapis.com
```
...
Tokenash also forwards Anthropic publisher calls on Vertex:
...
## Claude Code with Tokenash compression (validated)

To run **Claude Code** against Claude-on-Vertex **with Tokenash compressing the
context**, use the dedicated, tested runbook:

➡️ **[Claude Code + Vertex + Tokenash](../docs/claude-code-vertex-tokenash.md)**
...
--code-aware`; Tokenash holds the GCP ADC creds and calls Vertex.
