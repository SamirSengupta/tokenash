...
The main entry point for Tokenash SDK.
...
response = client.chat.completions.create(
    model="gpt-4o",
    messages=[...],
    tokenash_mode="optimize",  # Override default mode
)
```
...
|-----------|------|-------------|
| `tokenash_mode` | `str` | Override mode for this request |
| `tokenash_query` | `str` | Query for relevance scoring |
...
```python
from tokenash import SmartCrusherConfig
...
```python
from tokenash import CacheAlignerConfig
...
```python
from tokenash import RollingWindowConfig
...
```python
from tokenash.config import ScoringWeights
...
```python
from tokenash import RelevanceScorerConfig
...
```python
from tokenash import BM25Scorer
...
```python
from tokenash import EmbeddingScorer, embedding_available
...
```python
from tokenash import HybridScorer
...
```python
from tokenash import SmartCrusher
...
```python
from tokenash import CacheAligner
...
```python
from tokenash import RollingWindow
...
```python
from tokenash.transforms import IntelligentContextManager
from tokenash.config import IntelligentContextConfig
...
```python
from tokenash.transforms import MessageScorer, MessageScore
from tokenash.config import ScoringWeights
...
```python
from tokenash import TransformPipeline
...
```python
from tokenash import Tokenizer, count_tokens_text, count_tokens_messages
...
```python
from tokenash import generate_report

report = generate_report(
    store_url="sqlite:///tokenash.db",
    format="html",
    period="day",
)
```
...
The TypeScript SDK provides `compress()`, `TokenashClient`, and framework adapters for Vercel AI SDK, OpenAI, and Anthropic.
