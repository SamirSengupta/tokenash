...
Harden the shared Tokenash proxy so it can survive real multi-agent Codex traffic — especially the **large Anthropic `/v1/messages?beta=true` reconnect/retry storm** that hits the proxy immediately after a restart — without appearing dead (`/livez` timing out, new `/v1/responses` websocket handshakes hanging) and without bypassing compression.
...
- **Not** modifying `tokenash[ml]` dependencies, HuggingFace model IDs, or the embedder's own backend.
...
- `tokenash/proxy/handlers/openai.py:1206` — `handle_openai_responses_ws` (Codex WS entry point, 600+ lines)
- `tokenash/proxy/handlers/openai.py:1559-1767` — upstream WS connect retry loop, `open_timeout` handling
- `tokenash/proxy/handlers/openai.py:1815` — `_ws_http_fallback` (preserved as-is)
- `tokenash/proxy/handlers/openai.py:1439-1452` — memory context timeout fail-open (preserved as-is)
- `tokenash/proxy/handlers/anthropic.py:293` — `handle_anthropic_messages` (HTTP entry point, also covers the `?beta=true` replay traffic)
- `tokenash/proxy/server.py:586-733` — `TokenashProxy.startup` (where eager preload runs)
- `tokenash/proxy/server.py:634-637` — current eager preload iterates **only** `anthropic_pipeline.transforms` and breaks on first match
- `tokenash/proxy/server.py:301-308` — both pipelines currently share the same `transforms` list (so the module-level `_kompress_cache` is de facto shared, but this is fragile)
- `tokenash/proxy/server.py:1190-1212` — `/livez`, `/readyz`, `/health` handlers (trivial JSON, no I/O)
- `tokenash/proxy/memory_handler.py:134-207` — `MemoryHandler._ensure_initialized` (lazy, no `asyncio.Lock`)
- `tokenash/transforms/content_router.py:1221-1297` — `eager_load_compressors` (Kompress + Magika + Code-Aware + SmartCrusher)
- `tokenash/transforms/kompress_compressor.py:163-221` — `_load_kompress_onnx` / `_load_kompress` with module-level `_kompress_cache` + `threading.Lock`
- `tokenash/proxy/request_logger.py` — existing structured request log sink (extend, don't replace)
- `tokenash/proxy/prometheus_metrics.py` — existing `PrometheusMetrics` class; add counters/gauges here
- `tokenash/proxy/helpers.py:138-207` — `_read_request_json` (pre-upstream work on Anthropic path)
...
- *Q: Target upstream or the fork?* Resolved: target the fork (`fix/responses-retries-keep-compression`), structured so each unit is cherry-pickable into a PR against `SamirSengupta/tokenash#172`.
...
- **Exact default for the Anthropic pre-upstream semaphore.** Start at `max(2, min(8, os.cpu_count()))` and expose via `--anthropic-pre-upstream-concurrency` / `TOKENASH_ANTHROPIC_PRE_UPSTREAM_CONCURRENCY`. Tune after Unit 2's stage timings land.
...
- **Final placement of `WebSocketSessionRegistry`**: `tokenash/proxy/ws_session_registry.py` as a new module, or folded into `tokenash/proxy/server.py`. Likely new module; confirm during implementation once imports are real.
...
**Files:**
- Modify: `tokenash/proxy/server.py` (startup orchestration around lines 627-675)
- Modify: `tokenash/proxy/memory_handler.py` (add `asyncio.Lock` in `_ensure_initialized`)
- Modify: `tokenash/transforms/content_router.py` (no behavior change — ensure `eager_load_compressors` is idempotent when called via both pipelines)
- Create: `tokenash/proxy/warmup.py` (new `WarmupRegistry` holding preloaded handles)
- Test: `tests/test_proxy_warmup.py` (new)
- Test: `tests/test_memory_handler_concurrent_init.py` (new)

**Approach:**
- Introduce `WarmupRegistry` with typed slots for kompress, magika, code-aware, smart-crusher, memory-embedder, memory-backend. Populate during `TokenashProxy.startup`, expose via `proxy.warmup` for `/debug/warmup` (Unit 5) and `/readyz`.
...
**Files:**
- Modify: `tokenash/proxy/handlers/openai.py` (WS path at lines 1206+; HTTP path at 798+)
- Modify: `tokenash/proxy/handlers/anthropic.py` (`handle_anthropic_messages` around line 293 and downstream)
- Modify: `tokenash/proxy/request_logger.py` (extend schema)
- Modify: `tokenash/proxy/prometheus_metrics.py` (add histograms per stage)
- Modify: `tokenash/proxy/helpers.py` (thread `stage_timer` through `_read_request_json` / compression helpers)
- Create: `tokenash/proxy/stage_timer.py` (small context-manager util)
...
**Patterns to follow:**
- `tokenash/proxy/request_logger.py` structured-field schema.
...
**Files:**
- Create: `tokenash/proxy/ws_session_registry.py` (new)
- Modify: `tokenash/proxy/handlers/openai.py` (`handle_openai_responses_ws` at 1206; relay task construction around 1559-1767; `_upstream_to_client` at 1565)
- Modify: `tokenash/proxy/prometheus_metrics.py` (add `active_ws_sessions` gauge, `active_relay_tasks` gauge, `ws_session_duration` histogram)
...
**Files:**
- Modify: `tokenash/proxy/handlers/anthropic.py` (wrap the pre-upstream phases in `handle_anthropic_messages` at ~293 through first upstream call)
- Modify: `tokenash/proxy/models.py` or config module (add `anthropic_pre_upstream_concurrency: int` config field)
- Modify: `tokenash/proxy/server.py` (construct the `asyncio.Semaphore` on `TokenashProxy.__init__` so it's per-process, not per-request)
...
**Approach:**
- Add `self.anthropic_pre_upstream_sem = asyncio.Semaphore(config.anthropic_pre_upstream_concurrency or max(2, min(8, os.cpu_count())))` in `TokenashProxy.__init__`.
...
- Default config: `TOKENASH_ANTHROPIC_PRE_UPSTREAM_CONCURRENCY` env var honored; CLI flag overrides; unset defaults to the computed floor.

**Patterns to follow:**
- Existing config field addition pattern in `tokenash/proxy/models.py`.
- Existing CLI flag plumbing in `tokenash/cli/`.
...
**Files:**
- Modify: `tokenash/proxy/server.py` (add `/debug/tasks`, `/debug/ws-sessions`, `/debug/warmup` routes near the existing `/livez` at 1190)
- Create: `tokenash/proxy/debug_introspection.py` (pure functions that serialize state)
- Create: `tokenash/proxy/loopback_guard.py` (middleware / dep that 404s non-loopback requests)
...
- Integration: after Unit 4 lands, harness with default Anthropic concurrency limits shows livez p99 < 100ms; harness with `TOKENASH_ANTHROPIC_PRE_UPSTREAM_CONCURRENCY=10000` (unbounded) reproduces the original starvation (p99 > 5s).
...
- **Interaction graph:** the new `WarmupRegistry` (Unit 1) and `WebSocketSessionRegistry` (Unit 3) are accessed from `TokenashProxy` during request handling and by `/debug/*` routes. Both are single-writer-per-event-loop; no locks needed beyond the `asyncio.Lock` in `MemoryHandler._ensure_initialized` (Unit 1).
...
- Update `CHANGELOG.md` under an "Unreleased" heading with: (a) compression preserved — no behavioral change for existing Codex users; (b) new stage timings visible in logs; (c) new `TOKENASH_ANTHROPIC_PRE_UPSTREAM_CONCURRENCY` env var; (d) new `/debug/*` loopback endpoints.
...
- Related code:
  - `tokenash/proxy/handlers/openai.py` — `handle_openai_responses_ws`, `_client_to_upstream`, `_upstream_to_client`, `_ws_http_fallback`
  - `tokenash/proxy/handlers/anthropic.py` — `handle_anthropic_messages`
  - `tokenash/proxy/server.py` — `TokenashProxy.__init__`, `.startup`, health routes
  - `tokenash/proxy/memory_handler.py` — `MemoryHandler._ensure_initialized`
  - `tokenash/transforms/content_router.py` — `eager_load_compressors`
  - `tokenash/transforms/kompress_compressor.py` — `_load_kompress_onnx`, `_kompress_cache`
- Related PRs/issues:
  - Upstream: `https://github.com/SamirSengupta/tokenash/issues/172`
