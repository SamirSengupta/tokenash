...
This page is the authoritative reference for the **Python Tokenash CLI** exposed by the `tokenash` console script.

## Global behavior

### Entry points

- Console script: `tokenash`
- Python module entrypoint: `python -m tokenash.cli`
...
| `--version`, `-v` | root only | Show the Tokenash version and exit |

> `-v` is a **root-level version alias**. Inside subcommands such as `tokenash wrap claude -v`, `-v` keeps its subcommand meaning (`--verbose`), not version.

## Command index

| Command | Purpose | Docker-native parity |
|---|---|---|
| `tokenash install ...` | Install and manage persistent deployments | **python-native; Docker-native wrapper supports `persistent-docker` lifecycle subset** |
| `tokenash proxy` | Run the Tokenash proxy server | **native in container** |
| `tokenash learn` | Learn from past tool-call failures | **native in container** |
| `tokenash perf` | Summarize recent proxy performance | **native in container** |
| `tokenash evals ...` | Run memory evaluation workflows | **native in container** |
| `tokenash memory ...` | Inspect and manage stored memories | **native in container** |
| `tokenash mcp ...` | Install, inspect, remove, or serve MCP integration | **native in container** |
| `tokenash wrap claude` | Start proxy and launch Claude Code | **host-bridged** |
| `tokenash wrap copilot` | Start proxy and launch GitHub Copilot CLI | **python-native only** |
| `tokenash wrap codex` | Start proxy and launch Codex CLI | **host-bridged** |
| `tokenash wrap aider` | Start proxy and launch Aider | **host-bridged** |
| `tokenash wrap cursor` | Start proxy and print Cursor config guidance | **host-bridged** |
| `tokenash wrap openclaw` | Install and configure the OpenClaw plugin | **host-bridged** |
| `tokenash unwrap openclaw` | Disable the Tokenash OpenClaw plugin | **host-bridged** |
...
```text
Usage: tokenash [OPTIONS] COMMAND [ARGS]...
  ...
  Examples:
      ...
      tokenash memory list        List stored memories
      tokenash memory stats       Show memory statistics
...
Commands:
  ...
  install Install and manage persistent Tokenash deployments.
  learn   Learn from past tool call failures to prevent future ones.
  mcp     MCP server for Claude Code integration.
  memory  Manage memories stored in Tokenash.
  perf    Analyze proxy performance from logs.
  proxy   Start the optimization proxy server.
  unwrap  Undo durable Tokenash wrapping for supported tools.
  wrap    Wrap CLI tools to run through Tokenash.
```

### Top-level command help snapshots

<details>
<summary><code>tokenash proxy --help</code></summary>

```text
Usage: tokenash proxy [OPTIONS]

  Start the optimization proxy server.

  Examples:
      tokenash proxy                    Start proxy on port 8787
      tokenash proxy --port 8080        Start proxy on port 8080
      tokenash proxy --no-optimize      Passthrough mode (no optimization)
...
<details>
<summary><code>tokenash learn --help</code></summary>

```text
Usage: tokenash learn [OPTIONS]
...
<details>
<summary><code>tokenash perf --help</code></summary>

```text
Usage: tokenash perf [OPTIONS]
...
<details>
<summary><code>tokenash evals --help</code></summary>

```text
Usage: tokenash evals [OPTIONS] COMMAND [ARGS]...
...
<details>
<summary><code>tokenash memory --help</code></summary>

```text
Usage: tokenash memory [OPTIONS] COMMAND [ARGS]...

  Manage memories stored in Tokenash.
...
<details>
<summary><code>tokenash mcp --help</code></summary>

```text
Usage: tokenash mcp [OPTIONS] COMMAND [ARGS]...

  MCP server for Claude Code integration.

Commands:
  install    Install Tokenash MCP server into Claude Code config.
  serve      Start the MCP server (called by Claude Code).
  status     Check Tokenash MCP configuration status.
  uninstall  Remove Tokenash MCP server from Claude Code config.
```

</details>

<details>
<summary><code>tokenash install --help</code></summary>

```text
Usage: tokenash install [OPTIONS] COMMAND [ARGS]...

  Install and manage persistent Tokenash deployments.

Options:
  -?, --help  Show this message and exit.

Commands:
  apply    Install a persistent Tokenash deployment.
...
<details>
<summary><code>tokenash wrap --help</code></summary>

```text
Usage: tokenash wrap [OPTIONS] COMMAND [ARGS]...

  Wrap CLI tools to run through Tokenash.

Commands:
  aider     Launch aider through Tokenash proxy.
  claude    Launch Claude Code through Tokenash proxy.
  copilot   Launch GitHub Copilot CLI through Tokenash proxy.
  codex     Launch OpenAI Codex CLI through Tokenash proxy.
  cursor    Start Tokenash proxy for use with Cursor.
  openclaw  Install and configure Tokenash OpenClaw plugin in one command.
```

</details>

<details>
<summary><code>tokenash unwrap --help</code></summary>

```text
Usage: tokenash unwrap [OPTIONS] COMMAND [ARGS]...

  Undo durable Tokenash wrapping for supported tools.

Commands:
  openclaw  Disable the Tokenash OpenClaw plugin and restore the legacy engine slot.
```
...
| `--mode` | runtime default | Optimization mode: `token`, `cache`, `token_mode`, `cache_mode`, `token_savings`, `cost_savings`, `token_tokenash` |
...
| `--code-aware` | off | Enable code-aware compression in the proxy (env: TOKENASH_CODE_AWARE_ENABLED) |
...
| `--memory-db-path` | `""` | Override memory DB path (help text: `{cwd}/.tokenash/memory.db`) |
...
- Proxy startup can also read environment variables such as `TOKENASH_HOST`, `TOKENASH_PORT`, `TOKENASH_BUDGET`, `TOKENASH_MODE`, `TOKENASH_ANYLLM_PROVIDER`, `TOKENASH_ANTHROPIC_PRE_UPSTREAM_CONCURRENCY`, `TOKENASH_ANTHROPIC_PRE_UPSTREAM_ACQUIRE_TIMEOUT_SECONDS`, `TOKENASH_REQUEST_TIMEOUT`, `TOKENASH_ANTHROPIC_PRE_UPSTREAM_MEMORY_CONTEXT_TIMEOUT_SECONDS`, `ANTHROPIC_TARGET_API_URL`, `OPENAI_TARGET_API_URL`, and `GEMINI_TARGET_API_URL`. CLI flags take precedence over environment variables.
...
```bash
tokenash learn
tokenash learn --apply
tokenash learn --agent codex --all
```
...
- If `--project` is omitted, Tokenash resolves from the current directory upward.
- External agent integrations register through the `tokenash.learn_plugin` entry point.
...
The command reads `${TOKENASH_WORKSPACE_DIR}/logs/proxy.log` (defaults
to `~/.tokenash/logs/proxy.log` — see the
...
### `tokenash evals memory`

Run the LoCoMo memory evaluation benchmark.

```bash
tokenash evals memory -n 3
tokenash evals memory --answer-model gpt-4o --llm-judge
```
...
### `tokenash evals memory-v2`

Run the V2 memory evaluation flow with LLM-controlled tools.

```bash
tokenash evals memory-v2
tokenash evals memory-v2 --save-model gpt-4o-mini --llm-judge
```
...
- `tokenash memory-eval`
- `tokenash memory-eval-v2`
...
### `tokenash memory list`

```bash
tokenash memory list
tokenash memory list --scope USER --since 7d
tokenash memory list -q "budget"
```

| Option | Default | Meaning |
|---|---|---|
| `--db-path` | `tokenash_memory.db` | Memory database path |
...
### `tokenash memory show <memory_id>`

```bash
tokenash memory show 1234abcd
tokenash memory show 1234abcd --json
```
...
### `tokenash memory edit <memory_id>`

```bash
tokenash memory edit 1234abcd --content "Updated note"
tokenash memory edit 1234abcd --importance 0.9
```
...
### `tokenash memory delete <memory_ids...>`

```bash
tokenash memory delete 1234abcd 5678efgh
tokenash memory delete 1234abcd --force
```
...
### `tokenash memory prune`

```bash
tokenash memory prune --older-than 30d --dry-run
tokenash memory prune --scope SESSION --force
```
...
### `tokenash memory purge`

```bash
tokenash memory purge --confirm
```
...
### `tokenash memory export`

```bash
tokenash memory export
tokenash memory export --output export.json
```
...
### `tokenash memory import <file>`

```bash
tokenash memory import export.json
tokenash memory import export.json --force
```
...
Manage the Tokenash MCP server integration.

### `tokenash mcp install`

```bash
tokenash mcp install
tokenash mcp install --proxy-url http://127.0.0.1:9000
```

| Option | Default | Meaning |
|---|---|---|
| `--proxy-url` | `http://127.0.0.1:8787` | Proxy URL written into MCP config |
| `--force` | off | Overwrite an existing Tokenash MCP config |
...
This removes the Tokenash MCP server entry from the Claude configuration.
...
```bash
tokenash mcp serve
tokenash mcp serve --proxy-url http://127.0.0.1:9000 --debug
```

| Option | Default | Meaning |
|---|---|---|
| `--proxy-url` | `http://127.0.0.1:8787` | Proxy URL (also reads `TOKENASH_PROXY_URL`) |
...
Install and manage persistent local Tokenash deployments.

### `tokenash install apply --help`

```text
Usage: tokenash install apply [OPTIONS]

  Install a persistent Tokenash deployment.
...
Options:
  ...
  --runtime [python|docker]       Runtime used to execute Tokenash for
  ...
  --image TEXT                    Docker image to use when runtime=docker or
                                  ...
                                  ghcr.io/samirsengupta/tokenash:latest]
  -?, --help                      Show this message and exit.
```

### `tokenash install apply`

```bash
tokenash install apply --preset persistent-service --providers auto
tokenash install apply --preset persistent-task --providers manual --target claude --target codex
tokenash install apply --preset persistent-docker --scope user
```
...
| `--image` | `ghcr.io/samirsengupta/tokenash:latest` | Docker image for Docker-backed installs |

`apply` stores a manifest under
`${TOKENASH_WORKSPACE_DIR}/deploy/<profile>/manifest.json` (default
`~/.tokenash/deploy/<profile>/manifest.json`), applies managed tool
configuration, starts the chosen runtime, and waits for `readyz`.

Docker-native host wrappers expose a narrower `tokenash install` subset for `persistent-docker` only: `apply`, `status`, `start`, `stop`, `restart`, and `remove`. Those wrapper flows preserve the same port and manifest behavior, but they intentionally reject `persistent-service`, `persistent-task`, and provider mutation flags like `--scope`, `--providers`, and `--target`.

### `tokenash install status`

```bash
tokenash install status
tokenash install status --profile default
```

Shows the stored profile, preset, runtime, supervisor kind, scope, port, runtime status, readiness, and backend from `/health`.

### `tokenash install start`

```bash
tokenash install start
tokenash install start --profile default
```

Starts a previously installed deployment profile without reapplying mutations.

### `tokenash install stop`
...
Wrap external coding tools so their traffic flows through Tokenash.
...
### `tokenash wrap claude`

```bash
tokenash wrap claude
tokenash wrap claude --resume <session-id>
tokenash wrap claude --port 9999
```
...
```bash
tokenash wrap codex
tokenash wrap codex -- "fix the bug"
...
```bash
tokenash wrap aider
tokenash wrap aider -- --model gpt-4o
...
### `tokenash wrap cursor`

```bash
tokenash wrap cursor
tokenash wrap cursor --port 9999
tokenash wrap cursor --no-rtk
```
...
```bash
tokenash wrap openclaw
tokenash wrap openclaw --plugin-path ./plugins/openclaw
```

| Option | Default | Meaning |
|---|---|---|
| `--plugin-path` | unset | Local plugin source directory |
| `--plugin-spec` | `tokenash/openclaw` | NPM plugin spec |
| `--skip-build` | off | Skip local `npm install` / build steps |
| `--copy` | off | Copy plugin instead of linked install |
| `--proxy-port` | `8787` | Tokenash proxy port |
| `--startup-timeout-ms` | `20000` | Proxy startup timeout |
| `--gateway-provider-id` | repeatable | OpenClaw provider IDs routed through Tokenash |
...
Requires the `openclaw` binary on the host, and local-source mode may also require `npm`. In Docker-native mode, the installed host wrapper drives the host `openclaw` CLI while the plugin auto-starts the host `tokenash` wrapper from `PATH`.

## `tokenash unwrap`

Undo durable wrapping for supported tools.

### `tokenash unwrap openclaw`

```bash
tokenash unwrap openclaw
tokenash unwrap openclaw --no-restart
```
...
This disables the Tokenash OpenClaw plugin and restores the legacy context engine slot.
...
- **native in container** — the command runs entirely inside the Tokenash container
- **host-bridged** — Tokenash runs in Docker, but the wrapped external tool still runs on the host

| Command path | Python CLI | Docker-native wrapper | Parity |
|---|---|---|---|
| `tokenash proxy` | native | native in container | full |
| `tokenash learn` | native | native in container | full |
| `tokenash perf` | native | native in container | full |
| `tokenash evals memory` | native | native in container | full |
| `tokenash evals memory-v2` | native | native in container | full |
| `tokenash memory ...` | native (when memory deps are available) | native in container | full |
| `tokenash mcp install` | native | native in container | full |
| `tokenash mcp uninstall` | native | native in container | full |
| `tokenash mcp status` | native | native in container | full |
| `tokenash mcp serve` | native | native in container | full |
| `tokenash install apply|status|start|stop|restart|remove` | native | Docker-native wrapper for `persistent-docker`; compose remains an alternative | partial |
| `tokenash wrap claude` | native | host-bridged | partial |
| `tokenash wrap copilot` | native | not implemented in Docker-native wrapper | none |
| `tokenash wrap codex` | native | host-bridged | partial |
| `tokenash wrap aider` | native | host-bridged | partial |
| `tokenash wrap cursor` | native | host-bridged | partial |
| `tokenash wrap openclaw` | native | host-bridged | partial |
| `tokenash unwrap openclaw` | native | host-bridged | partial |
...
If you are documenting operational behavior or debugging internal wrapper flows, refer to the implementation in `tokenash/cli/wrap.py`.
