...
Tokenash provides explicit exceptions for debugging, with a safety guarantee that compression failures never break your LLM calls.

## Exception Hierarchy

```python
from tokenash import (
    TokenashError,        # Base class - catch all Tokenash errors
...
This is a core design principle. Your LLM calls never fail due to Tokenash:
...
# WARNING:tokenash.transforms.smart_crusher:Skipping compression: invalid JSON
```

## Error Details

All Tokenash exceptions include a `details` dict with context:
...
result = client.validate_setup()
if not result["valid"]:
    raise SystemExit(f"Tokenash setup invalid: {result['issues']}")
...
# DEBUG:tokenash.transforms.smart_crusher:Analyzing 1000 items...
# DEBUG:tokenash.transforms.smart_crusher:Kept 15 items (errors: 2, anomalies: 3)
```
