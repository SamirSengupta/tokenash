...
## What Makes Tokenash Memory Different?

| Feature | Tokenash | Letta (MemGPT) | Mem0 |
|---------|----------|----------------|------|
...
tokenash proxy --memory

# Or use wrap (auto-starts proxy)
tokenash wrap claude --memory    # Claude Code with persistent memory
tokenash wrap codex --memory     # Codex with the SAME memory store
tokenash wrap aider --memory     # Aider shares it too
```
...
Memory is stored per-project at `{cwd}/.tokenash/memory.db`. Each
...
> canonical `TOKENASH_WORKSPACE_DIR` env var. This preserves the
...
User ID is auto-detected from `$USER` (your OS username). Override per-request with the `x-tokenash-user-id` header. All memories are scoped to the user — multiple developers on the same project have separate memory stores.
...
When the LLM calls `memory_save`, tokenash:
...
The LLM then decides whether to merge — using the user's own LLM, not a separate model. No extra cost to tokenash.
...
from tokenash import with_memory
...
Memories evolve over time. When facts change, Tokenash creates a **supersession chain** preserving history:

```python
from tokenash.memory import HierarchicalMemory, MemoryConfig
...
```python
import asyncio
from tokenash.memory import (
    HierarchicalMemory,
    MemoryConfig,
    MemoryCategory,
    EmbedderBackend,
)
from tokenash.memory.ports import MemoryFilter, VectorFilter
...
```python
from tokenash.memory import MemoryConfig, EmbedderBackend
...
Tokenash Memory uses **Protocol interfaces** (ports) for all components, enabling easy swapping:
...
**Letta** pioneered inline memory extraction. Tokenash builds on this with:

| Aspect | Tokenash | Letta |
|--------|----------|-------|
...
**When to use Tokenash**: You want memory as a layer on your existing stack.

### vs Mem0

**Mem0** provides a managed memory service. Tokenash differs:

| Aspect | Tokenash | Mem0 |
|--------|----------|------|
...
**When to use Tokenash**: You want embedded memory with no external services.

### Feature Matrix

| Feature | Tokenash | Letta | Mem0 |
|---------|:--------:|:-----:|:----:|
