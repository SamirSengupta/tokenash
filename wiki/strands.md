...
Tokenash integrates with [Strands Agents](https://github.com/strands-agents/sdk-python) to provide automatic context optimization. Two integration patterns: wrap the model, or hook into tool calls.
...
```bash
pip install tokenash strands-agents
```
...
```python
from tokenash.integrations.strands import TokenashStrandsModel, TokenashHookProvider

optimized = TokenashStrandsModel(wrapped_model=model)
hooks = TokenashHookProvider(compress_tool_outputs=True)
...
TokenashStrandsModel supports Strands' structured output feature:
...
The model wrapper uses Tokenash's full pipeline (CacheAligner → ContentRouter → IntelligentContext). The hook provider uses SmartCrusher directly for fast JSON compression of individual tool results.

---

## Supported Providers

TokenashStrandsModel auto-detects the provider from the wrapped model:
