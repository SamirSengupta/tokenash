...
Tokenash can be configured via the SDK, proxy command line, or per-request overrides.
...
client = TokenashClient(
    ...
    # store_url="sqlite:////absolute/path/to/tokenash.db",
)
```
...
tokenash proxy \
  ...
  --log-file tokenash.jsonl  # Log file path
```
...
tokenash proxy --no-optimize

# Disable semantic caching
tokenash proxy --no-cache

# Disable CCR response handling
tokenash proxy --no-ccr-responses

# Disable proactive expansion
tokenash proxy --no-ccr-expansion
...
- **ONNX Runtime** — lightweight, CPU-first. Installed with
  `pip install tokenash[proxy]`. Optionally uses the CoreML execution
  provider on macOS.
- **PyTorch** — heavier, supports CUDA and Apple-Silicon MPS
  acceleration. Installed with `pip install tokenash[ml]`. With
  `device=auto` it selects `cuda`, then `mps`, then `cpu`.

Select the backend via the `TOKENASH_KOMPRESS_BACKEND` environment
variable:
...
```bash
export TOKENASH_KOMPRESS_BACKEND=mps
tokenash proxy ...
```
...
```python
from tokenash.transforms import SmartCrusherConfig
...
```python
from tokenash.transforms import CacheAlignerConfig
...
```python
from tokenash.transforms import RollingWindowConfig
...
```python
from tokenash.config import IntelligentContextConfig, ScoringWeights
...
```python
from tokenash.telemetry import get_toin
...
|----------|-------------|---------|
| `TOKENASH_MODEL_LIMITS` | Custom model config (JSON string or file path) | - |
| `TOKENASH_CONFIG_DIR` | Canonical config (read-mostly) root. Derives `models.json` and per-plugin config paths when set. | `~/.tokenash/config` |
| `TOKENASH_WORKSPACE_DIR` | Canonical workspace (read-write state) root. Derives savings ledger, memory DB, logs, TOIN, subscription state, and more when set. | `~/.tokenash` |
| `TOKENASH_SAVINGS_PATH` | Full path to the proxy savings JSON ledger. Always wins when set. | derived from `${TOKENASH_WORKSPACE_DIR}` |
| `TOKENASH_TOIN_PATH` | Full path to the TOIN telemetry JSON file. Always wins when set. | derived from `${TOKENASH_WORKSPACE_DIR}` |
| `TOKENASH_SUBSCRIPTION_STATE_PATH` | Full path to the subscription tracker state. Always wins when set. | derived from `${TOKENASH_WORKSPACE_DIR}` |
| `TOKENASH_EMBEDDER_RUNTIME` | Set to `pytorch_mps` to run the memory embedder via the torch sentence-transformers backend on the Apple GPU (MPS). Only engages when Apple MPS is actually available; otherwise it logs a warning and uses the existing default embedder selection path. `pytorch_mps` is the only accepted value. Requires the `[pytorch-mps]` extra. See [Memory](memory.md#embedding-runtime--gpu-offload-apple-silicon). | default embedder selection |
| `TOKENASH_BETA_HEADER_STICKY` | Controls per-session `anthropic-beta` / `OpenAI-Beta` re-echo. `enabled` (default): the proxy unions beta tokens across turns within a session — if the client sends a token in turn N and omits it in turn N+1, the proxy re-injects it to preserve prefix-cache stability. `disabled`: the client's value is forwarded verbatim with no accumulation. Any other value raises at request time. See [Session Beta Header Tracking](#session-beta-header-tracking). | `enabled` |
| `TOKENASH_BETA_TRACKER_MAX_SESSIONS` | LRU capacity of the in-memory session beta tracker. Once full, the oldest session entry is evicted. | `1000` |

## Session Beta Header Tracking

When running as a proxy, Tokenash maintains a per-session union of `anthropic-beta` (and `OpenAI-Beta`) tokens via `SessionBetaTracker`. The session key is derived from the `x-tokenash-session-id` header if present, otherwise from `md5(model + system_prompt[:500])[:16]` — stable across turns of the same conversation.
...
**Trade-off:** once the proxy has seen a beta token in a session it will continue re-sending it for the rest of that session, even if the client stops including it. Stopping the token on the client side alone is not sufficient — the proxy re-injects it. Set `TOKENASH_BETA_HEADER_STICKY=disabled` to pass the client's `anthropic-beta` value verbatim and bypass this accumulation.

```bash
# Disable sticky beta re-echo
export TOKENASH_BETA_HEADER_STICKY=disabled
tokenash proxy ...
```
...
Tokenash resolves every on-disk resource through a two-root model:

- `TOKENASH_CONFIG_DIR` (default `~/.tokenash/config`) — read-mostly
  configuration
- `TOKENASH_WORKSPACE_DIR` (default `~/.tokenash`) — read-write state
...
note (`TOKENASH_WORKSPACE` is *not* the same as `TOKENASH_WORKSPACE_DIR`).
...
- A new model is released before Tokenash is updated
...
1. Built-in defaults
2. `${TOKENASH_CONFIG_DIR}/models.json` (defaults to
   `~/.tokenash/config/models.json`); falls back to the legacy location
   `~/.tokenash/models.json` when the canonical file is absent
3. `TOKENASH_MODEL_LIMITS` environment variable
4. SDK constructor arguments

### Config File Format

Create `~/.tokenash/models.json`:
...
Set `TOKENASH_MODEL_LIMITS` as a JSON string or file path:

```bash
# JSON string
export TOKENASH_MODEL_LIMITS='{"anthropic":{"context_limits":{"claude-new":200000}}}'

# File path
export TOKENASH_MODEL_LIMITS=/path/to/models.json
```
...
```python
from tokenash import OpenAIProvider
...
```python
from tokenash import AnthropicProvider
...
```python
from tokenash import GoogleProvider
...
| Variable | Description | Default |
|----------|-------------|---------|
| `TOKENASH_BASE_URL` | Base URL of the Tokenash proxy or cloud API | `http://localhost:8787` |
| `TOKENASH_API_KEY` | API key for Tokenash Cloud authentication | - |

### Usage

```bash
export TOKENASH_BASE_URL=http://localhost:8787
export TOKENASH_API_KEY=your-api-key
```

```typescript
import { TokenashClient } from 'tokenash';

// Reads from TOKENASH_BASE_URL and TOKENASH_API_KEY automatically
const client = new TokenashClient();
