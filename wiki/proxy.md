...
The Tokenash proxy server is a production-ready HTTP server that applies context optimization to all requests passing through it.
...
# Custom port
tokenash proxy --port 8080

# With all options
tokenash proxy \
  --host 0.0.0.0 \
  --port 8787 \
  --log-file /var/log/tokenash.jsonl \
  --budget 100.0
```
...
`tokenash wrap copilot` uses Copilot CLI's BYOK provider settings under the hood. In `provider-type=auto`, it chooses Tokenash's Anthropic route for the default proxy backend and the OpenAI-compatible `/v1` route for translated backends such as `anyllm` and LiteLLM.

Anonymous aggregate telemetry is **off by default** (opt-in). Opt in with `TOKENASH_TELEMETRY=on` or `tokenash proxy --telemetry`. Downstream apps can set `TOKENASH_SDK=tokenash-app` to override the anonymous telemetry `sdk` label; the default remains `proxy`.

Operational OTEL metrics are configured separately and are **off by default**. Install `tokenash[proxy,otel]` and set:
...
Use `TOKENASH_OTEL_METRICS_EXPORTER=console` for local smoke testing. `TOKENASH_TELEMETRY` controls the anonymous data-flywheel beacon only; it does not disable or enable OTEL export.

Langfuse can be enabled alongside this OTEL path for **trace ingestion**. Langfuse does **not** ingest OTEL metrics, so Tokenash keeps metrics and Langfuse traces as complementary signals:
...
When configured, Tokenash emits OTLP traces for the shared compression pipeline to Langfuse while continuing to expose metrics through `/metrics` and OTEL metric exporters.
...
| `--code-aware` | true | Enable AST-based code compression (env: TOKENASH_CODE_AWARE_ENABLED) |
...
Tokenash proxy has two explicit run modes:
...
```bash
tokenash proxy --mode token
TOKENASH_MODE=cache tokenash proxy
```
...
Legacy values (`token_tokenash`, `cost_savings`) are still accepted as aliases.
...
tokenash proxy --no-intelligent-context

# Disable semantic scoring (faster, but less intelligent)
tokenash proxy --no-intelligent-scoring
```

### ML Compression — RETIRED `--llmlingua` flag

The `--llmlingua` / `--llmlingua-device` / `--llmlingua-rate` flags and
the `tokenash[llmlingua]` extra were retired and replaced by Kompress
(ModernBERT). For the current opt-in path, install `tokenash[ml]`
...
`/readyz` returns HTTP 503 when Tokenash has not completed startup or a required enabled subsystem is unavailable. This is the endpoint used by the container health checks.
...
other Tokenash frontends. It returns:
...
Set `TOKENASH_SAVINGS_PATH` to override the location directly, or set
`TOKENASH_WORKSPACE_DIR` to relocate the full state root. See the
...
**Headers:**
- `x-tokenash-bypass: true` — skip compression, return messages as-is
...
1. Start the proxy: `tokenash proxy`
...
> `tokenash[llmlingua]`, `LLMLinguaCompressor`) was retired and
> replaced by **Kompress** (ModernBERT). Install with `pip install
> 'tokenash[ml]'`. See [transforms.md](transforms.md) and
...
```
tokenash_requests_total
tokenash_tokens_saved_total
tokenash_cost_usd_total
tokenash_latency_ms_sum
```

## Configuration via Environment

```bash
export TOKENASH_HOST=0.0.0.0
export TOKENASH_PORT=8787
export TOKENASH_BUDGET=100.0
...
# Run with gunicorn
gunicorn tokenash.proxy.server:app \
  --workers 4 \
  --bind 0.0.0.0:8787 \
...
EXPOSE 8787
CMD ["tokenash", "proxy", "--host", "0.0.0.0"]
```

> **Note:** `build-essential` is required at install time because `tokenash` includes `hnswlib`, a C++ extension that must be compiled from source. It is removed after installation to keep the image slim.
