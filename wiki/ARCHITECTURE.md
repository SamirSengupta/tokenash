# Tokenash SDK: A Complete Explanation
...
flowchart TB
    subgraph Entry["Entry Points"]
        ...
        SDK["SDK Mode<br/><i>TokenashClient</i>"]
...
## What Problem Does Tokenash Solve?
...
**Tokenash creates "tokenash"** - it intelligently compresses your input tokens so you have more room (and budget) for what matters.

---

## How Tokenash Works: The Big Picture
...
### 1. TokenashClient (`client.py`) - The Wrapper
...
# After (with Tokenash)
base = OpenAI(api_key="...")
client = TokenashClient(original_client=base, provider=OpenAIProvider())
response = client.chat.completions.create(model="gpt-4o", messages=[...])
```
...
**Why this matters:** Token counting is model-specific. GPT-4 uses different tokenization than Claude. Tokenash needs accurate counts to know how much to compress.

---

### 3. Parser (`parser.py`) - Understanding Your Messages

Before optimizing, Tokenash needs to understand what's in your messages:
...
This is where the real work happens. Tokenash has 4 transforms that run in sequence:
...
# Smart compression:
{
    "__tokenash_constants": {"host": "prod-1"},  # Factor out
    "__tokenash_summary": "items 0-44: cpu stable at ~45",  # Summarize boring part
    "data": [
...
'tokenash[ml]'`; see `wiki/transforms.md` for current configuration.

**Note:** The earlier LLMLingua-2 integration (`--llmlingua` flag, the
`tokenash[llmlingua]` extra, and the `LLMLinguaCompressor` class) was
retired and replaced by Kompress. `pip install 'tokenash[llmlingua]'`
...
CREATE TABLE requests (
    ...
    tokens_input_before INTEGER,  -- Before Tokenash
    tokens_input_after INTEGER,   -- After Tokenash
...
response = client.chat.completions.create(
    ...
    tokenash_mode="optimize",
)
```

### Step 2: TokenashClient intercepts
```python
# In client.py:
...
def apply(self, messages, ...):
    ...
    # Transform 3: Kompress ML compressor (opt-in via tokenash[ml])
...
**Location:** `tokenash/cache/compression_store.py`
...
When compression happens, Tokenash injects retrieval instructions into the LLM context.
...
When running as MCP server, Tokenash exposes retrieval as a tool:
...
```json
{
    "__tokenash_compressed": true,
    "__tokenash_hash": "abc123def456",
    "__tokenash_stats": {
        "original_items": 1000,
        "kept_items": 20,
        "errors_preserved": 5
...
**Location:** `tokenash/cache/compression_feedback.py`
...
**Location:** `tokenash/ccr/response_handler.py`

**The Problem:** When the proxy injects the `tokenash_retrieve` tool, the LLM might call it. But who handles that tool call? Without response handling, the tool call would go back to the client unhandled.
...
│     └─ Contains: tool_use(tokenash_retrieve, hash=abc123)       │
│                                                                   │
...
**Location:** `tokenash/ccr/context_tracker.py`
...
Vision models charge by the token, and images are expensive (765-2900 tokens for a typical image). Tokenash's image compression uses a **trained ML router** to automatically select the optimal compression technique.
...
- **Model**: `SamirSengupta/technique-router`
...
├── client.py            # TokenashClient - the main wrapper
...
**Tokenash is a Context Budget Controller that:**
