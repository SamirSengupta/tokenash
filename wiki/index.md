...
[![PyPI](https://img.shields.io/pypi/v/tokenash.svg)](https://pypi.org/project/tokenash/)
[![Python](https://img.shields.io/pypi/pyversions/tokenash.svg)](https://pypi.org/project/tokenash/)
[![License](https://img.shields.io/badge/license-Apache%202.0-blue.svg)](https://github.com/SamirSengupta/tokenash/blob/main/LICENSE)
...
Every tool call, DB query, file read, and RAG retrieval your agent makes is 70-95% boilerplate. Tokenash compresses it away before it hits the model. The LLM sees less noise, responds faster, and costs less.
...
Your Agent / App
    ...
    ▼
 Tokenash  ← proxy, Python library, or framework integration
    │
    ▼
 LLM Provider  (OpenAI, Anthropic, Google, Bedrock, 100+ via LiteLLM)
```

Tokenash works as a **transparent proxy** (zero code changes), a **Python function** (`compress()`), or a **framework integration** (LangChain, Agno, Strands, LiteLLM, MCP).
...
model = TokenashStrandsModel(wrapped_model=bedrock_model)
agent = Agent(model=model)
```
...
Three tools for Claude Code, Cursor, or any MCP client: `tokenash_compress`, `tokenash_retrieve`, `tokenash_stats`.

```bash
tokenash mcp install && claude
```
...
[OpenClaw Plugin &rarr;](https://github.com/SamirSengupta/tokenash/tree/main/plugins/openclaw)
...
Tokenash runs a three-stage pipeline on every request:
...
**Nothing is lost.** Compressed content goes into the CCR store (Compress-Cache-Retrieve). The LLM gets a `tokenash_retrieve` tool and can fetch full originals when it needs more detail.
...
```bash
tokenash proxy                                          # Direct Anthropic/OpenAI
tokenash proxy --backend bedrock --region us-east-1     # AWS Bedrock
tokenash proxy --backend vertex_ai --region us-central1 # Google Vertex AI
tokenash proxy --backend azure                          # Azure OpenAI
tokenash proxy --backend openrouter                     # OpenRouter (400+ models)
```
...
```bash
pip install tokenash                # Core library (Python)
pip install "tokenash[all]"         # Everything (recommended)
npm install tokenash                # TypeScript / Node.js
pip install "tokenash[proxy]"       # Proxy server + MCP tools
pip install "tokenash[ml]"          # ML compression (Kompress, requires torch)
pip install "tokenash[langchain]"   # LangChain integration
pip install "tokenash[agno]"        # Agno integration
pip install "tokenash[evals]"       # Evaluation framework
```
...
- **[Integration Guide](integration-guide.md)** — Every way to add Tokenash to your stack
...
Apache 2.0 — Free for commercial use. [GitHub](https://github.com/SamirSengupta/tokenash) | [PyPI](https://pypi.org/project/tokenash/) | [Discord](https://discord.gg/yRmaUNpsPJ)
