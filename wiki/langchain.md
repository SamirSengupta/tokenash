...
Tokenash provides seamless integration with LangChain, enabling automatic context optimization across all LangChain patterns: chat models, memory, retrievers, agents, and observability.
...
This installs Tokenash with LangChain dependencies (`langchain-core`).
...
The `TokenashChatModel` wraps any LangChain `BaseChatModel`:
...
llm = TokenashChatModel(ChatAnthropic(model="claude-3-5-sonnet-20241022"))
...
`TokenashChatMessageHistory` wraps any chat history with automatic compression:
...
**Why this matters**: Long conversations can blow up to 50K+ tokens. TokenashChatMessageHistory automatically compresses older turns while preserving recent context.
...
`TokenashDocumentCompressor` filters retrieved documents by relevance:
...
# Wrap with Tokenash compression (keep best for precision)
...
**Why this matters**: Vector search often returns many marginally-relevant documents. TokenashDocumentCompressor uses BM25-style scoring to keep only the most relevant ones, reducing context size while improving answer quality.

---

### 4. Agent Tool Wrapping

`wrap_tools_with_tokenash` compresses tool outputs for agents:
...
```python
from tokenash.integrations import StreamingMetricsTracker
...
```python
from tokenash.integrations import StreamingMetricsCallback
...
Add Tokenash metrics to LangSmith traces:

```python
from tokenash.integrations import TokenashLangSmithCallbackHandler

# Create callback handler
langsmith_handler = TokenashLangSmithCallbackHandler()
...
# - tokenash.tokens_before
# - tokenash.tokens_after
# - tokenash.tokens_saved
# - tokenash.compression_ratio
```
...
**Without Tokenash**: Each tool call adds 10-50K tokens to context.
**With Tokenash**: Tool outputs compressed to 1-2K tokens, agent runs faster and cheaper.
...
```python
from tokenash.integrations.langchain import compress_tool_messages
...
from langchain.retrievers import ContextualCompressionRetriever
from tokenash.integrations import TokenashChatModel, TokenashDocumentCompressor
...
# Tokenash compressor for precision
compressor = TokenashDocumentCompressor(
...
- With Tokenash: 5 docs × ~500 tokens = 2.5K context tokens (90% reduction)
...
from tokenash.integrations import TokenashChatModel, TokenashChatMessageHistory
...
**Impact**: Without compression, 100-turn conversation = 100K+ tokens. With TokenashChatMessageHistory, it stays under 8K tokens while preserving recent context.
...
from langchain_core.tools import tool
from tokenash.integrations import (
    TokenashChatModel,
...
# Wrap everything
llm = TokenashChatModel(ChatOpenAI(model="gpt-4o"))
tools = wrap_tools_with_tokenash([search_arxiv, search_github, fetch_documentation])
...
TokenashChatModel(
    ...
    tokenash_config=TokenashConfig(),  # Tokenash configuration
...
from tokenash.integrations.langchain.memory import TokenashChatMessageHistory
```
...
```python
from tokenash.integrations import langchain_available

if not langchain_available():
    print("Install with: pip install tokenash[langchain]")
```
...
```python
history = TokenashChatMessageHistory(
    base_history,
