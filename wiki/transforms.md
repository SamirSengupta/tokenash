...
Tokenash provides several transforms that work together to optimize LLM context.
...
```python
from tokenash.transforms import IntelligentContextManager, IntelligentContextConfig
...
```python
from tokenash.config import IntelligentContextConfig, RollingWindowConfig
...
the `tokenash[llmlingua]` extra, and the `--llmlingua` proxy flag)
was retired in 0.9.x and replaced by **Kompress** (ModernBERT).
`pip install 'tokenash[llmlingua]'` no longer resolves; use the
...
```bash
pip install "tokenash[code]"  # Adds tree-sitter-language-pack
```

### Configuration

```python
from tokenash.transforms import CodeAwareCompressor, CodeCompressorConfig, DocstringMode
...
```python
from tokenash.transforms import CodeAwareCompressor
...
```python
from tokenash.transforms import is_tree_sitter_available, unload_tree_sitter
...
```python
from tokenash.transforms import ContentRouter, ContentRouterConfig, CompressionStrategy
...
```python
from tokenash import TransformPipeline, SmartCrusher, CacheAligner, RollingWindow
